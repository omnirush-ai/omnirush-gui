...
import { resolvePlace } from "@omnirush/testkit/stack";
import type { Den, Place } from "@omnirush/testkit/stack";
import {
  ACME_DOCS_APP,
  ACME_DOCS_MEMBER,
...
async function mintMcpToken(admin: DenSession, orgId: string): Promise<string> {
  const { response, body, text } = await denFetch(admin, "/v1/mcp/token", {
    method: "POST",
    headers: { authorization: `Bearer ${admin.token}`, "x-omnirush-org-id": orgId },
