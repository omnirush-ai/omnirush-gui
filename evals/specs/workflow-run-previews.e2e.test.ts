...
import { runWorkflow, saveWorkflow } from "@omnirush/behaviors";
...
const test = spec.world(async (seed) => {
  ...
  const setEnterprise = async (enabled: boolean) => {
    ...
    if (den.placement?.kind === "daytona") {
      const script = `import { createConnection } from "/workspace/ee/packages/den-db/node_modules/mysql2/promise.js";
        const connection = await createConnection("mysql://root:password@127.0.0.1:3306/omnirush_den");
  ...
  const token = field((await seed.api(den.admin, "/v1/mcp/token", {
    method: "POST", headers: { "x-omnirush-org-id": organizationId }, body: JSON.stringify({ scopes: ["mcp:read", "mcp:write"] }),
  })).body, "token");
  let requestId = 0;
