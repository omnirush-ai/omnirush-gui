...
test("desktop opens directly while the non-desktop welcome join field accepts a server URL or web invite", async ({ world, user: desktopUser, probe, step }) => {
  ...
  await step("Non-desktop welcome keeps its optional entry paths", async () => {
    ...
    await user.notSee({ text: /Using OmniRush.ai on-premises\?/ });
    await user.looks([
      "The Welcome to OmniRush.ai heading is visible",
      "Sign in to OmniRush.ai Cloud and Use Without Cloud are offered",
      "Join your organization says to paste an invite link, install link, or server URL",
      "The page does not say Using OmniRush.ai on-premises",
    ]);
  });
  ...
  await step("A server URL becomes the control plane", async () => {
    await user.type(joinInput, "https://omnirush.acme.test");
    await user.click("Connect");
    await user.see({ text: /Connected to omnirush\.acme\.test\. Sign in to continue\./ }, { timeoutMs: 20_000 });
    expect(await readBootstrapBaseUrl()).toBe("https://omnirush.acme.test");
    await user.looks([
      "The join dialog field label mentions invite link, install link, or server URL",
      "The dialog confirms it connected to omnirush.acme.test",
    ]);
  });
  ...
  await step("A web invite requests trust before browser handoff", async () => {
    ...
    expect(await probe.hash()).toMatch(/^#\/workspace\/[^/]+\/session$/);
    expect(await probe.storage("omnirush.den.baseUrl")).not.toBe("http://localhost:59991");
  });
});
