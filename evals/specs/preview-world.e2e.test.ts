...
import { main, isProcessAlive, readScriptWorldSnapshot } from "@omnirush/world";
import { denFetch, signIn } from "@omnirush/behaviors";
import { attachSurface, evaluateOnSurface } from "@omnirush/cdp";
import { screenshot } from "@omnirush/test-evidence";
...
test("preview worlds expose Den and real Electron, preserve progress on frontend update, and tear down only their own stage", { timeout: 1_500_000 }, async ({ evidence }) => {
  ...
  const snapshots = await mkdtemp(join(tmpdir(), "omnirush-preview-proof-"));
  const previous = process.env.OMNIRUSH_WORLD_SNAPSHOT_DIR;
  process.env.OMNIRUSH_WORLD_SNAPSHOT_DIR = snapshots;
  ...
  try {
    const pinnedRef = process.env.OMNIRUSH_EVAL_REF;
    assert.ok(pinnedRef);
    try {
      process.env.OMNIRUSH_EVAL_REF = "dev";
      ...
      process.env.OMNIRUSH_EVAL_REF = pinnedRef;
    }
    ...
    assert.ok(process.env.OMNIRUSH_EVAL_REF);
    await exec("python3", [join(root, ".opencode/skills/preview-my-work/scripts/update-preview.py"), "preview-desktop", "--stage", stage, "--ref", process.env.OMNIRUSH_EVAL_REF], { cwd: root, timeout: 300000, maxBuffer: 2_000_000 });
    ...
    if (previous === undefined) delete process.env.OMNIRUSH_WORLD_SNAPSHOT_DIR;
    else process.env.OMNIRUSH_WORLD_SNAPSHOT_DIR = previous;
