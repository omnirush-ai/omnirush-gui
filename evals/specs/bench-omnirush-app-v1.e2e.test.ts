...
import { clickButton, createAndSelectWorkspace, evalIn, readComposerState } from "@omnirush/behaviors";
import type { Surface } from "@omnirush/cdp";
import { desktop } from "@omnirush/hosts";
import type { DesktopHandle } from "@omnirush/hosts";
import { eventually, needs, test } from "@omnirush/testkit";
import { timeline } from "../packages/timeline/src/index.ts";
import { expect } from "vitest";

const enabled = process.env.OMNIRUSH_EVAL_E2E_TESTS === "1";
const benchEngine = process.env.OMNIRUSH_BENCH_ENGINE === "v2" ? "v2" : "v1";
const title = enabled
  ? `OmniRush.ai Electron ${benchEngine} engine completes the CDP benchmark with faithful witness traffic`
  : `OmniRush.ai Electron ${benchEngine} benchmark skipped — needs: set OMNIRUSH_EVAL_E2E_TESTS=1`;
...
async function startWitness(nonce: string): Promise<Witness> {
  ...
  const server = createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      const requestId = `chatcmpl-omnirush-v1-bench-${requestNumber}`;
...
async function readServerInfo(app: Surface): Promise<ServerInfo> {
  const value = await evalIn(app, async () => {
    ...
    let token = "";
    try {
      const invokeDesktop = window.__OMNIRUSH_ELECTRON__ && window.__OMNIRUSH_ELECTRON__.invokeDesktop;
      if (invokeDesktop) {
    ...
    if (!baseUrl || !token) {
      const port = (localStorage.getItem("omnirush.server.port") ?? "").trim();
      baseUrl = port ? "http://127.0.0.1:" + port : baseUrl;
      token = token || (localStorage.getItem("omnirush.server.token") ?? "").trim();
    }
...
async function pollExpression(
  app: Surface,
  expression: import("@omnirush/cdp").BrowserEvaluation,
  label: string,
  within = 120_000,
): Promise<void> {
...
async function createSecondWorkspaceViaUi(app: Surface, firstWorkspaceId: string, workspacePath: string): Promise<string> {
  ...
  const workspaceId = await eventually(async () => {
    const value = await evalIn(app, () => (localStorage.getItem("omnirush.react.activeWorkspace") ?? ""));
    return typeof value === "string" ? value : "";
  }, {
    within: 120_000,
    intervalMs: pollResolutionMs,
...
async function waitForChatSurface(app: Surface, chat: Chat): Promise<void> {
  await pollExpression(app, browserScript((sessionId, workspaceId) => {
    const surface = document.querySelector<HTMLElement>("[data-session-surface-id]");
    return surface?.getAttribute("data-session-surface-id") === sessionId
      && (localStorage.getItem("omnirush.react.activeWorkspace") ?? "") === workspaceId;
...
test.skipIf(!enabled)(title, { timeout: 900_000 }, async ({ evidence, place }) => {
  needs({ optIn: ["OMNIRUSH_EVAL_E2E_TESTS"] });
  const iterations = Number(process.env.OMNIRUSH_BENCH_ITERATIONS ?? "1");
  expect(Number.isInteger(iterations) && iterations > 0, "OMNIRUSH_BENCH_ITERATIONS must be a positive integer").toBe(true);
  const opencodeV2Bin = benchEngine === "v2" && place.kind === "local" ? process.env.OMNIRUSH_EVAL_OPENCODE2_BIN?.trim() || undefined : undefined;
  ...
  let uiCompactionAvailable = false;
    ...
    for (let coldIndex = 0; coldIndex < iterations; coldIndex += 1) {
      ...
      const profileDir = await mkdtemp(join(tmpdir(), `omnirush-bench-${benchEngine}-profile-`));
      const workspaceAPath = await mkdtemp(join(tmpdir(), `omnirush-bench-${benchEngine}-workspace-a-`));
      const workspaceBPath = coldIndex === iterations - 1
        ? await mkdtemp(join(tmpdir(), `omnirush-bench-${benchEngine}-workspace-b-`))
        : null;
      ...
      try {
        const appName = `bench-omnirush-app-${benchEngine}-${coldIndex}`;
        ...
        app = await desktop({
          ...
          env: {
            ...
            // (injected by apps/server/src/omnirush-runtime-config.ts) can hold its install lock for minutes and block /config + /provider, so the picker
            ...
            OMNIRUSH_API_KEY: "",
            OMNIRUSH_INFERENCE_BASE_URL: "",
            ...(opencodeV2Bin === undefined ? {} : { OMNIRUSH_OPENCODE2_BIN: opencodeV2Bin }),
          },
        });
    ...
    const resultsDir = process.env.OMNIRUSH_BENCH_RESULTS_DIR ?? tmpdir();
    await mkdir(resultsDir, { recursive: true });
    const resultsPath = join(resultsDir, `bench-omnirush-app-${benchEngine}-${Date.now()}.json`);
    await writeFile(resultsPath, `${JSON.stringify(report, null, 2)}\n`);
    console.info(`[bench-omnirush-app-${benchEngine}] medians\n${medianTable(results)}`);
    console.info(`[bench-omnirush-app-${benchEngine}] results=${resultsPath} uiCompactionAvailable=${uiCompactionAvailable}`);
});
