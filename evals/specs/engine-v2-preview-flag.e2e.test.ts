...
import { createAndSelectWorkspace, evalIn, go, waitFor } from "@omnirush/behaviors";
import { desktop } from "@omnirush/hosts";
import { needs, test } from "@omnirush/testkit";
...
const title = enabled
  ...
  : "OpenCode v2 engine preview flag skipped — needs: set OMNIRUSH_EVAL_E2E_TESTS=1";
...
test.skipIf(!enabled)(title, async ({ evidence, place }) => {
  ...
  const profileDir = place.kind === "local"
    ? await mkdtemp(join(tmpdir(), "omnirush-engine-v2-preview-eval-"))
    : undefined;
  ...
  try {
    app = await desktop({
      ...
      env: binPath === undefined ? {} : { OMNIRUSH_OPENCODE2_BIN: binPath },
    });
    ...
    const mirroredStatus = await untilStatus(
      app,
      (status) => status.mirroredProviderIds.includes("omnirush-witness-e2e"),
      60_000,
    ...
    expect(mirroredStatus.skippedProviderIds).toContain("omnirush-skip-e2e");
    expect(mirroredStatus.mirroredProviderIds).not.toContain("omnirush-skip-e2e");
    expect(mirroredStatus.skippedProviderIds).toContain("omnirush-untrusted-api-e2e");
    expect(mirroredStatus.mirroredProviderIds).not.toContain("omnirush-untrusted-api-e2e");
