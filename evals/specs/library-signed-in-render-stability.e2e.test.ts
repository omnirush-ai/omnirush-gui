...
import { app, needs, server, test } from "@omnirush/testkit";
...
const title = enabled
  ...
  : "signed-in Library stability skipped — needs: set OMNIRUSH_EVAL_E2E_TESTS=1";
...
test.skipIf(!enabled)(title, { timeout: 10 * 60_000 }, async ({ evidence, place }) => {
  ...
  const instrumented = await evalIn(desktopApp, browserScript((value) => {
    ...
    window.addEventListener("omnirush-den-settings-changed", () => {
  ...
  await waitFor(
    ...
    () => (document.body.innerText.includes("READY TO USE")
      && document.body.innerText.includes("OmniRush.ai Browser")),
