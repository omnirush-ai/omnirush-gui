...
import { clickButton, control, createAndSelectWorkspace, evalIn, go, renameSessionAndWait, waitFor } from "@omnirush/behaviors";
import type { Surface } from "@omnirush/cdp";
import { desktop } from "@omnirush/hosts";
import { needs, resolveEvalEngine, test } from "@omnirush/testkit";
...
const title = enabled
  ...
  : "OpenCode v2 chat routing skipped — needs: set OMNIRUSH_EVAL_E2E_TESTS=1";
const keyV1 = "key-v1";
const keyV2 = "key-v2";
...
async function serverFetchJson(
  ...
  const value = await evalIn(app, browserScript(async (path, value, inputValue, timeoutMs) => {
    ...
    const token = (localStorage.getItem("omnirush.server.token") ?? "").trim();
...
test.skipIf(!enabled)(title, { timeout: 600_000 }, async ({ evidence, place, skip }) => {
  ...
  const binPath = place.kind === "local" ? process.env.OMNIRUSH_EVAL_OPENCODE2_BIN?.trim() || undefined : undefined;
  ...
  const witness = createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      const nonce = titleRequest ? generatedTitle : `OMNIRUSH-V2-ROUTING-NONCE-${witnessRequests.length + 1}`;
  ...
  const profileDir = place.kind === "local"
    ? await mkdtemp(join(tmpdir(), "omnirush-v2-chat-routing-eval-"))
    : undefined;
  ...
  try {
    app = await desktop({
      ...
      env: {
        ...
        OPENCODE_PURE: "true",
        ...(binPath === undefined ? {} : { OMNIRUSH_OPENCODE2_BIN: binPath }),
        ANTHROPIC_API_KEY: "",
        OPENAI_API_KEY: "",
        OPENROUTER_API_KEY: "",
    ...
    if (harnessLaneStatus !== undefined) {
      evidence.recordAssertionEvidence(
        ...
        `OMNIRUSH_EVAL_ENGINE=v2 started the app with preview enabled, chat routing enabled, and sidecar pid ${harnessLaneStatus.pid}; the spec had not changed either preview setting.`,
        true,
      );
    }
    if (place.kind === "daytona") {
