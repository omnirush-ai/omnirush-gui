...
test("claim builder supports present, absent, and unexpected group claim shapes", () => {
  ...
  assert.deepEqual(present.groups, ["Engineering", "OmniRush.ai Lab"]);
  assert.equal("groups" in absent, false);
  assert.deepEqual(odd.groups, { primary: "Engineering", all: ["Engineering", "OmniRush.ai Lab"] });
});
...
test("blocked-user response uses the Entra-style policy wording", () => {
  ...
  assert.match(response.html, /OmniRush.ai Mock IdP policy/);
});
