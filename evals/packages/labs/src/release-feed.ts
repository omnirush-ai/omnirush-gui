...
const DEFAULT_REPO = "omnirush-ai/omnirush-gui";
...
export function desktopReleaseAssetFileName(
  ...
  const prefix = distribution === "public" ? "omnirush" : `omnirush-${distribution}`;
...
function assetBody(input: ReleaseAssetInput, version: string): Uint8Array {
  ...
  return textEncoder.encode(`omnirush release ${version} ${input.platform}\n`);
}
