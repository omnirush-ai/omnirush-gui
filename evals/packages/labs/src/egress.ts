...
export function opensslCertificateCommands(input: OpenSslCommandInput): OpenSslCommand[] {
  ...
  const rootSubject = input.corporateIssuer
    ? "/CN=OmniRush.ai Egress Lab Corporate Root CA"
    : "/CN=OmniRush.ai Egress Lab Root CA";
  const intermediateSubject = input.corporateIssuer
    ? "/CN=OmniRush.ai Egress Lab Corporate Interception CA"
    : "/CN=OmniRush.ai Egress Lab Intermediate CA";
  return [
...
function opensslBinary(env: NodeJS.ProcessEnv = process.env): string {
  return env.OMNIRUSH_EVAL_OPENSSL?.trim() || "openssl";
}
...
async function generateCertificateMaterial(input: { hostname: string; aiaUrl: string | null; corporateIssuer: boolean }): Promise<CertificateMaterial> {
  const dir = await mkdtemp(path.join(tmpdir(), "omnirush-egress-lab-"));
  ...
  if (method === "initialize") {
    jsonResponse(response, 200, {
      ...
      result: {
        ...
        serverInfo: { name: "omnirush-egress-lab", version: "1.0.0" },
      },
    }, {
...
export async function stageTrustedEnterpriseTlsRoot(
  ...
  try {
    stagingDir = await mkdtemp(path.join(tmpdir(), "omnirush-enterprise-tls-trust-"));
  } catch (error) {
...
export function linuxTrustStorePlan(rootPemPath: string): LinuxTrustStorePlan {
  const certificatePath = "/usr/local/share/ca-certificates/omnirush-enterprise-tls-edge.crt";
...
export async function writeTempPemBundle(pems: string[]): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "omnirush-egress-ca-bundle-"));
