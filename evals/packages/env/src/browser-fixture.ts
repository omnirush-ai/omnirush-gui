...
if(location.pathname!='/fallback'){
  ...
  if(location.pathname==='/execution-delay'){
    ...
    document.modelContext.getTools=async(...args)=>{
      ...
      const pending=window[Symbol.for('omnirush.webmcp.pending-executions')];
...
(async()=>{
  ...
  const direct=await window.__omnirushWebMcpPolicyV1.check();
  const forged=await window.__omnirushWebMcpPolicyV1.check({originAgentCluster:true,domainMatchesHost:true});
