import type { CdpClient, BrowserEvaluation } from "@omnirush/cdp";
 ...
 *   - MEMBER: the OmniRush.ai desktop app in Electron (CDP :9823) — signed into
...
import { connect, evaluate, captureScreenshot, pickAppTarget, debuggerUrlFor, listTargets } from "@omnirush/cdp";
...
/** Pick the den-web admin page target (title "OmniRush.ai Cloud"). */
async function connectAdmin() {
  const targets = await listTargets(ADMIN_CDP);
  const page = targets.find((t) => t.type === "page" && t.title.includes("OmniRush.ai Cloud"))
...
async function memberRefreshAndWait(member: CdpClient, condition: BrowserEvaluation, label: string, timeoutMs = 25000) {
  await evaluate(member, () => (window.dispatchEvent(new CustomEvent('omnirush-den-settings-changed', { detail: {} }))));
  await evaluate(member, () => (window.dispatchEvent(new CustomEvent('omnirush-den-session-updated', { detail: {} }))));
...
async function main() {
  ...
  await evaluate(member, () => (localStorage.setItem('omnirush.react.settings.theme-mode','light')));
  ...
  // =================================================================
  // JOURNEY 1: admin sets OmniRush.ai demo logo in the web UI → member sees it
  // =================================================================
  console.log("\nJourney 1: admin sets logo via web UI");
  await adminSetLogoViaUI(admin, DEMO_LOGO);
  await sleep(500);
  await shot(admin, "admin", "01-logo-typed",
    "Admin: typed OmniRush.ai demo logo URL into the Logo URL field.",
  ...
  await memberRefreshAndWait(member,
    ...
    "OmniRush.ai demo logo loaded in member app");
  ...
  await shot(member, "member", "01-logo-appeared",
    "Member: app fetched the change and rendered the OmniRush.ai demo logo at a legible size (no reload).",
    [
      { label: "OmniRush.ai demo logo rendered", passed: await evaluate(member, () => (Boolean(document.querySelector<HTMLImageElement>('[data-testid="brand-logo"] img')))) },
  ...
  // (badge + unread dot) together with the OmniRush.ai demo logo top-left.
  await evaluate(member, () => (window.location.hash = '#/session'));
  await sleep(800);
  await evaluate(member, () => {
    // Use the public notify path if exposed; otherwise write an unread entry.
    try {
      const raw = localStorage.getItem('omnirush:notifications:v1');
      ...
      localStorage.setItem('omnirush:notifications:v1', JSON.stringify(data));
    } catch {}
    return true;
  });
  ...
  await shot(member, "member", "02-accent-applied",
    "Member: accent switched to blue — visible on the notification badge + unread dots, with the OmniRush.ai demo logo top-left.",
    [
      ...
      { label: "OmniRush.ai demo logo still shown", passed: await evaluate(member, () => (Boolean(document.querySelector<HTMLImageElement>('[data-testid="brand-logo"] img')))) },
    ]);
