...
export function engineBinary(): string | null {
  const fromEnv = process.env.OMNIRUSH_OPENCODE_BIN?.trim();
...
/**
 * Spawns the omnirush-server CLI with a managed engine. Every stdout/stderr
...
export function bootServer(env: NodeJS.ProcessEnv, token: string, workspace: string, sink: (chunk: string) => void): { child: ChildProcess; listening: Promise<string> } {
  ...
  const listening = new Promise<string>((resolveBase, reject) => {
    const timer = setTimeout(() => reject(new Error(`omnirush-server did not report a port within 60s:\n${seen.slice(-2_000)}`)), 60_000);
    const onChunk = (chunk: Buffer) => {
      const text = chunk.toString("utf8");
      seen += text;
      sink(text);
      const match = seen.match(/OmniRush.ai server listening on (http:\/\/127\.0\.0\.1:\d+)/);
      if (match?.[1]) {
        clearTimeout(timer);
        resolveBase(match[1]);
      }
    ...
    child.on("exit", (code) => {
      clearTimeout(timer);
      reject(new Error(`omnirush-server exited early (code ${code}):\n${seen.slice(-2_000)}`));
    });
...
export interface ManagedOmniRushServer {
  child: ChildProcess;
  base: string;
  binary: string;
  workspaceId: string;
  ...
  try {
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      try {
        ...
        if (attempt >= 2 || !(error instanceof Error && error.message.startsWith("omnirush-server exited early"))) throw error;
      }
    }
    ...
    if (!workspaceId) throw new Error(`omnirush-server reported no workspace: ${JSON.stringify(workspaces)}`);
    ...
    if (!ready) throw new Error(`The managed engine never answered through omnirush-server:\n${output.slice(-3_000)}`);
