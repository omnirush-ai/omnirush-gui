...
import { app as startApp, faultProxy as startFaultProxy, resolveEvalEngine, SkipError } from "@omnirush/env";
import type { Den, MockHandle, Seed } from "@omnirush/env";
import { denFetch, evalIn as rawEvalIn } from "@omnirush/behaviors";
import type { DenFetchResult, DenSession } from "@omnirush/behaviors";
import { allocateFreePort } from "@omnirush/cdp";
import { startMockMcp } from "@omnirush/labs";
import { captureExternalBrowserUrls, electronProfilePaths } from "@omnirush/hosts";
...
async function mintMcpSession(seed: Seed, den: Den, organizationId: string): Promise<DenSession> {
  const result = await seed.api(den.admin, "/v1/mcp/token", {
    method: "POST",
    headers: { "x-omnirush-org-id": organizationId },
...
export const runtimeGenerationExpression = async () => {
  const invokeDesktop = window.__OMNIRUSH_ELECTRON__?.invokeDesktop;
  if (!invokeDesktop) return { running: false, baseUrl: "", generation: null };
  const info = await invokeDesktop("omnirushServerInfo");
  return {
...
export const cloudHealthExpression = (workspaceId: string) => {
    ...
    const request = new XMLHttpRequest();
    request.open("GET", "http://127.0.0.1:" + port + "/workspace/" + encodeURIComponent(workspaceId) + "/mcp/omnirush-cloud/health?probe=1", false);
    request.setRequestHeader("Authorization", "Bearer " + token);
    request.send();
    return JSON.parse(request.responseText || "{}");
  };

export async function connectPolicyRuntimeRestart(seed: Seed, { place }: { place: import("@omnirush/env").Place }) {
  const stamp = Date.now();
  const den = await seed.den({
    org: {
      name: "Connect Policy Convergence",
      admin: { email: `connect-policy-admin-${stamp}@omnirush.test`, name: "Connect Policy Admin" },
      members: { fresh: { email: `connect-policy-member-${stamp}@omnirush.test`, name: "Fresh Profile Member" } },
    },
  });
...
export async function clearConnectStateFiles(app: Awaited<ReturnType<typeof startApp>>): Promise<void> {
  ...
  const candidates = [
    `${paths.userDataDir}/omnirush-dev-data/xdg/config/omnirush/connect-state.json`,
    `${paths.configHome}/omnirush/connect-state.json`,
    `${paths.homeDir}/.config/omnirush/connect-state.json`,
  ];
...
export async function connectStateProvenance(seed: Seed) {
  ...
  const den = await seed.den({
    org: {
      ...
      admin: { email: `connect-state-admin-${stamp}@omnirush.test`, name: "Connect State Admin" },
      members: { fresh: { email: `connect-state-member-${stamp}@omnirush.test`, name: "Fresh Profile Member" } },
    },
  });
...
export async function preseededConnect(seed: Seed) {
  ...
  const provider = await seed.api(den.admin, "/v1/llm-providers", {
    ...
    body: JSON.stringify({
      ...
      apiKey: "sk-omnirush-connect-eval-only",
...
export async function connectorBranding(seed: Seed) {
  ...
  const steps = (name: string) => engine === "v2" ? [{
    tool: "execute",
    arguments: { code: `
      const found = await tools["omnirush-cloud"].search_capabilities(${JSON.stringify(search(name))});
      ...
      return await tools["omnirush-cloud"].execute_capability({ name: catalog.matches[0].name, body: ${JSON.stringify(toolArguments)} });
    ` },
  }] : [
...
export async function librarySignedInStability(seed: Seed) {
  ...
  await seed.evalIn(app, browserScript((workspaceId) => {
    ...
    window.addEventListener("omnirush-den-settings-changed", () => { window.__libraryStability.denEvents += 1; });
...
export async function largeSkill(seed: Seed) {
  ...
  const organizationId = await activeOrganizationId(seed, den.admin);
  const headers = { "x-omnirush-org-id": organizationId };
...
export async function capabilitySearchLatency(seed: Seed, { place }: { place: import("@omnirush/env").Place }) {
...
export async function libraryView(seed: Seed) {
  const password = process.env.OMNIRUSH_EVAL_DEMO_PASSWORD?.trim() || "OmniRushDemo123!";
  const caseyEmail = process.env.OMNIRUSH_EVAL_CREATOR_EMAIL?.trim() || "casey.spec@acme.test";
  const novaEmail = process.env.OMNIRUSH_EVAL_MEMBER_EMAIL?.trim() || "nova.spec@acme.test";
  const den = await seed.den({
    reuseMembers: {
    ...
    mocks: {
      connector: seed.mock({ publicUrl: process.env.OMNIRUSH_EVAL_LIBRARY_MOCK_PUBLIC_URL?.trim() || undefined }),
    },
  });
  const organizationId = await activeOrganizationId(seed, den.admin);
  const stamp = Date.now();
  ...
  const org = await seed.api(den.admin, "/v1/org", { headers: { "x-omnirush-org-id": organizationId } });
  ...
  return withDispose({
    ...
    const headers = {
      ...
      "x-omnirush-org-id": organizationId,
    };
...
async function configureWorkspaceModel(seed: Seed, input: {
  app: import("@omnirush/cdp").Surface;
  workspaceId: string;
  providerId: string;
  modelId: string;
  fixtureUrl: string;
  ...
  const result = await rawEvalIn(input.app, browserScript(async (inputWorkspaceId, providerId, value, modelId, inputValue, inputValue2, inputProviderId, inputModelId, inputValue3) => {
    ...
    if (inputValue2) {
      const reconcile = await request("/workspace/" + encodeURIComponent(workspaceId) + "/mcp/omnirush-cloud/reconcile", {
    ...
    localStorage.setItem("omnirush.defaultModel", inputValue3);
    localStorage.removeItem("omnirush.sessionModels." + workspaceId);
    return "ok";
...
async function reloadConfiguredApp(app: import("@omnirush/cdp").Surface): Promise<void> {
  // TODO(primitive): seed.reloadConfiguredDesktop
  await rawEvalIn(app, () => { location.reload(); return true; }).catch(() => undefined);
  const deadline = Date.now() + 60_000;
  while (Date.now() < deadline) {
    if (await rawEvalIn(app, () => (Boolean(window.__omnirushControl))).catch(() => false) === true) return;
...
export const skillCreatedResourceUri = "ui://omnirush/skill-created/v1/view.html";
...
export const inlineResourceUri = "ui://omnirush/artifacts/arv_eval_card/views/avr_eval_card/index.html";
...
export async function remoteMcpApps(seed: Seed) {
  ...
  const builder = await import("../../ee/apps/den-api/src/generated-artifact-view-builder.js");
  const built = await builder.buildGeneratedArtifactViewInWorker({
    reactSource: `export default function ProjectAtlas(props) { const app = props.data || { name: "Project Atlas", status: "Connected through OmniRush.ai Connect" }; return <main><h1>{app.name}</h1><p>{app.status}</p></main> }`,
  ...
  const standardRpc = (message: Record<string, unknown>): Record<string, unknown> => {
    if (message.method === "initialize") return {
      ...
      result: {
        ...
        serverInfo: { name: "project-atlas-connect-fixture", title: "Project Atlas Connect", version: "1.0.0", description: "A standard MCP App fixture served through OmniRush.ai Connect." },
      },
    };
    if (message.method === "tools/list") return {
      jsonrpc: "2.0",
    ...
    if (message.method === "tools/call") {
      ...
      if (params.name === "open_project_atlas") return {
        ...
        result: {
          ...
          structuredContent: {
            schemaVersion: "1",
            artifact: { title: "Project Atlas", description: "A standard MCP App served through OmniRush.ai Connect." },
            data: { name: "Project Atlas", status: "Connected through OmniRush.ai Connect" },
          },
  ...
  const fixtureUrl = await listen(fixture);
  const profileDir = `/tmp/omnirush-remote-mcp-apps-profile-${Date.now()}`;
  try {
