import { and, asc, eq, gt, lte, or, sql } from "@omnirush-ee/den-db/drizzle"
import type { Hono } from "hono"
import { InferenceKeyTable, InferenceOrgLimitPolicyTable, InferenceUsageLedgerBucketChargeTable, InferenceUsageLedgerEntryTable, InferenceOrgUsageBucketTable } from "@omnirush-ee/den-db"
...
import { INFERENCE_USAGE_CONVERSION_FACTOR, INFERENCE_WINDOW_TYPES } from "@omnirush/types/den/inference"
...
function parseSpan(span: JsonRecord, attrs: JsonRecord): ParsedSpan | null {
  ...
  const omnirushRequestId = stringAttr(attrs, ["trace.metadata.omnirush_request_id", "trace.omnirush_request_id", "metadata.omnirush_request_id", "omnirush_request_id", "trace_id"])
  ...
  if (!orgMembershipId || !inferenceKeyId || !omnirushRequestId || !reportedModel) {
    return null
  }
  ...
  if (!occurredAt || [omnirushRequestId, reportedModel, externalEventId].some((value) => value !== null && value.length > 255)) return null
  ...
  unknownModel(report) {
    Sentry.captureMessage("OpenRouter usage webhook could not infer cost for reported model", {
      ...
      tags: {
        ...
        omnirush_request_id: report.omnirushRequestId,
...
const defaultWebhookDependencies: WebhookDependencies = {
  ...
  async settleUsage({ inferenceKey, span, costAmount }) {
    return db.transaction(async (tx) => {
      ...
      const identity = or(
        and(eq(InferenceUsageLedgerEntryTable.external_job_id, span.omnirushRequestId), eq(InferenceUsageLedgerEntryTable.event_type, "openrouter_usage")),
      ...
      const matchesIdentity = (entry: typeof InferenceUsageLedgerEntryTable.$inferSelect) =>
        ...
        entry.inference_key_id === inferenceKey.id && entry.external_job_id === span.omnirushRequestId && entry.event_type === "openrouter_usage"
      ...
      if (!existing[0]) {
        await tx.insert(InferenceUsageLedgerEntryTable).values({
          ...
          inference_key_id: inferenceKey.id, external_job_id: span.omnirushRequestId, external_event_id: span.externalEventId,
...
function reportUnknownPricedModel(input: { span: ParsedSpan; inferenceKey: WebhookInferenceKey; reporter: OpenRouterUsageWebhookReporter }) {
  logWebhookError("retained unpriced provider usage", {
    ...
    omnirushRequestId: input.span.omnirushRequestId,
  ...
  app.post("/webhooks/openrouter", async (c) => {
    ...
    for (const span of spans) {
      try {
        ...
        failed += 1
        logWebhookError("Usage persistence failed; provider must retry", { requestId: span.omnirushRequestId })
      }
    }
