...
export type InferenceReporter = {
  ...
  completion?(report: ChatCompletionReport & { omnirushRequestId: string; organizationId: string; orgMembershipId: string; modelAlias: string }): void
}
...
function reportAttributes(report: InferenceRequestReport | InferenceHandledErrorReport) {
  return {
    ...
    omnirushRequestId: report.omnirushRequestId,
    route: report.route,
...
export const sentryInferenceReporter: InferenceReporter = {
  completion(report) {
    if (shouldEmitSentryLog(report.outcome === "completed" ? "info" : "error")) {
      if (report.outcome === "completed") Sentry.logger.info("OmniRush.ai inference completion", report)
      else Sentry.logger.error("OmniRush.ai inference completion", report)
    }
  },
  request(report) {
    ...
    Sentry.logger.info("OmniRush.ai chat completions inference request", {
  ...
  handledError(report) {
    ...
    if (shouldEmitSentryLog("error")) {
      Sentry.logger.error("OmniRush.ai inference handled error", attributes)
    }
    if (report.exception === undefined) {
      Sentry.captureMessage(`OmniRush.ai inference handled error: ${report.reason}`, {
        level: "error",
