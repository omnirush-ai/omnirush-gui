...
export function upstreamError(status: number) {
  if (status === 401 || status === 403) return inferenceError("upstream_access_denied", "The managed model provider could not authorize this request. Ask your organization admin to check OmniRush.ai Models access.")
  if (status === 402) return inferenceError("upstream_quota_exhausted", "The managed provider's allowance is exhausted. Ask your organization admin to check OmniRush.ai Models access.")
