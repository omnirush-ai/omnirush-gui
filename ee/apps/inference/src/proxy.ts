...
import { inferenceBearerKey } from "@omnirush-ee/utils/inference-bearer-key"
...
function sanitizeHeaders(request: Request, apiKey: string, omnirushRequestId: string) {
  const headers = new Headers()
  const accept = request.headers.get("accept")
  if (accept) headers.set("accept", accept)
  headers.set("authorization", `Bearer ${apiKey}`)
  headers.set("content-type", "application/json")
  headers.set("x-omnirush-request-id", omnirushRequestId)
  if (env.proxyBaseUrl) {
    headers.set("http-referer", env.proxyBaseUrl)
  }
  headers.set("x-title", "OmniRush.ai Inference")
  return headers
}
...
async function logUpstreamError(input: {
  ...
  input.reporter.handledError({
    ...
    omnirushRequestId: input.omnirushRequestId,
    route: input.route,
    method: input.method,
...
async function prepareBody(request: Request, input: {
  ...
  if (blockedSelection) {
    ...
    return { error: openAiError(400, "unsupported_model_selection", `OmniRush.ai inference does not allow alternate model selection (${blockedSelection}).`), incomingModel: model ? model.alias : null, upstreamModel: model ? model.upstreamModel : null }
  }
  ...
  const body = json
  if (!model) {
    logProxyError("Unknown OmniRush.ai model alias", {
      omnirushRequestId: input.omnirushRequestId,
      organizationId: input.organizationId,
      orgMembershipId: input.orgMembershipId,
      inferenceKeyId: input.inferenceKeyId,
    ...
    return { error: openAiError(404, "model_not_found", `Unknown OmniRush.ai model alias: ${requestedModel}`), incomingModel: null, upstreamModel: null }
  }

  body.model = model.upstreamModel
  body.user = input.orgMembershipId
  body.session_id = input.omnirushRequestId
  const trace = {
    trace_id: input.omnirushRequestId,
    trace_name: "OmniRush.ai Inference",
    generation_name: model.alias,
    org_membership_id: input.orgMembershipId,
    inference_key_id: input.inferenceKeyId,
    omnirush_request_id: input.omnirushRequestId,
  }
...
function localRouteRejection(path: string, method: string) {
  ...
  return openAiError(404, "not_found", `Unsupported OmniRush.ai inference route: ${method} ${path}.`)
}
...
export function registerProxyRoutes(app: Hono, dependencies: ProxyDependencies = defaultProxyDependencies) {
  ...
  async function handleApiRequest(c: Context) {
    const omnirushRequestId = buildRequestId()
    c.header("x-omnirush-request-id", omnirushRequestId)
    ...
    if (!bearerKey) {
      ...
      return c.json({ error: { message: "Missing OmniRush.ai inference API key.", type: "authentication_error", code: "missing_api_key" } }, 401)
    }
    ...
    if (!inferenceKey) {
      ...
      return c.json({ error: { message: "Invalid OmniRush.ai inference API key.", type: "authentication_error", code: "invalid_api_key" } }, 401)
    }
    ...
    if (new URL(c.req.url).search) {
      ...
      return openAiError(400, "unsupported_query_parameters", "OmniRush.ai chat completions does not accept query parameters.")
    }
    ...
    if (!limits.ok) {
      c.header("x-omnirush-limit-bucket-id", limits.limitedBy)
      c.header("x-omnirush-limit-window-type", limits.windowType)
    ...
    const analytics = await Promise.race([
      (async () => {
        const begin = await dependencies.analytics?.({ key: inferenceKey, request: c.req.raw, requestId: omnirushRequestId, model: prepared.upstreamModel, startedAt })
    ...
    try {
      const upstreamInit: ProxyRequestInit = {
        method: c.req.method,
        headers: sanitizeHeaders(c.req.raw, providerKey.encrypted_api_key, omnirushRequestId),
    ...
    const headers = new Headers({ "x-omnirush-request-id": omnirushRequestId, "cache-control": "no-store" })
    ...
    if (prepared.stream) {
      ...
      return new Response(relayChatStream({
        ...
        onFinish(result) {
          ...
          try {
            reporter.completion?.({ ...result, omnirushRequestId, organizationId: inferenceKey.organization_id, orgMembershipId: inferenceKey.org_membership_id, modelAlias: prepared.modelAlias })
