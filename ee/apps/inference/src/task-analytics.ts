import { readModelsAnalyticsSettings, appendModelsAnalyticsEvents, type ModelsAnalyticsEvent } from "@omnirush-ee/telemetry"
...
export async function beginModelAnalytics(input: { key: Key; request: Request; requestId: string; model: string | null; startedAt: number }) {
  ...
  return (streaming: boolean) => observeModelResponse({
    id: input.requestId,
    sessionId: identifier(input.request.headers.get("x-omnirush-session-id"), input.requestId),
    taskId: identifier(input.request.headers.get("x-omnirush-task-id"), input.requestId),
