...
process.env.DATABASE_URL = "mysql://root:password@127.0.0.1:3306/omnirush_den"
...
function inferenceRequest(input: { method: string; headers: Headers; body?: string; path?: string }) {
  return new Request(`http://omnirush.test${input.path ?? "/api/v1/chat/completions"}`, {
    method: input.method,
...
for (const streaming of [false, true]) {
  for (const completed of [false, true]) {
    test(`${streaming ? "streamed" : "JSON"} analytics requires managed protocol completion (${completed})`, async () => {
      const { observeModelResponse } = await import("../src/task-analytics.js")
      const events: import("@omnirush-ee/telemetry").ModelsAnalyticsEvent[] = []
      const payload = {
      ...
      assert.equal(events[0].id, response.headers.get("x-omnirush-request-id"))
...
test("rewrites approved model aliases before forwarding JSON requests", async () => {
  ...
  const response = await app.fetch(inferenceRequest({
    ...
    body: JSON.stringify({ model: "omnirush/z-ai/glm-5.2", messages: [{ role: "user", content: "Hello" }] }),
  }))
  ...
  assert.equal(body.session_id, upstream.headers.get("x-omnirush-request-id"))
  ...
  assert.equal(report.omnirushRequestId, upstream.headers.get("x-omnirush-request-id"))
...
test("returns model_not_found for unknown JSON model aliases", async () => {
  ...
  const response = await app.fetch(inferenceRequest({
    method: "POST",
    headers: authHeaders("application/json"),
    body: JSON.stringify({ model: "omnirush/unknown-model", messages: [{ role: "user", content: "Hello" }] }),
  }))
...
test("returns usage-limit 429 without reporting a handled error or contacting provider/upstream", async () => {
  ...
  try {
    ...
    assert.equal(response.headers.get("x-omnirush-limit-bucket-id"), "bucket_123")
    assert.equal(response.headers.get("x-omnirush-limit-window-type"), "monthly")
...
test("reports handled upstream errors with searchable request context", async () => {
  ...
  assert.equal(errorReport.omnirushRequestId, requestReport.omnirushRequestId)
...
test("accepts application/*+json media types", async () => {
  ...
  const response = await app.fetch(inferenceRequest({
    method: "POST",
    headers: authHeaders("application/vnd.omnirush.request+json; charset=utf-8"),
    body: JSON.stringify({ model: "omnirush/z-ai/glm-5.2", messages: [{ role: "user", content: "Hello" }] }),
  }))
...
test("returns the authenticated local model catalog without forwarding", async () => {
  ...
  assert.ok(!model.id.startsWith("omnirush/"))
  assert.equal(calls.findActiveInferenceKey, 1)
  assert.equal(calls.ensureUsableBuckets, 0)
  assert.equal(calls.getOpenRouterProviderKey, 0)
  assert.equal(upstreamRequests.length, 0)
