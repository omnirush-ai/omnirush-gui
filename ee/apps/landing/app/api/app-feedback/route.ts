...
import { EmailSendError, sendEmail, type FeedbackEmailProps } from "@omnirush/email";
...
function sanitizeContext(input: FeedbackContext | undefined) {
  return {
    ...
    omnirushServerVersion: sanitizeValue(input?.omnirushServerVersion),
...
function formatDiagnosticsSummary(context: ReturnType<typeof sanitizeContext>) {
  ...
  const lines = [
    ...
    ["OmniRush.ai server", context.omnirushServerVersion],
...
export async function POST(request: Request) {
  ...
  const internalEmail =
    process.env.OMNIRUSH_FEEDBACK_EMAIL?.trim() ||
