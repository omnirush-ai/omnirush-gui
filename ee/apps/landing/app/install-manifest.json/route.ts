...
//
// Resolves the latest OmniRush.ai desktop release on GitHub and returns an install
// manifest in the shape the `omnirush-bootstrap install app` command expects:
//
...
function isDesktopAppAsset(name: string): boolean {
  ...
  if (!lower.startsWith("omnirush")) return false;
  ...
  for (const asset of release.assets) {
    ...
    if (!existing || pref > existing._pref) {
      artifacts[target.platform][target.arch] = {
        ...
        ...(type === "dmg" ? { appName: "OmniRush.ai.app" } : {}),
      };
    }
  }
  ...
  try {
    const response = await fetch(RELEASES_API, {
      headers: {
        ...
        "user-agent": "omnirush-install-manifest",
      },
      next: { revalidate: 300 },
    });
    if (!response.ok) {
    ...
    // OmniRush.ai desktop installer, not sidecar or CLI bundles.
