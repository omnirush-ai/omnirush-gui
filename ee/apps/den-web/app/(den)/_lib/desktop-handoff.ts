export const LAST_DESKTOP_HANDOFF_GRANT_STORAGE_KEY = "omnirush.den.lastHandoffGrant";
...
export function getDesktopHandoffOmniRushUrl(payload: unknown): string | null {
  if (!isRecord(payload)) {
    return null;
  }

  const omnirushUrl = payload.omnirushUrl;
  return typeof omnirushUrl === "string" && omnirushUrl.trim() ? omnirushUrl.trim() : null;
}

export function getDesktopHandoffGrant(payload: unknown, omnirushUrl: string | null): string | null {
  if (isRecord(payload) && typeof payload.grant === "string" && payload.grant.trim()) {
    return payload.grant.trim();
  }

  return getDesktopGrant(omnirushUrl);
}
