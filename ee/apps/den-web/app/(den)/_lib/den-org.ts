...
export const PENDING_ORG_INVITATION_STORAGE_KEY = "omnirush:web:pending-org-invitation";
export const PENDING_WORKSPACE_CLAIM_STORAGE_KEY = "omnirush:web:pending-workspace-claim";
...
function parseOrgCapabilities(value: unknown): DenOrgCapabilities {
  if (!isRecord(value)) {
    return { orgManagedDashboards: false, installLinks: false, mcpConnections: false, workflows: true, omnirushWeb: false, cloud: false };
  }
  ...
  return {
    ...
    omnirushWeb: value.omnirushWeb === true,
...
export function parseInvitationPreviewPayload(payload: unknown): DenInvitationPreview | null {
  ...
  return {
    ...
    organization: {
      ...
      allowedEmailDomains: asStringArray(organization.allowedEmailDomains),
      branding: {
        appName: asString(branding?.appName) ?? "OmniRush.ai",
