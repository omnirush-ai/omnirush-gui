...
import {
  getOmniRushWebQuantityDescription,
  OMNIRUSH_WEB_CHECKOUT_TYPE,
  OMNIRUSH_WEB_QUANTITY_EXPLANATION,
...
export function isExistingWebAccessResponse(response: Response, payload: unknown): boolean {
  return response.status === 409
    && Boolean(
      ...
      && "error" in payload
      && (payload.error === "stripe_subscription_exists" || payload.error === "omnirush_web_complimentary_access_exists"),
    );
}
...
export function WebOpenButton({ omnirushWebUrl }: { omnirushWebUrl: string }) {
  return (
    <DenButton href={omnirushWebUrl} target="_blank" rel="noopener noreferrer" icon={ExternalLink}>
...
export function WebPurchaseButton({ disabled, loading, onClick }: { disabled: boolean; loading: boolean; onClick: () => void }) {
  return (
    <DenButton disabled={disabled} loading={loading} onClick={onClick}>
      Purchase OmniRush.ai Web — $50 per member/month
    </DenButton>
  );
}
...
export default function WebPage() {
  ...
  const webAvailable = runtimeConfigLoaded
    && orgContext?.capabilities.omnirushWeb === true;
  ...
  async function requestWebBilling(expectedOrgId: string, quiet: boolean): Promise<StripeWebBilling | null> {
    ...
    try {
      ...
      if (!response.ok) throw new Error(getErrorMessage(payload, `OmniRush.ai Web billing lookup failed (${response.status}).`));
      const parsed = parseStripeWebBilling(payload);
      if (!parsed) throw new Error("OmniRush.ai Web billing response was incomplete.");
      ...
      if (!quiet && mountedRef.current && currentOrgIdRef.current === expectedOrgId) {
        setErrorRecord({
          orgId: expectedOrgId,
          message: error instanceof Error ? error.message : "Could not load OmniRush.ai Web billing.",
        });
      }
      return null;
    }
  ...
  useEffect(() => {
    ...
    if (params.get("stripe_checkout") !== OMNIRUSH_WEB_CHECKOUT_TYPE) return;
      ...
      try {
        ...
        if (!syncPromise) {
          syncPromise = (async () => {
            ...
            await runReauthableAction("omnirush-web-checkout-sync", async () => {
              const { response, payload } = await requestJson(
                "/v1/billing/stripe/checkout/sync",
                {
                  method: "POST",
                  headers: { [ORG_SCOPE_HEADER]: expectedOrgId },
                  body: JSON.stringify({ sessionId, type: OMNIRUSH_WEB_CHECKOUT_TYPE }),
                },
                12000,
              );
        ...
        if (!stopped && mountedRef.current && currentOrgIdRef.current === expectedOrgId) {
          ...
          setErrorRecord({
            orgId: expectedOrgId,
            message: error instanceof Error ? error.message : "Could not confirm this checkout. OmniRush.ai Web remains locked.",
          });
          clearStripeReturnParameters();
        }
        return;
  ...
  async function startWebCheckout() {
    ...
    if (!canPurchaseWeb) {
      setErrorRecord({ orgId, message: "Ask a workspace owner or admin to purchase OmniRush.ai Web for this organization." });
      return;
    }
    if (!billing?.configured) {
      setErrorRecord({ orgId, message: "OmniRush.ai Web billing is not configured for this deployment." });
      return;
    }
    ...
    setErrorRecord(null);
    try {
      await runReauthableAction("omnirush-web-checkout", async () => {
        const { response, payload } = await requestJson(
          "/v1/billing/stripe/checkout",
          {
            method: "POST",
            headers: { [ORG_SCOPE_HEADER]: orgId },
            body: JSON.stringify({ type: OMNIRUSH_WEB_CHECKOUT_TYPE }),
          },
          12000,
        );
        ...
        if (!response.ok) throw getRequestError(payload, response, `OmniRush.ai Web checkout failed (${response.status}).`);
        const url = payload && typeof payload === "object" && "url" in payload && typeof payload.url === "string" ? payload.url : null;
        if (!url) throw new Error("OmniRush.ai Web checkout response did not include a URL.");
      ...
      if (mountedRef.current && currentOrgIdRef.current === orgId) {
        setErrorRecord({ orgId, message: error instanceof Error ? error.message : "Could not start OmniRush.ai Web checkout." });
      }
    } finally {
  ...
  return (
    <DashboardPageTemplate
      ...
      description="Use OmniRush.ai in your browser with an organization subscription."
      colors={["#EFF6FF", "#0F172A", "#2563EB", "#BAE6FD"]}
    >
      <div data-testid="omnirush-web-access" data-access-state={accessState} data-access-source={billing?.accessSource ?? "none"}>
        {accessState === "error" ? (
          <DenCard size="spacious" data-testid="omnirush-web-error">
            <p className="text-[18px] font-medium text-gray-950">OmniRush.ai Web remains locked</p>
        ...
        {accessState === "confirming" ? (
          <DenCard size="spacious" data-testid="omnirush-web-confirming">
            <div className="flex items-start gap-4">
              <Loader2 className="mt-1 size-5 animate-spin text-blue-600" aria-hidden="true" />
              <div>
                <p className="text-[18px] font-medium text-gray-950">Confirming your OmniRush.ai Web subscription</p>
        ...
        {accessState === "unsubscribed" && billing ? (
          <DenCard size="spacious" data-testid="omnirush-web-purchase">
            <p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-blue-600">OmniRush.ai Web</p>
            <h2 className="mt-2 text-[22px] font-semibold tracking-[-0.03em] text-gray-950">Browser access for your organization</h2>
            <p className="mt-3 text-[14px] leading-6 text-gray-600">{OMNIRUSH_WEB_QUANTITY_EXPLANATION}</p>
            <div className="mt-5 rounded-2xl border border-gray-200 bg-gray-50 p-4" data-testid="omnirush-web-price-breakdown">
              <p className="text-[22px] font-semibold tracking-[-0.03em] text-gray-950">
                {getOmniRushWebQuantityDescription(billing.quantity)} × {unitPrice}
              </p>
              <p className="mt-1 text-[14px] text-gray-600">
                {expectedTotal} per {billing.interval}
              </p>
            </div>
            <p className="mt-5 text-[13px] leading-5 text-gray-600" data-testid="omnirush-web-checkout-explainer">
            ...
            {!billing.configured ? (
              <DenNotice className="mt-5" tone="neutral" message="OmniRush.ai Web billing is not configured for this deployment." />
            ) : hasOngoingSubscription ? (
              <div className="mt-5 grid gap-4">
                <DenNotice
                  tone="warning"
                  message={billing.subscription?.paymentStatus === "payment_failed"
                    ? "This organization already has an OmniRush.ai Web subscription with a failed payment. Update the payment method from Billing to restore access."
                    : `This organization already has an OmniRush.ai Web subscription (${formatSubscriptionStatus(billing.subscription?.status ?? "unknown").toLowerCase()}). Manage the existing subscription from Billing.`}
                />
              ...
              <DenNotice
                className="mt-5"
                tone="warning"
                message="Ask a workspace owner or admin to purchase OmniRush.ai Web for this organization. You'll get access as soon as it's active."
              />
            )}
          </DenCard>
        ) : null}
        ...
        {accessState === "eligible" && billing ? (
          <DenCard size="spacious" data-testid="omnirush-web-eligible">
            ...
            <h2 className="mt-2 text-[22px] font-semibold tracking-[-0.03em] text-gray-950">OmniRush.ai Web is ready</h2>
            <p className="mt-3 text-[14px] leading-6 text-gray-600">
              {billing.accessSource === "complimentary"
                ? `OmniRush.ai Web is included for all ${getOmniRushWebQuantityDescription(billing.quantity)} in this organization, with no Stripe subscription or per-member charge.`
                : `OmniRush.ai Web is active for ${getOmniRushWebQuantityDescription(billing.quantity)} in this organization.`}
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              <WebOpenButton omnirushWebUrl={runtimeConfig.omnirushWebUrl} />
              <DenButton variant="secondary" href={getBillingRoute(activeOrg?.slug)}>View billing</DenButton>
            </div>
          </DenCard>
        ) : null}
