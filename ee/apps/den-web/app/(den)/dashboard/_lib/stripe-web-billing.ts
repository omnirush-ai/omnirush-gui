export const OMNIRUSH_WEB_CHECKOUT_TYPE = "web";
export const OMNIRUSH_WEB_QUANTITY_DEFINITION = "joined_non_removed_members";
export const OMNIRUSH_WEB_UNIT_AMOUNT = 5000;
export const OMNIRUSH_WEB_CURRENCY = "usd";
export const OMNIRUSH_WEB_INTERVAL = "month";
...
export type StripeWebBilling = {
  configured: boolean;
  priceId: string | null;
  unitAmount: typeof OMNIRUSH_WEB_UNIT_AMOUNT;
  currency: typeof OMNIRUSH_WEB_CURRENCY;
  interval: typeof OMNIRUSH_WEB_INTERVAL;
  quantityDefinition: typeof OMNIRUSH_WEB_QUANTITY_DEFINITION;
  quantity: number;
...
export function parseStripeWebBilling(payload: unknown): StripeWebBilling | null {
  ...
  const accessSource: OmniRushWebAccessSource | undefined = value.accessSource === "subscription" || value.accessSource === "complimentary"
    ? value.accessSource
  ...
  if (
    ...
    value.unitAmount !== OMNIRUSH_WEB_UNIT_AMOUNT ||
    value.currency !== OMNIRUSH_WEB_CURRENCY ||
    value.interval !== OMNIRUSH_WEB_INTERVAL ||
    value.quantityDefinition !== OMNIRUSH_WEB_QUANTITY_DEFINITION ||
    typeof value.quantity !== "number" ||
    !Number.isInteger(value.quantity) ||
    value.quantity < 1 ||
    typeof value.expectedMonthlyTotal !== "number" ||
    value.expectedMonthlyTotal !== value.quantity * OMNIRUSH_WEB_UNIT_AMOUNT ||
...
export function getOmniRushWebQuantityDescription(quantity: number): string {
  const memberLabel = quantity === 1 ? "member" : "members";
  return `${quantity} ${memberLabel}`;
}

export const OMNIRUSH_WEB_QUANTITY_EXPLANATION =
