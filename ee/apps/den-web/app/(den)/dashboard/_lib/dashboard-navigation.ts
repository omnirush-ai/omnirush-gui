...
export function buildDashboardNavSections({
  ...
  const showWeb = runtimeConfigLoaded && capabilities.omnirushWeb;
  const workItems: DashboardNavItem[] = [
    ...
    ...(workflowsEnabled && orgSlug
      ...
      : []),
    ...(showWeb
      ? [{ href: orgSlug ? getWebRoute(orgSlug) : "#", label: "OmniRush.ai Web", icon: Globe }]
      : []),
  ];

  // Hosted deployments expose OmniRush.ai Models; self-hosted deployments only
  // expose their own providers. Keep hidden until runtime config is known.
  const showOmniRushModels = runtimeConfigLoaded && orgMode === "multi_org";
  const modelsGroup: DashboardNavItem | null = access.isAdmin && orgSlug
    ? {
        href: showOmniRushModels
        ...
        children: [
          ...(showOmniRushModels
            ? [{ href: getInferenceRoute(orgSlug), label: "OmniRush.ai Models" }]
            : []),
...
const PAGE_KEYWORDS: Record<string, string[]> = {
  ...
  "OmniRush.ai Models": ["llm", "provider", "managed", "inference"],
  "OmniRush.ai Web": ["cloud", "sessions"],
