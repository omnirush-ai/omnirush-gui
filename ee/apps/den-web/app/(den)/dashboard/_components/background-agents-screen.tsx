...
function SandboxCard({
  ...
  const connectionUrl = details?.omnirushUrl ?? sandbox.instanceUrl ?? null;
  const ownerToken = details?.ownerToken ?? null;
  const clientToken = details?.clientToken ?? null;
  const openWebUrl = details?.omnirushAppConnectUrl ?? null;
  const openDesktopUrl = details?.omnirushDeepLink ?? null;
  ...
  async function loadConnectionDetails(workerId: string, workerName: string) {
    ...
    try {
      ...
      const nextDetails: ConnectionDetails = {
        ...
        omnirushAppConnectUrl: buildOmniRushAppConnectUrl(
          runtimeConfig.omnirushAppConnectUrl,
          tokens.previewOmniRushUrl,
          tokens.clientToken,
          workerId,
          workerName,
          { autoConnect: true },
        ),
        omnirushDeepLink: buildOmniRushDeepLink(
          tokens.omnirushUrl,
