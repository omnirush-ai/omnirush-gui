    ...
    <div data-testid="stripe-billing-screen">
      <DashboardPageTemplate
        ...
        description={webFeatureEnabled
          ? webComplimentary
            ? "OmniRush.ai Web is included for this organization at no charge. Team seats and built-in AI model access remain separate purchases."
            : "OmniRush.ai Web, team seats, and built-in AI model access are separate purchases. Your expected monthly total reflects the subscriptions shown below."
      ...
      <DenCard className="mb-6 !p-0" data-testid="billing-summary-card">
        <DenSectionHeader
          ...
          description={webFeatureEnabled
            ? webComplimentary
              ? "OmniRush.ai Web is included without a Stripe subscription or per-member charge."
              : "Each plan is billed separately. OmniRush.ai Web is $50 per joined organization member each month."
            ...
            <DenLineItemRow
              ...
              description={
                !webBilling
                  ...
                  : webComplimentary
                    ? `${getOmniRushWebQuantityDescription(webBilling.quantity)} covered · complimentary access`
                  : !webConfigured
                    ? "This deployment does not sell OmniRush.ai Web access"
                    : webSubscribed
                      ? `${getOmniRushWebQuantityDescription(webBilling.quantity)} × ${webPrice} · ${formatSubscriptionStatus(webStatus ?? "unknown")}`
                      : `${getOmniRushWebQuantityDescription(webBilling.quantity)} · not subscribed`
              }
      ...
      {webFeatureEnabled ? (
        <DenCard className="mb-6" data-testid="billing-omnirush-web-card">
          <DenSectionHeader
            title="OmniRush.ai Web"
          ...
          {!webBilling ? (
            <DenNotice
              className="mt-5"
              message="OmniRush.ai Web billing details are unavailable. Browser access remains locked until the subscription can be confirmed."
            />
          ) : (
            <>
              <DenNotice
                ...
                message={
                  webComplimentary
                    ? "OmniRush.ai Web is included for this organization without a Stripe subscription or per-member charge."
                    : !webConfigured
                    ? "OmniRush.ai Web billing is not configured for this deployment."
                    : webPaymentFailed
                      ? webPaymentStatus === "payment_failed"
                        ? "The latest payment failed, so OmniRush.ai Web is locked. Update the payment method to restore access."
                        : `The payment is ${formatSubscriptionStatus(webPaymentStatus ?? "failed").toLowerCase()}, so OmniRush.ai Web is locked. Update the payment method to restore access.`
                      : webCancelling
                        ...
                        : webEligible
                          ? "OmniRush.ai Web is active for this organization."
                          : webSubscribed
                            ? `This subscription is ${formatSubscriptionStatus(webStatus ?? "unknown").toLowerCase()}, so OmniRush.ai Web is locked.`
                            : "No OmniRush.ai Web subscription is active. Purchase it from the OmniRush.ai Web page."
                }
              />

              <p className="mt-5 text-[14px] leading-6 text-gray-600" data-testid="billing-omnirush-web-quantity-definition">
                {webComplimentary
                  ? "Every joined organization member is covered, including the owner. Pending invitations are not counted until they join."
                  : OMNIRUSH_WEB_QUANTITY_EXPLANATION}
              </p>
              <div className="mt-4 rounded-2xl border border-gray-200 bg-gray-50 p-4" data-testid="billing-omnirush-web-price-breakdown">
                <p className="text-[22px] font-semibold tracking-[-0.03em] text-gray-950">
                  {webComplimentary
                    ? `${getOmniRushWebQuantityDescription(webBilling.quantity)} covered`
                    : `${getOmniRushWebQuantityDescription(webBilling.quantity)} × ${webPrice}`}
                </p>
              ...
              <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <BillingStat label="Plan" value="OmniRush.ai Web" />
              ...
              {!webComplimentary ? (
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3" data-testid="billing-omnirush-web-lifecycle">
              ...
              <DenActionList className="mt-5">
                ...
                {webComplimentary ? null : webSubscribed ? (
                  ...
                  <DenActionRow
                    description={`Purchase from the OmniRush.ai Web page — ${getOmniRushWebQuantityDescription(webBilling.quantity)} × ${webPrice} per ${webBilling.interval}.`}
                    action={<DenButton onClick={() => router.push(getWebRoute(activeOrg?.slug))}>View OmniRush.ai Web</DenButton>}
                  />
                )}
              </DenActionList>
            </>
      ...
      <DenCard data-testid="billing-ai-card">
        <DenSectionHeader
          title="AI model access"
          description="Use OmniRush.ai's built-in models with no API keys to manage. Separate from seats."
          action={
            !aiConfigured
              ? <DenBadge tone="neutral">Not billed</DenBadge>
              : aiPaymentFailed
        ...
        <DenActionList className="mt-5">
          {aiActive ? (
            ...
            <DenActionRow
              description={
                aiConfigured
                  ? `Turning this on costs ${aiChargeLabel} per ${stripeBilling.interval} for your ${activeMemberCount} ${activeMemberCount === 1 ? "member" : "members"}, not ${stripePrice}. You subscribe from the OmniRush.ai Models page.`
                  : "See which models OmniRush.ai ships with and how your team connects their own provider keys."
              }
              action={
                <DenButton onClick={() => router.push(getInferenceRoute(activeOrg?.slug))}>
                  View OmniRush.ai Models
                </DenButton>
              }
            />
          )}
