...
export function LlmProvidersScreen() {
  ...
  const customProviders = useMemo(
    () => llmProviders.filter((provider) => provider.source !== "omnirush"),
    [llmProviders],
  );
  ...
  const hasOmniRushModels = llmProviders.some((provider) => provider.source === "omnirush");

  const accessOutcome = accessMode === "managed"
    ? modelCount > 0
      ? `Members see exactly the ${plural(modelCount, "model")} from the ${plural(providerCount, "provider")} below${hasOmniRushModels ? ", plus OmniRush.ai Models" : ""}.`
