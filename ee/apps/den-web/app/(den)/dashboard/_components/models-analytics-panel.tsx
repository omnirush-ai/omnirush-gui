...
import { z } from "zod";
import { modelsAnalyticsSettingsSchema, modelsAnalyticsActivitySchema, modelsConsumptionSchema, type ModelsAnalyticsSettings } from "@omnirush-ee/telemetry-contracts";
...
function modelName(value: string) {
  ...
  return model ? model[1].displayName.replace(/^OmniRush.ai:\s*/, "") : value;
}
...
export function ModelsAnalyticsPanel() {
  ...
  return <section className="grid gap-5" data-testid="models-task-analytics" aria-label="Task analytics">
    {!settings.enabled ? <div className={`${analyticsSurfaceClass} grid gap-6 p-6 sm:grid-cols-[1fr_220px]`}>
      <div>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#f2ecff] px-2.5 py-1 text-xs font-medium text-[#6F3DFF]"><Sparkles className="h-3 w-3" aria-hidden="true" />Included with OmniRush.ai Models</span>
        <h2 className="mt-4 text-xl font-semibold tracking-tight text-[#07192C]">Unlock custom insights</h2>
        <p className="mt-2 max-w-xl text-sm leading-6 text-[#637291]">See model usage, costs, task activity, and the skills and tools your team uses. Would you like to turn on analytics for your team’s tasks in OmniRush.ai?</p>
        ...
        {settings.consentedAt ? <p className="mt-3 text-sm text-[#637291]">Task analytics is off. You can keep using OmniRush.ai Models as usual.</p> : null}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><h2 className="text-lg font-semibold tracking-tight text-[#07192C]">Task analytics</h2>
          <p className="mt-1 text-xs text-[#637291]">OmniRush.ai Models only · Included with your subscription</p></div>
      ...
      {tab !== "Integrations" ? <>
        ...
        {!dataQuery.isError || dataQuery.data ? <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard icon={<Sparkles className="text-[#18A34A]" />} title="Models used" value={loading ? "…" : new Set(usage.flatMap((row) => row.model ? [row.model] : [])).size.toLocaleString()} sub="Through OmniRush.ai Models" tone="green" />
        </div> : null}
      </> : null}
      {tab === "Activity" ? <div className={`${analyticsSurfaceClass} overflow-hidden`}>
        ...
        ]} /> : <AnalyticsEmptyState title="Your next OmniRush.ai Models task appears here" action={<AnalyticsAdoptionLink orgSlug={activeOrg?.slug} />}>
          <p>Choose a model from <strong className="font-medium text-[#30405F]">OmniRush.ai Models</strong> in the desktop model picker, then run a new task. This page refreshes automatically.</p>
          <p className="mt-2">Tasks using your own provider connections are covered by Usage &amp; adoption. Model costs here cover OmniRush.ai Models only.</p>
        </AnalyticsEmptyState>}
      ...
      {tab === "Consumption" ? <>
        {loading ? <p role="status" className="py-12 text-center text-sm text-[#637291]">Loading consumption…</p> : dataQuery.isError && !dataQuery.data ? null : !usage.length ? <div className={analyticsSurfaceClass}><AnalyticsEmptyState title="No model usage recorded yet">Run a new task with OmniRush.ai Models to see its model calls and provider-reported consumption. Usage from your own provider accounts is not included here.</AnalyticsEmptyState></div> : <>
          ...
          <div className={`${analyticsSurfaceClass} overflow-hidden`}>
            <div className="border-b border-[#edf0f5] px-5 py-4"><h3 className="text-sm font-semibold text-[#07192C]">Consumption by {groupBy}</h3><p className="mt-1 text-xs text-[#637291]">Reported usage for OmniRush.ai Models</p></div>
