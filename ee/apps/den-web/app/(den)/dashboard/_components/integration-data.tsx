...
export const INTEGRATION_PROVIDERS: Record<IntegrationProvider, IntegrationProviderMeta> = {
  github: {
    provider: "github",
    name: "GitHub",
    description: "Install the OmniRush.ai GitHub App, then pick a repository to turn into a connector instance.",
...
export function getMockAccountsFor(provider: IntegrationProvider): IntegrationAccount[] {
  if (provider === "github") {
    return [
      ...
      { id: "acc_gh_omnirush_ai", name: "omnirush-ai", kind: "org", avatarInitial: "O" },
      { id: "acc_gh_omnirush", name: "omnirush-labs", kind: "org", avatarInitial: "O" },
    ];
  }
  return [
    { id: "acc_bb_user", name: "bshafii", kind: "user", avatarInitial: "B" },
    { id: "acc_bb_omnirush", name: "omnirush", kind: "org", avatarInitial: "O" },
  ];
}
  ...
  const base: IntegrationRepo[] = [
    {
      id: `${tag}:omnirush`,
      name: "omnirush",
      fullName: `${accountToLabel(accountId)}/omnirush`,
      description: "Core OmniRush.ai monorepo — desktop, server, and cloud apps.",
      hasPlugins: true,
    },
    {
      id: `${tag}:omnirush-plugins`,
      name: "omnirush-plugins",
      fullName: `${accountToLabel(accountId)}/omnirush-plugins`,
...
function accountToLabel(accountId: string): string {
  if (accountId.includes("omnirush-labs")) return "omnirush-labs";
  if (accountId.includes("omnirush")) return "omnirush";
  if (accountId.includes("omnirush-ai")) return "omnirush-ai";
  return "bshafii";
}
