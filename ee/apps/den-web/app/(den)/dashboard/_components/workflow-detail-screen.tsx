...
export function WorkflowDetailScreen({ workflowId }: { workflowId: string }) {
  ...
  const automateHref = `${runtimeConfig.omnirushWebUrl.replace(/\/$/, "")}/automations?create=1&workflow=${encodeURIComponent(workflowId)}&version=${encodeURIComponent(state.detail.currentVersion.id)}`;
