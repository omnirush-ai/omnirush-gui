...
async function createBrandAssetDraft(file: File, kind: BrandAssetKind): Promise<BrandAssetDraft> {
  ...
  try {
    ...
    throw new Error("OmniRush.ai could not decode that image.");
  }
    ...
    <div className="grid min-w-0 gap-3 rounded-2xl border border-gray-200 bg-white p-4" data-testid={`brand-${kind}-asset-field`}>
      ...
      <div className="flex min-h-28 items-center justify-center overflow-hidden rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4">
        {previewUrl ? (
          ...
          <span className="text-center text-[12px] text-gray-400">Default OmniRush.ai {kind}</span>
        )}
      </div>
            ...
            <DenCard size="spacious" className="grid gap-6">
              ...
              <div className="grid gap-5" data-testid="brand-identity-fields">
                <label className="grid gap-3">
                  ...
                  <DenInput type="text" value={appNameDraft} onChange={(event) => setAppNameDraft(event.target.value)} placeholder="OmniRush.ai" maxLength={64} disabled={!canManageBrandAppearance} />
                  <span className="text-[11px] text-gray-400">The signed application identity stays OmniRush.ai.</span>
                </label>
                ...
                <label className="grid gap-3">
                  ...
                  <select value={accentColorDraft} onChange={(event) => setAccentColorDraft(event.target.value)} disabled={!canManageBrandAppearance} className="h-11 rounded-xl border border-gray-200 bg-white px-4 text-[14px] text-gray-900 outline-hidden">
                    <option value="">Default (OmniRush.ai)</option>
