...
function lastBoundaryFromDiagnostic(diagnostic: ExternalMcpDiagnostic | null): string | undefined {
  if (diagnostic?.highestPassed === "operation_ready") return "OmniRush.ai started remote tool execution";
  if (diagnostic?.highestPassed === "catalog_ready") return "OmniRush.ai loaded the remote MCP tool catalog";
  if (diagnostic?.highestPassed === "protocol_ready") return "OmniRush.ai initialized the remote MCP session";
  if (diagnostic?.highestPassed === "authorized") return "Remote MCP accepted the connection credential";
  if (diagnostic?.highestPassed === "reachable") return "OmniRush.ai reached the remote MCP endpoint";
  if (diagnostic?.highestPassed === "configured") return "OmniRush.ai loaded the MCP connection configuration";
  return undefined;
}
...
export function attributeExternalMcpToolFailure(input: {
  ...
  if (browserTimeout) {
    ...
    return {
      summary: `OmniRush.ai stopped waiting after ${duration} seconds. The operation’s outcome is unknown.`,
      lastConfirmedBoundary: "OmniRush.ai dashboard sent the request",
      likelySource: "Source unclear after OmniRush.ai timeout",
      confidence: "Inferred",
  ...
  if (blockedBeforeSend) {
    return {
      summary: "OmniRush.ai blocked the request before it was sent.",
      lastConfirmedBoundary: "OmniRush.ai evaluated the outbound request",
      likelySource: "OmniRush.ai",
      confidence: "Confirmed",
      retryGuidance: diagnostic?.operatorAction ?? "Resolve the OmniRush.ai policy or connection configuration, then run the tool again.",
      outcome: "failed",
      ...details,
    };
  }
  ...
  if (deadlineAfterSend) {
    return {
      summary: "OmniRush.ai sent the request, but the remote MCP did not respond before OmniRush.ai’s deadline.",
      lastConfirmedBoundary: "OmniRush.ai started the outbound tools/call",
  ...
  if (inspection?.request && !inspection.response) {
    return {
      summary: "OmniRush.ai started the outbound request, but no HTTP response was captured. This does not prove the remote MCP caused the failure.",
      lastConfirmedBoundary: "OmniRush.ai started the outbound tools/call",
  ...
  return {
    summary: inspection?.diagnosis?.summary
      ?? diagnostic?.message
      ?? "The request failed before OmniRush.ai received a tool result.",
    lastConfirmedBoundary: lastBoundaryFromDiagnostic(diagnostic)
      ?? (diagnostic ? "OmniRush.ai returned a structured diagnostic" : "OmniRush.ai dashboard sent the request"),
    likelySource: networkSetup ? "Connection path or remote MCP" : "OmniRush.ai or MCP setup",
    confidence: "Inferred",
    retryGuidance: diagnostic?.operatorAction ?? "Use the diagnostic reference to inspect OmniRush.ai and MCP connection health before retrying.",
    outcome: "failed",
    ...details,
  };
}
