...
const MCP_OAUTH_START_UNREADABLE_MESSAGE =
  "OmniRush.ai could not read the answer from its API when starting the sign-in. The browser blocked the response or the request never completed. Try again; if it keeps happening, tell your workspace admin the time of this attempt.";
...
function parseToolCallInspection(value: unknown): ExternalMcpToolCallInspection | null {
  ...
  if (
    ...
    || (layer !== "omnirush" && layer !== "network" && layer !== "mcp_connection" && layer !== "remote_http" && layer !== "mcp_tool")
