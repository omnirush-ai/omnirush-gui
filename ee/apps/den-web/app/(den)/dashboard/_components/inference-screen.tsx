...
const MODEL_LINEUP: LineupModel[] = Object.entries(INFERENCE_MODEL_ALIASES)
  ...
  .map(([id, model]) => {
    ...
    return {
      id,
      name: model.displayName.replace(/^OmniRush.ai:\s*/, ""),
...
export function InferenceScreen() {
  ...
  async function startSubscribeCheckout() {
    if (!canManageModels) {
      setError("Only workspace admins can start OmniRush.ai Models checkout.");
      return;
    }
  ...
  async function toggleEnabled() {
    if (!canManageModels) {
      setError("Only workspace admins can manage OmniRush.ai Models.");
      return;
    }
    if (!status) return;
  ...
  return (
    <div className="mx-auto grid w-full max-w-[960px] gap-6 px-4 pb-12 pt-5 sm:px-6 lg:px-8">
      <DenPageHeader title="OmniRush.ai Models"
      ...
      {canManageModels ? null : (
        <DenNotice
          tone="info"
          message="Only workspace admins can subscribe or enable OmniRush.ai Models. Ask an owner, super-admin, or admin for this workspace."
        />
      )}

      {showGettingStarted ? <DenCard>
        <p className="text-sm leading-6 text-[#637291]">One subscription activates models for everyone in your workspace. After subscribing, choose a model from the OmniRush.ai group in the app and start a task.</p>
      </DenCard> : null}
