...
export function MarketplaceOnboardingScreen() {
  ...
  const { data: modelsEnabled, isPending: modelsLoading, error: modelsError } = useQuery({
    ...
    queryFn: async () => {
      if (!orgId) throw new Error("Choose a workspace to check OmniRush.ai Models.");
      const { response, payload } = await requestJson("/v1/inference", { method: "GET", headers: { "x-omnirush-org-id": orgId } }, 12000);
      if (!response.ok) throw new Error(getErrorMessage(payload, "Could not check OmniRush.ai Models."));
  ...
  return (
    <SetupFrame step="ready" title="Choose what powers your work." description="Use OmniRush.ai Models or bring your own provider. You can decide now or set this up later.">
      <div className="grid gap-6" data-testid="marketplace-onboarding">
        <section aria-labelledby="setup-models-heading" className="grid gap-4">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--dls-text-secondary)]" role="status">
              {modelsLoading ? "Checking OmniRush.ai Models..." : modelsError ? "Model status is unavailable. You can still complete setup." : modelsEnabled ? "OmniRush.ai Models are on for this workspace." : "Signing in does not enable models. Keep your existing provider, or choose one when you are ready."}
            </p>
          ...
          <div className="divide-y divide-[var(--dls-border)] overflow-hidden rounded-2xl border border-[var(--dls-border)]">
            <div className="flex items-start gap-3 p-4 sm:p-5" data-testid="onboarding-choice-omnirush-models">
              <div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[var(--dls-hover)]">
                <img src="/omnirush-mark.svg" alt="" aria-hidden className="h-5 w-5" />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-semibold">OmniRush.ai Models</h3>
        ...
        <section aria-labelledby="setup-finish-heading" className="grid gap-4 border-t border-[var(--dls-border)] pt-6" data-testid="onboarding-finish">
          ...
          {desktopRedirectUrl ? <DesktopHandoffAction omnirushUrl={desktopRedirectUrl} grant={getDesktopGrant(desktopRedirectUrl)} organizationName={activeOrg?.name ?? null} showCopyLinkByDefault /> : (
