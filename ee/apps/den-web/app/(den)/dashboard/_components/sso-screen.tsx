...
function readSsoTestMessage(value: unknown) {
  if (typeof value !== "object" || value === null || !("type" in value) || value.type !== "omnirush:sso-test-complete") return null;
...
export function SsoScreen() {
  ...
  function getOrgScopedHeaders() {
    ...
    if (orgId) {
      headers.set("x-omnirush-legacy-org-id", orgId);
    }
    return headers;
  }
  ...
  async function handleStartTest() {
    ...
    const popup = window.open("", "omnirush-sso-test", "popup,width=520,height=720");
    if (!popup) {
      setError("OmniRush.ai could not open the SSO test window. Allow popups, then try again.");
      return;
    }
                ...
                {formMode === "saml" ? (
                  <>
                    ...
                    <label className="block text-[14px] text-gray-700 md:col-span-2">
                      ...
                      <input className="w-full rounded-[18px] border border-gray-200 px-4 py-3" value={audience} onChange={(event) => setAudience(event.target.value)} placeholder="Defaults to the OmniRush.ai auth URL" disabled={ssoFormDisabled} />
                    </label>
                    <label className="block text-[14px] text-gray-700 md:col-span-2">
                    ...
                    <div className="rounded-[18px] border border-gray-200 px-4 py-3 text-[14px] leading-6 text-gray-600 md:col-span-2">
                      OmniRush.ai always requires signed SAML assertions, timestamps, and SP-initiated responses for organization SAML connections.
                    </div>
                  </>
                ) : (
                  <>
