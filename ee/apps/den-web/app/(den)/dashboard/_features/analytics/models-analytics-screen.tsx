...
export function ModelsAnalyticsScreen() {
  ...
  return <div className={analyticsPageClass}>
    <AnalyticsPageHeader orgSlug={activeOrg?.slug} active="models" title="Models & usage"
      description="Understand your team’s OmniRush.ai Models activity, consumption, and shared limits."
      caption="Included with OmniRush.ai Models · Task analytics requires your team’s opt-in" />
    {!runtimeConfigLoaded ? <p role="status">Loading model usage…</p> : !hosted ? (
      <div className={analyticsSurfaceClass}><AnalyticsEmptyState title="OmniRush.ai Models is available on OmniRush.ai Cloud">
      ...
      {status.data?.enabled && status.data.subscribed ? <>
        ...
        <AnalyticsEmptyState title="Model insights are included with OmniRush.ai Models"
          action={<DenButton href={getInferenceRoute(activeOrg?.slug)}>Set up OmniRush.ai Models</DenButton>}>
          Enable OmniRush.ai Models for your workspace to see shared limits and choose whether to collect task analytics.
        </AnalyticsEmptyState>
      </div> : null}
    </>}
  </div>;
