...
import {
  ...
  buildOmniRushAppConnectUrl,
  buildOmniRushDeepLink,
...
type DenFlowContextValue = {
  ...
  omnirushDeepLink: string | null;
  omnirushAppConnectUrl: string | null;
...
export function DenFlowProvider({ children }: { children: ReactNode }) {
  ...
  const desktopAuthScheme = continuation?.desktopScheme ?? "omnirush";
  ...
  const activeWorker =
    worker && workerLookupId === worker.workerId
      ...
      : selectedWorker
        ...
        : worker;
  const { desktopUrl: omnirushConnectUrl, webUrl: previewConnectUrl } = getWorkerConnectionTargets(activeWorker);
  const { desktopToken: desktopOmniRushToken, webToken: webOmniRushToken } = getWorkerConnectionTokens(activeWorker);
  const hasWorkspaceScopedUrl = Boolean(omnirushConnectUrl && /\/w\/[^/?#]+/.test(omnirushConnectUrl));
  const omnirushDeepLink = buildOmniRushDeepLink(
    omnirushConnectUrl,
    desktopOmniRushToken,
    activeWorker?.workerId ?? null,
    activeWorker?.workerName ?? null
  );
  const omnirushAppConnectUrl = buildOmniRushAppConnectUrl(
    runtimeConfig.omnirushAppConnectUrl,
    previewConnectUrl,
    webOmniRushToken,
    activeWorker?.workerId ?? null,
    activeWorker?.workerName ?? null,
  ...
  async function finalizeEmailPasswordSignIn(
    ...
    if (desktopAuthRequested || webAuthRequested) {
      setAuthInfo("Signed in. Returning to OmniRush.ai...");
      return null;
    }
  ...
  async function withResolvedOmniRushCredentials(candidate: WorkerLaunch, options: { quiet?: boolean } = {}) {
    const existingConnectUrl = candidate.omnirushUrl?.trim() ?? "";
    ...
    if (existingConnectUrl && (existingWorkspaceId || existingConnectUrl.includes("/v1/cloud/workers/"))) {
      return {
        ...candidate,
        omnirushUrl: existingConnectUrl,
    ...
    if (!accessToken) {
      ...
      return {
        ...candidate,
        omnirushUrl: instanceUrl.trim().replace(/\/+$/, ""),
        workspaceId: mountedWorkspaceId
      };
    }

    try {
      const resolved = await resolveOmniRushWorkspaceUrl(instanceUrl, accessToken);
      if (resolved) {
        return {
          ...candidate,
          omnirushUrl: resolved.omnirushUrl,
    ...
    try {
      const { response, payload } = await requestJson(
        `/v1/workers/${encodeURIComponent(targetWorkerId)}/runtime/upgrade`,
        {
          method: "POST",
          headers: authToken ? { Authorization: `Bearer ${authToken}` } : undefined,
          body: JSON.stringify({ services: ["omnirush-server", "opencode"] })
        },
        12000
      );
  ...
  async function completeDesktopAuthHandoff(organizationId?: string) {
    ...
    try {
      ...
      const omnirushUrl = getDesktopHandoffOmniRushUrl(payload) ?? "";
      if (!omnirushUrl) {
        setAuthError("Desktop handoff succeeded, but no OmniRush.ai redirect URL was returned.");
        return false;
      }

      rememberDesktopHandoffGrant(getDesktopHandoffGrant(payload, omnirushUrl));
      setDesktopRedirectUrl(omnirushUrl);
      persistContinuation(null);
      clearPendingAuthIntent();
      window.location.assign(omnirushUrl);
      return true;
    } catch (error) {
      if (continuationRef.current === current && epoch === sessionEpochRef.current) setAuthError(error instanceof Error ? error.message : "Failed to open OmniRush.ai.");
      return false;
    } finally {
  ...
  async function completeWebAuthHandoff() {
    ...
    try {
      ...
      setAuthError(error instanceof Error ? error.message : "Failed to return to OmniRush.ai Cloud.");
    } finally {
  ...
  async function beginSocialAuth(provider: SocialAuthProvider) {
    ...
    try {
      ...
      const callbackURL = getSocialCallbackUrl(latestRuntimeConfig.omnirushAuthCallbackUrl);
  ...
  async function launchWorker(options: { source?: "manual" | "signup_auto"; workerNameOverride?: string } = {}) {
    ...
    try {
      ...
      const resolvedWorker = await withResolvedOmniRushCredentials(parsedWorker);
  ...
  async function checkWorkerStatus(options: { workerId?: string; quiet?: boolean; background?: boolean } = {}) {
    ...
    try {
      ...
      const nextWorker: WorkerLaunch =
        worker && worker.workerId === summary.workerId
          ...
          : {
              ...
              provider: summary.provider,
              instanceUrl: summary.instanceUrl,
              omnirushUrl: summary.instanceUrl,
      ...
      if (shouldUpdateActiveWorker) {
        const resolvedWorker = await withResolvedOmniRushCredentials(nextWorker, { quiet: true });
        setWorker(resolvedWorker);
        setPendingRestoredWorkerId(null);
        if (!background) {
  ...
  async function generateWorkerToken(options: { background?: boolean; workerId?: string } = {}) {
    ...
    try {
      const { response, payload } = await requestJson(`/v1/workers/${encodeURIComponent(id)}/tokens`, {
        method: "POST",
        headers: authToken ? { Authorization: `Bearer ${authToken}` } : undefined,
        body: JSON.stringify({ includeExpiringOmniRushUrl: true })
      });
  ...
  useEffect(() => {
    ...
    try {
      ...
      const restored: WorkerLaunch = {
        ...parsed,
        omnirushUrl: parsed.provider === "daytona" && parsed.omnirushUrl && !parsed.omnirushUrl.includes("/v1/cloud/workers/")
          ? null
          : parsed.omnirushUrl ?? parsed.instanceUrl,
        previewOmniRushUrl: null,
        previewExpiresAt: null,
  ...
  }, [actionBusy, launchBusy, pendingRestoredWorkerId, user?.id, worker?.workerId, worker?.status, worker?.clientToken, worker?.hostToken, worker?.omnirushUrl, worker?.previewOmniRushUrl, worker?.previewExpiresAt]);
