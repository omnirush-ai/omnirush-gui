  ...
  return (
    <section className="den-page py-4 lg:py-6">
      <div className="den-frame grid max-w-[44rem] gap-4 p-6 md:p-7">
        <p className="den-eyebrow">OmniRush.ai Cloud</p>
...
const AUTO_ACCEPT_WORKSPACE_CLAIM_STORAGE_KEY = "omnirush:web:auto-accept-workspace-claim";
...
function getOmniRushUrl(payload: unknown): string | null {
  if (typeof payload !== "object" || payload === null) {
    return null;
  }

  const url = (payload as { omnirushUrl?: unknown }).omnirushUrl;
...
export function WorkspaceClaimScreen({
  ...
  async function createDesktopHandoff(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(getErrorMessage(payload, `Could not prepare an OmniRush.ai link (${response.status}).`));
    }

    const omnirushUrl = getOmniRushUrl(payload);
    if (!omnirushUrl) {
      throw new Error("No OmniRush.ai link was returned.");
    }
  ...
  async function handleOpenDesktop() {
    ...
    try {
      ...
      setHandoffError(error instanceof Error ? error.message : "Could not open OmniRush.ai.");
    } finally {
      setHandoffBusy(false);
    }
  }

  async function handleCopyOmniRushLink() {
    setCopyBusy(true);
    setLinkCopied(false);
    setHandoffError(null);
    ...
    try {
      ...
      const omnirushUrl = await createDesktopHandoff();
      await navigator.clipboard.writeText(omnirushUrl);
      setLinkCopied(true);
      window.setTimeout(() => setLinkCopied(false), 1800);
    } catch (error) {
      setHandoffError(error instanceof Error ? error.message : "Could not copy the OmniRush.ai link.");
    } finally {
      setCopyBusy(false);
    }
  }
        ...
        <div className={`den-frame mx-auto grid w-full max-w-[38rem] gap-7 p-7 text-center md:p-10 ${isLoopback ? "border-blue-200/80 bg-white/90 shadow-[0_28px_80px_-44px_rgba(37,99,235,0.45)]" : ""}`}>
          ...
          <div className="grid justify-items-center gap-3">
            ...
            <p className="den-copy max-w-[46ch]">
              Copy the OmniRush.ai link, open OmniRush.ai Enterprise, and paste it to connect. You will not need to enter your password again.
            </p>
          </div>
          ...
          <div className="grid gap-3">
            <button
              ...
              onClick={() => void handleCopyOmniRushLink()}
              disabled={handoffBusy || copyBusy}
            >
              <Copy className="size-4" aria-hidden />
              {copyBusy ? "Copying..." : linkCopied ? "Link copied" : "Copy OmniRush.ai link"}
            </button>
            ...
            <div className="flex flex-col items-center justify-center gap-2 text-sm sm:flex-row sm:gap-5">
              ...
              >
                ...
                {handoffBusy ? "Opening OmniRush.ai..." : "Open OmniRush.ai"}
              </button>
          ...
          {linkCopied ? (
            <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-medium text-blue-800">
              Open OmniRush.ai Enterprise and paste the OmniRush.ai link to connect.
            </div>
          ) : null}

          {handoffAttempted && !handoffError ? (
            <p className="den-copy text-sm">
              Opening OmniRush.ai now. If nothing happens (for example, the app isn&apos;t installed on this machine), use &quot;Continue in browser instead&quot;.
            </p>
          ) : null}
