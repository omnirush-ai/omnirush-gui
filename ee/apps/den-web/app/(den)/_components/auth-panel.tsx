    ...
    <div className="grid gap-2" data-testid="desktop-handoff-copy-link">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <input
          ...
          aria-label="OmniRush.ai sign-in link"
        />
...
export function DesktopHandoffAction({
  ...
  /** When true, always show the pasteable omnirush:// link (signed-in desktop handoff). */
  ...
  if (status === "consumed") {
    return (
      <div className="den-frame-inset rounded-[1.5rem] px-4 py-3 text-center text-sm font-medium text-emerald-700" data-testid="desktop-connected" aria-live="polite">
        ✓ Connected — OmniRush.ai is set up for {resolvedOrganizationName}
      </div>
    );
  }
  ...
  if (showTroubleshoot && !showCopyLinkByDefault) {
    return (
      <div className="den-frame-inset grid gap-3 rounded-[1.5rem] px-4 py-3 text-sm text-[var(--dls-text-secondary)]" data-testid="desktop-handoff-troubleshoot" aria-live="polite">
        <p className="m-0">
          Nothing opened?{" "}
          <button type="button" className="font-medium text-[var(--dls-text-primary)] underline-offset-4 hover:underline" onClick={() => window.location.assign(omnirushUrl)}>
            Open OmniRush.ai again
          </button>
        </p>
        <DesktopHandoffCopyLink
          omnirushUrl={omnirushUrl}
          label="Still stuck? Paste this sign-in code in OmniRush.ai:"
        />
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-3">
      <button
        ...
        onClick={() => window.location.assign(omnirushUrl)}
      >
        Open OmniRush.ai
      ...
      {showCopyLink ? (
        <DesktopHandoffCopyLink
          omnirushUrl={omnirushUrl}
          label={showTroubleshoot ? "Nothing opened? Paste this sign-in code in OmniRush.ai:" : "Or paste this sign-in code in OmniRush.ai:"}
        />
      ) : null}
    </div>
  );
...
export function AuthPanel({
  ...
  const singleOrgName = runtimeConfig.singleOrgName || "OmniRush.ai";
  ...
  const emailFirstContent: PanelContent =
    emailFirstStep === "email"
      ...
      : {
          ...
          copy: "Set up your OmniRush.ai Cloud account.",
          submitLabel: "Sign up",
          ...signUpContent,
        };
  ...
  const startSingleOrgSso = () => {
    ...
    nextUrl.searchParams.set("callbackURL", getSocialCallbackUrl(runtimeConfig.omnirushAuthCallbackUrl));
      ...
      <div className={shellClass("gap-6", "p-6 md:p-7")} data-testid="desktop-signed-in-handoff">
        ...
        {desktopRedirectUrl ? (
          <DesktopHandoffAction
            omnirushUrl={desktopRedirectUrl}
            grant={desktopGrant}
          ...
          <div className="grid gap-3">
            ...
            <button type="button" className="den-button-primary w-full" disabled={desktopRedirectBusy} onClick={retryDesktopAuthHandoff}>Retry opening OmniRush.ai</button>
          </div>
        ) : (
