...
const OMNIRUSH_MARK_SRC = "/omnirush-mark.svg";
...
function BrandIcon({ iconUrl }: { iconUrl: string | null }) {
  ...
  const iconSrc = iconUrl && iconUrl !== failedIconUrl ? iconUrl : OMNIRUSH_MARK_SRC;
  const isFallbackIcon = iconSrc === OMNIRUSH_MARK_SRC;
...
function BrandLabel({ organization }: { organization: OnboardingCardOrganization | null }) {
  if (!organization) {
    return <span>OmniRush.ai Cloud</span>;
  }
