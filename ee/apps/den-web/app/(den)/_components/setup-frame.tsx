...
function WorkPreview({ step }: { step: SetupStep }) {
  ...
  return (
    <figure className={styles.preview} aria-label={`Product example: ${copy.caption}`}>
      <div className={styles.previewLabel}><span>OMNIRUSH, IN PRACTICE</span><span>Example</span></div>
      <div className={styles.appWindow} aria-hidden="true">
        <div className={styles.appTitlebar}>
          <img src="/omnirush-mark.svg" alt="" width={13} height={13} />
          <span>OmniRush.ai</span><span className={styles.appWindowLabel}>New conversation</span>
        </div>
        <div className={styles.appBody}>
          ...
          <div className={styles.appContent}>
            ...
            <div className={styles.previewResponse}><img src="/omnirush-mark.svg" alt="" width={13} height={13} /><p>{copy.reply}</p></div>
            {step === "space" ? (
            ...
            {gateway ? <div className={styles.gatewayContext}><Network size={12} /><div><strong>OmniRush.ai gateway</strong><small>Team tools in Desktop, Codex, and Claude Code</small></div></div> : null}
            <PreviewComposer />
          </div>
        </div>
      </div>
  ...
  return (
    <section className={`${styles.frame} ${embedded ? styles.embedded : ""}`} data-testid="setup-frame" data-step={step}>
      <header className={styles.top}>
        <div className={styles.brand}><img src="/omnirush-mark.svg" alt="" width={25} height={25} /><span>OmniRush.ai<span className={styles.cloud}> / Cloud</span></span></div>
