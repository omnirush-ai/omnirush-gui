...
import { detectPlatform, type DetectedPlatform } from "@omnirush/ui/react";
...
function ReturnToOmniRushStatus({
  ...
  async function copyOmniRushUrl() {
    await navigator.clipboard.writeText(omnirushUrl);
    setCopied(true);
  ...
  if (status === "consumed") {
    return (
      <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700" data-testid="desktop-connected" aria-live="polite">
        Connected — {organizationName} is ready in OmniRush.ai.
      </div>
    );
  }
  ...
  if (timedOut || status === "unknown") {
    return (
      <div className="grid gap-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600" data-testid="desktop-handoff-troubleshoot" aria-live="polite">
        <p className="m-0">
          Nothing opened?{" "}
          <button type="button" className="font-medium text-slate-950 underline-offset-4 hover:underline" onClick={() => window.location.assign(omnirushUrl)}>
            Return to OmniRush.ai again
          </button>
        </p>
        <div className="grid gap-2">
          <p className="m-0">Still stuck? Copy this sign-in link into OmniRush.ai:</p>
          <div className="flex flex-col gap-2 sm:flex-row">
            <input className="den-input min-w-0 flex-1 text-xs" value={omnirushUrl} readOnly onFocus={(event) => event.currentTarget.select()} />
            <button type="button" className="den-button-secondary sm:w-auto" onClick={() => void copyOmniRushUrl()}>
...
export function JoinOrgSuccess({
  ...
  const [desktopOmniRushUrl, setDesktopOmniRushUrl] = useState<string | null>(null);
  ...
  async function handleReturnToOmniRush() {
    setHandoffBusy(true);
    setActionError(null);
    ...
    try {
      ...
      if (!response.ok) {
        setActionError(getErrorMessage(payload, `Could not return to OmniRush.ai (${response.status}).`));
        return;
      }

      const omnirushUrl = getDesktopHandoffOmniRushUrl(payload);
      if (!omnirushUrl) {
        setActionError("OmniRush.ai sign-in was prepared, but no app link was returned.");
        return;
      }

      const grant = getDesktopHandoffGrant(payload, omnirushUrl);
      rememberDesktopHandoffGrant(grant);
      setDesktopOmniRushUrl(omnirushUrl);
      setDesktopGrant(grant);
      window.location.assign(omnirushUrl);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Could not return to OmniRush.ai.");
    } finally {
          ...
          <div className="grid gap-3">
            <h1 className="m-0 grid max-w-full gap-1 text-[30px] font-semibold leading-[38px] tracking-[-0.03em] text-slate-950 sm:text-[38px] sm:leading-[46px]">
              ...
              {/* No horizontal gap: the possessive must hug the org identity
                  ("Acme Robotics's OmniRush.ai", not "Acme Robotics 's"). */}
            ...
            <p className="m-0 max-w-2xl text-[15px] leading-[23px] text-slate-600">
              {desktopAuthRequested
                ? "Your team setup is ready. Return to OmniRush.ai to continue where you left off."
                : "The desktop app is where OmniRush.ai runs on your computer and puts your team's setup to work."}
            </p>
          </div>
          ...
          {isMobile === null ? (
            ...
            <div className="grid gap-3">
              <div className="grid gap-2 rounded-2xl bg-slate-50 p-4" data-testid="join-org-mobile-note">
                <p className="m-0 text-sm font-medium text-slate-950">OmniRush.ai runs on your computer.</p>
            ...
            desktopOmniRushUrl ? (
              <ReturnToOmniRushStatus omnirushUrl={desktopOmniRushUrl} grant={desktopGrant} organizationName={organizationName} />
            ) : (
              <button
                type="button"
                className="den-button-primary min-h-12 w-full"
                onClick={() => void handleReturnToOmniRush()}
                disabled={handoffBusy}
                data-testid="join-org-return-omnirush"
              >
                {handoffBusy ? "Returning to OmniRush.ai..." : "Return to OmniRush.ai"}
              </button>
            )
          ) : !platformReady ? (
            <p className="m-0 text-sm text-slate-500">Preparing your next step...</p>
            ...
            <div className="grid gap-3">
              ...
              >
                {handoffBusy ? "Opening OmniRush.ai..." : "Already have OmniRush.ai? Open it."}
              </button>
            </div>
          )}

          {desktopAuthRequested && desktopOmniRushUrl ? null : (
            <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700" data-testid="join-org-connected">
              Connected — OmniRush.ai is set up for {organizationName}
            </div>
          )}
          ...
          {actionError ? (
            <div className="grid gap-3">
              ...
              {desktopAuthRequested ? null : (
                <a href={OMNIRUSH_DOWNLOAD_URL} className="den-button-secondary min-h-12 w-full">
