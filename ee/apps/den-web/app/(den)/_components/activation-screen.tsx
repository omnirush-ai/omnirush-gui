...
const RETURN_TO_OMNIRUSH_URL = "omnirush://open";
...
function connectUrlFor(code: string, apiBaseUrl: string) {
  const url = new URL("omnirush://connect");
...
export function ActivationScreen() {
  ...
  if (state.kind === "expired" || state.kind === "error") {
    const message = state.kind === "expired"
      ? "This one-time activation link has expired. Return to the install page and open OmniRush.ai again."
      : state.message;
    return (
      <OnboardingShell state="activation-error" width="wide">
        <section className="grid gap-5 rounded-[1.75rem] border border-slate-200/80 bg-white p-6 sm:p-8" data-testid="activation-page">
          <p className="den-eyebrow">OmniRush.ai Desktop</p>
      ...
      >
        <div className="grid justify-items-start gap-3">
          ...
          <p className="den-copy">
            {connected
              ...
              : `OmniRush.ai will show ${details.organizationName} and its server before anything changes.`}
          </p>
        </div>
        ...
        {connected ? (
          <div className="grid gap-4" aria-live="polite">
            ...
            <a className="den-button-primary w-full justify-center sm:w-fit" href={RETURN_TO_OMNIRUSH_URL} data-testid="activation-return-omnirush">
              Return to OmniRush.ai
              <ExternalLink className="size-4" aria-hidden="true" />
            </a>
            <div className="grid gap-2 rounded-2xl bg-slate-50 p-4">
              <p className="m-0 text-sm text-slate-600">Nothing opened? Copy this OmniRush.ai link and open it from your browser.</p>
              <div className="flex flex-col gap-2 sm:flex-row">
                <input className="den-input min-w-0 flex-1 text-xs" value={RETURN_TO_OMNIRUSH_URL} readOnly onFocus={(event) => event.currentTarget.select()} />
                <button type="button" className="den-button-secondary sm:w-auto" onClick={() => void copyLink("return", RETURN_TO_OMNIRUSH_URL)}>
          ...
          <div className="grid gap-4" aria-live="polite">
            <button
              ...
              data-testid="activation-open-omnirush"
              onClick={() => {
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500">
              ...
              {openAttempted ? "Waiting for OmniRush.ai to accept this setup…" : "Waiting for your approval…"}
            </div>
            {openAttempted ? (
              <div className="grid gap-2 rounded-2xl bg-slate-50 p-4" data-testid="activation-open-fallback">
                <p className="m-0 text-sm text-slate-600">OmniRush.ai did not appear? Copy this one-time link and open it anywhere that handles OmniRush.ai links.</p>
                <div className="flex flex-col gap-2 sm:flex-row">
                  ...
                  <button type="button" className="den-button-secondary sm:w-auto" onClick={() => void copyLink("connect", connectUrl)}>
                    <Copy className="size-4" aria-hidden="true" />
                    {copied === "connect" ? "Copied" : "Copy OmniRush.ai link"}
                  </button>
                </div>
              </div>
            ) : null}
