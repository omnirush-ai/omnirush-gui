...
function getReauthCompleteMessage(data: unknown): ReauthCompleteMessage | null {
  ...
  if (!("type" in data) || data.type !== "omnirush:reauth-complete") {
    return null;
  }
  ...
  return { type: "omnirush:reauth-complete", nonce: data.nonce, error };
}
...
export function ReauthDialog({
  ...
  async function continueSocial(provider: SocialAuthProvider) {
    const popup = window.open("", "omnirush-reauth", "popup,width=480,height=640");
    if (!popup) {
      setError("OmniRush.ai could not open the sign-in window. Allow popups for OmniRush.ai, then try again.");
      return;
    }
