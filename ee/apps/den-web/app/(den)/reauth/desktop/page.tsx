...
function DesktopReauth() {
  ...
  async function verified() {
    ...
    const result = await requestJson("/api/auth/desktop-handoff", { method: "POST", body: JSON.stringify({ desktopScheme: "omnirush" }) });
    if (!result.response.ok) throw new Error(getErrorMessage(result.payload, "Could not return verification to OmniRush.ai. Try again."));
    ...
    const returned = new URL("omnirush://den-reauth");
  ...
  return <div className="den-page flex min-h-screen items-center justify-center p-6">
    <div className="den-frame grid w-full max-w-[520px] gap-4 p-6">
      <h1 className="den-title-lg">{link ? "Return to OmniRush.ai to finish sharing" : "Confirm your identity to share apps"}</h1>
      {!identity ? <p>Loading security check…</p> : !valid ? <p>Open verification from the Share dialog in OmniRush.ai to start a new security check.</p>
        : loadingSession ? <p>Checking your browser session…</p>
        : cancelled ? <><p>Verification cancelled. Return to the Share dialog in OmniRush.ai.</p><DenButton onClick={() => { setEmail(""); setCancelled(false); }}>Try verification again</DenButton></>
        : !email ? <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); setEmail(emailInput.trim()); }}>
          <p>Enter the email you use in OmniRush.ai to continue verification.</p>
          <label className="grid gap-2"><span>OmniRush.ai email</span><DenInput type="email" autoComplete="email" required value={emailInput} onChange={(event) => setEmailInput(event.target.value)} /></label>
        ...
        : link ? <>
          ...
          <a className={buttonVariants()} href={link}>Return to OmniRush.ai</a>
    ...
    <ReauthDialog open={valid && !loadingSession && Boolean(email) && !cancelled && !link} user={user} orgContext={null} onCancel={() => setCancelled(true)} onVerified={verified}
      title="Confirm your identity to share apps" description="After verification, return to OmniRush.ai to finish sharing your selected apps." />
  </div>;
}
