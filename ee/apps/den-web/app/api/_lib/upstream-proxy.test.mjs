...
describe("Den upstream proxy", () => {
  ...
  test("forwards only OmniRush.ai Den and legacy Better Auth cookies through the auth proxy", async () => {
    const { proxyUpstream } = await import("./upstream-proxy.ts");
    const request = new NextRequest("https://app.example.com/api/auth/sign-in/social", {
      method: "POST",
      headers: {
        cookie: "ph_posthog=analytics; __Secure-omnirush-den.state=oauth-state; omnirush-den.session_token=session; better-auth.state=legacy-oauth-state; __Secure-better-auth.session_token=legacy-session; other=value",
      },
    });

    await proxyUpstream(request, [], { routePrefix: "/api/auth", upstreamPathPrefix: "api/auth" });

    expect(observed.cookie).toBe("__Secure-omnirush-den.state=oauth-state; omnirush-den.session_token=session; better-auth.state=legacy-oauth-state; __Secure-better-auth.session_token=legacy-session");
  });
  ...
  test("preserves auth Set-Cookie parent domains shared by sibling web and API hosts", async () => {
    const { proxyUpstream } = await import("./upstream-proxy.ts");
    const originalFetch = globalThis.fetch;
    process.env.DEN_API_BASE = "https://api.omnirushlabs.com";
    globalThis.fetch = async () => {
      const headers = new Headers({ "content-type": "text/plain" });
      headers.append(
        "set-cookie",
        "__Secure-better-auth.session_token=abc; Path=/; Domain=omnirushlabs.com; Secure; HttpOnly; SameSite=Lax",
      );
      return new Response("signed in", { headers });
    };
    const request = new NextRequest("https://app.omnirushlabs.com/api/auth/callback/google", {
      method: "GET",
    });
