...
1. Install workspace deps from repo root:
   `pnpm install`
2. Run the app:
   `pnpm --filter @omnirush-ee/den-web dev`
3. Open:
   `http://localhost:3005`
...
- `DEN_WEB_OMNIRUSH_AUTH_CALLBACK_URL` (runtime): Canonical URL where the app returns after auth completes.
...
- `DEN_MCP_CLAIM_NAMESPACE`: namespace used for MCP token claim URIs. Leave blank to use `BETTER_AUTH_URL`; set a stable value before issuing tokens if hosts may change. Use `https://omnirushlabs.com` to preserve the original hosted MCP claim names.
...
- Build command: `cd ../../.. && pnpm --filter @omnirush-ee/den-web build`
- Output directory: `.next`
- Install command: use the command committed in `vercel.json`

The filtered install includes Den Web's workspace dependency closure, so shared packages like `@omnirush-ee/utils` remain available without installing unrelated native desktop and server dependencies.
