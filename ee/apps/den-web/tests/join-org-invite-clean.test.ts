...
describe("join organization invite clean layout contract", () => {
  ...
  test("removes stacked frames while using the shared wide welcome card hierarchy", () => {
    ...
    expect(cardSource).toContain("/omnirush-mark.svg");
    expect(cardSource).toContain("OmniRush.ai Cloud");
  ...
  test("keeps onboarding focused and reuses the responsive platform grid", () => {
    ...
    expect(successSource).toContain("Already have OmniRush.ai? Open it.");
    ...
    expect(successSource).toContain("Return to OmniRush.ai");
    ...
    expect(successSource).not.toContain("Open OmniRush.ai");
    ...
    expect(installSource).toContain("Set up OmniRush.ai Enterprise");
    expect(installSource).toContain("Download OmniRush.ai");
