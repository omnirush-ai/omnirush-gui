...
test("preserves the complete OmniRush.ai desktop handoff URL", () => {
  const omnirushUrl = "omnirush://den-auth?grant=one-time-code&denBaseUrl=https%3A%2F%2Fapi.example.test";
  const payload = { grant: "one-time-code", omnirushUrl };

  expect(getDesktopHandoffOmniRushUrl(payload)).toBe(omnirushUrl);
  expect(getDesktopHandoffGrant(payload, omnirushUrl)).toBe("one-time-code");
});

test("extracts a one-time grant from an OmniRush.ai desktop handoff", () => {
  expect(
    getDesktopGrant(
      "omnirush://den-auth?grant=one-time-code&baseUrl=https%3A%2F%2Fapi.example.test"
    )
  ).toBe("one-time-code");
});
...
test("rejects missing and malformed desktop handoffs", () => {
  expect(
    getDesktopGrant(
      "omnirush://den-auth?baseUrl=https%3A%2F%2Fapi.example.test"
    )
  ).toBeNull();
...
test("restores only fresh tab-scoped setup with a user and a known route", () => {
  const pending = { userId: "user-1", desktopScheme: "omnirush", setup: { organizationId: "org-1", route: "/dashboard/onboarding/tools" }, at: Date.now() };
  ...
  for (const invalid of [
    ...
    { ...pending, desktopScheme: "omnirush://" },
...
const account = { id: "user-1", email: "member@omnirush.test", name: "Member" };
...
test("a different authenticated user cannot inherit an earlier user's setup or desktop return", async () => {
  await withFlow({ stored: { userId: "old-user", desktopScheme: "omnirush", setup: { organizationId: "old-org", route: "/dashboard/onboarding/tools" }, at: Date.now() } }, async ({ state, paths }) => {
...
test.each([runtime.EMPTY_RUNTIME_CONFIG, { ...runtime.EMPTY_RUNTIME_CONFIG }])("fallback or single-org config never discards unfinished setup", async (config) => {
  ...
  await withFlow({ config: Promise.resolve(config), stored: { userId: account.id, desktopScheme: "omnirush", setup, at: Date.now() } }, async ({ state, paths }) => {
...
test("sign-out discards a late handoff result without opening the app", async () => {
  ...
  await withFlow({ reply: async (path) => path === "/api/auth/desktop-handoff" ? handoff.promise
    ...
    await act(async () => state().signOut());
    await act(async () => handoff.resolve({ payload: { grant: "stale-test-grant", omnirushUrl: "omnirush://den-auth?grant=stale-test-grant" } }));
...
test("a returning user's failed handoff can retry without signing in again", async () => {
  ...
  await withFlow({ reply: async (path) => {
    if (path === "/api/auth/desktop-handoff") {
      attempts += 1;
      return attempts === 1 ? { status: 503, payload: { message: "Try again" } } : { payload: { grant: "test-grant", omnirushUrl: "omnirush://den-auth?grant=test-grant" } };
    }
    ...
    expect(opened).toEqual(["omnirush://den-auth?grant=test-grant"]);
    expect(sessionStorage.getItem(SETUP_CONTINUATION_KEY)).toBeNull();
  });
});
