...
import { DEFAULT_OMNIRUSH_WEB_URL } from "../app/(den)/_lib/runtime-config";
import {
...
const originalEnv = {
  ...
  DEN_WEB_OMNIRUSH_WEB_URL: process.env.DEN_WEB_OMNIRUSH_WEB_URL,
};
...
afterEach(() => {
  ...
  restoreEnvValue("DEN_WEB_OMNIRUSH_WEB_URL");
});
...
describe("Web dashboard page", () => {
  ...
  test("renders the external Web button with the configured href", () => {
    ...
    expect(html).toContain(`href="${DEFAULT_OMNIRUSH_WEB_URL}"`);
    expect(html).toContain('target="_blank"');
    expect(html).toContain('rel="noopener noreferrer"');
    expect(html).toContain("Open OmniRush.ai Web");
  });
  ...
  test("renders the exact per-user purchase action", () => {
    ...
    expect(html).toContain("Purchase OmniRush.ai Web — $50 per member/month");
  });
  ...
  test("pins checkout and confirmation to Web billing and never unlocks from the redirect alone", () => {
    ...
    expect(source).toContain("JSON.stringify({ type: OMNIRUSH_WEB_CHECKOUT_TYPE })");
    expect(source).toContain("JSON.stringify({ sessionId, type: OMNIRUSH_WEB_CHECKOUT_TYPE })");
    expect(source).toContain("headers: { [ORG_SCOPE_HEADER]: orgId }");
    expect(source).toContain("nextBilling?.hasAccess");
    expect(source).toContain("OmniRush.ai Web remains locked");
    ...
    expect(source).toContain("orgContext?.capabilities.omnirushWeb === true");
  ...
  test("runtime config exposes the default Web URL and deployment override", async () => {
    ...
    process.env.DEN_BASE_URL = "https://app.omnirushlabs.com";
    delete process.env.DEN_WEB_OMNIRUSH_WEB_URL;

    const defaultPayload: unknown = await (await GET()).json();
    expect(readStringProperty(defaultPayload, "omnirushWebUrl")).toBe(DEFAULT_OMNIRUSH_WEB_URL);

    process.env.DEN_WEB_OMNIRUSH_WEB_URL = "https://self-hosted.example.test";
    const overridePayload: unknown = await (await GET()).json();
    expect(readStringProperty(overridePayload, "omnirushWebUrl")).toBe("https://self-hosted.example.test");
  });
  ...
  test("runtime config exposes the public Den API URL without leaking the internal API base", async () => {
    ...
    process.env.DEN_API_BASE = "http://omnirush-ee-den-api:8788";
    ...
    expect(JSON.stringify(publicPayload)).not.toContain("omnirush-ee-den-api");
    ...
    expect(JSON.stringify(derivedPayload)).not.toContain("omnirush-ee-den-api");
  });
});
