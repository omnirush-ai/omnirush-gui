...
describe("Den API build-time redirect rules", () => {
  ...
  test("prefers an explicit DEN_API_BASE origin", () => {
    const env = { DEN_API_BASE: "https://api.omnirushlabs.com/", DEN_BASE_URL: "https://app.omnirushlabs.com" };

    expect(denApiRedirectOrigin(env)).toBe("https://api.omnirushlabs.com");
    expect(denApiRedirects(env)).toEqual([
      {
        source: "/api/den/:path*",
        destination: "https://api.omnirushlabs.com/:path*",
        permanent: false,
      },
    ]);
  });
  ...
  test("derives the api-prefixed host from DEN_BASE_URL", () => {
    expect(denApiRedirectOrigin({ DEN_BASE_URL: "https://app.omnirushlabs.com" })).toBe("https://api.app.omnirushlabs.com");
  ...
  test("ignores a DEN_API_BASE without a scheme, like the route handler", () => {
    const env = { DEN_API_BASE: "api.omnirushlabs.com", DEN_BASE_URL: "https://app.omnirushlabs.com" };

    expect(denApiRedirectOrigin(env)).toBe("https://api.app.omnirushlabs.com");
  });

  test("matches the route handler's Location for every supported env shape", () => {
    const shapes: RedirectEnv[] = [
      { DEN_API_BASE: "https://api.omnirushlabs.com", DEN_BASE_URL: "https://app.omnirushlabs.com" },
      { DEN_BASE_URL: "https://app.omnirushlabs.com" },
      { DEN_BASE_URL: "http://localhost:3005", DEN_API_BASE: "http://127.0.0.1:8790" },
      { DEN_WEB_PUBLIC_ORIGIN: "https://den.example.org" },
      { DEN_API_BASE: "not a url", DEN_BASE_URL: "https://app.omnirushlabs.com" },
    ];
