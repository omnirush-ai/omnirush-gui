...
describe("OmniRush.ai Web billing data", () => {
  ...
  test("billing presents plan, quantity definition, unit-total math, lifecycle, and management", () => {
    ...
    expect(OMNIRUSH_WEB_QUANTITY_EXPLANATION).toContain("Pending invitations are not billed");
    expect(source).toContain('data-testid="billing-omnirush-web-card"');
    expect(source).toContain('label="Plan" value="OmniRush.ai Web"');
    ...
    expect(source).toContain('data-testid="billing-omnirush-web-lifecycle"');
  ...
  test("platform admin UI exposes an audited complimentary access action separate from capabilities", () => {
    ...
    expect(source).toContain('data-testid="admin-omnirush-web-access"');
    expect(source).toContain("OmniRush.ai Web billing access");
    ...
    expect(source).toContain("/omnirush-web-access");
  });
});
