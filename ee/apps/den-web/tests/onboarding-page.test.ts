...
describe("Marketplace onboarding page", () => {
  test("finishes setup without downloads or an installation checklist", () => {
    expect(screen).not.toContain("DownloadOmniRushCard");
  ...
  test("offers OmniRush.ai Models and Bring your Own Keys as the model path", () => {
    expect(screen).toContain("onboarding-choice-omnirush-models");
    ...
    expect(screen).toContain("/omnirush-mark.svg");
  });

  test("checks model status without enabling models", () => {
    expect(screen).toContain("/v1/inference");
    expect(screen).toContain('headers: { "x-omnirush-org-id": orgId }');
