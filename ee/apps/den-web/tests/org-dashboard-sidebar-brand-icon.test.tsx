...
describe("Den dashboard sidebar brand icon", () => {
  ...
  test("renders the canonical managed square icon without first rendering the OmniRush.ai mark", () => {
    const markup = renderToStaticMarkup(
  ...
  test("uses the OmniRush.ai mark when no managed square icon exists", () => {
    const markup = renderToStaticMarkup(
      <SidebarBrandMark metadata={null} organizationName="Acme" />,
    );

    expect(markup).toContain("<svg");
    expect(markup).toContain('aria-label="OmniRush.ai"');
