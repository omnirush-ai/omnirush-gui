...
test("Den Web never treats Daytona list or detail URLs as durable connections", () => {
  ...
  expect(getWorker({ worker, instance: daytonaInstance, tokens: {} })?.omnirushUrl).toBeNull();
...
test("Den Web keeps durable URLs and separates stable connections from direct previews", () => {
  ...
  const tokens = getWorkerTokens({
    ...
    connect: { omnirushUrl: "https://den.example.test/v1/cloud/workers/worker-1/w/workspace", workspaceId: "workspace" },
    directPreview: {
      version: 1,
      omnirushUrl: "https://fresh.preview.example.test/w/workspace",
      workspaceId: "workspace",
      expiresAt: "2026-08-27T12:00:00.000Z",
    },
  });
  expect(tokens?.omnirushUrl).toBe("https://den.example.test/v1/cloud/workers/worker-1/w/workspace");
  expect(tokens?.previewOmniRushUrl).toBe("https://fresh.preview.example.test/w/workspace");
  ...
  const desktopLink = buildOmniRushDeepLink(
    connectedWorker.omnirushUrl,
    connectionTokens.desktopToken,
    connectedWorker.workerId,
    connectedWorker.workerName,
  );
  const webLink = buildOmniRushAppConnectUrl(
    "https://app.example.test/connect-remote",
    connectedWorker.previewOmniRushUrl ?? null,
    connectionTokens.webToken,
    connectedWorker.workerId,
    connectedWorker.workerName,
  );
  expect(new URL(desktopLink ?? "").searchParams.get("omnirushToken")).toBe("host-token");
  expect(new URL(webLink ?? "").searchParams.get("omnirushToken")).toBe("client-token");
})
...
test("create tokens without a URL keep polling until the late resolver URL is adopted", () => {
  ...
  expect(created.omnirushUrl).toBeNull();
  ...
  const late = getWorkerTokens({
    tokens: { client: "client-token", owner: "host-token", host: "host-token" },
    connect: {
      omnirushUrl: "https://den.example.test/v1/cloud/workers/worker-late-url/w/workspace",
      workspaceId: "workspace",
    },
    directPreview: {
      version: 1,
      omnirushUrl: "https://late.preview.example.test/w/workspace",
      workspaceId: "workspace",
      expiresAt: "2026-08-27T12:00:00.000Z",
    },
  });
  ...
  expect(ready.omnirushUrl).toBe("https://den.example.test/v1/cloud/workers/worker-late-url/w/workspace");
  expect(ready.previewOmniRushUrl).toBe("https://late.preview.example.test/w/workspace");
