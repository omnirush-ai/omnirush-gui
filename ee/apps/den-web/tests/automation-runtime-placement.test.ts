...
describe("Automation runtime placement", () => {
  test("Den's My Automations is a monitor that routes management to the creating surface", () => {
    ...
    expect(screen).toContain("Create and edit Cloud Automations in OmniRush.ai Web; Desktop Automations are managed in the desktop app.");
    expect(screen).toContain("Open in OmniRush.ai Web");
    expect(screen).toContain("Manage in OmniRush.ai Web");
    expect(screen).toContain("Manage in OmniRush.ai Desktop");
  ...
  test("Workflow → Automate deep-links into the OmniRush.ai Web editor with the exact version pinned", () => {
    const detail = read("../app/(den)/dashboard/_components/workflow-detail-screen.tsx");

    expect(detail).toContain("Automate in OmniRush.ai Web");
