...
const upstream = createServer(async (request, response) => {
  ...
  if (url.pathname === "/api/den/v1/auth/desktop-handoff" && request.method === "POST") {
    ...
    const omnirushUrl = new URL("omnirush://den-auth")
    omnirushUrl.searchParams.set("grant", "one-time-smoke-grant")
    omnirushUrl.searchParams.set("denBaseUrl", `${upstreamOrigin}/api/den`)
    return json(response, 200, { omnirushUrl: omnirushUrl.toString() })
  }
...
try {
  ...
  const routeCookie = browserCookies.find((cookie) => cookie.startsWith("omnirush_connect_debug_route="))?.split(";", 1)[0]
  ...
  const handoffDenBaseUrl = new URL(handoffPayload.omnirushUrl).searchParams.get("denBaseUrl")
