# OmniRush.ai Diagnostics

OmniRush.ai Diagnostics is a deliberately small, Vercel-native MCP compatibility
...
record of metadata-only OmniRush.ai Connect incidents. It correlates what a
...
NetworkPolicy path. OmniRush.ai support can filter the dashboard by the resulting
...
```bash
pnpm --filter @omnirush-ee/diagnostics dev
```

Open `http://localhost:3010` and sign in with:

- username: `diagnostics-admin`
- password: `OmniRushDiagnosticsLocal!`

The local MCP endpoint is `http://localhost:3010/mcp` with synthetic bearer
token `OmniRushDiagnosticsToken!`. Local history is process-memory only.
...
The same editor is visible on the OmniRush.ai Connect sign-in surface while signed
out. Run the standalone local MCP smoke journey with:

```bash
pnpm --filter @omnirush-ee/diagnostics smoke:debug-proxy
```
...
DEN_DIAGNOSTICS_BEARER_TOKEN=OmniRushDiagnosticsToken!
```
...
shared `@omnirush/types` package. Link an Upstash Redis database from the
...
| `NEXT_PUBLIC_DIAGNOSTICS_ORIGIN` | Fixed production origin, normally `https://diagnostic.omnirushlabs.com`. Preview deployments use Vercel's deployment-specific `VERCEL_URL` instead. |
...
| `DEBUG_PROXY_DEFAULT_UPSTREAM` | Default Den origin. Falls back to `https://app.omnirushlabs.com`; set it explicitly on the Vercel project. |
...
Attach `diagnostic.omnirushlabs.com` in the project's Vercel **Domains**
...
```text
https://diagnostic.omnirushlabs.com/mcp
```
...
mint its short-lived MCP token, and manage the hidden `omnirush-cloud` MCP
entry automatically.
...
one-time `omnirush://den-auth` handoff also has its encoded Den base URL
...
```bash
pnpm --filter @omnirush-ee/diagnostics test
pnpm --filter @omnirush-ee/diagnostics build
pnpm --filter @omnirush-ee/diagnostics smoke:debug-proxy
```
...
1. `GET https://diagnostic.omnirushlabs.com/health` returns HTTP 200 and
   `{"service":"omnirush-diagnostics","status":"ok"}`.
...
- app, OmniRush.ai server, engine, and platform versions;
...
HTTP reached OmniRush.ai. If a row exists, its response status and next missing
...
returns it to the browser. Den uses `https://diagnostic.omnirushlabs.com` by
...
```dotenv
DEN_DIAGNOSTICS_ORIGIN=https://diagnostic.omnirushlabs.com
