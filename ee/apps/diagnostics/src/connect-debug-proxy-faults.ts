...
declare global {
  var __omnirushConnectDebugProxyFlakyWindows: Map<string, FlakyWindow> | undefined
}

const flakyWindows = globalThis.__omnirushConnectDebugProxyFlakyWindows ??= new Map()
...
export function connectDebugProxyFault(input: {
  ...
  if (input.scenario.fault === "auth-expired") {
    return {
      kind: "response",
      label: "auth-expired",
      response: jsonError(401, "invalid_token", "The debug scenario expired the OmniRush.ai Connect token.", {
        "www-authenticate": "Bearer error=\"invalid_token\", error_description=\"OmniRush.ai Connect debug token expired\"",
      }),
    }
  }
