...
async function makeWebRoot() {
  ...
  await writeFile(join(root, "index.html"), "<!doctype html><div id=\"root\">OmniRush.ai App</div>")
  await writeFile(join(root, "assets", "app.js"), "globalThis.__omnirushTest = true;")
  return root
}
...
function startDenApi(resolvePayload) {
  ...
  const server = startServer((request) => {
    ...
    observed.gatewayKey = request.headers.get("x-omnirush-gateway-key")
...
function startPassthroughDenApi() {
  ...
  const server = startServer(async (request) => {
    ...
    observed.requests.push({
      ...
      hostToken: request.headers.get("x-omnirush-host-token"),
      cookie: request.headers.get("cookie"),
      gatewayKey: request.headers.get("x-omnirush-gateway-key"),
...
describe("gateway build version", () => {
  test("prefers the explicit version and falls back to Render's commit", () => {
    expect(resolveGatewayBuildVersion({ denGatewayVersion: " omnirush-0.19.0 ", renderGitCommit: " render-sha " })).toBe("omnirush-0.19.0")
...
describe("den-gateway static UI", () => {
  test("serves index, falls back for deep routes, hard-404s asset misses, rejects traversal, and caches assets immutably", async () => {
    ...
    expect(await index.text()).toContain("OmniRush.ai App")

    const deep = await fetch(`${base}/sessions/deep/link`)
    expect(deep.status).toBe(200)
    expect(await deep.text()).toContain("OmniRush.ai App")
    ...
    expect(await missingAsset.text()).not.toContain("OmniRush.ai App")
  ...
  test("injects the gateway runtime marker into index.html without a bootstrap token", async () => {
    ...
    expect(html).toContain("window.__OMNIRUSH_GATEWAY__ = {\"version\":1}")
    expect(html).not.toContain("__OMNIRUSH_BOOTSTRAP__")
  ...
  test("identifies the configured gateway build in the runtime marker and status", async () => {
    const root = await makeWebRoot()
    const gateway = startGateway({ webRoot: root, buildVersion: "omnirush-0.19.0" })
    const base = serverBase(gateway)
    ...
    expect(await index.text()).toContain("window.__OMNIRUSH_GATEWAY__ = {\"version\":1,\"build\":\"omnirush-0.19.0\"}")
    await expect(health.json()).resolves.toEqual({ ok: true, service: "den-gateway", build: "omnirush-0.19.0" })
    await expect(ready.json()).resolves.toEqual({ ok: true, service: "den-gateway", build: "omnirush-0.19.0" })
  })
})
...
describe("den-gateway proxy", () => {
  test("passes through Den's Web access denial without resolving an instance", async () => {
    ...
    const gateway = startGateway({
      ...
      fetchImpl: async (url) => {
        if (new URL(url).pathname === "/v1/cloud/gateway/resolve") {
          return Response.json({ error: "omnirush_web_access_required" }, { status: 403 })
        }
        instanceCalls += 1
  ...
  test("passes /api/den through to den-api with the caller bearer, no cookies, and the prefix stripped", async () => {
    ...
    const response = await fetch(`${serverBase(gateway)}/api/den/v1/me?expand=org`, {
      ...
      headers: {
        ...
        "X-OmniRush-Host-Token": "browser-host-token",
  ...
  test("serves workspace document navigations from the SPA but proxies workspace API calls", async () => {
    ...
    expect(await navigation.text()).toContain("OmniRush.ai App")
    expect(upstream.observed.requests).toHaveLength(0)
  ...
  test("keeps /w navigations proxied and non-api navigations on the SPA", async () => {
    ...
    expect(await settings.text()).toContain("OmniRush.ai App")
