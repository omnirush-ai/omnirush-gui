...
import { createJsonStdoutLogger, type JsonObject, type JsonStdoutLogger } from "@omnirush-ee/utils/observability"
...
export function shouldProxyToInstance(request: Request) {
  ...
  // /workspace/ is ambiguous: omnirush-server owns API routes here, while the
...
function injectGatewayMarker(html: string, buildVersion: string | undefined) {
  ...
  const script = `<script>window.__OMNIRUSH_GATEWAY__ = ${marker}</script>`
...
function upstreamRequestHeaders(headers: Headers, clientToken: string, hostToken: string) {
  ...
  upstream.delete("x-omnirush-host-token")
  ...
  upstream.set("x-omnirush-host-token", hostToken)
  return upstream
}
...
function denApiRequestHeaders(request: Request) {
  ...
  request.headers.forEach((value, name) => {
    ...
    if (normalized === "x-omnirush-host-token") return
