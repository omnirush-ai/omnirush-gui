...
import { and, asc, eq, inArray, isNull } from "@omnirush-ee/den-db/drizzle"
import { WorkerTable, WorkerTokenTable } from "@omnirush-ee/den-db/schema"
...
const omniRushWebAccessRequiredSchema = z.object({
  ...
  message: z.string(),
}).meta({ ref: "OmniRushWebAccessRequiredError" })
...
const gatewayKeyHeader = "X-OmniRush-Gateway-Key"
  ...
  async insertCloudWorkerWithTokens(input) {
    await db.transaction(async (tx) => {
      await tx.insert(WorkerTable).values({
        ...
        name: input.name,
        description: "OmniRush.ai Cloud browser instance",
        destination: "cloud",
        status: "provisioning",
...
export function registerCloudRoutes<T extends { Variables: OrgRouteVariables }>(
  ...
  const getOmniRushWebAccess = options.getOmniRushWebAccess ?? getOmniRushWebRuntimeAccess
  ...
  app.get(
    ...
    describeRoute({
      ...
      description: "Starts the active organization's OmniRush.ai Cloud browser instance when needed and returns its browser URL once ready.",
      responses: {
        ...
        403: jsonResponse("OmniRush.ai Web access is not active for the organization.", omniRushWebAccessRequiredSchema),
    ...
    async (c) => {
      ...
      // runtime, after OmniRushWebAccessGate (v0.18.42+) has already resolved
      // Web access for the organization; see omnirush-web-runtime-access.ts.
      const webAccess = await getOmniRushWebAccess(payload.organization.id)
      if (!webAccess.hasAccess) {
        return c.json(omniRushWebAccessRequiredPayload(), 403)
      }
  ...
  app.get(
    ...
    describeRoute({
      ...
      description: "Starts or wakes the caller's own OmniRush.ai Cloud browser instance when needed and returns the collaborator token only to the trusted gateway.",
      responses: {
