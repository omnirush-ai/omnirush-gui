import { and, eq, gt, isNotNull, isNull } from "@omnirush-ee/den-db/drizzle"
...
import { createDenTypeId, normalizeDenTypeId, parseSkillMarkdown } from "@omnirush-ee/utils"
...
const STARTER_SKILL_OUTPUT = "OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED"

const bootstrapWorkspaceSchema = z.object({
  workspaceName: z.string().trim().min(2).max(120),
  skillName: z.string().trim().min(1).max(120).default("First OmniRush.ai Skill"),
...
function starterSkillText(name: string) {
  return `---\nname: ${name}\ndescription: Starter skill created by OmniRush.ai agent bootstrap.\nomnirushBootstrapTrigger: bootstrap.verify\nomnirushBootstrapOutput: ${JSON.stringify(STARTER_SKILL_OUTPUT)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${STARTER_SKILL_OUTPUT}\`\n`
}
...
function skillMetadata(skillText: string) {
  ...
  if (parsed.hasFrontmatter) {
    return {
      ...
      description: (parsed.description.trim() || "Starter skill created by OmniRush.ai agent bootstrap.").slice(0, 65535),
    }
  }
