import { and, asc, desc, eq, gte, inArray, isNotNull, isNull, sql } from "@omnirush-ee/den-db/drizzle"
import type { SQL } from "@omnirush-ee/den-db/drizzle"
import {
  AuthAccountTable,
  AuthApiKeyTable,
  AuthSessionTable,
...
import { createDenTypeId, isDenTypeId } from "@omnirush-ee/utils/typeid"
import { ManagedModelsPolicyError, readOrganizationMetadata } from "@omnirush/types/den/managed-models-policy"
...
import { hasOmniRushWebComplimentaryAccess, resolveOmniRushWebAccess, setOmniRushWebComplimentaryAccess } from "../../omnirush-web-access.js"
import { isOmniRushWebAvailable } from "../../omnirush-web-availability.js"
import { calculateOrganizationSeatBillingCounts, getOrganizationSeatBillingCounts, isEligibleOmniRushWebSubscriptionStatus, isOngoingOmniRushWebSubscriptionStatus, organizationHasOngoingOmniRushWebSubscription, refreshOrgSubscriptionFromStripe, syncSeatSubscriptionQuantityAfterMemberChange } from "../../stripe-billing.js"
...
const updateOrganizationOmniRushWebAccessSchema = z.object({
  enabled: z.boolean(),
...
function readAdminOmniRushWebAccess(
  metadata: Record<string, unknown> | string | null | undefined,
  subscription: AdminOmniRushWebSubscription | null,
) {
  const complimentaryAccess = hasOmniRushWebComplimentaryAccess(metadata)
  const hasEligibleSubscription = Boolean(
    subscription
    && isEligibleOmniRushWebSubscriptionStatus(subscription.status)
    && env.stripe.omnirushWebPriceId
    && subscription.stripe_price_id === env.stripe.omnirushWebPriceId
  ...
  return {
    ...access,
    hasEligibleSubscription,
    hasOngoingSubscription: Boolean(subscription && isOngoingOmniRushWebSubscriptionStatus(subscription.status)),
...
function readUnmanagedCapabilityMetadata(metadata: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    // OmniRush.ai Web access instead), so stale stored overrides stay managed
...
type AdminOmniRushWebSubscription = Pick<
  typeof OrgSubscriptionTable.$inferSelect,
  "status" | "stripe_price_id" | "payment_failed"
>

type AdminOmniRushWebAccess = ReturnType<typeof readAdminOmniRushWebAccess>
...
async function shapeAdminOrganizationRows(rows: Array<Pick<typeof OrganizationTable.$inferSelect, "id" | "name" | "slug" | "metadata" | "createdAt" | "updatedAt">>): Promise<AdminOrganizationRow[]> {
  ...
  return rows.map((entry) => {
    ...
    return {
      ...
      omnirushWebAccess: readAdminOmniRushWebAccess(metadata, webSubscriptionByOrg.get(entry.id) ?? null),
    }
  })
}
...
export function registerAdminRoutes<T extends { Variables: AuthContextVariables }>(app: Hono<T>) {
  ...
  app.put(
    "/v1/admin/organizations/:organizationId/omnirush-web-access",
    adminRoute(),
    async (c) => {
      const body = updateOrganizationOmniRushWebAccessSchema.safeParse(await c.req.json().catch(() => null))
      if (!body.success) {
        return c.json({ error: "invalid_request", message: body.error.issues[0]?.message ?? "Invalid OmniRush.ai Web access request." }, 400)
      }
      ...
      if (body.data.enabled && await organizationHasOngoingOmniRushWebSubscription(organizationId)) {
        return c.json({
          error: "omnirush_web_subscription_exists",
          message: "Cancel or finish the existing paid OmniRush.ai Web subscription before granting complimentary access.",
        }, 409)
      }
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (body.data.enabled && webSubscription && isOngoingOmniRushWebSubscriptionStatus(webSubscription.status)) {
          return "subscription_exists"
        }

        const metadata = setOmniRushWebComplimentaryAccess(organization.metadata, body.data.enabled)
        const auditEvent = buildOrganizationAuditEvent({
          organizationId,
          actorUserId,
          action: body.data.enabled
            ? ORGANIZATION_AUDIT_ACTIONS.omniRushWebComplimentaryAccessGranted
            : ORGANIZATION_AUDIT_ACTIONS.omniRushWebComplimentaryAccessRevoked,
          payload: {
      ...
      return c.json({
        ...
        organization: {
          id: organizationId,
          omnirushWebAccess: readAdminOmniRushWebAccess(result.metadata, result.webSubscription),
        },
      })
    },
  )
