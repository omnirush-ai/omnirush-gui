...
import { AuthAccountTable, AuthUserTable, RateLimitTable } from "@omnirush-ee/den-db/schema"
import { createDenTypeId, normalizeDenTypeId } from "@omnirush-ee/utils/typeid"
import { desktopConfigSchema } from "@omnirush/types/den/desktop-policies"
...
function normalizeAuthProvider(providerId: string) {
  ...
  if (normalized.startsWith("omnirush-sso-")) {
    return "sso"
  }
  if (normalized.startsWith("omnirush-scim-")) {
    return "scim"
  }
...
export function registerMeRoutes<T extends { Variables: AuthContextVariables & Partial<UserOrganizationsContext> & Partial<OrganizationContextVariables> }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      tags: ["Users"],
      summary: "Send current user the OmniRush.ai desktop download link",
      description: "Emails the authenticated user a link to download the OmniRush.ai desktop app.",
      responses: {
    ...
    async (c) => {
      ...
      try {
        await sendEmail({
          ...
          props: {
            downloadUrl: OMNIRUSH_DOWNLOAD_URL,
          },
        })
      } catch (error) {
