...
import { OmniRushWebAccessRequiredError } from "../../omnirush-web-runtime-access.js"
...
const omniRushWebAccessRequiredSchema = z.object({
  error: z.literal("omnirush_web_access_required"),
  message: z.string(),
}).meta({ ref: "AutomationOmniRushWebAccessRequiredError" })
...
function failure(error: unknown): { status: 400 | 403 | 404 | 409; body: { error: string; message?: string } } | null {
  if (error instanceof OmniRushWebAccessRequiredError) {
  ...
  if (error.message === "automation_action_target_mismatch") {
    return { status: 400, body: { error: "automation_action_target_mismatch", message: "Desktop creates local Automations; Web creates OmniRush.ai Cloud Automations." } }
  }
  ...
  if (error.message === "automation_cloud_worker_required") {
    return { status: 409, body: { error: error.message, message: "Set up OmniRush.ai Cloud before creating a Cloud Automation." } }
  }
...
const routeDescription = [
  ...
  "Automations created by Desktop run on the owner's connected desktop; Automations created by Web run in OmniRush.ai Cloud.",
  "If no desktop runner is connected when a desktop occurrence is due, that occurrence is recorded as missed.",
  "Creation makes an Automation active immediately and uses the owner's current OmniRush.ai Connect integrations.",
...
export function registerAutomationRoutes<T extends { Variables: RouteVariables }>(
  ...
  app.post(
    ...
    describeNonMcpRoute({
      ...
      responses: {
        ...
        403: jsonResponse("OmniRush.ai Web access is required.", omniRushWebAccessRequiredSchema),
  ...
  app.post(
    ...
    describeMcpRoute({
      ...
      summary: "Create an active OmniRush.ai Cloud Automation",
      description: `${routeDescription} This is the Web and Cloud Chat creation surface. Placement is fixed to OmniRush.ai Cloud and the Automation can wake a stopped Cloud container without a desktop. Create only when the person explicitly asks to create or schedule it; there is no draft step.`,
      responses: {
  ...
  app.patch(
    ...
    describeMcpRoute({
      ...
      responses: {
        ...
        400: jsonResponse("Invalid request.", invalidRequestSchema),
        403: jsonResponse("OmniRush.ai Web access is required for Cloud Automations.", omniRushWebAccessRequiredSchema),
      },
    }),
  ...
  const stateRoute = (
    ...
    describeMcpRoute({
      ...
      responses: {
        ...
        ...(action === "activate" ? {
          403: jsonResponse("OmniRush.ai Web access is required to activate a Cloud Automation.", omniRushWebAccessRequiredSchema),
        } : {}),
  ...
  app.post(
    ...
    describeMcpRoute({
      ...
      responses: {
        ...
        403: jsonResponse("OmniRush.ai Web access is required to run a Cloud Automation.", omniRushWebAccessRequiredSchema),
        404: jsonResponse("Not found.", notFoundSchema),
      },
    }),
    orgMemberRoute(), paramValidator(idParamsSchema),
