import { desc, eq } from "@omnirush-ee/den-db/drizzle"
...
import { getOmniRushWebRuntimeAccess, omniRushWebAccessRequiredPayload } from "../../omnirush-web-runtime-access.js"
...
const workerTokensResponseSchema = z.object({
  ...
  connect: z.object({
    omnirushUrl: z.string().nullable(),
    workspaceId: z.string().nullable(),
  }).nullable(),
  directPreview: z.object({
    version: z.literal(1),
    omnirushUrl: z.string(),
    workspaceId: z.string().nullable(),
...
const workerTokensRequestSchema = z.object({
  ...
  includeExpiringOmniRushUrl: z.boolean().optional(),
...
const omniRushWebAccessRequiredSchema = z.object({
  ...
  message: z.string(),
}).meta({ ref: "WorkerOmniRushWebAccessRequiredError" })
...
export function registerWorkerCoreRoutes<T extends { Variables: WorkerRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      responses: {
        ...
        403: jsonResponse("OmniRush.ai Web access is required to launch a cloud worker.", omniRushWebAccessRequiredSchema),
    ...
    if (input.destination === "cloud") {
      const webAccess = await getOmniRushWebRuntimeAccess(orgId)
      ...
      const access = await requireCloudAccessOrPayment({
        ...
        email,
        name: user.name ?? user.email ?? "OmniRush.ai User",
      })

      if (!access.allowed) {
        return c.json({
          error: "cloud_worker_billing_unavailable",
          message: "Creating new cloud workers requires an existing OmniRush.ai Cloud plan. New self-serve purchases are no longer available.",
        }, 402)
      }
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Returns connection tokens and the resolved OmniRush.ai connect URL for an existing worker.",
      responses: {
        ...
        403: jsonResponse("OmniRush.ai Web access is required to use cloud worker tokens.", omniRushWebAccessRequiredSchema),
        404: jsonResponse("The worker could not be found.", notFoundSchema),
    ...
    const resolved = await getWorkerTokensAndConnect(worker, {
      includeExpiringOmniRushUrl: requestBody.success && requestBody.data.includeExpiringOmniRushUrl === true,
    })
