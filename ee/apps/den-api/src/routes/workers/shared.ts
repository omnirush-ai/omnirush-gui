...
import { and, asc, desc, eq, inArray, isNull } from "@omnirush-ee/den-db/drizzle"
import {
  AuditEventTable,
  AuthUserTable,
  DaytonaSandboxTable,
...
function parseWorkspaceSelection(payload: unknown): { workspaceId: string; omnirushUrl: string } | null {
  ...
  return {
    workspaceId,
    omnirushUrl: `${baseUrl}/w/${encodeURIComponent(workspaceId)}`,
  }
}
...
export async function fetchWorkerRuntimeJson(input: {
  ...
  // Published desktops hold cloud worker tokens only after OmniRushWebAccessGate
  // (v0.18.42+) granted Web access; this recheck covers lapsed entitlement and
  // callers that bypass the gate. See omnirush-web-runtime-access.ts.
  if (input.worker.destination === "cloud") {
    const webAccess = await (options.getOmniRushWebAccess ?? getOmniRushWebRuntimeAccess)(input.worker.org_id)
    if (!webAccess.hasAccess) {
      return {
        ok: false as const,
        status: 403,
        payload: omniRushWebAccessRequiredPayload(),
      }
    }
  }
...
async function runCloudProvisioning(input: {
  ...
  try {
    ...
    await requireOmniRushWebRuntimeAccess(
      input.orgId,
      options.getOmniRushWebAccess ?? getOmniRushWebRuntimeAccess,
    )
...
export async function getWorkerTokensAndConnect(worker: WorkerRow, options: {
  ...
  fetchImpl?: typeof fetch
  includeExpiringOmniRushUrl?: boolean
  apiPublicUrl?: string
} = {}) {
  ...
  if (worker.destination === "cloud") {
    const webAccess = await (options.getOmniRushWebAccess ?? getOmniRushWebRuntimeAccess)(worker.org_id)
    if (!webAccess.hasAccess) {
      return {
        error: {
          status: 403,
          body: omniRushWebAccessRequiredPayload(),
        },
      }
    }
  }
  if (worker.destination === "cloud" && worker.sandbox_backend === CLOUD_INSTANCE_BACKEND) {
    ...
    if (!options.includeExpiringOmniRushUrl) {
      return {
        tokens: { owner: hostToken, host: hostToken, client: clientToken },
        connect: stableRootUrl ? { omnirushUrl: stableRootUrl, workspaceId: null } : null,
      }
    }
    ...
    const previewConnect = resolved?.status === "ready"
      ...
      : null
    const stableOmniRushUrl = cloudWorkerCompatibilityUrl(
      worker.id,
    ...
    return {
      tokens: { owner: hostToken, host: hostToken, client: clientToken },
      connect: stableOmniRushUrl
        ? { omnirushUrl: stableOmniRushUrl, workspaceId: previewConnect?.workspaceId ?? null }
        : null,
      directPreview: resolved?.status === "ready" && previewConnect
        ? {
            version: 1 as const,
            omnirushUrl: previewConnect.omnirushUrl,
  ...
  return {
    ...
    connect: connect ?? (instance?.url ? { omnirushUrl: instance.url, workspaceId: null } : null),
  }
}
