...
import { OrganizationTable, ScimProviderTable, SsoConnectionTable } from "@omnirush-ee/den-db/schema"
  ...
  app.get(
    ...
    async (c) => {
      ...
      const cloudEnabled = cloudHostingAvailable({ orgMode: env.orgMode })
        && (await getOmniRushWebAccess(payload.organization.id)).hasAccess
      ...
      return c.json({
        ...
        capabilities: {
          ...
          omnirushWeb: isOmniRushWebAvailableForOrganization(payload.organization.metadata),
