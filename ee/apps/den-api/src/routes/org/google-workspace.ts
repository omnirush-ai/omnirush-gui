...
import { z } from "zod"
import type { DenTypeId } from "@omnirush-ee/utils/typeid"
...
const CONNECT_GOOGLE_ACCOUNT_MESSAGE = "Connect your Google account first: open Settings > Connect and use Connect your account on the Google Workspace row, or connect from the OmniRush.ai Cloud dashboard."
...
function missingPermissionMessage(label: string): string {
  return `Your connected Google account is missing the ${label} permission. An admin can enable it on the Google Workspace connector in OmniRush.ai Cloud -> Connectors; then reconnect your account in Settings -> Extensions.`
}

function driveReadPermissionMessage(account: ConnectedAccountRow): string | null {
  const scopes = account.scopes
  if (!Array.isArray(scopes) || scopes.length === 0) {
    return "OmniRush.ai could not verify the Google Drive permissions granted to this connection. An organization admin must enable Read all Drive files, then reconnect Google Workspace and grant Drive read access."
  }
  const grantedScopes = new Set(scopes)
  if (grantedScopes.has(DRIVE_READ_SCOPE) || grantedScopes.has(DRIVE_FULL_SCOPE)) return null
  return "Your connected Google account does not grant Read all Drive files. The selected-file Drive permission can upload or share files created through OmniRush.ai, but it cannot search or read your general Drive. An organization admin must enable Read all Drive files, then reconnect Google Workspace and grant Drive read access."
}
...
export function registerGoogleWorkspaceRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Authenticated host transport for omnirush-cloud-uploads. The route immediately forwards the file to Google and does not persist it or expose its bytes to the model.",
      responses: {
    ...
    async (c) => {
      ...
      const boundary = `omnirush-${randomUUID()}`
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Authenticated host transport for omnirush-cloud-uploads. The route immediately creates the draft and does not persist attachment bytes or expose them to the model.",
      responses: {
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Creates a plain-text Gmail draft in the calling member own mailbox. For workspace attachments, use the omnirush-cloud-uploads gmail_create_draft_with_attachments action so file bytes stay outside model context. Set threadId for replies and forwards. Always share the returned draftUrl.",
      responses: {
