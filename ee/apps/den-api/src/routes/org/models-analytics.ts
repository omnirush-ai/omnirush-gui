import { and, desc, eq, gte, inArray, sql } from "@omnirush-ee/den-db/drizzle"
import { ModelsAnalyticsEventTable as Event, ModelsAnalyticsSettingsTable as Settings } from "@omnirush-ee/den-db/schema"
import {
...
export function registerModelsAnalyticsRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  app.get("/v1/inference/analytics/settings", describeRoute({
    tags: ["Inference"], summary: "Read the OmniRush.ai Models task analytics choice",
  ...
  app.patch("/v1/inference/analytics/settings", describeRoute({
    tags: ["Inference"], summary: "Choose whether to collect task analytics included with OmniRush.ai Models",
    ...
    if (choice.enabled && (!settings.available || !settings.subscribed || !settings.modelsEnabled)) {
      return c.json({ error: "models_analytics_unavailable", message: "Task analytics requires access to this feature and an active OmniRush.ai Models subscription." }, 403)
    }
  ...
  app.get("/v1/inference/analytics/consumption", describeRoute({
    tags: ["Inference"], summary: "Read provider-reported consumption for OmniRush.ai Models",
