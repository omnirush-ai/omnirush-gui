...
const saveSchema = z.object({
  pluginId: z.string().trim().min(1).max(160).optional().describe("Existing OmniRush.ai Connect Plugin that will contain and share this Workflow. Omit to use the member's private My Workflows Plugin."),
...
function appRouteFailure(error: unknown) {
  ...
  const message = code.includes("not_found")
    ...
    : code === "artifact_view_schema_incompatible"
      ? "The workflow’s results have changed. Ask OmniRush.ai to update this app before saving."
...
export function registerOrgWorkflowRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      tags: ["Workflows"], summary: "Save a successful Code Mode run as a Workflow inside an OmniRush.ai Connect Plugin",
      responses: {
