import { installConfigSchema, installExperienceConfigSchema } from "@omnirush/install-config"
import { connectLinkClaimsSchema } from "@omnirush/connect-link"
import { and, eq, gt, isNull, or } from "@omnirush-ee/den-db/drizzle"
import { InstallLinkTable, OrganizationTable } from "@omnirush-ee/den-db/schema"
...
import { z } from "zod"
import { OMNIRUSH_DOWNLOAD_URL } from "../../CONSTS.js"
...
const defaultInstallerDependencies: InstallExperienceDependencies = {
  ...
  resolveDirectUrl: (platform, releaseTag) => {
    ...
    return fileName ? installerReleaseAssetUrl(fileName, { releaseTag }) : OMNIRUSH_DOWNLOAD_URL
  },
...
function buildInstallConfig(input: { organization: { name: string; logo: string | null; metadata: unknown }; request: Request }) {
  ...
  return installConfigSchema.parse({
    appName: typeof metadata.brandAppName === "string" ? metadata.brandAppName : "OmniRush.ai",
...
export function registerOrgInstallLinkRoutes<T extends { Variables: OrgRouteVariables }>(
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Mints a shareable OmniRush.ai desktop install link for a signed-in organization member. Older active links remain valid unless an owner or admin explicitly requests rotation.",
      responses: {
  ...
  app.get(
    ...
    describeRoute({
      tags: ["Users"],
      summary: "Download current user's managed OmniRush.ai desktop",
  ...
  app.get(
    ...
    describeRoute({
      tags: ["Organizations"],
      summary: "Download managed OmniRush.ai desktop",
