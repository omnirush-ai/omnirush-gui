...
import { createInferenceCheckoutSession, createInferencePortalSession, createOmniRushWebCheckout, createSeatCheckoutSession, getOmniRushWebBillingSummary, getOrgBillingSummary, syncStripeCheckoutSession } from "../../stripe-billing.js"
...
import { isOmniRushWebAvailableForOrganization } from "../../omnirush-web-availability.js"
...
const omniRushWebUnavailableSchema = z.object({
  error: z.literal("omnirush_web_not_available"),
  message: z.string(),
}).meta({ ref: "OmniRushWebUnavailableError" })

function omniRushWebUnavailableResponse(): { error: "omnirush_web_not_available"; message: string } {
  return {
    error: "omnirush_web_not_available",
    message: "OmniRush.ai Web is not available for this organization.",
  }
}
...
function checkoutSuccessUrl(c: { req: { raw: Request } }) {
  // `return=models` sends the user back to the OmniRush.ai Models page after a
...
function omniRushWebCheckoutSuccessUrl(c: { req: { raw: Request } }) {
...
function omniRushWebCheckoutCancelUrl(c: { req: { raw: Request } }) {
...
export function registerOrgBillingRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  app.get(
    ...
    describeRoute({
      tags: ["Organizations"],
      hide: true,
      summary: "Get OmniRush.ai Web billing eligibility",
      responses: {
        200: jsonResponse("OmniRush.ai Web billing eligibility returned successfully.", stripeBillingResponseSchema),
        401: jsonResponse("The caller must be an organization member.", unauthorizedSchema),
        404: jsonResponse("OmniRush.ai Web is not available for this organization.", omniRushWebUnavailableSchema),
      },
    }),
    orgRoleRoute(["member"]),
    async (c) => {
      const payload = c.get("organizationContext")
      if (!isOmniRushWebAvailableForOrganization(payload.organization.metadata)) {
        return c.json(omniRushWebUnavailableResponse(), 404)
      }
      const web = await getOmniRushWebBillingSummary(payload.organization.id)
  ...
  app.post(
    ...
    describeRoute({
      tags: ["Organizations"],
      hide: true,
      summary: "Create Stripe Checkout session for OmniRush.ai Models",
      responses: {
    ...
    async (c) => {
      ...
      if (subscriptionType === "web" && !isOmniRushWebAvailableForOrganization(payload.organization.metadata)) {
        return c.json(omniRushWebUnavailableResponse(), 404)
      }
      if (subscriptionType === "web") {
        const webBilling = await getOmniRushWebBillingSummary(payload.organization.id)
        if (webBilling.complimentaryAccess) {
          return c.json({
            error: "omnirush_web_complimentary_access_exists",
            message: "OmniRush.ai Web is already included for this organization without a Stripe subscription.",
          }, 409)
        }
      }
      ...
      const session = await createCheckoutSession({
        ...
        successUrl: subscriptionType === "seat"
          ...
          : subscriptionType === "web"
            ? omniRushWebCheckoutSuccessUrl(c)
            : checkoutSuccessUrl(c),
        cancelUrl: subscriptionType === "web" ? omniRushWebCheckoutCancelUrl(c) : checkoutCancelUrl(c),
      }).catch((error) => {
        if (error instanceof ManagedModelsPolicyError) return error
        if (error instanceof Error && error.message === "stripe_omnirush_web_subscription_exists") {
      ...
      if (session === "subscription_exists") {
        return c.json({
          ...
          message: "OmniRush.ai Web is already subscribed for this organization. Manage it from Billing.",
        }, 409)
      }
  ...
  app.post(
    ...
    describeRoute({
      tags: ["Organizations"],
      hide: true,
      summary: "Create Stripe billing portal session for OmniRush.ai Models",
      responses: {
