import { and, asc, count, desc, eq, inArray, isNull, or } from "@omnirush-ee/den-db/drizzle"
import {
  AuthUserTable,
...
import { createDenTypeId, normalizeDenTypeId } from "@omnirush-ee/utils/typeid"
import { hasSkillFrontmatterName, parseSkillMarkdown } from "@omnirush-ee/utils"
...
import {
  ...
  DEFAULT_OMNIRUSH_MARKETPLACE_DESCRIPTION,
  DEFAULT_OMNIRUSH_MARKETPLACE_LOGO_URL,
  DEFAULT_OMNIRUSH_MARKETPLACE_NAME,
...
import { omnirushYourConnectionsUrl } from "../../../mcp/connection-navigation.js"
import {
...
const DEFAULT_OMNIRUSH_EXTENSION_MANIFESTS = [
  {
    schemaVersion: 1,
    id: "omnirush-browser",
    ...
    icon: { src: "/omnirush-mark.svg" },
    composer: { prompt: "Use the OmniRush.ai Browser extension to " },
    setup: { instructions: "OmniRush.ai Browser is ready by default in desktop workspaces." },
    ...
    enablement: [{ type: "toggle-enabled", ref: "omnirush-browser", label: "Enabled" }],
  ...
  },
  {
    ...
    resources: [
      { type: "mcp", id: "computer-use-mcp", label: "Computer Use MCP", mcpServerName: "computer-use", command: ["npx", "-y", "@omnirush/handsfree", "mcp"], localCommandRef: "omnirush.computerUseMcp", required: true },
      { type: "native-binary", id: "computer-use-native", label: "macOS accessibility runtime", packageName: "@omnirush/handsfree", required: true },
    ],
    contributions: [
      { type: "setup-instructions", ref: "omnirush.computerUse.setup", location: "settings-detail" },
  ...
  },
  {
    ...
    setup: { instructions: "Add an OpenAI API key, then agents can generate image artifacts through OmniRush.ai extension actions." },
    resources: [
    ...
    contributions: [
      { type: "settings-panel", ref: "omnirush.imageGen.settings", location: "settings-detail" },
  ...
  },
  {
    ...
    description: "Let OmniRush.ai help with meetings, selected Drive files, and Gmail drafts.",
    source: { format: "omnirush-builtin", origin: "builtin", trusted: true },
    icon: { simpleIconSlug: "google" },
    composer: { prompt: "Use Google Workspace to " },
    setup: { instructions: "Connect your Google account to use Calendar, Drive, and Gmail drafts in OmniRush.ai." },
    resources: [
    ...
    contributions: [
      { type: "settings-panel", ref: "omnirush.googleWorkspace.settings", location: "settings-detail" },
...
function defaultOmniRushManifestForPlugin(row: PluginRow) {
  return DEFAULT_OMNIRUSH_EXTENSION_MANIFESTS.find((manifest) => manifest.name === row.name && manifest.description === row.description) ?? null
}
...
function serializedPluginSourceFormat(row: PluginRow) {
  switch (row.sourceFormat) {
    ...
    case "omnirush-extension-manifest":
...
function serializePluginExtension(row: PluginRow, componentCounts: Record<string, number>) {
  const builtInManifest = defaultOmniRushManifestForPlugin(row)
  if (builtInManifest) {
    return {
    ...
    manifest: {
      ...
      setup: {
        instructions: agentPlugin
          ? `Imported from Agent Plugins ${row.sourceSchemaVersion ?? AGENT_PLUGIN_V1_VERSION}. OmniRush.ai installs supported skills and remote MCP resources into this workspace.`
          : "Imported from a Claude-compatible plugin. OmniRush.ai installs its resources into this workspace as extension components.",
      },
      lifecycle: {
...
export async function listMarketplaces(input: { context: PluginArchActorContext; cursor?: string; limit?: number; q?: string; status?: MarketplaceRow["status"] }) {
  await ensureDefaultOmniRushMarketplace(input.context)
...
async function ensureDefaultOmniRushMarketplace(context: PluginArchActorContext) {
  const organizationId = context.organizationContext.organization.id
  if (await defaultOmniRushMarketplaceSeedComplete(organizationId)) {
    return
  }
  ...
  await db.transaction(async (tx) => {
    ...
    const marketplace = await ensureDefaultMarketplace({
      ...
      createdAt: now,
      database: tx,
      description: DEFAULT_OMNIRUSH_MARKETPLACE_DESCRIPTION,
      logoUrl: DEFAULT_OMNIRUSH_MARKETPLACE_LOGO_URL,
      name: DEFAULT_OMNIRUSH_MARKETPLACE_NAME,
    })
    await ensureDefaultMarketplacePlugins({
      context,
      createdAt: now,
      database: tx,
      entries: DEFAULT_OMNIRUSH_EXTENSION_MANIFESTS.map((manifest) => ({ description: manifest.description, name: manifest.name })),
      marketplaceId: marketplace.id,
    })
  })
}

async function defaultOmniRushMarketplaceSeedComplete(organizationId: OrganizationId) {
  const defaultMarketplaces = await db
    ...
    .where(and(
      eq(MarketplaceTable.organizationId, organizationId),
      inArray(MarketplaceTable.name, [DEFAULT_ANTHROPIC_MARKETPLACE_NAME, DEFAULT_OMNIRUSH_MARKETPLACE_NAME]),
      eq(MarketplaceTable.status, "active"),
      isNull(MarketplaceTable.deletedAt),
    ))
  ...
  const omniRushMarketplaceId = marketplaceIdByName.get(DEFAULT_OMNIRUSH_MARKETPLACE_NAME)
  if (!anthropicMarketplaceId || !omniRushMarketplaceId) {
    return false
  }
  ...
  if (!defaultMarketplaces.some((marketplace) => marketplace.name === DEFAULT_OMNIRUSH_MARKETPLACE_NAME && marketplace.logoUrl === DEFAULT_OMNIRUSH_MARKETPLACE_LOGO_URL)) {
    return false
  }

  const marketplaceIds = [anthropicMarketplaceId, omniRushMarketplaceId]
  ...
  const omniRushPluginEntries = DEFAULT_OMNIRUSH_EXTENSION_MANIFESTS.map((manifest) => ({ description: manifest.description, name: manifest.name }))
  const defaultPluginEntries = [...anthropicPluginEntries, ...omniRushPluginEntries]
  ...
  for (const entry of omniRushPluginEntries) {
    const pluginId = pluginIdByEntry.get(defaultMarketplacePluginEntryKey(entry))
    if (pluginId) expectedMemberships.add(defaultMarketplacePluginMembershipKey(omniRushMarketplaceId, pluginId))
  }
...
export async function getMarketplaceResolved(input: { context: PluginArchActorContext; marketplaceId: MarketplaceId }) {
  ...
  const cloudReadinessByPlugin = memberFacingMcpConnectionsEnabled(input.context.organizationContext.organization.metadata, { gatingEnabled: env.mcpConnectionsGatingEnabled })
    ? await resolveMarketplacePluginCloudReadiness({
        ...
        pluginIds,
        desktopManifestPluginIds: pluginRows.flatMap((row) => defaultOmniRushManifestForPlugin(row) ? [row.id] : []),
      })
  ...
  return {
    mcpServers: {
      [serverName]: {
        type: "remote",
        url: input.server.url,
        omnirushManaged: "den_external_mcp",
...
export async function configureMarketplacePluginMcpRequirement(input: {
  ...
  return {
    ...
    links: {
      yourConnections: omnirushYourConnectionsUrl(connection.id),
    },
  }
}
  ...
  for (const server of supportedServers) {
    ...
    const configObject = await createConfigObject({
      ...
      value: {
        ...
        schemaVersion: "omnirush.den_external_mcp.v1",
      },
    })
