...
function agentPluginAssetWarnings(input: {
  ...
  return extraAssetCount > 0
    ? [`${extraAssetCount} additional skill asset${extraAssetCount === 1 ? " was" : "s were"} found. OmniRush.ai imports the SKILL.md entrypoint; referenced skill assets are not installed yet.`]
    : []
}
...
export function buildGithubRepoDiscovery(input: {
  ...
  if (agentPluginCandidates.length > 0) {
    const discoveredPlugins = agentPluginCandidates.map((candidate) => {
      ...
      const noComponentsWarning = !validation.ok || hasSupportedComponents
        ? []
        : ["This Agent Plugin has no skills or MCP configuration that OmniRush.ai can import."]
  ...
  warnings.push(`OmniRush.ai supports Agent Plugins ${AGENT_PLUGIN_V1_SUPPORTED_VERSIONS.join(" and ")} and Claude-compatible plugins. Add a standards-compliant plugin.json, .claude-plugin/marketplace.json, or .claude-plugin/plugin.json to this repository.`)
