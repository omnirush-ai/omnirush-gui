...
import { and, desc, eq, inArray, isNotNull, isNull, sql } from "@omnirush-ee/den-db/drizzle"
...
async function loadLlmProviders(input: {
  ...
  const providers = await db
    .select()
    .from(LlmProviderTable)
    .where(and(providerWhere, managedModelsAllowed ? undefined : sql`${LlmProviderTable.source} <> 'omnirush'`))
...
export function registerOrgLlmProviderRoutes<T extends { Variables: OrgRouteVariables & Partial<MemberTeamsContext> }>(app: Hono<T>) {
  ...
  app.get(
    ...
    async (c) => {
      ...
      // but this member's OmniRush.ai provider/key was deleted, re-provision before
      ...
      if (query.scope === "usable") {
        try {
          ...
          // Keep listing other providers even if OmniRush.ai re-provision fails.
        }
      }
