...
} from "@omnirush/enterprise-mcp-client"
import { and, desc, eq, inArray, isNull } from "@omnirush-ee/den-db/drizzle"
import {
 ...
 */
const CONNECT_MCP_APP_HOST_NAME_PREFIX = "omnirush-app-host-connect-"
...
const clientMetadataResponseSchema = z.object({
  client_id: z.string(),
  client_name: z.literal("OmniRush.ai"),
...
const connectionToolInspectionDiagnosisSchema = z.object({
  ...
  layer: z.enum(["omnirush", "network", "mcp_connection", "remote_http", "mcp_tool"]),
  summary: z.string(),
...
const externalMcpDiagnosticSchema = z.object({
  ...
  retryable: z.boolean(),
  actionOwner: z.enum(["omnirush", "network_admin", "provider_admin", "organization_admin", "member"]),
...
function legacyExternalMcpConnectionIdsFromPayload(payload: Record<string, unknown> | null): string[] {
  ...
  const collect = (value: unknown) => {
    ...
    if (value.omnirushManaged !== "den_external_mcp") return
...
async function createExternalConnectionResponse(
  ...
  if (isAgentOAuthClientConnection({ oauthClient: body.oauthClient, sessionId })) {
    return c.json({ error: "invalid_request", message: "OAuth client credentials cannot be set from the agent. Add them in the OmniRush.ai Cloud dashboard under Extensions." }, 400)
  }
  if (isAgentApiKeyConnection({ authType: body.authType, sessionId })) {
    return c.json({ error: "invalid_request", message: "API-key connections cannot be created from the agent. Add them in the OmniRush.ai Cloud dashboard under Extensions." }, 400)
  }
...
async function replaceExternalConnectionResponse(
  ...
  if (sessionId === "mcp_internal" && (body.apiKey !== undefined || body.oauthClient !== undefined)) {
    return c.json({
      ...
      message: "Connection credentials cannot be edited from the agent. Use the OmniRush.ai Cloud dashboard under Connections.",
    }, 400)
  }
...
export function registerMcpConnectionRoutes<T extends { Variables: OrgRouteVariables & RequestIdVariables }>(app: Hono<T>) {
  app.get(
    ...
    describeRoute({
      tags: ["Authentication"],
      summary: "OmniRush.ai external MCP OAuth client metadata",
    ...
    (c) => {
      ...
      return c.json({
        client_id: clientId,
        client_name: "OmniRush.ai" as const,
      ...
      try {
        ...
        if (diagnostic.code === "MCP_OAUTH_CONFIGURATION_REQUIRED") {
          return c.json({
            ...
            message: "This authorization server requires a pre-registered OAuth client before OmniRush.ai can connect.",
            ...
            manualRequirements: [
              ...
              "Allowlist the callback URL shown by OmniRush.ai.",
              "Save the client ID and optional client secret in OmniRush.ai.",
            ],
          }, 409)
        }
        if (diagnostic.code === "MCP_OAUTH_ISSUER_MISMATCH") {
          return c.json({
            ...
            message: issuerRepairRequiresAdmin
              ...
              : "OmniRush.ai could not safely verify the authorization server selected for this MCP connection. Ask a workspace admin to review its OAuth setup.",
          }, 409)
        }
        return c.json({
