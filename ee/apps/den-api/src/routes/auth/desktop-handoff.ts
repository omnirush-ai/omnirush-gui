...
import { and, desc, eq, gt, isNull } from "@omnirush-ee/den-db/drizzle"
import { AuthSessionTable, AuthUserTable, DaytonaSandboxTable, DesktopHandoffGrantTable, WorkerTable } from "@omnirush-ee/den-db/schema"
...
const createGrantSchema = z.object({
  ...
  desktopScheme: z.literal("omnirush").optional().describe("The registered OmniRush.ai desktop URL scheme."),
  returnUrl: z.string().trim().max(2048).optional().describe("Optional HTTPS OmniRush.ai Cloud web return URL. Accepted only for multi-organization Cloud instances after server-side origin validation."),
...
function isWebAppHost(hostname: string) {
  ...
  return normalized === "app.omnirushlabs.com"
    || normalized === "app.omnirush.software"
...
function buildOmniRushDeepLink(input: {
  grant: string
  denBaseUrl: string
}) {
  const url = new URL("omnirush://den-auth")
...
export function registerDesktopAuthRoutes<T extends { Variables: AuthContextVariables }>(app: Hono<T>) {
  app.post(
    ...
    describeRoute({
      ...
      description: "Creates a short-lived handoff grant for a signed-in web user. Desktop clients receive an OmniRush.ai deep link; approved Cloud web clients also receive a validated return URL.",
      responses: {
    ...
    return c.json({
      ...
      omnirushUrl: buildOmniRushDeepLink({
        grant,
        denBaseUrl,
      }),
