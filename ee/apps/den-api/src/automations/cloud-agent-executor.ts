...
import { createHeadlessThreadClient, type HeadlessThreadTranscript } from "@omnirush/headless-threads"
import { and, asc, eq, isNull } from "@omnirush-ee/den-db/drizzle"
import { MemberTable, WorkerTable } from "@omnirush-ee/den-db/schema"
import { normalizeDenTypeId } from "@omnirush-ee/utils/typeid"
import type { AutomationAction, AutomationError, AutomationUsage } from "@omnirush/types/automations"
...
export async function cloudAgentRuntimeAvailable(scope: OwnerScope): Promise<boolean> {
  ...
  const webAccess = await getOmniRushWebRuntimeAccess(organizationId)
...
export async function resolveCloudAgentReadyWorker(
  ...
  if (!userId) return { ok: false, reason: "missing", message: "Set up OmniRush.ai Cloud before creating a Cloud Automation." }
  ...
  let pending: Extract<CloudAgentRuntimeResult, { ok: false }> = {
    ok: false,
    reason: "waking",
    message: "OmniRush.ai Cloud is still starting for this Automation run.",
  }
  while (!signal.aborted && deps.now() < deadline) {
    ...
    if (access.status === "missing") {
      return { ok: false, reason: "missing", message: "Set up OmniRush.ai Cloud before creating a Cloud Automation." }
    }
    if (
      !waitedForLifecycle &&
    ...
    if (access.status !== "ready" && access.reason === "unreachable") {
      pending = { ok: false, reason: "unreachable", message: "The OmniRush.ai Cloud runtime is healthy but unreachable for this Automation run." }
      await deps.sleep(WORKER_READY_POLL_MS, signal)
      continue
    }
    if (access.status === "failed") {
      return { ok: false, reason: "failed", message: "The OmniRush.ai Cloud runtime needs repair before this Automation can run." }
    }
    if (access.status === "ready") {
      ...
      pending = { ok: false, reason: "unreachable", message: "The OmniRush.ai Cloud runtime session API is unreachable for this Automation run." }
    } else {
      pending = { ok: false, reason: "waking", message: "OmniRush.ai Cloud is still starting for this Automation run." }
    }
    await deps.sleep(WORKER_READY_POLL_MS, signal)
  }
  return pending
...
export function cloudAgentRuntimeUnavailableResult(input: {
  ...
  if (input.timedOut) {
    return {
      ...
      message: "The Automation run exceeded its maximum runtime while starting OmniRush.ai Cloud.",
      retryable: false,
    }
  }
  return {
...
async function connectHealth(input: {
  ...
  let value = await request("GET", `/workspace/${encodedWorkspace}/mcp/omnirush-cloud/health?${query}`)
  let health = value
  if (health?.usable !== true || health.usableByCurrentModel !== true) {
    value = await request("POST", `/workspace/${encodedWorkspace}/mcp/omnirush-cloud/engine-refresh`, {
      provider: input.action.model.providerId,
      model: input.action.model.modelId,
  ...
  if (health?.usable === true && health.usableByCurrentModel !== true) {
    return { ok: false, code: "model_access_lost", message: "The selected model cannot use the current OmniRush.ai Connect capabilities." }
  }
  ...
  return {
    ...
    message: typeof failure?.message === "string"
      ? failure.message
      : "OmniRush.ai Connect is not ready in the Cloud runtime. Reconnect it before retrying this Automation.",
  }
}
...
async function currentAgentAuthority(input: OwnerScope & { action: AgentAction }): Promise<CloudAgentExecution | null> {
  const webAccess = await getOmniRushWebRuntimeAccess(input.organizationId)
  if (!webAccess.hasAccess) {
    return {
      ok: false,
      status: "failed",
      code: OMNIRUSH_WEB_ACCESS_REQUIRED_CODE,
...
export async function executeCloudAgent(input: CloudAgentExecutorInput): Promise<CloudAgentExecution> {
  ...
  try {
    ...
    if (authorityFailure) {
      ...
      if (recoveringNativeThread && !await abortAndObserve(client, nativeThreadId)) {
        return {
          ...
          message: "Model authority was revoked, but OmniRush.ai Cloud could not confirm that the native thread stopped. Inspect the native run before retrying.",
          retryable: false,
          needsAttention: true,
    ...
    if (waited.outcome !== "settled") {
      ...
      if (!stopped) {
        return {
          ...
          status: "failed",
          code: "execution_failed",
          message: "OmniRush.ai Cloud could not confirm that the cancelled agent thread stopped. Inspect the native run before retrying.",
          retryable: false,
          needsAttention: true,
    ...
    const resultSummary = transcript.finalAssistantText.trim() || "OmniRush.ai Cloud completed the Automation run."
    return {
      ok: true,
    ...
    if (client && nativeThreadId) {
      ...
      if (!stopped) {
        return {
          ...
          status: "failed",
          code: "execution_failed",
          message: "OmniRush.ai Cloud could not confirm that the interrupted agent thread stopped. Inspect the native run before retrying.",
          retryable: false,
          needsAttention: true,
          events: [{ type: "warning", payload: { code: "abort_not_observed", nativeThreadId } }],
        }
