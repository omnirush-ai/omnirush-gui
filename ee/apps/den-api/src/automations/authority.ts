import { and, eq, inArray, isNull, or } from "@omnirush-ee/den-db/drizzle"
import {
...
import { AUTOMATION_FREE_MODEL } from "@omnirush/types/automations"
...
export type ResolvedAutomationModel = AutomationModelSelection & {
  accessKind: "free" | "omnirush_managed" | "authorized_custom"
...
export type AutomationModelAuthorityStore = {
  ...
  findOmniRushProvider(input: { organizationId: string; ownerMemberId: string }): Promise<AutomationAuthorityProvider | null>
...
const databaseAuthorityStore: AutomationModelAuthorityStore = {
  ...
  async findOmniRushProvider(input) {
    ...
    const providers = await db.select().from(LlmProviderTable).where(and(
      ...
      eq(LlmProviderTable.providerId, "omnirush"),
    )).limit(1)
...
function enabledOmniRushModel(modelId: string) {
...
export async function resolveAutomationModelAccessWithStore(
  ...
  if (input.providerId === "omnirush") {
    const model = enabledOmniRushModel(input.modelId)
    if (!model) {
      return { ok: false, code: "model_access_lost", message: "The selected OmniRush.ai-managed model is not available." }
    }
    const provider = await store.findOmniRushProvider(input)
    if (!provider) {
      return { ok: false, code: "provider_unavailable", message: "OmniRush.ai Models are not available for the Automation owner." }
    }
    if (!await store.canAccessProvider({ member, providerRecordId: provider.id })) {
      return { ok: false, code: "model_access_lost", message: "The Automation owner no longer has access to OmniRush.ai Models." }
    }
    return {
      ok: true,
      value: {
  ...
  if (!provider || provider.source === "omnirush") {
