...
import type { AutomationClaimResult, AutomationListItem } from "@omnirush/automations"
import { AUTOMATION_MIN_CLAIM_WINDOW_MS, desktopRunnerConnected } from "@omnirush/automations"
import type {
...
import {
  ...
  requireOmniRushWebRuntimeAccess,
...
export class AutomationService {
  ...
  private readonly getOmniRushWebAccess: OmniRushWebRuntimeAccessResolver

  constructor(options: AutomationServiceOptions = {}) {
    this.getOmniRushWebAccess = options.getOmniRushWebAccess ?? getOmniRushWebRuntimeAccess
  }
  ...
  async create(scope: OwnerScope, definition: CreateAutomationDefinition) {
    if ("action" in definition) {
      ...
      await requireOmniRushWebRuntimeAccess(scope.organizationId, this.getOmniRushWebAccess)
  ...
  async runNow(scope: OwnerScope, automationId: string): Promise<AutomationRun | null> {
    ...
    // Cloud Automations execute on an OmniRush.ai VM, so a manual run is gated
    // like every other VM boundary. Desktop-target Automations are untouched.
    // omnirush_web_access_required is already part of the shared Automation
  ...
  private async executeCloudRun(runId: string): Promise<void> {
    ...
    if (!claimed) return
    const webAccess = await this.getOmniRushWebAccess(claimed.automation.organizationId)
    ...
    if (!executor) {
      await automationRepository.completeCloud({
        ...
        leaseOwner,
        status: "failed",
        resultSummary: "OmniRush.ai Cloud Workflow execution is unavailable.",
        error: { code: "execution_runtime_unavailable", message: "OmniRush.ai Cloud Workflow execution is unavailable.", retryable: true },
        now: Date.now(),
      })
      return
    }
  ...
  private async executeCloudAgentRun(
    ...
    if (!executor) {
      await automationRepository.completeCloud({
        ...
        leaseOwner,
        status: "failed",
        resultSummary: "OmniRush.ai Cloud agent execution is unavailable.",
        updateArtifactState: false,
        error: { code: "execution_runtime_unavailable", message: "OmniRush.ai Cloud agent execution is unavailable.", retryable: true },
        now: Date.now(),
      })
      return
    }
    ...
    try {
      ...
      result = await executor({
        ...
        onAdmitted: async (receipt) => automationRepository.setCloudExecution({
          runId: claimed.run.id,
          leaseOwner,
          engineKind: "omnirush-cloud-agent-v1",
          receipt,
          now: Date.now(),
        }),
    ...
    if (!result.ok && result.needsAttention) {
      const attentionCode = [
        ...
        "omnirush_web_access_required",
        "execution_runtime_unavailable",
      ].includes(result.code) ? result.code as "owner_membership_lost" | "model_access_lost" | "provider_unavailable" | "connect_access_unavailable" | "omnirush_web_access_required" | "execution_runtime_unavailable" : "execution_runtime_unavailable"
      await automationRepository.markNeedsAttention({
        automationId: claimed.automation.id,
        expectedRevisionId: claimed.revision.id,
