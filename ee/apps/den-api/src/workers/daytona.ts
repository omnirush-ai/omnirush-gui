...
import {
  ...
  type OmniRushCheckpointConfig,
} from "@omnirush-ee/cloud-runtime/bootstrap"
import { eq } from "@omnirush-ee/den-db/drizzle"
import { DaytonaSandboxTable, WorkerTable } from "@omnirush-ee/den-db/schema"
...
function sandboxLabels(workerId: WorkerId) {
  return {
    "omnirush.den.provider": "daytona",
    "omnirush.den.worker-id": workerId,
  }
}
...
function checkpointConfig(): OmniRushCheckpointConfig {
  return {
...
function bootstrapConfig(input: ProvisionInput): OmniRushBootstrapConfig {
  return {
    ...checkpointConfig(),
    workspaceMountPath: env.daytona.workspaceMountPath,
    port: env.daytona.omnirushPort,
...
export function buildOmniRushStartCommand(input: ProvisionInput) {
  return renderOmniRushBootstrapCommand(bootstrapConfig(input))
}
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      if (typeof command.exitCode === "number" && command.exitCode !== 0) {
        ...
        throw new Error(
          [
            `omnirush session exited with ${command.exitCode}`,
      ...
      if (error instanceof Error && error.message.startsWith("omnirush session exited")) {
        throw error
      }
    }
...
async function verifyRestoreMarker(sandbox: DaytonaSandboxRuntime) {
  const exitCode = await runSandboxShellCommand(
    sandbox,
    `omnirush-restore-verify-${Date.now()}`,
...
export async function refreshDaytonaSignedPreview(workerId: WorkerId) {
  ...
  const preview = await sandbox.getSignedPreviewUrl(env.daytona.omnirushPort, expiresInSeconds)
...
async function checkpointExistsOnDaytonaVolume(daytona: Daytona, workerId: WorkerId, sharedVolume: DaytonaVolumeRuntime) {
  ...
  try {
    ...
    const exitCode = await runSandboxShellCommand(
      probeSandbox,
      `omnirush-ckpt-probe-${workerHint(workerId)}-${Date.now()}`,
...
async function startOmniRushProcessOnDaytonaSandbox(input: {
  ...
  const preview = await input.sandbox.getSignedPreviewUrl(env.daytona.omnirushPort, expiresInSeconds)
...
async function persistDaytonaSandbox(input: {
  ...
  runtime: DaytonaProvisioningRuntime
  sandbox: DaytonaSandboxRuntime
  started: StartedOmniRushProcess
  workspaceVolumeId: string
  dataVolumeId: string
}) {
...
async function startOmniRushOnDaytonaSandbox(input: {
  provisionInput: ProvisionInput
  runtime: DaytonaProvisioningRuntime
  sandbox: DaytonaSandboxRuntime
  sessionId: string
  ...
  const started = await startOmniRushProcessOnDaytonaSandbox(input)
...
async function wakeExistingDaytonaSandbox(input: {
  ...
  if (isStartedSandboxState(input.sandbox.state)) {
    // A failed health probe means an already-running OmniRush.ai process cannot be
  ...
  return startOmniRushOnDaytonaSandbox({
    provisionInput: input.provisionInput,
    runtime: input.runtime,
    sandbox: input.sandbox,
    sessionId: `omnirush-wake-${workerHint(input.provisionInput.workerId)}-${Date.now()}`,
    workspaceVolumeId: input.workspaceVolumeId,
    dataVolumeId: input.dataVolumeId,
...
async function recycleDaytonaSandbox(input: {
  ...
  try {
    ...
    const started = await startOmniRushProcessOnDaytonaSandbox({
      provisionInput: input.provisionInput,
      runtime: input.runtime,
      sandbox: replacementSandbox,
      sessionId: `omnirush-recycle-${workerHint(input.provisionInput.workerId)}-${Date.now()}`,
    })
    const restored = input.requireRestoreMarker === false
      ? true
      : await input.runtime.verifyRestoreMarker(replacementSandbox)
    if (!restored) {
      throw new Error("Daytona replacement did not restore an OmniRush.ai checkpoint")
    }
...
export async function provisionWorkerOnDaytonaWithRuntime(
  ...
  try {
    ...
    return startOmniRushOnDaytonaSandbox({
      ...
      sessionId: `omnirush-${workerHint(input.workerId)}`,
...
export async function flushWorkerCheckpointOnDaytona(workerId: WorkerId) {
  ...
  const exitCode = await runSandboxShellCommand(
    ...
    `omnirush-update-flush-${workerHint(workerId)}-${Date.now()}`,
