...
import type { WorkerTable } from "@omnirush-ee/den-db/schema"
...
export function classifyCloudStartupFailure(error: unknown): CloudStartupFailureCode {
  if (error instanceof OmniRushWebAccessRequiredError) return "web_access_required"
  ...
  if (message.includes("omnirush session exited") || message.includes("binary missing")) return "runtime_start_failed"
