import { and, eq, inArray, isNull } from "@omnirush-ee/den-db/drizzle"
...
function requestTokens(request: Request) {
  return Array.from(new Set([
    ...
    request.headers.get("x-omnirush-host-token")?.trim() || null,
...
function upstreamRequestHeaders(request: Request, authorization: WorkerAuthorization, access: {
  ...
  if (authorization.scope === "host") {
    headers.set("X-OmniRush-Host-Token", access.hostToken)
  }
  return headers
}
...
function omniRushWebAccessRequiredResponse() {
  return Response.json(omniRushWebAccessRequiredPayload(), {
    status: 403,
    headers: {
      "Cache-Control": "no-store",
      Pragma: "no-cache",
...
export async function proxyCloudWorkerCompatibilityRequest(input: {
  ...
  // unchanged. Published desktops connect here only after OmniRushWebAccessGate
  ...
  let webAccess
  try {
    webAccess = await (options.getOmniRushWebAccess ?? getOmniRushWebRuntimeAccess)(authorization.organizationId)
  } catch {
    return jsonError(503, "worker_runtime_unavailable")
  }
  if (!webAccess.hasAccess) return omniRushWebAccessRequiredResponse()
