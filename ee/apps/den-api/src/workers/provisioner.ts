import { WorkerTable } from "@omnirush-ee/den-db/schema"
...
async function provisionWorkerOnRender(
  ...
  const omnirushServerPackage = env.render.workerOmniRushVersion?.trim()
    ? `omnirush-server@${env.render.workerOmniRushVersion.trim()}`
    : "omnirush-server"
  const buildCommand = [
    `npm install -g ${shellQuote(omnirushServerPackage)}`,
    "node ./scripts/install-opencode.mjs",
  ].join(" && ")
  const startScript = `
set -u
mkdir -p /tmp/workspace
plugin_dir="$(npm root -g)/omnirush-server/dist/opencode-plugins"
if [ ! -d "$plugin_dir" ]; then
  echo "omnirush-server extension plugins missing at $plugin_dir" >&2
  exit 1
fi
attempt=0
while [ "$attempt" -lt 3 ]; do
  attempt=$((attempt + 1))
  if OMNIRUSH_MANAGE_OPENCODE=1 OMNIRUSH_OPENCODE_BIN=./bin/opencode OMNIRUSH_EXTENSIONS_PLUGIN_DIR="$plugin_dir" omnirush-server --workspace /tmp/workspace --host 0.0.0.0 --port "\${PORT:-10000}" --cors '*' --approval manual --verbose; then
    exit 0
  fi
  status=$?
  echo "omnirush-server failed (attempt $attempt, exit $status); retrying in 3s"
  sleep 3
done
exit 1
`.trim()
  ...
  const payload = {
    ...
    envVars: [
      { key: "OMNIRUSH_TOKEN", value: input.clientToken },
      { key: "OMNIRUSH_HOST_TOKEN", value: input.hostToken },
