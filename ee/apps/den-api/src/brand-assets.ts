...
export async function validateAndNormalizeBrandAsset(input: {
  ...
  try {
    ...
    if (!metadata.width || !metadata.height) {
      return failure("invalid-image", "OmniRush.ai could not read that image's dimensions.")
    }
    ...
    return failure("invalid-image", "OmniRush.ai could not decode that image. Use a valid PNG or JPEG file.")
  }
}
