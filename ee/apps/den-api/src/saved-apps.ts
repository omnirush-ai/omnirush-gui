import type { SavedAppDetail, SavedAppSummary } from "@omnirush/types/workflows"
...
import { AuthUserTable, DashboardAppTable, MemberTable } from "@omnirush-ee/den-db/schema"
...
export async function getSavedApp(input: {
  ...
  if (!revision || revision.buildStatus !== "ready") {
    return { ...base, html: null, payload: null, previewNotice: "This app is still being prepared. Ask OmniRush.ai to finish its preview." }
  }
  ...
  if (snapshot.outputSchemaDigest !== revision.outputSchemaDigest) {
    return { ...base, html: null, payload: null, previewNotice: "The workflow’s results have changed. Ask OmniRush.ai to update this app to match." }
  }
  return {
    ...base,
