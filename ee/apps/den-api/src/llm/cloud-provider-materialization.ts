  ...
  async listProviders(organizationId) {
    ...
    const providers = await db
      ...
      .where(and(
        ...
        managedModelsAllowed ? undefined : sql`${LlmProviderTable.source} <> 'omnirush'`,
      ))
...
function runtimeProviderId(provider: Pick<CloudProviderMaterializationProvider, "id" | "source">) {
  return provider.source === "omnirush" ? "omnirush" : provider.id.trim()
}
...
function readOmniRushInferenceBaseUrl(providerConfig: JsonRecord) {
...
function providerEnvEntries(provider: CloudProviderMaterializationProvider): EnvEntry[] {
  ...
  if (provider.source === "omnirush" && primaryCredential) {
    ...
    if (baseUrl) {
      upsertEnvEntry(entries, "OMNIRUSH_INFERENCE_BASE_URL", baseUrl)
    }
  }
...
function readRuntimeSnapshotVersion(payload: JsonRecord) {
  ...
  for (const service of services) {
    ...
    if (serviceName?.includes("omnirush") || serviceName?.includes("server")) {
      return version
    }
  }
...
async function patchRuntimeProviders(input: {
  ...
  // worker still running omnirush-server 0.18.3: the patch "succeeded", the
