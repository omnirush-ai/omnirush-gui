...
import {
  OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_KIND,
  OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_SOURCE,
  OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_VERSION,
} from "@omnirush/types/den/mcp-connection-action"
import { normalizeDenTypeId, type DenTypeId } from "@omnirush-ee/utils/typeid"
import {
...
import { omnirushOrganizationConnectionsUrl, omnirushYourConnectionsUrl } from "./connection-navigation.js"
import {
...
export type ExternalConnectionStatus = {
  version: typeof OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_VERSION
  kind: typeof OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_KIND
  source: typeof OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_SOURCE
  ...
  action: {
    type: "connect" | "reconnect" | "update_credentials" | "inspect_connection" | "fix_provider" | "fix_network" | "contact_omnirush"
    label: string
    surface: "omnirush_your_connections" | "omnirush_organization_connections" | "provider_admin_console" | "network_infrastructure" | "omnirush_support"
...
export function externalConnectionErrorHint(
  ...
  if (externalMcpAuthErrorCode(error, message)) {
    const destination = credentialMode === "per_member"
      ? "OmniRush.ai Cloud -> Your Connections"
      : "the OmniRush.ai Cloud dashboard -> Connections"
    return `The stored credential for "${connectionName}" is invalid or expired. Reconnect "${connectionName}" from ${destination}, then search again. OmniRush.ai Cloud itself is still connected. ${LIVE_PROBE_HINT}`
  }
  if (PROVIDER_ADMIN_ACTION_PATTERN.test(message)) {
    return `The provider's server rejected the request for "${connectionName}": ${message}. A provider admin must fix it in the provider's own admin console, then search again. OmniRush.ai Cloud itself is still connected. ${LIVE_PROBE_HINT}`
  }
  return `The downstream provider for "${connectionName}" returned an error: ${message}. Ask an org admin to inspect "${connectionName}" in the OmniRush.ai Cloud dashboard -> Connections, then search again. OmniRush.ai Cloud itself is still connected. ${LIVE_PROBE_HINT}`
}
...
function diagnosticConnectionAction(input: {
  ...
  if (actor === "omnirush") {
    type = "contact_omnirush"
    surface = "omnirush_support"
    ...
    type = input.state === "reauth_required"
      ...
      : "inspect_connection"
    surface = "omnirush_organization_connections"
  }
  return {
    actor,
    action: {
...
function actionNavigationUrl(input: {
  ...
  if (input.surface === "omnirush_your_connections") return omnirushYourConnectionsUrl(input.connectionId)
  if (input.surface === "omnirush_organization_connections") return omnirushOrganizationConnectionsUrl()
  return undefined
}
...
export function buildExternalConnectionStatus(input: {
  ...
  const actionContract = {
    version: OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_VERSION,
    kind: OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_KIND,
    source: OMNIRUSH_CLOUD_MCP_CONNECTION_ACTION_SOURCE,
  } as const
      ...
      action: diagnosticAction
        ...
        : addConnectionActionUrl({
          ...
          action: {
            ...
            surface: providerAdminAction ? "provider_admin_console" : "omnirush_organization_connections",
            retry: "search_capabilities",
          },
        }),
    }
  ...
  if (connection.oauthIssuerReviewRequiredAt) {
    ...
    if (score > 0) {
      ...
      add(statusMatch({
        ...
        status: "error",
        hint: `Ask an org admin to open OmniRush.ai Cloud -> Connectors, review the live OAuth issuer for "${connection.name}", and reconnect if requested. ${CONNECTION_CARD_HINT}`,
  ...
  if (connection.credentialMode === "per_member") {
    ...
    if (!account?.accessToken) {
      ...
      if (score > 0) {
        ...
        add(statusMatch({
          ...
          hint: `Ask the user to click Connect on the "${connection.name}" card in OmniRush.ai desktop, then search again. In clients without inline connection controls, use OmniRush.ai Cloud -> Your Connections. ${CONNECTION_CARD_HINT}`,
    ...
    if (score > 0) {
      ...
      add(statusMatch({
        ...
        status: "needs_connection",
        hint: `Ask an org admin to open the OmniRush.ai Cloud dashboard -> Connections and connect "${connection.name}", then search again. ${CONNECTION_CARD_HINT}`,
        connectionStatus: buildExternalConnectionStatus({ connection, state: "needs_connection", errorCode: "not_connected", message }),
      }))
    }
    return matches
...
function advisorySchemaGuidance(
  ...
  return {
    advisory: true,
    providerCallAttempted: true,
    message: "OmniRush.ai forwarded the call to the provider. These local schema checks are guidance only; use the provider result as the source of truth.",
    warnings,
  }
}
  ...
  if (connection.credentialMode === "per_member") {
    ...
    if (!account?.accessToken) {
      return {
        ok: false,
        error: "needs_connection",
        message: `You haven't connected your ${connection.name} account yet. Open OmniRush.ai Cloud -> Your Connections and click Connect on "${connection.name}".`,
        connectionStatus: buildExternalConnectionStatus({
          connection,
          state: "needs_connection",
          errorCode: "not_connected",
  ...
  try {
    ...
    if (input.requireReadOnly && (tool.annotations?.readOnlyHint !== true || tool.annotations?.destructiveHint === true)) {
      return {
        ...
        error: "policy_blocked",
        capability: buildExternalCapabilityName(connection.id, input.toolName),
        message: `${input.toolName} is no longer advertised as strictly read-only, so OmniRush.ai blocked the Remote MCP App call.`,
        sameArgumentsRetryable: false,
        retry: { action: "search_capabilities", searchRequired: true },
      }
    }
    ...
    if (input.requireSchemaMatch && input.schemaDigest && input.schemaDigest !== schemaDigest) {
      return {
        ...
        error: "policy_blocked",
        capability: buildExternalCapabilityName(connection.id, input.toolName),
        message: `${input.toolName} now advertises a different input schema, so OmniRush.ai blocked the Remote MCP App call until its cached revision is refreshed.`,
        sameArgumentsRetryable: false,
        retry: { action: "search_capabilities", searchRequired: true },
      }
    }
    ...
    if (input.schemaDigest && input.schemaDigest !== schemaDigest) {
      schemaWarnings.push({
        code: "capability_schema_changed",
        message: "The provider advertised a different capability schema after discovery, but OmniRush.ai still forwarded the call.",
    ...
    if (!validation.ok && validation.error === "invalid_arguments") {
      schemaWarnings.push({
        code: "arguments_schema_mismatch",
        message: "The arguments do not match the provider's advertised argumentsSchema, but OmniRush.ai still forwarded the call because the provider may accept them.",
