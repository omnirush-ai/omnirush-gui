import { createHeadlessThreadClient, toTranscript, type AgentSessionClient, type HeadlessThreadModel } from "@omnirush/headless-threads"
import { and, eq, isNull } from "@omnirush-ee/den-db/drizzle"
import { MemberTable } from "@omnirush-ee/den-db/schema/org"
import { createDenTypeId, normalizeDenTypeId, type DenTypeId } from "@omnirush-ee/utils/typeid"
import { z } from "zod"
import { desktopRunnerConnected } from "@omnirush/automations"
import { REMOTE_SESSION_DESKTOP_RUNNER_CAPABILITY } from "@omnirush/types/automations"
import { db } from "../db.js"
import { env } from "../env.js"
import {
  getOmniRushWebRuntimeAccess,
  OMNIRUSH_WEB_ACCESS_REQUIRED_CODE,
  OMNIRUSH_WEB_ACCESS_REQUIRED_MESSAGE,
  type OmniRushWebRuntimeAccessResolver,
} from "../omnirush-web-runtime-access.js"
 ...
 * OmniRush.ai session on the member's OmniRush.ai Cloud worker or connected
...
const REMOTE_SESSION_DEFINITIONS: RemoteSessionDefinition[] = [
  {
    action: "create",
    summary:
      "Start a remote session: a native OmniRush.ai chat on your OmniRush.ai Web instance (runs in the cloud, visible in the browser). Automatically sets up your workspace on first use; on cloud_runtime_provisioning, wait retryAfterMs before retrying with the same arguments. Give it the task to run as prompt. target \"desktop\" runs it on your connected OmniRush.ai desktop instead.",
    searchExtraTokens:
      "remote session sessions chat thread cloud web instance browser omnirush desktop create start new open run do task work delegate hand off handoff background continue workspace",
    argumentsSchema: {
      type: "object",
      properties: {
        target: { type: "string", enum: ["cloud", "desktop"], description: "Where the session runs. Defaults to \"cloud\" (your OmniRush.ai Web instance)." },
        title: { type: "string", maxLength: 120, description: "Session title shown in OmniRush.ai." },
  ...
  },
  {
    action: "send",
    summary:
      "Send a follow-up prompt to an existing remote session on your OmniRush.ai Web instance. Returns an acceptance receipt; poll remote-session:read for the reply.",
    searchExtraTokens:
  ...
  },
  {
    action: "read",
    summary:
      "Read a remote session's recent transcript and status from your OmniRush.ai Web instance, or the status of a queued desktop command.",
    searchExtraTokens:
...
export type RemoteSessionExecuteDeps = {
  getOmniRushWebAccess: OmniRushWebRuntimeAccessResolver
...
const NEEDS_SETUP_MESSAGE =
  "No OmniRush.ai Cloud workspace exists for your account yet. Use remote-session:create to start a new task and set up your workspace automatically."

function provisioningResult(): RemoteSessionRuntimeResult {
  return {
    ok: false,
    error: "cloud_runtime_provisioning",
    message: "Your OmniRush.ai Cloud workspace is being set up. No task has been submitted yet. Retry remote-session:create with the same arguments in about 30 seconds; you do not need to open the web app.",
    retryable: true,
    retryAfterMs: 30_000,
  }
}
...
const CLOUD_NOT_AVAILABLE_MESSAGE =
  "OmniRush.ai Cloud is not available on this deployment, so remote sessions are unavailable."
 ...
 * the OmniRush.ai Web access check, which returns a clear access-required error.
 */
...
function workerHeaders(access: CloudWorkerAccess) {
  return {
    ...
    "X-OmniRush-Host-Token": access.hostToken,
  }
}
...
async function defaultResolveRuntime(
  ...
  // already confirmed by the OmniRush.ai Web access check in executeRemoteSessionCapability.
  ...
  for (;;) {
    ...
    if (access.status !== "ready" && access.reason === "unreachable") {
      return {
        ...
        message: "Your OmniRush.ai Cloud workspace is running but cannot be reached right now. Retry after the network path recovers.",
        retryable: true,
      }
    }
    if (access.status === "failed") {
      return {
        ...
        message: "Your OmniRush.ai Cloud workspace needs repair before remote sessions can run. Open OmniRush.ai Cloud in the browser to let it recover, then retry.",
        retryable: false,
      }
    }
    if (access.status === "ready") {
      ...
      return {
        ok: false,
        error: "cloud_runtime_unreachable",
        message: "Your OmniRush.ai Cloud workspace is healthy but its session API cannot be reached right now. Retry after the network path recovers.",
        retryable: true,
      }
    }
  ...
  return {
    ...
    message: "Your OmniRush.ai Cloud workspace is still starting. Retry the same call in about 30 seconds.",
    retryable: true,
  }
}
...
export const DEFAULT_REMOTE_SESSION_DEPS: RemoteSessionExecuteDeps = {
  getOmniRushWebAccess: getOmniRushWebRuntimeAccess,
...
function threadErrorResult(action: RemoteSessionAction, sessionId: string | null, error: unknown): RemoteSessionToolResult {
  ...
  if (action === "create" && status === 400 && isRecord(error)
    ...
    return errorResult({
      error: "cloud_runtime_waking",
      message: "Your OmniRush.ai Cloud workspace is reachable, but its engine is not ready yet. Retry the same call in about 30 seconds; no session was created.",
      retryable: true,
      retryAfterMs: 30_000,
    })
  }
  if (status === 404 && sessionId) {
    return errorResult({
      ...
      message: `No remote session "${sessionId}" exists on your OmniRush.ai Cloud workspace. It may have been deleted; create a new one with remote-session:create.`,
      retryable: false,
    })
  }
  const message = error instanceof Error ? error.message : "The OmniRush.ai Cloud workspace request failed."
  return errorResult({
...
export async function executeRemoteSessionCapability(
  ...
  const webAccess = await deps.getOmniRushWebAccess(input.organizationId)
    ...
    if (body.target === "desktop") {
      ...
      if (!presence.connected || !presence.ownerMemberId) {
        return errorResult({
          ...
          message: "No desktop is connected for your account. Open the OmniRush.ai desktop app and try again.",
        })
      }
  ...
  if (input.action === "create") {
    ...
    try {
      ...
      return jsonResult({
        ...
        note: "This is a native OmniRush.ai session on your OmniRush.ai Web instance; it is visible in OmniRush.ai Web. Use remote-session:send for follow-ups and remote-session:read to read replies.",
      })
    } catch (error) {
