...
name: create-skill
description: |
  Create a new OmniRush.ai Cloud skill as a plugin with one skill component.
...
Call the direct MCP tool \`create_skill\` (projected as \`omnirush-cloud_create_skill\` in OmniRush.ai Desktop). Do not route this flow through \`execute_capability\` or \`postPlugins\`.
...
Call \`omnirush-cloud_execute_capability\` with these names. Skip broad search unless a call returns \`unknown_capability\`.
...
name: share-plugin
description: |
  Share an OmniRush.ai Cloud skill's plugin with a person, team, or organization.
...
export const BUILTIN_SKILLS: BuiltinSkillDefinition[] = [
  {
    descriptor: {
      name: "create-skill",
      title: "Create Skill",
      description: "Create a new OmniRush.ai Cloud skill that its creator can use immediately.",
  ...
  },
  {
    descriptor: {
      name: "share-plugin",
      title: "Share Plugin",
      description: "Share an OmniRush.ai Cloud skill's plugin with a person, team, or organization.",
...
export function executeBuiltinSkillCapability(name: string): BuiltinSkillExecutePayload | null {
  ...
  return {
    ...
    provenance: "Built into OmniRush.ai Cloud.",
