...
import { eq } from "@omnirush-ee/den-db/drizzle"
import { OrganizationTable } from "@omnirush-ee/den-db/schema"
import { normalizeDenTypeId } from "@omnirush-ee/utils/typeid"
import { omnirushCloudMcpConnectionActionSchema } from "@omnirush/types/den/mcp-connection-action"
import type { Hono } from "hono"
import type { RequestIdVariables } from "hono/request-id"
import { z } from "zod"
import { connectorCatalogSchema, type ConnectorCatalog } from "@omnirush/types/connection-action-app"
...
const connectionStatusOutputSchema = omnirushCloudMcpConnectionActionSchema.extend({
  ...
  message: z.string(),
  action: z.object({
    type: z.enum(["connect", "reconnect", "update_credentials", "inspect_connection", "fix_provider", "fix_network", "contact_omnirush"]),
    label: z.string(),
    surface: z.enum(["omnirush_your_connections", "omnirush_organization_connections", "provider_admin_console", "network_infrastructure", "omnirush_support"]),
...
export const AGENT_MCP_INSTRUCTIONS = [
  ...
  "This OmniRush.ai Cloud MCP server uses standard MCP tools, resources, structured results, and list-changed notifications.",
  "For connection actions and saved Workflows, call search_capabilities with 2-4 keyword variants before concluding something is unavailable. Direct MCP tools are not capability search results; call them directly. Use execute_capability only with exact names returned by search_capabilities. A successful search_capabilities call proves this connection is authorized: Never tell the user to reconnect OmniRush.ai Cloud because a downstream connector failed.",
  "Capabilities include native Google Workspace operations (Gmail read/search, Calendar list/create, Drive search/read, and Gmail draft creation) executed with the signed-in member's organization credentials, plus any MCP connections the organization has added. Allowlisted platform admins also discover namespaced OmniRush.ai Admin capabilities here; other members cannot.",
  "A remote session is the member's OmniRush.ai Web instance: a native OmniRush.ai chat running in the cloud, visible in the browser. When asked to do something \"on the remote session\", \"in the web\", or \"in the cloud\" (e.g. \"run a Slack search for messages on the remote session\"), do not do the work here: execute remote-session:create with the whole request as prompt (or remote-session:send to an existing sessionId), then poll remote-session:read and relay the reply. target \"desktop\" runs it on the member's connected desktop instead.",
  ...
  "Do not invent OAuth-client, credential, or local-extension setup. Organization connections are managed in the OmniRush.ai Cloud dashboard / Settings > Connect; when a connection or marketplace readiness state requires administrator setup or member sign-in, relay that exact action.",
  "External MCP matches include the provider-advertised argumentsSchema, schemaDigest, and invocation.argumentsField. Put an object matching argumentsSchema in execute_capability.body and copy schemaDigest into execute_capability.schemaDigest. OmniRush.ai always attempts the downstream provider call even when local schema checks find a mismatch; schemaGuidance is advisory: if the provider succeeded, accept the result and do not retry because of the warning; if it failed, use the warning to correct the arguments or search again.",
...
export function registerAgentSkillResources(input: {
  ...
  input.server.registerResource("agent-skills-index", AGENT_SKILL_INDEX_URI, {
    ...
    description: "Authorized Agent Skills discovery index for this OmniRush.ai member.",
 ...
 * desktop app's "OmniRush.ai Cloud Control" connection, which is what an
    ...
    server.registerTool(
      SEARCH_CAPABILITIES_TOOL_NAME,
      {
        ...
        description: [
          ...
          "Search covers native Google Workspace capabilities (Gmail, Calendar, Drive, Gmail drafts), org-connected external MCPs, and namespaced OmniRush.ai Admin tools for allowlisted platform admins.",
    ...
    server.registerTool(
      EXECUTE_CAPABILITY_TOOL_NAME,
      {
        ...
        description: [
          ...
          "When the exact capability is a standard MCP App launch tool, this call preserves its originating tool and ui:// binding so compatible OmniRush.ai hosts render it without requiring a generated direct-tool name.",
      ...
      create: async ({ pluginName, skillMarkdown }) => {
        ...
        if (!libraryContext) {
          return {
            ...
            message: "The OmniRush.ai Cloud membership for this connection is unavailable.",
          }
        }
        try {
