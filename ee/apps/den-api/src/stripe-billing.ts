...
import { and, eq, isNotNull, isNull, sql } from "@omnirush-ee/den-db/drizzle"
import {
  MemberTable,
  ...
  OrganizationTable,
} from "@omnirush-ee/den-db/schema"
import { createDenTypeId } from "@omnirush-ee/utils/typeid"
import { ManagedModelsPolicyError } from "@omnirush/types/den/managed-models-policy"
...
import { isOmniRushWebAvailable } from "./omnirush-web-availability.js"
import { hasOmniRushWebComplimentaryAccess, resolveOmniRushWebAccess } from "./omnirush-web-access.js"
...
export const OMNIRUSH_WEB_UNIT_AMOUNT = 5000
export const OMNIRUSH_WEB_CURRENCY = "usd" as const
export const OMNIRUSH_WEB_INTERVAL = "month" as const
export const OMNIRUSH_WEB_QUANTITY_DEFINITION = "joined_non_removed_members" as const
...
function requireOmniRushWebPriceId() {
  const priceId = env.stripe.omnirushWebPriceId
  if (!isOmniRushWebAvailable() || !priceId) {
    throw new Error("stripe_omnirush_web_not_available")
  }
  return priceId
}
...
function requirePriceIdForSubscriptionType(subscriptionType: StripeCheckoutSubscriptionType) {
  switch (subscriptionType) {
    ...
    case WEB_SUBSCRIPTION_TYPE:
      return requireOmniRushWebPriceId()
  }
}
...
function subscriptionTypeFromStripeSubscription(subscription: Stripe.Subscription, item: Stripe.SubscriptionItem | null) {
  ...
  if (env.stripe.omnirushWebPriceId && priceId === env.stripe.omnirushWebPriceId) {
    return WEB_SUBSCRIPTION_TYPE
  }
...
async function organizationOmniRushWebComplimentaryAccess(organizationId: OrgId) {
  const rows = await db
    .select({ metadata: OrganizationTable.metadata })
    .from(OrganizationTable)
    .where(eq(OrganizationTable.id, organizationId))
    .limit(1)
  return hasOmniRushWebComplimentaryAccess(rows[0]?.metadata)
}

export function isOmniRushWebBillableMember(input: { joinedAt: Date | null; removedAt: Date | null }) {
  return input.joinedAt !== null && input.removedAt === null
}

export function calculateOmniRushWebBilling(input: { joinedMemberCount: number }) {
  const quantity = normalizeSeatCount(input.joinedMemberCount)
  return {
    quantity,
    unitAmount: OMNIRUSH_WEB_UNIT_AMOUNT,
    expectedMonthlyTotal: quantity * OMNIRUSH_WEB_UNIT_AMOUNT,
  }
}

export function isEligibleOmniRushWebSubscriptionStatus(status: string | null | undefined) {
  return status === "active" || status === "trialing"
}

export function isOngoingOmniRushWebSubscriptionStatus(status: string | null | undefined) {
  return typeof status === "string" && ONGOING_STATUSES.has(subscriptionStatus(status)) && status !== "expired"
}

export function omniRushWebPaymentStatus(status: string | null | undefined, paymentFailed = false) {
  if (paymentFailed) {
...
function isEligibleOmniRushWebSubscriptionRow(row: Awaited<ReturnType<typeof findWebSubscriptionByOrg>>) {
  return Boolean(
    row
    && isEligibleOmniRushWebSubscriptionStatus(row.status)
    && env.stripe.omnirushWebPriceId
    && row.stripe_price_id === env.stripe.omnirushWebPriceId
    && row.payment_failed !== true,
  )
}

function isOngoingConfiguredOmniRushWebSubscriptionRow(row: Awaited<ReturnType<typeof findWebSubscriptionByOrg>>) {
  return Boolean(
    row
    && isOngoingOmniRushWebSubscriptionStatus(row.status)
    && env.stripe.omnirushWebPriceId
    && row.stripe_price_id === env.stripe.omnirushWebPriceId,
  )
}

export async function organizationHasEligibleOmniRushWebSubscription(organizationId: OrgId) {
  return isEligibleOmniRushWebSubscriptionRow(await findWebSubscriptionByOrg(organizationId))
}

export async function organizationHasOngoingOmniRushWebSubscription(organizationId: OrgId) {
  ...
  return Boolean(row && isOngoingOmniRushWebSubscriptionStatus(row.status))
}
...
export async function findOrCreateStripeCustomer(input: {
  ...
  if (organizationId) {
    ...
    const customer = await stripe().customers.create({
      ...
      idempotencyKey: `omnirush-org-customer:${organizationId}`,
    })
    return customer.id
  }
...
export function omniRushWebCheckoutIdempotencyKey(input: {
  organizationId: string
  quantity: number
  previousSessionId?: string | null
}) {
  return `omnirush-web-checkout:${input.organizationId}:${input.quantity}:${input.previousSessionId ?? "initial"}`
}

function subscriptionHasConfiguredOmniRushWebPrice(subscription: Stripe.Subscription) {
  const item = firstSubscriptionItem(subscription)
  return Boolean(
    env.stripe.omnirushWebPriceId
    && typeof item?.price?.id === "string"
    && item.price.id === env.stripe.omnirushWebPriceId,
  )
}

async function validateOmniRushWebPrice(priceId: string) {
  ...
  if (
    ...
    || price.unit_amount !== OMNIRUSH_WEB_UNIT_AMOUNT
    || price.currency.toLowerCase() !== OMNIRUSH_WEB_CURRENCY
    || price.recurring?.interval !== OMNIRUSH_WEB_INTERVAL
    || price.recurring.interval_count !== 1
    || price.recurring.usage_type !== "licensed"
  ) {
    throw new Error("stripe_omnirush_web_price_contract_invalid")
  }
}

function checkoutSessionMatchesOmniRushWeb(input: {
...
async function createOmniRushWebCheckoutSession(input: {
  ...
  await validateOmniRushWebPrice(input.priceId)
  const quantity = await joinedMemberCount(input.organizationId)
  if (quantity < 1) {
    throw new Error("stripe_omnirush_web_quantity_empty")
  }
  ...
  if (storedWebSubscription?.stripe_subscription_id) {
    ...
    if (isOngoingConfiguredOmniRushWebSubscriptionRow(refreshedWebSubscription)) {
      throw new Error("stripe_omnirush_web_subscription_exists")
    }
  }
  ...
  const existingOngoingSubscription = subscriptions.data.find((subscription) => {
    ...
    return metadata.organizationId === input.organizationId
      && metadata.subscriptionType === WEB_SUBSCRIPTION_TYPE
      && isOngoingOmniRushWebSubscriptionStatus(subscription.status)
      && subscriptionHasConfiguredOmniRushWebPrice(subscription)
  })
  ...
  const latestMatchingSession = checkoutSessions.data.find((session) => checkoutSessionMatchesOmniRushWeb({
    session,
    organizationId: input.organizationId,
  }))
  ...
  if (latestMatchingSession?.status === "open" && latestMatchingSession.url) {
    ...
    await stripe().checkout.sessions.expire(latestMatchingSession.id).catch((error) => {
      logger.warn("failed to expire stale OmniRush.ai Web Checkout session", {
        organization_id: input.organizationId,
  ...
  return stripe().checkout.sessions.create({
    ...
    idempotencyKey: omniRushWebCheckoutIdempotencyKey({
      organizationId: input.organizationId,
      quantity,
...
export async function createOrgSubscriptionCheckoutSession(input: {
  ...
  const omnirushProduct = input.subscriptionType === SEAT_SUBSCRIPTION_TYPE
    ? "omnirush_seats"
...
export async function createOmniRushWebCheckout(input: Omit<Parameters<typeof createOrgSubscriptionCheckoutSession>[0], "subscriptionType">) {
...
function serializeSubscription(row: Awaited<ReturnType<typeof findOrgSubscriptionByStripeId>>) {
  return row ? {
    id: row.id,
    status: row.status,
    paymentStatus: omniRushWebPaymentStatus(row.status, row.payment_failed),
...
function serializeOmniRushWebSubscription(row: Awaited<ReturnType<typeof findWebSubscriptionByOrg>>) {
  return row ? {
    status: row.status,
    paymentStatus: omniRushWebPaymentStatus(row.status, row.payment_failed),
    quantity: row.quantity,
...
async function loadOmniRushWebBillingSummary(organizationId: OrgId) {
  const [row, memberCount, complimentaryAccess] = await Promise.all([
    ...
    organizationOmniRushWebComplimentaryAccess(organizationId),
  ])
  const billing = calculateOmniRushWebBilling({ joinedMemberCount: memberCount })
  const hasEligibleSubscription = isEligibleOmniRushWebSubscriptionRow(row)
  const access = resolveOmniRushWebAccess({
    deploymentAvailable: isOmniRushWebAvailable(),
  ...
  return {
    row,
    summary: {
      configured: isOmniRushWebAvailable()
        && Boolean(env.stripe.secretKey && env.stripe.omnirushWebPriceId),
      unitAmount: OMNIRUSH_WEB_UNIT_AMOUNT,
      currency: OMNIRUSH_WEB_CURRENCY,
      interval: OMNIRUSH_WEB_INTERVAL,
      quantityDefinition: OMNIRUSH_WEB_QUANTITY_DEFINITION,
      ...
      hasEligibleSubscription,
      ...access,
      subscription: serializeOmniRushWebSubscription(row),
    },
  }
}

export async function getOmniRushWebAccess(organizationId: OrgId) {
  const [row, complimentaryAccess] = await Promise.all([
    findWebSubscriptionByOrg(organizationId),
    organizationOmniRushWebComplimentaryAccess(organizationId),
  ])
  return resolveOmniRushWebAccess({
    deploymentAvailable: isOmniRushWebAvailable(),
    hasEligibleSubscription: isEligibleOmniRushWebSubscriptionRow(row),
    complimentaryAccess,
  })
}

export async function getOmniRushWebBillingSummary(organizationId: OrgId) {
  return (await loadOmniRushWebBillingSummary(organizationId)).summary
}
...
export async function getOrgBillingSummary(input: { organizationId: OrgId; includePortalUrl?: boolean; returnUrl: string }) {
  ...
  const webBillingState = await loadOmniRushWebBillingSummary(input.organizationId)
  ...
  const hasEligibleWebSubscription = isEligibleOmniRushWebSubscriptionRow(webRow)
  ...
  return {
    stripe: {
      ...
      web: {
        ...
        priceId: env.stripe.omnirushWebPriceId ?? null,
...
export async function syncWebSubscriptionQuantityAfterMemberChange(input: { organizationId: OrgId; memberCount: number }) {
  ...
  if (!isEligibleOmniRushWebSubscriptionRow(row) || !row?.stripe_subscription_item_id) {
    return
  }
  ...
  const subscription = await stripe().subscriptions.create(
    {
      ...
      metadata: {
        ...
        omnirush_product: "omnirush_seats",
        subscription_type: SEAT_SUBSCRIPTION_TYPE,
      },
    },
    { idempotencyKey: `omnirush-seat-subscription-${session.id}` },
  )
...
export async function syncStripeCheckoutSession(input: { organizationId: OrgId; sessionId: string }) {
  ...
  if (row?.type === WEB_SUBSCRIPTION_TYPE) {
    row = await syncOmniRushWebPaymentStateFromCurrentInvoice({
      row,
...
async function syncCurrentStripeSubscription(stripeSubscriptionId: string, eventId: string) {
  ...
  if (subscriptionType === WEB_SUBSCRIPTION_TYPE && metadata.organizationId) {
    ...
    if (existing?.stripe_subscription_id && existing.stripe_subscription_id !== subscription.id) {
      ...
      if (isOngoingConfiguredOmniRushWebSubscriptionRow(refreshedExisting)) {
...
async function syncOmniRushWebPaymentStateFromCurrentInvoice(input: {
  ...
  switch (event.type) {
    ...
    case "checkout.session.async_payment_succeeded": {
      ...
      if (session.mode === "setup") {
        ...
        if (row?.type === WEB_SUBSCRIPTION_TYPE && isEligibleOmniRushWebSubscriptionStatus(subscription.status)) {
    ...
    case "customer.subscription.deleted": {
      ...
      if (row?.type === WEB_SUBSCRIPTION_TYPE && isEligibleOmniRushWebSubscriptionStatus(row.status)) {
        await syncWebSubscriptionQuantityAfterMemberChange({
          organizationId: row.organization_id,
          memberCount: row.quantity,
        })
    ...
    case "invoice.payment_failed": {
      ...
      if (row && isEligibleOmniRushWebSubscriptionRow(row)) {
        await syncWebSubscriptionQuantityAfterMemberChange({
          organizationId: row.organization_id,
          memberCount: row.quantity,
        })
