...
import { denUrls } from "@omnirush-ee/utils/den-urls"
...
export const DEFAULT_DEN_DIAGNOSTICS_ORIGIN = "https://diagnostic.omnirushlabs.com"
...
const EnvSchema = z.object({
  ...
  OMNIRUSH_DEV_MODE: z.string().optional(),
  ...
  OMNIRUSH_INSTALLER_ARTIFACTS_DIR: z.string().optional(),
  OMNIRUSH_INSTALLER_RELEASE_TAG: z.string().optional(),
  OMNIRUSH_INSTALLER_RELEASE_REPO: z.string().optional(),
  OMNIRUSH_INSTALLER_CACHE_DIR: z.string().optional(),
  ...
  DEN_OMNIRUSH_WEB_ENABLED: z.string().optional(),
  ...
  OMNIRUSH_DAYTONA_ENV_PATH: z.string().optional(),
  ...
  RENDER_WORKER_OMNIRUSH_VERSION: z.string().optional(),
  ...
  DAYTONA_OMNIRUSH_PORT: z.string().optional(),
  ...
  STRIPE_OMNIRUSH_WEB_PRICE_ID: z.string().optional(),
...
const omnirushWebEnabled = parseBooleanFlag(parsed.DEN_OMNIRUSH_WEB_ENABLED ?? "false")

const devMode = (parsed.OMNIRUSH_DEV_MODE ?? "0").trim() === "1"
...
// (OMNIRUSH_DEV_MODE=1) is exempt automatically so evals against a local
...
export const env = {
  ...
  orgMode,
  singleOrg: {
    name: optionalString(parsed.DEN_SINGLE_ORG_NAME) ?? "OmniRush.ai",
  ...
  installerArtifactsDir: optionalString(parsed.OMNIRUSH_INSTALLER_ARTIFACTS_DIR),
  ...
  installerReleaseTag: optionalString(parsed.OMNIRUSH_INSTALLER_RELEASE_TAG) ?? `v${denApiAppVersion.latestAppVersion}`,
  installerReleaseTagExplicit: optionalString(parsed.OMNIRUSH_INSTALLER_RELEASE_TAG) !== undefined,
  installerReleaseRepo: optionalString(parsed.OMNIRUSH_INSTALLER_RELEASE_REPO) ?? "omnirush-ai/omnirush-gui",
  installerCacheDir: optionalString(parsed.OMNIRUSH_INSTALLER_CACHE_DIR) ?? path.join(os.tmpdir(), "omnirush-desktop-artifacts"),
  ...
  stripe: {
    ...
    omnirushWebPriceId: optionalString(parsed.STRIPE_OMNIRUSH_WEB_PRICE_ID),
  ...
  render: {
    ...
    workerRepo:
      // TODO(ent): require RENDER_WORKER_REPO for hosted/customer Render deployments instead of using OmniRush.ai's public repo default.
      parsed.RENDER_WORKER_REPO ?? "https://github.com/omnirush-ai/omnirush-gui",
    ...
    workerOmniRushVersion: parsed.RENDER_WORKER_OMNIRUSH_VERSION,
  ...
  daytona: {
    envPath: optionalString(parsed.OMNIRUSH_DAYTONA_ENV_PATH),
    ...
    dataMountPath:
      optionalString(parsed.DAYTONA_DATA_MOUNT_PATH) ?? "/persist/omnirush",
    runtimeWorkspacePath:
      optionalString(parsed.DAYTONA_RUNTIME_WORKSPACE_PATH) ??
      "/tmp/omnirush-workspace",
    runtimeDataPath:
      optionalString(parsed.DAYTONA_RUNTIME_DATA_PATH) ?? "/tmp/omnirush-data",
    sidecarDir:
      optionalString(parsed.DAYTONA_SIDECAR_DIR) ?? "/tmp/omnirush-sidecars",
    omnirushPort: Number(parsed.DAYTONA_OMNIRUSH_PORT ?? "8787"),
