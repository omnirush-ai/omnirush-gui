...
export type ExternalMcpToolCallDiagnosis = {
  ...
  layer: "omnirush" | "network" | "mcp_connection" | "remote_http" | "mcp_tool"
  summary: string
}
...
export function diagnoseExternalMcpToolCall(input: {
  ...
  if (!input.inspection.request) {
    if (input.diagnostic?.phase.startsWith("NETWORK_")) {
      return {
        status: "failed",
        layer: "network",
        summary: "OmniRush.ai could not reach the remote MCP while preparing the session, so tools/call was not sent.",
      }
    }
    if (
    ...
    return {
      status: "failed",
      layer: "omnirush",
      summary: "The call failed inside OmniRush.ai before an outbound tools/call request was sent.",
    }
  }
  if (!input.inspection.response) {
    ...
    if (
      ...
      return {
        status: "failed",
        layer: "omnirush",
        summary: "OmniRush.ai's outbound network safety policy blocked this tools/call request, so it was not sent to the remote MCP.",
      }
    }
    ...
    return {
      status: "failed",
      layer: "network",
      summary: "OmniRush.ai started tools/call but did not capture an HTTP response. This does not prove the remote MCP caused the failure.",
    }
  }
  ...
  return {
    ...
    summary: input.diagnostic?.code === "MCP_PROVIDER_AUTH_REQUIRED"
      ...
      : "The remote MCP answered, but OmniRush.ai could not accept the MCP response as a successful tool result.",
  }
}
