...
import { isEnterpriseMcpLifecycleDeadline } from "@omnirush/enterprise-mcp-client"
...
export type ExternalMcpDiagnostic = {
  ...
  retryable: boolean
  actionOwner: "omnirush" | "network_admin" | "provider_admin" | "organization_admin" | "member"
...
function safeBaseMessageFor(input: {
  ...
  if (input.code === "MCP_LIFECYCLE_DEADLINE") {
    return input.phase === "MCP_TOOL_EXECUTION"
      ? "The capability did not finish within the time OmniRush.ai allows a single tool call."
      : "The MCP lifecycle exceeded OmniRush.ai's bounded diagnostic deadline."
  }
  ...
  if (input.code === "MCP_PROVIDER_DECLARED_ERROR") {
    const message = input.jsonRpcCode === undefined
      ? "The provider answered with a JSON-RPC error OmniRush.ai does not recognize."
      : `The provider answered with a JSON-RPC error OmniRush.ai does not recognize (code ${input.jsonRpcCode}).`
    return input.providerErrorMessage
      ? `${message} Provider-declared message (untrusted): "${input.providerErrorMessage}".`
      : message
  }
  if (input.code === "MCP_RESPONSE_BODY_LIMIT") {
    return "The MCP server returned a response body larger than OmniRush.ai can safely process."
  }
  if (input.highestPassed && HEALTH_RANK[input.highestPassed] >= HEALTH_RANK.protocol_ready && isUninformativeClassification(input)) {
    return "The MCP server answered, but OmniRush.ai could not interpret its response for the current request."
  }
  ...
  if (input.phase === "AUTH_CLIENT_REGISTRATION") {
    return "OmniRush.ai could not register or identify its OAuth client with the authorization server."
  }
  ...
  if (input.phase === "MCP_TOOL_DISCOVERY") {
    return input.code === "MCP_CATALOG_CURSOR_LOOP"
      ? "The MCP server repeated a tool-catalog cursor, so OmniRush.ai stopped safely."
      : "OmniRush.ai could not retrieve a complete, valid MCP tool catalog."
  }
  ...
  if (input.category === "mcp_tool_input_invalid") {
    if (input.code === "MCP_TOOL_ARGUMENT_INVALID_JSON") {
      return "OmniRush.ai rejected the tool arguments before contacting the provider because they are not valid JSON data (for example an undefined field, a non-finite number, or a non-plain object)."
    }
    if (input.code === "MCP_TOOL_ARGUMENT_SIZE_LIMIT") {
      return "OmniRush.ai rejected the tool arguments before contacting the provider because they exceed the 1 MiB argument limit."
    }
    if (input.code === "MCP_TOOL_ARGUMENT_DEPTH_LIMIT") {
      return "OmniRush.ai rejected the tool arguments before contacting the provider because they are nested too deeply."
    }
    if (input.code === "MCP_TOOL_ARGUMENT_CYCLE") {
      return "OmniRush.ai rejected the tool arguments before contacting the provider because they contain a circular reference."
    }
    return "OmniRush.ai rejected the tool arguments before contacting the provider because they are invalid."
  }
  return "The MCP connection failed before OmniRush.ai could complete the protocol lifecycle."
}
...
function classifyError(error: unknown, fallbackPhase: ExternalMcpDiagnosticPhase): Classification {
  ...
  if (hasUnsupportedVersionMessage(error)) {
    return {
      ...
      retryable: false,
      actionOwner: "provider_admin",
      operatorAction: "Configure the provider to support an MCP protocol version compatible with OmniRush.ai.",
    }
  }
