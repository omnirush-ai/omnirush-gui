...
export function connectCallbackPage(input:
  ...
  const body = input.ok
    ? `<div class="status-row success">
        ...
        <span><strong>${escapeHtml(input.name)} is connected to OmniRush.ai.</strong><small>Connection complete</small></span>
      </div>
      <p>You can close this window and return to OmniRush.ai.</p>`
    : `<div class="status-row failure">
      ...
      <p>Return to OmniRush.ai to try connecting again. You can close this window.</p>`
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
