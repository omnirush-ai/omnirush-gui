...
import { and, asc, eq, gte, inArray, isNull } from "@omnirush-ee/den-db/drizzle"
import { ModelsAnalyticsSettingsTable as Settings, ModelsAnalyticsEventTable as Event } from "@omnirush-ee/den-db/schema"
import { modelsAnalyticsEventSchema, readModelsAnalyticsSettings } from "@omnirush-ee/telemetry"
...
async function send(config: z.infer<typeof configSchema>, spans: unknown[]) {
  ...
  }, JSON.stringify({ resourceSpans: [{ resource: { attributes: [{ key: "service.name", value: { stringValue: "omnirush-models" } }] }, scopeSpans: [{ scope: { name: "omnirush.task-analytics", version: "1" }, spans }] }] }))
...
export function modelsAnalyticsSpan(row: typeof Event.$inferSelect) {
  ...
  const attributes: Record<string, string | number | boolean> = {
    ...
    "langfuse.trace.name": "OmniRush.ai task", "langfuse.trace.metadata.organization_id": row.org_id,
  ...
  return {
    ...
    name: terminal ? "OmniRush.ai task" : event.model ?? event.skill ?? event.tool ?? event.type,
