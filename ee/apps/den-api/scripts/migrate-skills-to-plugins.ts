 ...
 *
 * Usage:
 *   pnpm --filter @omnirush-ee/den-api migrate:skills-to-plugins        # dry run
 *   pnpm --filter @omnirush-ee/den-api migrate:skills-to-plugins -- --yes
 *
 ...
 *        pnpm --filter @omnirush-ee/den-api migrate:skills-to-plugins -- --table-prefix recovered_ --yes
 *   4. Drop the recovered_* tables after verifying the migrated plugins.
 */
import { createDenDb, type DenDbMode } from "@omnirush-ee/den-db"
import { and, asc, eq, isNull, sql } from "@omnirush-ee/den-db/drizzle"
import {
...
async function ensureDefaultOmniRushMarketplace(input: {
  ...
  const existing = (await input.database
    ...
    .where(and(
      ...
      eq(MarketplaceTable.name, DEFAULT_OMNIRUSH_MARKETPLACE_NAME),
...
async function attachPluginToDefaultMarketplace(input: {
  ...
  const marketplaceId = await ensureDefaultOmniRushMarketplace(input)
  const existing = (await input.database
    .select()
...
async function main() {
  ...
  const pointerVersions = await db
    ...
    .from(ConfigObjectVersionTable)
    .where(eq(ConfigObjectVersionTable.schemaVersion, "omnirush.den_skill.v1"))
