...
beforeAll(async () => {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/omnirush_test_gwsreconnect"
  ...
  drizzle = await import("@omnirush-ee/den-db/drizzle")
  oauthCredentials = await import("../src/capability-sources/oauth-credentials.js")
  schema = await import("@omnirush-ee/den-db/schema")
})
...
test("Better Auth's native remove and leave routes delete the member's connected account", async () => {
  ...
  const password = "OmniRush.ai-test-password-123!"
