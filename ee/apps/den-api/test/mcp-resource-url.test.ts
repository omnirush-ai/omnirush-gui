...
describe("getMcpResourceUrl", () => {
  ...
  test("auto-trusts a path-prefixed public API resource", () => {
    runMcpResourceProbe({
      betterAuthUrl: "https://app.example.com",
      apiPublicUrl: "https://omnirush.example/api/den",
      route: "agent",
      requestUrl: "https://omnirush.example/api/den/mcp/agent",
      expectedResource: "https://omnirush.example/api/den/mcp/agent",
      expectedMetadataUrl: "https://omnirush.example/.well-known/oauth-protected-resource/api/den/mcp/agent",
      metadataUrl: "https://omnirush.example/api/den/mcp/agent",
      expectedMetadataResource: "https://omnirush.example/api/den/mcp/agent",
      expectedAuthorizationServer: "https://app.example.com/api/auth",
    })
  })
  ...
  test("honors an additional direct API-origin resource", () => {
    runMcpResourceProbe({
      betterAuthUrl: "https://app.omnirushlabs.com",
      additionalResources: " https://api.omnirushlabs.com/mcp/ ",
      requestUrl: "https://api.omnirushlabs.com/mcp/agent",
      expectedResource: "https://api.omnirushlabs.com/mcp",
      metadataUrl: "https://api.omnirushlabs.com/mcp/agent",
      expectedMetadataResource: "https://api.omnirushlabs.com/mcp",
      expectedAuthorizationServer: "https://app.omnirushlabs.com/api/auth",
    })
  })
  ...
  test("falls back to the configured resource when the request origin is not allowlisted", () => {
    runMcpResourceProbe({
      ...
      route: "agent",
      requestUrl: "https://api.omnirushlabs.com/mcp/agent",
      expectedResource: "https://app.omnirushlabs.com/api/den/mcp/agent",
    })
  })
...
describe("resolveMcpResourceFromRequest", () => {
  test("checks bare and proxied candidates against a static allowlist", () => {
    expect(resolveMcpResourceFromRequest(
      ...
      ["https://api.omnirushlabs.com/mcp"],
      "https://app.omnirushlabs.com/api/den/mcp",
    )).toBe("https://api.omnirushlabs.com/mcp")

    expect(resolveMcpResourceFromRequest(
      "https://api.omnirushlabs.com/mcp/agent",
      ["https://app.omnirushlabs.com/api/den/mcp"],
      "https://app.omnirushlabs.com/api/den/mcp",
    )).toBe("https://app.omnirushlabs.com/api/den/mcp")
...
describe("deriveDenMcpResource", () => {
  test("keeps hosted web-app and direct API origin behavior unchanged", () => {
    expect(deriveDenMcpResource("https://app.omnirushlabs.com", [])).toBe(
      "https://app.omnirushlabs.com/api/den/mcp",
    )
    expect(deriveDenMcpResource("https://api.omnirushlabs.com", [])).toBe(
      "https://api.omnirushlabs.com/mcp",
    )
  })
})
