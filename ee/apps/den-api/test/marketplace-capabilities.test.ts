...
import { and, eq, inArray } from "@omnirush-ee/den-db/drizzle"
import {
  AuthUserTable,
...
function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/omnirush_test_pr3"
...
beforeAll(async () => {
  seedRequiredEnv()
  mock.restore()
  db = (await import("@omnirush-ee/den-db")).createDenDb({
...
describe("marketplace capabilities source", () => {
  ...
  test("mcp objects return server specs and existing-connection or admin-install hints", async () => {
    ...
    expect(missingConnection.result.hint).toContain("OmniRush.ai Cloud -> Connectors")
  ...
  test("team-assigned skill discovery reports missing plugin MCP admin setup deterministically", async () => {
    ...
    expect(match?.action?.surface).toBe("omnirush_organization_connections")
    ...
    expect(result.result.action?.surface).toBe("omnirush_organization_connections")
