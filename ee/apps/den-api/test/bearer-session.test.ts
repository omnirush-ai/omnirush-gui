...
test("x-api-key can call ordinary organization routes using its scoped org", async () => {
  ...
  const response = await app.request("/ordinary-org-route", {
    headers: {
      ...
      "x-omnirush-org-id": createDenTypeId("organization"),
    },
  })
...
test("signed OmniRush.ai Den auth cookie sessions resolve through the Den cache", async () => {
  const now = new Date("2026-07-09T12:00:00.000Z")
  setSystemTime(now)
  stored = makeStoredSession({
    now,
  ...
  const cookie = await generateSignedCookie("omnirush-den.session_token", token, process.env.BETTER_AUTH_SECRET ?? "")
...
test("unsigned OmniRush.ai Den auth cookies are ignored", async () => {
  const now = new Date("2026-07-09T12:00:00.000Z")
  setSystemTime(now)
  stored = makeStoredSession({
    now,
  ...
  const response = await app.request("/session", { headers: { cookie: `omnirush-den.session_token=${token}` } })
