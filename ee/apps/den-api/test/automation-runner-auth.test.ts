...
import { AUTOMATION_MODEL_ATTENTION_CAPABILITY } from "@omnirush/types/automations"
...
describe("Automation runner credentials", () => {
  ...
  test("binds a Den Web proxied credential to its trusted public route", () => {
    const request = new Request("http://api.omnirushlabs.com/v1/automation-runners/token", {
      headers: {
        "x-forwarded-host": "app.omnirushlabs.com",
    ...
    expect(automationRunnerAudienceFromRequest(request, {
      trustedOrigins: ["https://app.omnirushlabs.com"],
    })).toBe("https://app.omnirushlabs.com/api/den")
  })
  ...
  test("ignores an untrusted forwarded runner destination", () => {
    const request = new Request("https://api.omnirushlabs.com/v1/automation-runners/token", {
      headers: {
    ...
    expect(automationRunnerAudienceFromRequest(request, {
      trustedOrigins: ["https://app.omnirushlabs.com"],
    })).toBe("https://api.omnirushlabs.com")
  })
  ...
  test("keeps legacy v1 credentials capability-free", () => {
    ...
    const signature = createHmac("sha256", secret)
      .update(`omnirush-automation-runner-v1.${payload}`)
      .digest("base64url")
