...
describe("MicrosoftGraphClient", () => {
  ...
  test("creates drafts and events, writes OneDrive text, and reads/sends Teams chat", async () => {
    ...
    const fetchMock: typeof fetch = async (input, init) => {
      ...
      if (decodeURIComponent(url.pathname).endsWith("/me/drive/root:/OmniRush.ai/Review notes.txt:/content")) {
    ...
    expect((await client.putDriveTextFile({ path: "OmniRush.ai/Review notes.txt", content: "Notes" })).id).toBe("file_1")
...
test("OneDrive path search escapes OData apostrophes", () => {
  ...
  expect(encodeOneDrivePath("OmniRush.ai/Review notes.txt")).toBe("OmniRush.ai/Review%20notes.txt")
})
