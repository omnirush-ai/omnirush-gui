...
import { HeadlessThreadError } from "@omnirush/headless-threads"
  ...
  const result = await executeRemoteSessionCapability(
    executeInput("create", {}),
    {
      ...
      resolveRuntime: async () => ({
        ...
        message: "Your OmniRush.ai Cloud workspace is still starting.",
        retryable: true,
      }),
      createClient: () => {
...
test("remote sessions route workspace discovery only to the signed preview", async () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    expect(new Headers(init?.headers).get("x-omnirush-host-token")).toBe("host-token")
  ...
  const result = await executeRemoteSessionCapability(
    executeInput("create", {}),
    {
      ...
      resolveRuntime: async () => ({
        ...
        message: "No OmniRush.ai Cloud workspace is available for your account yet.",
        retryable: false,
      }),
      createClient: () => {
        throw new Error("createClient must not be called when the runtime is not ready")
...
async function registryContext(input: { remoteSessionsEnabled: boolean }) {
  ...
  const { createDenTypeId: createId } = await import("@omnirush-ee/utils/typeid")
