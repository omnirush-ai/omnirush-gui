...
const origin = "https://diagnostic.omnirush.test"
...
describe("Den private-cloud egress diagnostic", () => {
  test("defaults to the OmniRush.ai Labs diagnostic host and accepts an operator override", () => {
    expectConfiguredOrigin("https://diagnostic.omnirushlabs.com")
  ...
  test("identifies DNS failure before HTTP reaches OmniRush.ai", async () => {
  ...
  test("identifies a proxy-replaced response that lacks OmniRush.ai receipt proof", async () => {
    const result = await runEgressDiagnostic({
      bearerToken: "synthetic-diagnostics-secret",
