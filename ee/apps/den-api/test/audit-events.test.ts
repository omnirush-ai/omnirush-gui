...
test("organization audit events support SCIM management actions", () => {
  ...
  const event = buildOrganizationAuditEvent({
    ...
    payload: {
      scimProviderId,
      providerId: "omnirush-scim-org_id",
    },
  })
...
test("organization audit events support SSO management actions", () => {
  ...
  const event = buildOrganizationAuditEvent({
    ...
    payload: {
      ssoConnectionId,
      providerId: "omnirush-sso-org_id",
      kind: "saml",
...
test("organization audit alerting covers sensitive access changes", () => {
  ...
  expect(isOrganizationAuditAlertAction(ORGANIZATION_AUDIT_ACTIONS.omniRushWebComplimentaryAccessGranted)).toBe(true)
  expect(isOrganizationAuditAlertAction(ORGANIZATION_AUDIT_ACTIONS.omniRushWebComplimentaryAccessRevoked)).toBe(true)
...
test("organization audit events record complimentary Web access changes without storing access metadata", () => {
  const event = buildOrganizationAuditEvent({
    organizationId: createDenTypeId("organization"),
    actorUserId: createDenTypeId("user"),
    action: ORGANIZATION_AUDIT_ACTIONS.omniRushWebComplimentaryAccessGranted,
    payload: {
  ...
  expect(event.action).toBe("organization.omnirush_web.complimentary_access_granted")
  expect(event.payload).toEqual({
    reason: "Internal administration organization",
    complimentaryAccess: true,
  })
