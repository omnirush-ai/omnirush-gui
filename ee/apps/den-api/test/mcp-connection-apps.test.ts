...
test("connect app-host server names mirror the desktop naming convention", () => {
  ...
  expect(connections.connectMcpAppHostServerName(connectionId)).toBe(`omnirush-app-host-connect-${digest}`)
})

test("projected tool names sanitize like the desktop host", () => {
  expect(connections.projectedMcpToolName("omnirush-app-host-connect-abc123", "render_report"))
    .toBe("omnirush-app-host-connect-abc123_render_report")
