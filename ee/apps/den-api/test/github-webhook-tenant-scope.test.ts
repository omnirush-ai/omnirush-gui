...
const vulnerableCrossInstallationMatch = {
  ...
  target: {
    ...
    connectorType: "github",
    remoteId: "omnirush-ai/omnirush-gui",
    targetConfigJson: { repositoryFullName: "omnirush-ai/omnirush-gui", repositoryId: 42 },
  },
}
...
test("repository rename updates connector identity, records continuity, queues resync, and acks 202", async () => {
  ...
  try {
    const response = await deliverWebhook("repository", {
      action: "renamed",
      changes: { repository: { name: { from: "omnirush" } } },
      installation: { id: 12345 },
      repository: { full_name: "omnirush-ai/omnirush-renamed", id: 42 },
    })

    expect(response.status).toBe(202)
    expect(updatedValues).toEqual([
      expect.objectContaining({
        remoteId: "omnirush-ai/omnirush-renamed",
        targetConfigJson: expect.objectContaining({ repositoryFullName: "omnirush-ai/omnirush-renamed", repositoryId: 42 }),
      }),
      expect.objectContaining({ remoteId: "omnirush-ai/omnirush-renamed" }),
    ])
    expect(insertedValues).toEqual([
      expect.objectContaining({
        ...
        summaryJson: {
          ...
          from: "omnirush-ai/omnirush-gui",
          to: "omnirush-ai/omnirush-renamed",
          trigger: "webhook",
        },
      }),
...
test("removed installation repository disables its target and instance and acks 202", async () => {
  ...
  try {
    const response = await deliverWebhook("installation_repositories", {
      action: "removed",
      installation: { id: 12345 },
      repositories_removed: [{ full_name: "omnirush-ai/omnirush-gui", id: 42 }],
    })
...
test("unmatched repository rename is ignored with a 200 ack", async () => {
  ...
  const response = await deliverWebhook("repository", {
    ...
    installation: { id: 12345 },
    repository: { full_name: "omnirush-ai/renamed", id: 404 },
  })
