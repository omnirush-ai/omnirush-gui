...
describe("public API URL configuration", () => {
  test("callback base resolution keeps the configured prefix", async () => {
    ...
    expect(resolvePublicApiBaseUrl(
      ...
      "https://omnirush.example/api/den/",
    )).toBe("https://omnirush.example/api/den")
  })

  test("preserves and normalizes an HTTPS pathname prefix", () => {
    expect(normalizeConfiguredPublicApiBaseUrl("https://omnirush.example/api/den/", {
      allowInsecureHttp: false,
    })).toBe("https://omnirush.example/api/den")
  })

  test("requires HTTPS outside development and localhost", () => {
    expect(() => normalizeConfiguredPublicApiBaseUrl("http://omnirush.example/api/den", {
      allowInsecureHttp: false,
    ...
    })).toBe("http://omnirush.example/api/den")
  })
  ...
  test("rejects malformed and non-base URLs", () => {
    ...
    expect(() => normalizeConfiguredPublicApiBaseUrl("https://omnirush.example/api?tenant=one", {
      allowInsecureHttp: false,
