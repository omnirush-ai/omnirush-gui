...
const omniRushProvider: AutomationAuthorityProvider = {
...
function authorityStore(overrides: Partial<AutomationModelAuthorityStore> = {}): AutomationModelAuthorityStore {
  return {
    ...
    async findOmniRushProvider() { return omniRushProvider },
...
describe("Automation normalized model authority", () => {
  ...
  test("resolves enabled OmniRush.ai aliases through the owner's managed provider", async () => {
    const result = await resolveAutomationModelAccessWithStore({
      ...base,
      providerId: "omnirush",
    ...
    expect(result).toMatchObject({
      ...
      value: {
        ...
        providerRecordId: omniRushProvider.id,
        providerId: "omnirush",
        modelId: "z-ai/glm-5.2",
      },
    })
