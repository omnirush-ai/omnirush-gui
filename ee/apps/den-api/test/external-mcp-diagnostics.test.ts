...
class RecordingOAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata() {
    return {
      ...
      client_name: "OmniRush.ai deadline test",
...
describe("external MCP diagnostics", () => {
  ...
  test("attributes our own lifecycle deadline to OmniRush.ai rather than the provider", () => {
  ...
  test("attributes a wrapped lifecycle deadline to OmniRush.ai through its cause chain", () => {
  ...
  test("uses an honest fallback message after MCP protocol readiness", () => {
    ...
    expect(diagnostic.message).toBe("The MCP server answered, but OmniRush.ai could not interpret its response for the current request.")
  })
  ...
  test("successful callbacks stay in the browser without opening the desktop app", () => {
    ...
    expect(html).toContain("Enterprise MCP &lt;test&gt; is connected to OmniRush.ai.")
    expect(html).not.toContain("window.close")
    expect(html).not.toContain("<button")
    expect(html).toContain("You can close this window and return to OmniRush.ai.")
    expect(html).toContain('<div class="brand">')
    expect(html).toContain("OmniRush.ai</span>")
    expect(html).not.toContain("@keyframes")
    expect(html).not.toContain("omnirush://")
    expect(html).not.toContain("Open OmniRush.ai")
  })
