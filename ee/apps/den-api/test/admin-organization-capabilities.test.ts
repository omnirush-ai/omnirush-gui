...
let schema: typeof import("@omnirush-ee/den-db/schema") | null = null
let drizzle: typeof import("@omnirush-ee/den-db/drizzle") | null = null
...
async function putOmniRushWebAccess(enabled: boolean, reason: string) {
  return routeApp().request(`http://den.local/v1/admin/organizations/${organizationId}/omnirush-web-access`, {
    method: "PUT",
    headers: { "content-type": "application/json" },
...
beforeAll(async () => {
  ...
  let schemaModule: typeof import("@omnirush-ee/den-db/schema")
  let drizzleModule: typeof import("@omnirush-ee/den-db/drizzle")
  try {
    [dbModule, schemaModule, drizzleModule] = await Promise.all([
      import("../src/db.js"),
      import("@omnirush-ee/den-db/schema"),
      import("@omnirush-ee/den-db/drizzle"),
    ])
...
test("admin capability routes show effective defaults while preserving raw overrides", async () => {
  ...
  // entitled by OmniRush.ai Web access instead.
...
test("platform admins grant and revoke audited complimentary Web access", async () => {
  ...
  const invalid = await putOmniRushWebAccess(true, "x")
  expect(invalid.status).toBe(400)

  const grant = await putOmniRushWebAccess(true, "Internal administration organization")
  ...
  await expect(listed.json()).resolves.toMatchObject({
    organizations: [{
      id: organizationId,
      omnirushWebAccess: { complimentaryAccess: true, hasOngoingSubscription: false },
    }],
  })
  ...
  expect(auditRows[0]).toMatchObject({
    action: "organization.omnirush_web.complimentary_access_granted",
    payload: { reason: "Internal administration organization", complimentaryAccess: true },
  })

  const revoke = await putOmniRushWebAccess(false, "Return organization to standard billing")
  expect(revoke.status).toBe(200)
  expect(await readOrganizationMetadata()).not.toHaveProperty("complimentaryAccess.omnirushWeb")
  ...
  expect(revokedAuditRows.map((row) => row.action).sort()).toEqual([
    "organization.omnirush_web.complimentary_access_granted",
    "organization.omnirush_web.complimentary_access_revoked",
  ])
  ...
  const paidConflict = await putOmniRushWebAccess(true, "Would overlap paid billing")
  expect(paidConflict.status).toBe(409)
  await expect(paidConflict.json()).resolves.toMatchObject({ error: "omnirush_web_subscription_exists" })
  expect(await readOrganizationMetadata()).not.toHaveProperty("complimentaryAccess.omnirushWeb")
})
