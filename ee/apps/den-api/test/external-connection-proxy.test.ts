...
mock.module("../src/auth.js", () => ({
  ...
  DEN_MCP_GRANT_ID_CLAIM: "https://omnirushlabs.com/grant_id",
  DEN_MCP_ORG_ID_CLAIM: "https://omnirushlabs.com/org_id",
  DEN_MCP_OAUTH_RESOURCE: "http://127.0.0.1:8790/mcp/agent",
  DEN_MCP_RESOURCE: "http://127.0.0.1:8790/mcp",
  DEN_MCP_RESOURCE_CLAIM: "https://omnirushlabs.com/resource",
  DEN_MCP_RESOURCES: ["http://127.0.0.1:8790/mcp"],
  DEN_MCP_TOKEN_USE_CLAIM: "https://omnirushlabs.com/token_use",
}))
...
const operation = {
  connection,
  redirectUri: "https://omnirush.example/v1/mcp-connections/fixture/connect/callback",
...
test("ordinary MCP clients receive only bounded search and execute without the per-provider App surface", async () => {
  await withClient({ tools: {}, resources: {} }, async (client) => {
    ...
    await expect(client.readResource({ uri: resourceUri }))
      .rejects.toThrow("only through the OmniRush.ai App host")
  }, {}, false)
})
...
test("a directly exposed connection serves its provider catalog to ordinary clients", async () => {
  ...
  await withClient({ tools: {}, resources: {} }, async (client) => {
    ...
    await expect(client.readResource({ uri: resourceUri })).rejects.toThrow("only through the OmniRush.ai App host")
    expect(downstreamCalls).toBe(1)
  }, {
    listTools: async () => [
...
test("the request handler never enables direct exposure unless the route confirms the organization flag", async () => {
  ...
  const request = new Request("https://omnirush.example/mcp/agent/connections/fixture", { method: "POST" })
...
test("a forged App-host audience header cannot unlock the provider surface", async () => {
  let toolNames: string[] = []
  const request = new Request("https://omnirush.example/mcp/agent/connections/fixture", {
    method: "POST",
    headers: { "x-omnirush-mcp-client-audience": "app-host" },
  })
  await handleExternalConnectionProxyRequest({
    context: requestContext(request),
    operation,
...
test("OAuth registration and network failures become sanitized protocol errors", async () => {
  ...
  const oauthRequest = new Request("https://omnirush.example/mcp", {
    method: "POST",
  ...
  const networkRequest = new Request("https://omnirush.example/mcp", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
test("unsupported GET requests never trigger downstream discovery", async () => {
  ...
  const request = new Request("https://omnirush.example/mcp", { method: "GET" })
...
test("a client that does not advertise the App host capability receives an empty provider index", () => {
  expect(buildConnectMcpServerIndex({
    ...
    publicOrigin: "https://omnirush.example",
  }).servers).toEqual([])
})
