...
import { createDenTypeId, type DenTypeId } from "@omnirush-ee/utils/typeid"
import { afterAll, beforeAll, expect, mock, test } from "bun:test"

function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/omnirush_test_pr7"
...
let schema: typeof import("@omnirush-ee/den-db/schema")
let drizzle: typeof import("@omnirush-ee/den-db/drizzle")
...
beforeAll(async () => {
  seedRequiredEnv()
  mock.restore()
  const realDb = (await import("@omnirush-ee/den-db")).createDenDb({
...
test("connect start keeps the shared callback for a pre-registered confidential client without response issuer support", async () => {
  ...
  try {
    ...
    const sharedCallback = "https://app.omnirushlabs.com/api/den/v1/mcp-connections/oauth/callback"
...
test("version-two legacy callbacks use enterprise issuer validation", async () => {
  ...
  expect(response.status).toBe(400)
  const html = await response.text()
  expect(html).toContain("OmniRush.ai could not register or identify its OAuth client with the authorization server")
