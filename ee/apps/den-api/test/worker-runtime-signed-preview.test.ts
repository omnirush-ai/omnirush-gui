...
function seedRequiredEnv() {
  process.env.DATABASE_URL ??= "mysql://root:password@127.0.0.1:3306/omnirush_test"
...
test("cloud worker tokens expose expiring URLs only by explicit opt-in", async () => {
  ...
  const resolved = await shared.getWorkerTokensAndConnect(runtimeWorker, {
    getOmniRushWebAccess: async () => ({ hasAccess: true }),
    includeExpiringOmniRushUrl: true,
    apiPublicUrl: "https://den.example.test/api/den",
    loadActiveTokens,
  ...
  expect(legacy).toEqual({
    ...
    connect: {
      omnirushUrl: `https://den.example.test/api/den/v1/cloud/workers/${runtimeWorker.id}`,
      workspaceId: null,
    },
  })
  expect(resolved).toEqual({
    tokens: { owner: "host-token", host: "host-token", client: "client-token" },
    connect: {
      omnirushUrl: `https://den.example.test/api/den/v1/cloud/workers/${runtimeWorker.id}/w/created-workspace`,
      workspaceId: "created-workspace",
    },
    directPreview: {
      version: 1,
      omnirushUrl: "https://create-token.preview.example.test/w/created-workspace",
      workspaceId: "created-workspace",
  ...
  expect("connect" in legacy ? legacy.connect?.omnirushUrl : null).not.toContain("preview.example.test")
...
function registerCompatibilityTestApp(input: {
  ...
  compatibility.registerCloudWorkerCompatibilityRoutes(app, {
    getOmniRushWebAccess: async () => ({ hasAccess: input.hasWebAccess ?? true }),
    authenticate: async ({ request, workerId }) => {
      if (workerId !== input.workerId) return null
      const bearer = request.headers.get("authorization")?.replace(/^Bearer\s+/i, "") ?? ""
      const host = request.headers.get("x-omnirush-host-token") ?? ""
...
test("cloud worker OPTIONS bypasses global credentialed CORS for native origins", async () => {
  ...
  for (const origin of ["null", "omnirush://desktop"]) {
    const response = await app.request("https://den.example.test/v1/cloud/workers/worker_native/health", {
      method: "OPTIONS",
      headers: {
        Origin: origin,
        "Access-Control-Request-Method": "PATCH",
        "Access-Control-Request-Headers": "content-type,x-omnirush-host-token,x-opencode-directory",
      },
    })
    expect(response.status).toBe(204)
    expect(response.headers.get("access-control-allow-origin")).toBe(origin)
    expect(response.headers.get("access-control-allow-credentials")).toBeNull()
    expect(response.headers.get("access-control-allow-headers")?.toLowerCase()).toContain("x-omnirush-host-token")
    expect(response.headers.get("access-control-allow-methods")).toContain("PATCH")
  }

  const unrelated = await app.request("https://den.example.test/v1/other", {
    method: "OPTIONS",
    headers: { Origin: "omnirush://desktop", "Access-Control-Request-Method": "POST" },
  })
...
test("stable cloud worker route requires Web access before resolving or calling the VM", async () => {
  ...
  const response = await app.request(
    ...
    { method: "POST", headers: { "X-OmniRush-Host-Token": "host-token" } },
  )
...
test("stable cloud worker route enforces worker token and method scope", async () => {
  ...
  const hostWrite = await app.request(route, {
    method: "PUT",
    headers: { "X-OmniRush-Host-Token": "host-token", "Content-Type": "application/json" },
    body: "{}",
  })
  ...
  expect(handoffHeaders.get("x-omnirush-host-token")).toBe("fresh-host-token")
})
...
test("stable cloud worker route safely proxies bodies, headers, and streaming responses", async () => {
  ...
  const write = await app.request(`${route}/workspace/demo/config?mode=replace`, {
    ...
    headers: {
      ...
      "X-OmniRush-Host-Token": "host-token",
    },
  ...
  expect(writeHeaders.get("x-omnirush-host-token")).toBe("fresh-host-token")
  ...
  expect(streamHeaders.get("x-omnirush-host-token")).toBeNull()
...
test("stable cloud worker route streams a multi-chunk large request without buffering", async () => {
  ...
  const requestInit: RequestInit & { duplex: "half" } = {
    method: "POST",
    headers: { "X-OmniRush-Host-Token": "host-token" },
    body,
    duplex: "half",
  }
