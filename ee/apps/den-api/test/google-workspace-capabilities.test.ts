...
function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/omnirush_test_gwscaps"
...
test("calendar create requests a Google Meet link when asked", async () => {
  ...
  expect(requestId.startsWith("omnirush-")).toBe(true)
...
test("no connected account returns needs_connection", async () => {
  ...
  expect(body).toEqual({
    ...
    message: "Connect your Google account first: open Settings > Connect and use Connect your account on the Google Workspace row, or connect from the OmniRush.ai Cloud dashboard.",
  })
})
