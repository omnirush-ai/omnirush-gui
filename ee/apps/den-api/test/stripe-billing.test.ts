...
mock.module("../src/env.js", () => ({
  env: {
    ...
    stripe: {
      ...
      omnirushWebPriceId: "price_web_fake",
...
const {
  OMNIRUSH_WEB_QUANTITY_DEFINITION,
  OMNIRUSH_WEB_UNIT_AMOUNT,
  calculateOmniRushWebBilling,
  createOmniRushWebCheckout,
  findOrCreateStripeCustomer,
  getOmniRushWebBillingSummary,
  handleStripeWebhook,
  isEligibleOmniRushWebSubscriptionStatus,
  isOngoingOmniRushWebSubscriptionStatus,
  isOmniRushWebBillableMember,
  omniRushWebCheckoutIdempotencyKey,
...
const { omniRushWebAvailableForOrganization, omniRushWebDeploymentAvailable } = await import("../src/omnirush-web-availability.js")
...
beforeEach(() => {
  ...
  env.omnirushWebEnabled = true
  env.stripe.secretKey = "sk_test_fake"
  env.stripe.omnirushWebPriceId = "price_web_fake"
...
test("OmniRush.ai Web bills joined non-removed members at $50 each", () => {
  expect(OMNIRUSH_WEB_QUANTITY_DEFINITION).toBe("joined_non_removed_members")
  expect(OMNIRUSH_WEB_UNIT_AMOUNT).toBe(5000)
  expect(isOmniRushWebBillableMember({ joinedAt: new Date(), removedAt: null })).toBe(true)
  expect(isOmniRushWebBillableMember({ joinedAt: null, removedAt: null })).toBe(false)
  expect(isOmniRushWebBillableMember({ joinedAt: new Date(), removedAt: new Date() })).toBe(false)
  expect(calculateOmniRushWebBilling({ joinedMemberCount: 3 })).toEqual({
    quantity: 3,
    unitAmount: 5000,
...
test("only active and trialing Web subscriptions are eligible", () => {
  for (const status of ["active", "trialing"]) {
    expect(isEligibleOmniRushWebSubscriptionStatus(status)).toBe(true)
  }
  for (const status of ["incomplete", "incomplete_expired", "past_due", "canceled", "unpaid", "paused", "expired"]) {
    expect(isEligibleOmniRushWebSubscriptionStatus(status)).toBe(false)
  }
})

test("the deployment flag is global while the complimentary override is organization-scoped", () => {
  expect(omniRushWebDeploymentAvailable(true)).toBe(true)
  expect(omniRushWebDeploymentAvailable(false)).toBe(false)
  expect(omniRushWebAvailableForOrganization(false, {})).toBe(false)
  expect(omniRushWebAvailableForOrganization(false, { capabilities: { omnirushWeb: true } })).toBe(false)
  expect(omniRushWebAvailableForOrganization(false, { complimentaryAccess: { omnirushWeb: true } })).toBe(true)
  expect(omniRushWebAvailableForOrganization(true, {})).toBe(true)

  env.orgMode = "single_org"
  expect(omniRushWebDeploymentAvailable(env.omnirushWebEnabled)).toBe(true)
})
...
test("ongoing Web statuses suppress duplicate checkout until the subscription is terminal", () => {
  for (const status of ["active", "trialing", "incomplete", "past_due", "unpaid", "paused"]) {
    expect(isOngoingOmniRushWebSubscriptionStatus(status)).toBe(true)
  }
  for (const status of ["canceled", "incomplete_expired", "expired"]) {
    expect(isOngoingOmniRushWebSubscriptionStatus(status)).toBe(false)
  }
})
...
test("member-readable Web billing summary omits Stripe identifiers and portal access", async () => {
  ...
  const summary = await getOmniRushWebBillingSummary("org_test")
...
test("complimentary Web access works without Stripe configuration and overrides the deployment flag for that organization", async () => {
  env.stripe.secretKey = ""
  selectResults.push([], [{ count: 2 }], [{ metadata: { complimentaryAccess: { omnirushWeb: true } } }])

  const enabledSummary = await getOmniRushWebBillingSummary("org_test")
  ...
  env.omnirushWebEnabled = false
  selectResults.push([], [{ count: 2 }], [{ metadata: { complimentaryAccess: { omnirushWeb: true } } }])
  const disabledSummary = await getOmniRushWebBillingSummary("org_test")
  ...
  selectResults.push([], [{ count: 2 }], [{ metadata: {} }])
  const ungrantedSummary = await getOmniRushWebBillingSummary("org_other")
...
test("Web checkout uses the configured monthly price and authoritative joined-member quantity", async () => {
  ...
  const session = await createOmniRushWebCheckout(checkoutInput())
  ...
  expect(checkoutCreates[0]?.params).toMatchObject({
    ...
    metadata: { org_id: "org_test", subscription_type: "web", omnirush_product: "omnirush_web" },
    subscription_data: { metadata: { org_id: "org_test", subscription_type: "web" } },
  })
  expect(checkoutCreates[0]?.options).toEqual({
    idempotencyKey: omniRushWebCheckoutIdempotencyKey({ organizationId: "org_test", quantity: 2 }),
  })
})
...
test("deployments without the Web flag cannot create a checkout even when Stripe billing is configured", async () => {
  ...
  await expect(createOmniRushWebCheckout(checkoutInput())).rejects.toThrow("stripe_omnirush_web_not_available")
...
test("Web checkout rejects a configured price that is not exactly $50 per licensed user each month", async () => {
  ...
  await expect(createOmniRushWebCheckout(checkoutInput())).rejects.toThrow("stripe_omnirush_web_price_contract_invalid")
  expect(checkoutCreates).toHaveLength(0)
})
...
test("an eligible Stripe Web subscription blocks a second checkout", async () => {
  ...
  await expect(createOmniRushWebCheckout(checkoutInput())).rejects.toThrow("stripe_omnirush_web_subscription_exists")
...
test("organization-scoped customers never reuse another organization by email", async () => {
  ...
  expect(customerCreates.map((call) => call.options)).toEqual([
    { idempotencyKey: "omnirush-org-customer:org_alpha" },
    { idempotencyKey: "omnirush-org-customer:org_beta" },
  ])
})
...
test("checkout sync rejects cross-organization sessions and stores ineligible payment state without granting eligibility", async () => {
  ...
  expect(isEligibleOmniRushWebSubscriptionStatus(typeof insertedStatus === "string" ? insertedStatus : null)).toBe(false)
})
