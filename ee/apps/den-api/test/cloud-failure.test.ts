...
import { OmniRushWebAccessRequiredError } from "../src/omnirush-web-access-error.js"

describe("Cloud startup failure diagnostics", () => {
  test("classifies a revoked OmniRush.ai Web entitlement as its own failure code", () => {
    expect(classifyCloudStartupFailure(new OmniRushWebAccessRequiredError())).toBe("web_access_required")
  ...
  test("classifies actionable startup stages without exposing raw provider output", () => {
    ...
    expect(classifyCloudStartupFailure(new Error("omnirush session exited with 1\nstderr: token=secret")))
      .toBe("runtime_start_failed")

    const failure = createCloudStartupFailure({
      stage: "recovery",
      error: new Error("omnirush session exited with 1\nstderr: token=secret"),
