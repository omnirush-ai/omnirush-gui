...
const GRANT_CLAIM = "https://omnirushlabs.com/grant_id"
...
function validMcpJwtPayload(includeGrant: boolean) {
  const payload: Record<string, unknown> = {
    ...
    "https://omnirushlabs.com/resource": AGENT_RESOURCE,
...
beforeAll(async () => {
  seedRequiredEnv()
  mock.module("@omnirush-ee/den-db/schema", () => ({
    AuthSessionTable,
    AuthUserTable,
    InvitationTable,
    MemberTable,
  ...
  mock.module("@omnirush-ee/den-db/drizzle", () => ({
...
test("grant-claim tokens stay valid without consulting their dead login session", async () => {
  ...
  try {
    ...
    if (!(principal instanceof Response)) {
      expect(principal.organizationId).toBe(jwtPayload["https://omnirushlabs.com/org_id"])
    }
