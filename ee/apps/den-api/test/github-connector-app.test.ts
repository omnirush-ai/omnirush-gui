...
describe("github connector app helpers", () => {
  ...
  test("lists repositories through the GitHub installation token flow", async () => {
    ...
    const repositories = await listGithubInstallationRepositories({
      ...
      fetchFn: async (url, init) => {
        ...
        if (String(url).endsWith("/contents/.claude-plugin/marketplace.json")) {
          if (String(url).includes("omnirush-ai/omnirush-gui")) {
        ...
        if (String(url).endsWith("/contents/.claude-plugin/plugin.json")) {
          if (String(url).includes("omnirush-ai/opencode")) {
        ...
        return new Response(JSON.stringify({
          repositories: [
            { default_branch: "main", full_name: "omnirush-ai/omnirush-gui", id: 42, private: true },
            { default_branch: "dev", full_name: "omnirush-ai/opencode", id: 99, private: false },
          ],
        }), { status: 200 })
      },
      installationId: 777,
    ...
    expect(requestUrls.slice(2).sort()).toEqual([
      "https://api.github.com/repos/omnirush-ai/omnirush-gui/contents/.claude-plugin/marketplace.json",
      "https://api.github.com/repos/omnirush-ai/opencode/contents/.claude-plugin/marketplace.json",
      "https://api.github.com/repos/omnirush-ai/opencode/contents/.claude-plugin/plugin.json",
    ].sort())
    expect(requestUrls.filter((url) => url.includes("/repos/omnirush-ai/opencode/contents/"))).toEqual([
      "https://api.github.com/repos/omnirush-ai/opencode/contents/.claude-plugin/marketplace.json",
      "https://api.github.com/repos/omnirush-ai/opencode/contents/.claude-plugin/plugin.json",
    ])
    expect(requestUrls).not.toContain("https://api.github.com/installation/repositories?per_page=100&page=2")
    expect(repositories).toEqual([
      { defaultBranch: "main", fullName: "omnirush-ai/omnirush-gui", hasPluginManifest: true, id: 42, manifestKind: "marketplace", marketplacePluginCount: 3, private: true },
      { defaultBranch: "dev", fullName: "omnirush-ai/opencode", hasPluginManifest: true, id: 99, manifestKind: "plugin", marketplacePluginCount: null, private: false },
    ])
  })
  ...
  test("detects a root Agent Plugin manifest", async () => {
    const repositories = await listGithubInstallationRepositories({
      ...
      fetchFn: async (url) => {
        ...
        return new Response(JSON.stringify({
          repositories: [
            { default_branch: "main", full_name: "omnirush-ai/team-tools", id: 7, private: true },
          ],
        }), { status: 200 })
      },
      installationId: 777,
    ...
    expect(repositories).toEqual([{
      defaultBranch: "main",
      fullName: "omnirush-ai/team-tools",
          ...
          return new Response(JSON.stringify({
            repositories: firstId === null
              ...
              : Array.from({ length: count }, (_, index) => {
                  ...
                  return { default_branch: "main", full_name: `omnirush-ai/repo-${id}`, id, private: false }
                }),
          }), { status: 200 })
        }
  ...
  test("builds install URLs and validates signed state tokens", async () => {
    const app = await getGithubAppSummary({
      ...
      fetchFn: async () => new Response(JSON.stringify({
        html_url: "https://github.com/apps/omnirush-test",
        name: "OmniRush.ai Test",
        slug: "omnirush-test",
      }), { status: 200 }),
    })
    ...
    expect(buildGithubAppInstallUrl({ app, state: token })).toBe(`https://github.com/apps/omnirush-test/installations/new?state=${encodeURIComponent(token)}`)
  ...
  test("reads GitHub installation account details", async () => {
    ...
    expect(installation).toEqual({
      accountLogin: "omnirush-ai",
      accountType: "Organization",
      displayName: "omnirush-ai",
      installationId: 777,
  ...
  test("skips content fetches for files already seen at the current revision", async () => {
    ...
    const fetchFile = (path: string) => getGithubRepositoryTextFile({
      ...
      path,
      ref: "main",
      repositoryFullName: "omnirush-ai/omnirush-gui",
    ...
    expect(contentRequests).toEqual([
      "https://api.github.com/repos/omnirush-ai/omnirush-gui/contents/skills/c/SKILL.md?ref=main",
      "https://api.github.com/repos/omnirush-ai/omnirush-gui/contents/skills/d/SKILL.md?ref=main",
    ])
  })
  ...
  test("validates repository identity and branch existence against GitHub", async () => {
    const result = await validateGithubInstallationTarget({
      ...
      fetchFn: async (url) => {
        ...
        if (String(url).endsWith("/repos/omnirush-ai/omnirush-gui")) {
          return new Response(JSON.stringify({
            default_branch: "main",
            full_name: "omnirush-ai/omnirush-gui",
            id: 42,
          }), { status: 200 })
        }

        if (String(url).endsWith("/repos/omnirush-ai/omnirush-gui/branches/main")) {
