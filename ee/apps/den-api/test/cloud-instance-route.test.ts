...
mock.module("../src/omnirush-web-runtime-access.js", () => {
  const code = "omnirush_web_access_required" as const
  const message = "An active OmniRush.ai Web subscription or complimentary access is required to use OmniRush.ai Cloud."
  class OmniRushWebAccessRequiredError extends Error {
    readonly code = code
  ...
  const getOmniRushWebRuntimeAccess = async () => ({ hasAccess: true })
  return {
    OMNIRUSH_WEB_ACCESS_REQUIRED_CODE: code,
    OMNIRUSH_WEB_ACCESS_REQUIRED_MESSAGE: message,
    OmniRushWebAccessRequiredError,
    getOmniRushWebRuntimeAccess,
    requireOmniRushWebRuntimeAccess: async (
      organizationId: string,
      resolveAccess = getOmniRushWebRuntimeAccess,
    ) => {
      const access = await resolveAccess(organizationId)
      if (!access.hasAccess) throw new OmniRushWebAccessRequiredError()
      return access
    },
    omniRushWebAccessRequiredPayload: () => ({ error: code, message }),
  }
})
...
function seedRequiredEnv() {
  ...
  process.env.DAYTONA_SNAPSHOT = "omnirush-0.18.8"
}
...
describe("Cloud instance route gate", () => {
  test("opens Cloud for an organization with Web access and no per-organization Cloud flag", async () => {
    ...
    routes.registerCloudRoutes(app, {
      ...
      getOmniRushWebAccess: async () => ({ hasAccess: true }),
  ...
  test("ignores a stale literal Cloud opt-in when Web access is not active", async () => {
    ...
    routes.registerCloudRoutes(app, {
      ...
      getOmniRushWebAccess: async () => ({ hasAccess: false }),
    ...
    await expect(response.json()).resolves.toMatchObject({ error: "omnirush_web_access_required" })
  ...
  test("denies member resolution before provisioning when Web access is not active", async () => {
    ...
    await expect(response.json()).resolves.toEqual({
      error: "omnirush_web_access_required",
      message: "An active OmniRush.ai Web subscription or complimentary access is required to use OmniRush.ai Cloud.",
    })
    expect(ensureCalls).toBe(0)
  })
...
describe("Cloud gateway resolve route", () => {
  const grantedOmniRushWebAccess = async () => ({ hasAccess: true })
  ...
  test("returns 404 when the gateway key is not configured", async () => {
    ...
    const response = await app.request("http://den.local/v1/cloud/gateway/resolve", {
      headers: { "X-OmniRush-Gateway-Key": "gateway-secret" },
    })
  ...
  test("returns 404 when the gateway key is wrong", async () => {
    ...
    const response = await app.request("http://den.local/v1/cloud/gateway/resolve", {
      headers: { "X-OmniRush-Gateway-Key": "bad-secret" },
    })
  ...
  test("returns the gateway tokens only when the instance is ready", async () => {
    ...
    routes.registerCloudRoutes(provisioningApp, {
      ...
      daytonaApiKey: "daytona-test-key",
      gatewayKey: "gateway-secret",
      getOmniRushWebAccess: grantedOmniRushWebAccess,
      ensureCloudWorker: async () => provisioningWorker,
      getSandboxRecord: async () => null,
    })
...
describe("Cloud instance route lifecycle states", () => {
  test("returns the Daytona sandbox identifier on the member instance response", async () => {
    const worker = { ...fakeWorker("healthy"), image_version: "omnirush-0.18.7" }
    const app = new Hono<{ Variables: OrgRouteVariables }>()
  ...
  test("claims and wakes a stale stopped sandbox without checkpoint probing on resolve", async () => {
    const worker = { ...storedWorker({ status: "healthy" }), image_version: "omnirush-0.18.7" }
    ...
    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual(expectedCloudInstance({ status: "waking", url: null, imageVersion: "omnirush-0.18.7" }))
  ...
  test("does not inspect the sandbox for an up-to-date stopped worker wake", async () => {
    const worker = { ...fakeWorker("stopped"), image_version: "omnirush-0.18.8" }
    const app = new Hono<{ Variables: OrgRouteVariables }>()
    let wakeCalls = 0
    let inspectCalls = 0
    ...
    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual(expectedCloudInstance({ status: "waking", url: null, imageVersion: "omnirush-0.18.8" }))
...
describe("Cloud instance update route", () => {
  test("denies an update request before touching the sandbox when Web access is not active", async () => {
    ...
    const worker = { ...storedWorker({ orgId, userId, status: "healthy" }), image_version: "omnirush-0.18.7" }
    const store = makeCloudWorkerStore({ initialWorkers: [worker] })
    const app = new Hono<{ Variables: OrgRouteVariables }>()
    let flushCalls = 0
    let stopCalls = 0
  ...
  test("refuses to stop an already-current worker", async () => {
    const orgId = createDenTypeId("organization")
    const userId = createDenTypeId("user")
    const worker = { ...storedWorker({ orgId, userId, status: "healthy" }), image_version: "omnirush-0.18.8" }
    const store = makeCloudWorkerStore({ initialWorkers: [worker] })
    const app = new Hono<{ Variables: OrgRouteVariables }>()
    let inspectCalls = 0
    let stopCalls = 0
