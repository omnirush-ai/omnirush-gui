...
function seedRequiredEnv() {
  ...
  process.env.DAYTONA_RUNTIME_DATA_PATH = "/tmp/omnirush-data"
  process.env.DAYTONA_RUNTIME_WORKSPACE_PATH = "/tmp/omnirush-workspace"
  process.env.DAYTONA_DATA_MOUNT_PATH = "/persist/omnirush"
  process.env.DAYTONA_WORKSPACE_MOUNT_PATH = "/workspace"
  process.env.DAYTONA_SIDECAR_DIR = "/tmp/omnirush-sidecars"
...
describe("Daytona OmniRush.ai checkpoint start command", () => {
  test("keeps checkpoint seams and safety invariants in the sandbox script", () => {
    const command = daytona.buildOmniRushStartCommand({
    ...
    expect(command).toContain("OMNIRUSH_STATE_MANIFEST=")
    ...
    expect(command).toContain("ENGINE_STATE_PATH=${OMNIRUSH_ENGINE_STATE_PATH:-$HOME/.local/share/opencode}")
    ...
    expect(command).toContain("/tmp/omnirush-data /tmp/omnirush-workspace")
    expect(command).toContain("CHECKPOINT_DIR=")
    expect(command).toContain("/persist/omnirush/checkpoints")
