...
import { AUTOMATION_MIN_CLAIM_WINDOW_MS } from "@omnirush/automations"
import {
...
test("desktop occurrences stay claimable through the recovery window with named missed causes", () => {
  ...
  // The recovery window is one policy in @omnirush/automations: the claim path
...
test("every dispatch path revalidates the owner's model access", () => {
  ...
  assert.match(executorSource, /currentAgentAuthority[\s\S]*getOmniRushWebRuntimeAccess\(input\.organizationId\)/)
