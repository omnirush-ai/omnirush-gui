...
Read-only admin analytics MCP server for the OmniRush.ai Den database. Ask OmniRush.ai
...
> connect "OmniRush.ai Admin Analytics" from Settings -> Connections -> MCP
...
GRANT SELECT ON omnirush_den.* TO 'den_readonly'@'%';
```

## Register in OmniRush.ai
...
```jsonc
{
  "mcp": {
    "den-admin": {
      ...
      "environment": { "DATABASE_URL": "mysql://den_readonly:...@host:3306/omnirush_den" },
      "enabled": true
    }
  }
}
...
pnpm --filter @omnirush-ee/den-admin-mcp test
```
