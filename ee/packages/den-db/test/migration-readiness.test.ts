...
describe("Den DB migration readiness wiring", () => {
  ...
  test("Helm migration defaults execute the precompiled dist runner", () => {
    const values = readRepoFile("packaging/helm/omnirush-ee/values.yaml")
  ...
  test("hosted Den API build includes den-db assets but start does not run migrations", () => {
    ...
    assert.match(denApiPackage, /"build:workspace-dependencies": "pnpm --filter '@omnirush-ee\/den-api\^\.\.\.' --if-present run build"/)
  ...
  test("hosted migration workflow remains the explicit PlanetScale migration owner", () => {
    ...
    assert.match(workflow, /run_with_ddl_retry pnpm --filter @omnirush-ee\/den-db db:migrate/)
  ...
  test("PR guardrails run readiness tests and smoke Den DB assets in the Den API image", () => {
    ...
    assert.match(checkWorkflow, /pnpm --filter @omnirush-ee\/den-db test/)
    ...
    assert.match(checkWorkflow, /"packaging\/helm\/omnirush-ee\/templates\/migration-job\.yaml"/)
