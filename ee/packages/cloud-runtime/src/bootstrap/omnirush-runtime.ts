 ...
 * layout, checkpoint hydrate/flush, and the supervised `omnirush-server`
...
export type OmniRushCheckpointConfig = {
...
export type OmniRushBootstrapConfig = OmniRushCheckpointConfig & {
...
export function checkpointDirectory(config: Pick<OmniRushCheckpointConfig, "dataMountPath">) {
  return `${config.dataMountPath}/checkpoints`
}

export function checkpointRestoreMarkerPath(config: Pick<OmniRushCheckpointConfig, "runtimeDataPath">) {
  return `${config.runtimeDataPath}/.omnirush-restore-marker`
}

function checkpointStateManifest(config: OmniRushCheckpointConfig) {
  return `${config.runtimeDataPath} ${config.runtimeWorkspacePath}`
}

function checkpointLastFlushMarkerPath(config: OmniRushCheckpointConfig) {
  return `${config.sidecarDir}/checkpoint.last-flush`
}

function checkpointEnvironmentScript(config: OmniRushCheckpointConfig) {
  ...
  return `ENGINE_STATE_PATH=\${OMNIRUSH_ENGINE_STATE_PATH:-\$HOME/.local/share/opencode}
OMNIRUSH_STATE_MANIFEST="${checkpointStateManifest(config)} \$ENGINE_STATE_PATH"
...
function checkpointFlushFunctions(config: OmniRushCheckpointConfig, input: { failOnError: boolean }) {
  const failureReturn = input.failOnError ? "1" : "0"
  return `checkpoint_changed() {
  if [ ! -e "$LAST_FLUSH_MARKER" ]; then
    return 0
  fi
  for state_path in $OMNIRUSH_STATE_MANIFEST; do
...
export function renderCheckpointFlushCommand(config: OmniRushCheckpointConfig) {
  return `set -u
...
export function renderCheckpointExistsCommand(config: Pick<OmniRushCheckpointConfig, "dataMountPath">) {
...
export function renderRestoreMarkerExistsCommand(config: Pick<OmniRushCheckpointConfig, "runtimeDataPath">) {
  return `test -s ${shellQuote(checkpointRestoreMarkerPath(config))}`
}

/** The supervised OmniRush.ai server process with checkpoint hydrate and flush, as a POSIX sh script. */
export function renderOmniRushBootstrapScript(config: OmniRushBootstrapConfig) {
  const verifyRuntimeStep = [
    `if ! command -v omnirush-server >/dev/null 2>&1; then echo 'omnirush-server binary missing from ${config.imageDescription}; ${config.rebuildHint}' >&2; exit 1; fi`,
  ...
  const omnirushServe = [
    ...
    " OMNIRUSH_WEB_BOOTSTRAP_TOKEN=",
    shellQuote("0"),
    " OMNIRUSH_EXTENSIONS_PLUGIN_DIR=",
    shellQuote("/opt/omnirush/opencode-plugins"),
...
hydrate_checkpoint() {
  ...
  if ! state_dirs_pristine; then
    echo "checkpoint hydrate skipped; local OmniRush.ai state is not pristine or was already restored"
    return 0
  fi
  ...
  echo "checkpoint hydrate failed for $latest_checkpoint; continuing with fresh OmniRush.ai state" >&2
...
checkpoint_pid=""
on_term() {
  echo "termination requested; flushing OmniRush.ai checkpoint"
  flush_checkpoint
...
while [ "$attempt" -lt 3 ]; do
  ...
  echo "omnirush-server failed (attempt $attempt, exit $status); ${config.rebuildHint} if this persists; retrying in 3s"
  sleep 3
done
flush_checkpoint
exit 1
...
/** `renderOmniRushBootstrapScript` wrapped for a provider that execs one command line. */
export function renderOmniRushBootstrapCommand(config: OmniRushBootstrapConfig) {
  return `sh -lc ${shellQuote(renderOmniRushBootstrapScript(config))}`
}
