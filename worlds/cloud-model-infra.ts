...
async function grantOmniRushWebAccess(admin: DenSession, organizationId: string): Promise<void> {
  // Cloud is entitled by OmniRush.ai Web access (paid subscription or the
  // platform-admin complimentary grant); there is no per-organization Cloud flag.
  const route = `/v1/admin/organizations/${organizationId}/omnirush-web-access`;
  const result = await denFetch(admin, route, {
    method: "PUT",
    headers: { authorization: `Bearer ${admin.token}` },
    body: JSON.stringify({ enabled: true, reason: "cloud-model-infra world: complimentary OmniRush.ai Web access for Cloud" }),
  });
 ...
 * organization holds complimentary OmniRush.ai Web access (the Cloud entitlement) and whose Daytona
 ...
 * source-first omnirush-server launched from `cloud-model-infra-worker.ts`,
...
export async function bootCloudModelInfra(
  ...
  await grantOmniRushWebAccess(admin, org.id);
