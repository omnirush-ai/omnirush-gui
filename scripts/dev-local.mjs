...
const composeProject = "omnirush-den-local"
...
const appPort = process.env.OMNIRUSH_APP_PORT?.trim() || process.env.PORT?.trim() || "5173"
const extraAppPorts = (process.env.OMNIRUSH_EXTRA_APP_PORTS?.trim() || "5174")
  .split(",")
  .map((value) => value.trim())
  .filter(Boolean)
const databaseUrl = process.env.DATABASE_URL?.trim() || "mysql://root:password@127.0.0.1:3306/omnirush_den"
...
async function main() {
  ...
  await run("bash", ["-c", "pnpm --filter @omnirush-ee/den-db build && pnpm --filter @omnirush-ee/den-db exec node --import tsx ./node_modules/drizzle-kit/bin.cjs push --config drizzle.config.ts --force"], {
    env: {
      ...process.env,
  ...
  turboChild = spawn(
    ...
    [
      ...
      "dev:local",
      "--output-logs=full",
        "--filter=@omnirush-ee/den-api",
        "--filter=@omnirush-ee/inference",
        "--filter=@omnirush-ee/den-web",
    ],
    {
      cwd: rootDir,
      stdio: "inherit",
