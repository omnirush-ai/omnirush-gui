param(
    [string]$Hostname = "poc.omnirush.test",
...
function Stop-ReproJobs {
    try {
        $jobs = Get-Job -Name "OmniRushTlsRepro-*" -ErrorAction SilentlyContinue
        foreach ($job in $jobs) {
...
function Invoke-Cleanup {
    ...
    if (-not $Quiet) {
        Write-Step "Cleaning OmniRush.ai TLS repro artifacts..."
    }
...
function Start-ReproListenerJob {
    ...
    $jobName = "OmniRushTlsRepro-{0}" -f $Port
    ...
    $job = Start-Job -Name $jobName -ArgumentList $Port, $Label -ScriptBlock {
        ...
        try {
            while ($true) {
                ...
                $context.Response.Headers.Add("X-OmniRush-TLS-Repro", $Label)
...
Write-Step "OmniRush.ai TLS repro setup"
...
$rootSubject = "CN=OmniRush.ai TLS Repro Root CA, O=$Marker"
$healthyIntermediateSubject = "CN=OmniRush.ai TLS Repro Healthy Intermediate CA, O=$Marker"
$brokenIntermediateSubject = "CN=OmniRush.ai TLS Repro Broken Intermediate CA, O=$Marker"
...
Write-Step ("   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\omnirush-doctor.ps1 -WebUrl {0} -ApiUrl {1} -ExpectedIssuerMatch `"OmniRush.ai TLS Repro`"" -f $healthyUrl, $brokenUrl)
...
Write-Step "   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-omnirush-tls-repro.ps1 -Cleanup"
