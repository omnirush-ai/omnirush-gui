...
SNAPSHOT_NAME="${1:-${DAYTONA_SNAPSHOT_NAME:-omnirush-runtime}}"
...
LOCAL_IMAGE_TAG="${DAYTONA_LOCAL_IMAGE_TAG:-omnirush-daytona-snapshot:${SNAPSHOT_NAME//[^a-zA-Z0-9_.-]/-}}"

OMNIRUSH_SERVER_VERSION="${OMNIRUSH_SERVER_VERSION:-$(node -e 'const fs=require("fs"); const pkg=JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(pkg.version));' "$ROOT_DIR/apps/server/package.json")}"
...
echo "- omnirush-server@$OMNIRUSH_SERVER_VERSION" >&2
...
docker buildx build \
  ...
  -t "$LOCAL_IMAGE_TAG" \
  -f "$DOCKERFILE" \
  --build-arg "OMNIRUSH_SERVER_VERSION=$OMNIRUSH_SERVER_VERSION" \
