#!/usr/bin/env bash
#
# omnirush-debug.sh — one-stop observability + lifecycle control for the
# OmniRush.ai dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan omnirush/opencode processes (ppid == launchd)
...
#   wait-healthy    block until omnirush-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Dev app: pnpm --filter @omnirush/desktop dev:electron launches the Electron
...
#   4. opencode sidecars and omnirush-server helpers
#
...
DEV_LOG_FILE="${OMNIRUSH_DEV_LOG_FILE:-$HOME/.omnirush/debug/omnirush-dev.log}"
PNPM_DEV_LOG="${OMNIRUSH_PNPM_DEV_LOG:-/tmp/omnirush-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${OMNIRUSH_PNPM_DEV_PID:-/tmp/omnirush-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${OMNIRUSH_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${OMNIRUSH_ELECTRON_REMOTE_DEBUG_PORT:-9823}"

# ---------------------------------------------------------------------------
# Helpers

log() { printf '[omnirush-debug] %s\n' "$*"; }
...
discover_omnirush_server_port() {
  ps -Ao command \
    | grep -E "omnirush-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /ai\.omnirush\.desktop/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.OMNIRUSH_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|apps\/desktop\/resources\/sidecars\/opencode( |\/)|omnirush-server|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== omnirush-server ==="
  local port
  port=$(discover_omnirush_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev omnirush-server running)"
  else
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /omnirush-server|opencode( |\/)/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with OMNIRUSH_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /omnirush-server|opencode( |\/)/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(OMNIRUSH_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_omnirush_server_port)
  if [[ -n "$port" ]]; then
    echo "  omnirush-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no omnirush-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/omnirush-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set OMNIRUSH_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "omnirush-server"
...
start() {
  ...
  env OMNIRUSH_DEV_LOG_FILE="$DEV_LOG_FILE" \
    nohup pnpm --filter @omnirush/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
  pid=$!
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "omnirush-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "omnirush-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the omnirush-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/ai.omnirush.desktop.dev"
