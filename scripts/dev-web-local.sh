...
PROJECT_NAME="omnirush-web-local"
...
: "${DATABASE_URL:=mysql://root:password@127.0.0.1:3306/omnirush_den}"
...
pnpm --filter @omnirush-ee/den-db db:push <<'EOF'
y
EOF
...
(
  ...
  env \
    ...
    NEXT_PUBLIC_OMNIRUSH_AUTH_CALLBACK_URL="$DEN_WEB_ORIGIN" \
