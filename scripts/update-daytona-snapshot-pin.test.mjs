...
const snapshotName = "omnirush-0.18.10"
const expectedUrl =
...
test("an already-correct pin makes zero write requests and reports unchanged", async () => {
  ...
  assert.deepEqual(messages, [
    "unchanged: DAYTONA_SNAPSHOT is already omnirush-0.18.10",
  ])
})
...
test("a different pin makes exactly one documented PUT and reports the transition", async () => {
  ...
  assert.equal(writes[0].options.body, '{"value":"omnirush-0.18.10"}')
  assert.equal(writes[0].options.headers["Content-Type"], "application/json")
  assert.deepEqual(messages, [
    "updated: DAYTONA_SNAPSHOT omnirush-0.18.9 -> omnirush-0.18.10",
  ])
})
...
test("compare mode reports agreement without writing", async () => {
  ...
  assert.deepEqual(messages, ["agreement: DAYTONA_SNAPSHOT=omnirush-0.18.10"])
})
...
test("compare mode fails with a clear diff when drift is detected", async () => {
  await assert.rejects(
    ...
    /expected DAYTONA_SNAPSHOT=omnirush-0\.18\.10, found DAYTONA_SNAPSHOT=omnirush-0\.18\.9/,
  )
})
