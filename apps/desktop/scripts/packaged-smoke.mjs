...
const output = resolve(process.env.OMNIRUSH_PACKAGED_SMOKE_DIR || join(tmpdir(), `omnirush-packaged-smoke-${process.pid}`));
...
try {
  ...
  const binary = join(output, "linux-unpacked/omnirush");
  ...
  run("desktop-boot", "xvfb-run", ["-a", "pnpm", "evals:e2e", "app-smoke", "--local"], 90_000, {
    OMNIRUSH_EVAL_ELECTRON_BINARY: binary,
    OMNIRUSH_EVAL_ELECTRON_RESOURCES_PREPARED: "1",
    OMNIRUSH_EVAL_ENGINE: "v1",
    OMNIRUSH_EVAL_SURFACES_DIR: join(output, "profiles"),
