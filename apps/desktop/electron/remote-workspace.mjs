...
export function selectOmniRushWorkspaceForConnection(list, directory) {
...
export function omnirushWorkspaceDisplayName(workspace) {
  return (
    trim(workspace?.displayName) ||
    trim(workspace?.omnirushWorkspaceName) ||
    trim(workspace?.name) ||
    trim(workspace?.id) ||
    null
  );
