...
export class WebContentsView {
  constructor(options) {
    ...
    const requestHook = options?.webPreferences?.partition === "persist:omnirush-browser" ? requestHooks.at(-1) : null;
    let attached = false;
...
function createPanel(checkPolicy = async (_request) => {}, remoteDebugPort = 0) {
  ...
  const approve = (allowed = true, tabId = invoke("omnirush:browser:state").activeTabId) => {
    const tab = invoke("omnirush:browser:state").tabs.find((tab) => tab.id === tabId);
    assert.ok(tab?.browserApproval, "the tab has a pending approval");
    return invoke("omnirush:browser:approve", tabId, tab.browserApproval.id, allowed);
  };
  async function openLinkMenu(payload = LINK) {
    invoke("omnirush:browser:linkContextMenu", payload);
    await flush();
    ...
    emit("omnirush:menu-overlay:ready", { sender: view.webContents });
    await flush();
    const request = view.webContents.sent.findLast((entry) => entry.channel === "omnirush:menu-overlay:show")?.payload;
    assert.ok(request, "the ready overlay receives its menu");
    const choose = (itemId) => emit("omnirush:menu-overlay:choose", { sender: view.webContents }, { requestId: request.id, itemId });
...
test("browser manager construction before app readiness defers session hooks until the first tab", async (t) => {
  ...
  controls.ready = true;
  invoke("omnirush:browser:createTab", "about:blank", "A");
  invoke("omnirush:browser:createTab", "about:blank", "B");
  await flush();
  assert.equal(browserSession.listenerCount("will-download"), 1, "download tracking is installed once");
  invoke("omnirush:browser:destroy");
});
...
test("showing the panel sizes the active tab and resets viewport emulation left on it", async () => {
  ...
  invoke("omnirush:browser:createTab", "https://example.com");
  assert.equal(onScreen(), null, "a tab created while the panel is hidden stays off screen");
  await flush();

  invoke("omnirush:browser:show", PANEL_BOUNDS);
  await flush();
...
test("selecting a tab from the tab strip resets that tab only", async () => {
  const { invoke, onScreen, commands } = createPanel();
  invoke("omnirush:browser:show", PANEL_BOUNDS);
  const first = invoke("omnirush:browser:createTab", "https://one.example");
  const firstView = onScreen();
  invoke("omnirush:browser:createTab", "https://two.example");
  ...
  invoke("omnirush:browser:selectTab", first.tabId);
  await flush();
...
test("agent navigation that brings a background tab on screen leaves its viewport emulation alone", async () => {
  const { invoke, onScreen, commands } = createPanel();
  invoke("omnirush:browser:show", PANEL_BOUNDS);
  invoke("omnirush:browser:createTab", "https://one.example");
  const firstView = onScreen();
  invoke("omnirush:browser:createTab", "https://two.example");
...
test("a tab opened for a background conversation loads silently and leaves the visible conversation's tab on screen", async () => {
  ...
  invoke("omnirush:browser:show", PANEL_BOUNDS, "A");
  invoke("omnirush:browser:createTab", "https://a.example", "A");
  const visibleView = onScreen();
  await flush();
  commands(visibleView).length = 0;

  const { tabId } = invoke("omnirush:browser:createTab", "https://b.example", "B");
  await flush();

  const state = invoke("omnirush:browser:state");
  ...
  assert.equal(messages("omnirush:browser:panel-opened").at(-1).tab.id, tabId, "the explicit open selects B's page only in B's panel");
  assert.equal(messages("omnirush:browser:panel-opened").at(-1).ownerSessionId, "B");
  ...
  invoke("omnirush:browser:hide");
  assert.deepEqual(children, []);
  assert.equal(onScreen(), null);
  assert.ok(invoke("omnirush:browser:state").nativeViews.every((view) => !view.aboveApp));
});
...
test("navigating a background conversation's tab reports its owner instead of taking the screen", async () => {
  ...
  const opens = messages("omnirush:browser:panel-opened");
...
test("switching to the background conversation swaps its tab on screen and restores a normal viewport", async () => {
  ...
  invoke("omnirush:browser:createTab", "https://b.example", "B");
  const bView = views().find((view) => view !== aView);
  await flush();
  commands(aView).length = 0;
  commands(bView).length = 0;

  invoke("omnirush:browser:setVisibleSession", "B");
  await flush();
...
test("closing a conversation's last tab tells only that conversation its panel is empty", async () => {
  ...
  invoke("omnirush:browser:closeTab", tabId);

  assert.equal(onScreen(), aView, "A keeps browsing");
  assert.deepEqual(messages("omnirush:browser:panel-closed"), [{ ownerSessionId: "B" }]);
  assert.deepEqual(invoke("omnirush:browser:state").tabs.map((tab) => tab.ownerSessionId), ["A"]);
});
...
test("capacity refuses allocation without replacing existing tabs and closing frees a slot", async () => {
  ...
  const limit = invoke("omnirush:browser:state").tabLimit;
  assert.equal(limit, 12);
  for (let i = 0; i < limit; i++) invoke("omnirush:browser:createTab", "about:blank", `owner-${i}`);
  const before = invoke("omnirush:browser:state");
  assert.throws(() => invoke("omnirush:browser:createTab", "about:blank", "overflow"), /12 browser tabs open.*Close.*try again/);
  await assert.rejects(invoke("omnirush:browser:openUrl", "https://example.com", "builtin", { sessionId: "overflow" }), /12 browser tabs open/);
  assert.equal(views().length, limit, "rejection allocates no native view");
  assert.deepEqual(invoke("omnirush:browser:state").tabs, before.tabs);
  invoke("omnirush:browser:closeTab", before.tabs[0].id);
  assert.equal(views()[0].webContents.isDestroyed(), true);
  invoke("omnirush:browser:createTab", "about:blank", "retry");
  assert.equal(invoke("omnirush:browser:state").tabs.length, limit);
});
...
test("owner cleanup is exact and idempotent and releases only an empty background host", async () => {
  ...
  invoke("omnirush:browser:show", PANEL_BOUNDS, "A");
  invoke("omnirush:browser:createTab", "about:blank", "A");
  invoke("omnirush:browser:createTab", "about:blank", null);
  const b = invoke("omnirush:browser:createTab", "about:blank", "B");
  const c = invoke("omnirush:browser:createTab", "about:blank", "C");
  await flush();
  for (const invalid of [undefined, null, "", "   ", 1]) assert.deepEqual(invoke("omnirush:browser:closeSessionTabs", invalid), []);
  assert.deepEqual(invoke("omnirush:browser:closeSessionTabs", "B"), [b.tabId]);
  assert.deepEqual(invoke("omnirush:browser:closeSessionTabs", "B"), []);
  assert.equal(views()[2].webContents.isDestroyed(), true);
  assert.equal(invoke("omnirush:browser:state").backgroundWindowCount, 1, "C still uses the hidden host");
  assert.deepEqual(invoke("omnirush:browser:closeSessionTabs", "C"), [c.tabId]);
  assert.equal(invoke("omnirush:browser:state").backgroundWindowCount, 0);
  assert.deepEqual(invoke("omnirush:browser:state").tabs.map(tab => tab.ownerSessionId), ["A", null]);
  assert.ok(views().slice(0, 2).every(view => !view.webContents.isDestroyed()));
  assert.deepEqual(messages("omnirush:browser:panel-closed"), [{ ownerSessionId: "B" }, { ownerSessionId: "C" }]);
  invoke("omnirush:browser:closeAllTabs");
...
test("external target destruction releases owner state and the empty hidden host", async () => {
  ...
  assert.deepEqual(invoke("omnirush:browser:state").tabs, []);
  assert.equal(invoke("omnirush:browser:state").backgroundWindowCount, 0);
});
...
test("failed navigation rolls back its allocation while another owner's page survives", async (t) => {
  ...
  const opening = invoke("omnirush:browser:openUrl", "http://127.0.0.1:1", "builtin", { sessionId: "B" });
  await flush();
  invoke("omnirush:browser:setVisibleSession", "B");
  approve();
  await assert.rejects(opening, { code: "browser_operation_failed" });
  invoke("omnirush:browser:setVisibleSession", "A");
  assert.deepEqual(invoke("omnirush:browser:state").tabs, before.tabs);
  assert.equal(invoke("omnirush:browser:state").backgroundWindowCount, 0);
...
test("tabs created without a conversation stay shared and behave as before", async () => {
  const { invoke, onScreen, messages } = createPanel();
  invoke("omnirush:browser:show", PANEL_BOUNDS);
  invoke("omnirush:browser:createTab", "https://shared.example");
  await flush();
  ...
  invoke("omnirush:browser:closeAllTabs");
  assert.deepEqual(messages("omnirush:browser:panel-closed"), [{ ownerSessionId: null }]);
});

test("suspension requires explicit confirmation with Cancel as both defaults and never falls back to the active tab", async () => {
  const { invoke, views } = createPanel();
  const { tabId } = invoke("omnirush:browser:createTab", "https://example.com", "A");
  await flush();
  const before = invoke("omnirush:browser:state").tabs;
  ...
  assert.equal(await invoke("omnirush:browser:suspendTab", tabId), null);
  for (const id of [undefined, null, "", "missing"]) await assert.rejects(invoke("omnirush:browser:suspendTab", id), /Unknown/);
  assert.deepEqual(invoke("omnirush:browser:state").tabs, before);
  assert.equal(views()[0].webContents.isDestroyed(), false);
...
test("confirmed suspension frees native resources but retains identity and owner until an explicit selection reloads", async () => {
  const { invoke, views, messages } = createPanel();
  const { tabId } = invoke("omnirush:browser:createTab", "https://example.com/form", "B");
  controls.confirm = async () => 1;
  for (let cycle = 0; cycle < 4; cycle++) {
    await flush();
    const previous = views().at(-1);
    assert.equal(await invoke("omnirush:browser:suspendTab", tabId), tabId);
    assert.equal(previous.webContents.isDestroyed(), true);
    invoke("omnirush:browser:setVisibleSession", "A");
    invoke("omnirush:browser:show", PANEL_BOUNDS, "B");
    invoke("omnirush:browser:bounds", PANEL_BOUNDS);
    const state = invoke("omnirush:browser:state");
    ...
    assert.deepEqual(messages("omnirush:browser:panel-closed"), []);
    assert.equal(await invoke("omnirush:browser:selectTab", tabId), tabId);
    ...
    assert.equal(invoke("omnirush:browser:state").tabs[0].automationProtected, false);
  }
  invoke("omnirush:browser:closeSessionTabs", "B");
  assert.deepEqual(invoke("omnirush:browser:state").tabs, []);
});
...
test("automation is protected before navigation consent and returns its native target only after first-document background emulation", async (t) => {
  ...
  let returned = false;
  const opening = invoke("omnirush:browser:openUrl", "https://example.com", "builtin", { sessionId: "B" });
  void opening.then(() => { returned = true; });
  await flush();
  const tab = invoke("omnirush:browser:state").tabs[0];
  ...
  await assert.rejects(invoke("omnirush:browser:suspendTab", tab.id), /protected or busy/);
  assert.throws(() => invoke("omnirush:browser:releaseTab", tab.id, "B"), /busy/);
  ...
  assert.equal((await invoke("omnirush:browser:restoreTab", tab.id, "B")).target_id, handle.target_id);
...
test("restore and release enforce exact ownership and protection lasts until explicit release", async (t) => {
  ...
  for (const owner of [undefined, null, "", "A"]) {
    await assert.rejects(invoke("omnirush:browser:restoreTab", tabId, owner), /owner mismatch/);
    assert.throws(() => invoke("omnirush:browser:releaseTab", tabId, owner), /owner mismatch/);
  }
  await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /protected/);
  assert.deepEqual(invoke("omnirush:browser:releaseTab", tabId, "B"), { tabId, released: true });
  assert.equal(views()[0].webContents.isDestroyed(), false, "release is not a close or navigation");
  await invoke("omnirush:browser:suspendTab", tabId);
  const suspended = invoke("omnirush:browser:state").tabs;
  await assert.rejects(invoke("omnirush:browser:restoreTab", tabId, "A"), /owner mismatch/);
  assert.equal(views().length, 1, "ownership is checked before allocating a native page");
  assert.deepEqual(invoke("omnirush:browser:state").tabs, suspended);
  const restored = await invoke("omnirush:browser:restoreTab", tabId, "B");
  ...
  assert.deepEqual(invoke("omnirush:browser:state").tabs, [], "ordinary CDP close still deletes the logical tab");
});
...
test("confirmation rechecks the captured page for active work, loading, downloads, and media", async (t) => {
  ...
  const { tabId } = invoke("omnirush:browser:createTab", "https://example.com", "B");
  await flush();
  const contents = views()[0].webContents;
  for (const field of ["loading", "audible"]) {
    controls.confirm = async () => { contents[field] = true; return 1; };
    await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /loading, downloading, or playing/);
    contents[field] = false;
  }
  controls.confirm = async () => { contents.emit("media-started-playing"); return 1; };
  await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /playing media/);
  ...
  await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /downloading/);
  download.emit("done");
  controls.confirm = async () => { await invoke("omnirush:browser:restoreTab", tabId, "B"); return 1; };
  await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /protected/);
  invoke("omnirush:browser:releaseTab", tabId, "B");
  const other = invoke("omnirush:browser:createTab", "about:blank", "B");
  controls.confirm = async () => { invoke("omnirush:browser:closeTab", tabId); return 1; };
  await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /Unknown/);
  assert.deepEqual(invoke("omnirush:browser:state").tabs.map(tab => tab.id), [other.tabId]);
});
...
test("beforeunload veto leaves the live document intact and pending close retains capacity even after timeout", async (t) => {
  ...
  await assert.rejects(invoke("omnirush:browser:suspendTab", tabId), /page prevented/);
  assert.equal(contents.isDestroyed(), false);
  assert.equal(invoke("omnirush:browser:state").tabs[0].status, "ready");
  for (let i = 1; i < 12; i++) invoke("omnirush:browser:createTab", "about:blank", "A");
  await invoke("omnirush:browser:selectTab", tabId);
  contents.closeMode = "pending";
  t.mock.timers.enable({ apis: ["setTimeout"] });
  const pending = invoke("omnirush:browser:suspendTab", tabId);
  const rejected = assert.rejects(pending, /still waiting to close/);
  await flush();
  assert.equal(invoke("omnirush:browser:state").tabs[0].status, "suspending");
  await assert.rejects(invoke("omnirush:browser:restoreTab", tabId, "A"), /busy/);
  await assert.rejects(invoke("omnirush:browser:selectTab", tabId), /suspending/);
  assert.throws(() => invoke("omnirush:browser:reload"), /suspending/);
  t.mock.timers.tick(2501);
  await rejected;
  assert.throws(() => invoke("omnirush:browser:createTab", "about:blank"), /12 browser tabs/);
  assert.equal(invoke("omnirush:browser:state").nativeViews.length, 12);
  contents.close();
  assert.equal(invoke("omnirush:browser:state").tabs[0].status, "suspended");
  invoke("omnirush:browser:createTab", "about:blank");
  assert.equal(invoke("omnirush:browser:state").tabs.length, 13);
});
...
test("failed and capacity-blocked restoration preserve saved metadata and a retry returns the same logical tab", async (t) => {
  ...
  controls.confirm = async () => 1;
  await invoke("omnirush:browser:suspendTab", tabId);
  const saved = invoke("omnirush:browser:state").tabs[0];
  controls.beforeLoad = async () => { throw new Error("Navigation failed"); };
  await assert.rejects(invoke("omnirush:browser:restoreTab", tabId, "B"), /Navigation failed/);
  assert.deepEqual(invoke("omnirush:browser:state").tabs, [saved]);
  assert.ok(views().every(view => view.webContents.isDestroyed()));
  assert.equal(invoke("omnirush:browser:state").backgroundWindowCount, 0);
  controls.beforeLoad = async () => {};
  controls.beforeCommand = async (method) => { if (method === "Target.getTargetInfo") throw new Error("Target failed"); };
  await assert.rejects(invoke("omnirush:browser:restoreTab", tabId, "B"), /Target failed/);
  assert.deepEqual(invoke("omnirush:browser:state").tabs, [saved]);
  controls.beforeCommand = async () => {};
  for (let i = 0; i < 12; i++) invoke("omnirush:browser:createTab", "about:blank", "A");
  await assert.rejects(invoke("omnirush:browser:restoreTab", tabId, "B"), /12 browser tabs/);
  assert.deepEqual(invoke("omnirush:browser:state").tabs[0], saved);
  invoke("omnirush:browser:closeSessionTabs", "A");
  assert.equal((await invoke("omnirush:browser:restoreTab", tabId, "B")).tab_id, tabId);
});
...
test("deletion cancels pending suspension and restoration without resurrecting saved tabs", async (t) => {
  ...
  const suspending = invoke("omnirush:browser:suspendTab", tabId);
  ...
  const saved = invoke("omnirush:browser:createTab", "https://example.com", "B");
  await flush();
  await invoke("omnirush:browser:suspendTab", saved.tabId);
  const loading = gate();
  controls.beforeLoad = () => loading.promise;
  const restoring = invoke("omnirush:browser:restoreTab", saved.tabId, "B");
  const cancelled = assert.rejects(restoring, /destroyed|closed/);
  await assert.rejects(invoke("omnirush:browser:restoreTab", saved.tabId, "B"), /busy/);
  assert.equal(invoke("omnirush:browser:state").nativeViews.length, 1);
  invoke("omnirush:browser:closeAllTabs");
  loading.finish();
  await cancelled;
  assert.deepEqual(invoke("omnirush:browser:state").tabs, []);
  ...
  for (const channel of ["closeTab", "closeSessionTabs", "closeAllTabs", "destroy"]) {
    const next = invoke("omnirush:browser:createTab", "about:blank", "B");
    await flush();
    await invoke("omnirush:browser:suspendTab", next.tabId);
    invoke(`omnirush:browser:${channel}`, channel === "closeSessionTabs" ? "B" : next.tabId);
    assert.deepEqual(invoke("omnirush:browser:state").tabs, []);
    await assert.rejects(invoke("omnirush:browser:selectTab", next.tabId), /Unknown/);
  }
});
...
test("external policy denial prevents catalog and default launches without a built-in fallback", async () => {
  for (const itemId of ["browser:firefox", "open-external"]) {
    ...
    assert.deepEqual(invoke("omnirush:browser:state").tabs, [], itemId);
  }
});
...
test("link menus reject untrusted senders, subframes, and non-HTTP payloads", async () => {
  ...
  emit("omnirush:browser:linkContextMenu", { sender: {}, senderFrame: mainContents.mainFrame }, LINK);
  emit("omnirush:browser:linkContextMenu", { sender: mainContents, senderFrame: {} }, LINK);
  for (const url of ["javascript:alert(1)", "file:///tmp/link.html", "data:text/html,link", "omnirush://settings"]) {
    invoke("omnirush:browser:linkContextMenu", { ...LINK, url });
  }
  await flush();
...
test("the built-in choice retains the captured owner when focus changes before policy completes", async () => {
  ...
  assert.deepEqual(invoke("omnirush:browser:state").tabs, [], "navigation waits for policy");
  invoke("omnirush:browser:setVisibleSession", "C");
...
test("forged menu requests, senders, and action IDs are ignored without dismissing the valid menu", async () => {
  ...
  invoke("omnirush:browser:createTab", "https://existing.example");
  const { view, request, choose } = await openLinkMenu();
  const tabs = invoke("omnirush:browser:state").tabs;
  policies.length = 0;
  emit("omnirush:menu-overlay:choose", { sender: view.webContents }, { requestId: "forged", itemId: "browser:firefox" });
  invoke("omnirush:menu-overlay:choose", { requestId: request.id, itemId: "browser:firefox" });
  ...
  assert.deepEqual(policies, []);
  assert.deepEqual(effects, []);
  assert.deepEqual(invoke("omnirush:browser:state").tabs, tabs);
...
test("automation open waits for its owner's consent and then reuses only that owned task tab", async () => {
  ...
  const opening = invoke("omnirush:browser:openUrl", "https://b.example/", "builtin", { sessionId: "B" });
  await flush();
  ...
  assert.deepEqual(opened, {
    ...
    tab_id: invoke("omnirush:browser:state").activeTabIdByOwner.B, url: "https://b.example/", owner_session_id: "B", visible: true,
  });
  invoke("omnirush:browser:setVisibleSession", "A");
  assert.equal(onScreen(), foreground);
  ...
  assert.deepEqual(await invoke("omnirush:browser:openUrl", opened.url, "builtin", { sessionId: "B" }), { ...opened, visible: false });
...
test("automation open rejects paused and disabled control before creating or navigating a tab", async () => {
  ...
  invoke("omnirush:browser:show", PANEL_BOUNDS, "A");
  const { tabId } = invoke("omnirush:browser:createTab", "https://a.example/", "A");
  await flush();
  invoke("omnirush:browser:taskControl", tabId, "pause");
  const before = invoke("omnirush:browser:state");
  const foreground = onScreen();
  const loads = [...foreground.webContents.loads];
  await assert.rejects(invoke("omnirush:browser:openUrl", "https://a.example/new", "builtin", { sessionId: "A" }), { code: "paused" });
  invoke("omnirush:browser:setControlEnabled", false);
  await assert.rejects(invoke("omnirush:browser:openUrl", "https://a.example/new", "builtin", { sessionId: "A" }), { code: "browser_disabled" });
  assert.equal(views().length, 1);
  assert.equal(onScreen(), foreground);
  assert.deepEqual(invoke("omnirush:browser:state").tabs.map((tab) => tab.id), before.tabs.map((tab) => tab.id));
  assert.deepEqual(foreground.webContents.loads, loads);
  invoke("omnirush:browser:navigate", "https://a.example/person");
  await flush();
...
test("takeover cancels automation opening during policy and during navigation", async (t) => {
  ...
  const { tabId } = invoke("omnirush:browser:createTab", "https://a.example/", "A");
  await flush();
  const beforeDispatch = invoke("omnirush:browser:openUrl", "https://a.example/policy", "builtin", { sessionId: "A" });
  invoke("omnirush:browser:taskControl", tabId, "pause");
  await assert.rejects(beforeDispatch, { code: "paused" });
  invoke("omnirush:browser:taskControl", tabId, "resume");
  releasePolicy();
  await flush();
  ...
  const inFlight = invoke("omnirush:browser:openUrl", "https://a.example/slow", "builtin", { sessionId: "A" });
  await flush();
  ...
  assert.ok(invoke("omnirush:browser:state").tabs.every((tab) => tab.browserTask.status === "paused"));
});
...
test("a first task open stays blank through asynchronous panel mounting and localhost needs explicit consent", async () => {
  ...
  await flush();
  const pending = invoke("omnirush:browser:state").tabs[0];
  ...
  invoke("omnirush:browser:show", PANEL_BOUNDS, "A");
  assert.equal(invoke("omnirush:browser:state").tabs[0].browserApproval.id, pending.browserApproval.id, "mounting preserves the pending review");
  approve();
  const result = await opening;
  assert.equal(result.ok, true);
  assert.equal(invoke("omnirush:browser:state").tabs[0].browserTask.status, "idle");
  ...
  await flush();
  assert.equal(invoke("omnirush:browser:state").tabs[0].browserApproval.title, "Allow website access?", "navigation did not grant reading or action access");
  approve(false);
...
test("local development auto-approval opens browser tasks without a permission card", async (t) => {
  ...
  const previousAutoApprove = process.env.OMNIRUSH_BROWSER_AUTO_APPROVE;
  t.after(() => {
    if (previousDevMode === undefined) delete process.env.OMNIRUSH_DEV_MODE;
    else process.env.OMNIRUSH_DEV_MODE = previousDevMode;
    if (previousAutoApprove === undefined) delete process.env.OMNIRUSH_BROWSER_AUTO_APPROVE;
    else process.env.OMNIRUSH_BROWSER_AUTO_APPROVE = previousAutoApprove;
  });
  ...
  assert.equal(invoke("omnirush:browser:state").tabs[0].browserApproval, null);
});
...
test("denied, canceled, closed and background task opens never load and release their blank tabs", async () => {
  for (const end of ["deny", "cancel", "close", "background"]) {
    ...
    if (end === "close") invoke("omnirush:browser:closeTab", tab.id);
    ...
    if (end === "background") {
      ...
      assert.equal(invoke("omnirush:browser:state").backgroundWindowCount, 0, "pending consent needs no hidden native host");
      assert.equal(approve(true, tab.id), false, "another visible conversation cannot approve");
      assert.ok(invoke("omnirush:browser:state").tabs[0].browserApproval);
      controller.abort();
    }
    ...
    assert.deepEqual(invoke("omnirush:browser:state").tabs, [], end);
  }
});
...
test("the task timeout cancels the longer approval dialog and late acceptance cannot navigate", async (t) => {
  ...
  assert.equal(invoke("omnirush:browser:approve", tab.id, tab.browserApproval.id, true), false);
  assert.deepEqual(views()[0].webContents.loads, []);
  assert.deepEqual(views()[0].webContents.destinations, []);
  assert.deepEqual(invoke("omnirush:browser:state").tabs, []);
});
...
test("the request hook holds a cross-origin main-frame redirect before any target request", async (t) => {
  for (const outcome of ["deny", "allow", "cancel", "close", "background"]) {
    ...
    assert.match(invoke("omnirush:browser:state").tabs[0].browserApproval.message, /http:\/\/127.0.0.1:4173/);
    const tab = invoke("omnirush:browser:state").tabs[0];
    if (outcome === "allow" || outcome === "deny") approve(outcome === "allow");
    if (outcome === "cancel") controller.abort();
    if (outcome === "close") invoke("omnirush:browser:closeTab", tab.id);
...
test("takeover cancels pending navigation, permits manual browsing without grants, and requires fresh consent on resume", async () => {
  ...
  invoke("omnirush:browser:show", PANEL_BOUNDS, "A");
  const { tabId } = invoke("omnirush:browser:createTab", "about:blank", "A");
  await flush();
  const url = "http://localhost:4173/";
  const navigate = () => panel.browserTask({ sessionId: "A", operation: "navigate", args: { tabId, url } });
  const pending = navigate();
  await flush();
  const approvalId = invoke("omnirush:browser:state").tabs[0].browserApproval.id;
  invoke("omnirush:browser:taskControl", tabId, "pause");
  ...
  for (const channel of ["navigate", "back", "forward", "reload"]) {
    for (const event of [{ sender: contents, senderFrame: {} }, { sender: mainContents, senderFrame: {} }]) {
      assert.throws(() => emit(`omnirush:browser:${channel}`, event, url), /browser toolbar/);
  ...
  invoke("omnirush:browser:navigate", url);
  await flush();
  assert.deepEqual(views()[0].webContents.destinations, [url], "manual takeover navigation is still available");
  invoke("omnirush:browser:taskControl", tabId, "resume");
  assert.equal(invoke("omnirush:browser:approve", tabId, approvalId, true), false);
...
test("adopting a manual popup guards its existing opener without sharing the popup's grant", async () => {
  ...
  const openerTab = invoke("omnirush:browser:createTab", url, "A");
  await flush();
  ...
  const popupId = invoke("omnirush:browser:state").activeTabId;
  ...
  invoke("omnirush:browser:selectTab", openerTab.tabId);
  const sameOrigin = opener.request(url);
  await flush();
  assert.ok(invoke("omnirush:browser:state").tabs[0].browserApproval, "the popup's grant does not authorize its opener");
  approve(false);
...
test("parent observations preserve popup approval and grants, but deliberate lifecycle endings still revoke them", async () => {
  for (const ending of ["cancel", "takeover", "close"]) {
    ...
    await flush();
    const childId = invoke("omnirush:browser:state").activeTabId;
    const approvalId = invoke("omnirush:browser:state").tabs.at(-1).browserApproval.id;
    assert.equal((await observe()).ok, true);
    assert.equal(invoke("omnirush:browser:state").tabs.at(-1).browserApproval?.id, approvalId, "observing the parent preserves the pending popup review");
    assert.deepEqual(child.destinations, []);
    approve();
    ...
    await flush();
    const canceledApprovalId = invoke("omnirush:browser:state").tabs.at(-1).browserApproval.id;
    if (ending === "cancel") {
      /** @type {() => void} */
    ...
    if (ending === "takeover") invoke("omnirush:browser:taskControl", tabId, "pause");
    if (ending === "close") invoke("omnirush:browser:closeTab", tabId);
    assert.deepEqual(await canceledNavigation, { cancel: true }, ending);
    assert.equal(invoke("omnirush:browser:approve", childId, canceledApprovalId, true), false, ending);
