...
const roots: string[] = [];
const priorDataDir = process.env.OMNIRUSH_DATA_DIR;
...
async function createWorkspaceRoot() {
  return createTempRoot("omnirush-authorized-folders-");
}

async function startOmniRushServer(workspaceRoot: string, options?: { readOnly?: boolean }) {
...
beforeEach(async () => {
  const envRoot = await createTempRoot("omnirush-authorized-folders-env-");
  process.env.OMNIRUSH_DATA_DIR = join(envRoot, "data");
  process.env.OMNIRUSH_TOKEN_STORE = join(envRoot, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorDataDir === undefined) {
    delete process.env.OMNIRUSH_DATA_DIR;
  } else {
    process.env.OMNIRUSH_DATA_DIR = priorDataDir;
  }
...
describe("authorized folders routes", () => {
  ...
  test("lists visible folders and counts preserved hidden entries", async () => {
    ...
    const { base, config } = await startOmniRushServer(root);
  ...
  test("requires client auth, collaborator scope, and writable server", async () => {
    ...
    const { base } = await startOmniRushServer(root);
    ...
    const readOnly = await startOmniRushServer(readOnlyRoot, { readOnly: true });
