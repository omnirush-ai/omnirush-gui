...
function assertStrictBody(body: Record<string, unknown>, workspace: WorkspaceInfo): void {
  ...
  if (typeof body.name === "string" && body.name.trim() !== OMNIRUSH_CLOUD_MCP_NAME) {
    throw new ApiError(400, "invalid_mcp_name", "Only omnirush-cloud can be reconciled by this endpoint");
  }
}
...
export function registerCloudMcpRoutes(options: RegisterCloudMcpRoutesOptions): void {
  ...
  addRoute(routes, "POST", "/workspace/:id/mcp/omnirush-cloud/engine-refresh", "client", async (ctx) => {
    ...
    const result = await refreshOmniRushCloudMcpEngine({
      config,
      workspace,
      directory: resolveOpencodeDirectory(workspace),
  ...
  addRoute(routes, "POST", "/workspace/:id/mcp/omnirush-cloud/reconcile", "client", async (ctx) => {
    ...
    const health = await reconcileOmniRushCloudMcp({
      config,
      workspace,
      directory: resolveOpencodeDirectory(workspace),
      body,
