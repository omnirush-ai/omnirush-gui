...
all-folders policy on, any other folder too, see "All folders" below; with
the touched-files policy on, any other folder gets only the files the agent
touched there, see "Touched files" below):

- at session start: a **base** archive, including `.git/` and without anything
  git ignores (see "Gitignored content" below);
...
archiver.recordTouched(sessionId, path);                  // a path the session touched (touched-files mode)
archiver.forgetTouched(sessionId);                        // drop them (a child session)
...
(`unchanged` is also a touched-files session registered, or captured, with
no touched file yet: no base). This implementation adds five more:
...
- **Stopping a session.** A chain-ending 409, a 413 or a 422 drops that session's jobs and stops the session (one `warn` line; the code is `archive_marker_not_allowed` when the server says so, else `archive_request_invalid`). A 7-day-old job does the same. The next session starts over with a new base. The exception is `archive_parent_mismatch` on a final archive before the server accepted one (see "Final archives").
...
- **Touched files.** GET /archives/key also carries `policy.touched_files` (off unless it is the boolean `true`; absent on an older backend). Precedence per session root, in `captureBaseLocked`'s gate: a git repository or a folder inside one (`gitMarkerDetector`, `gitParentDetector`) gets the `.git` archive exactly as before; else, with `all_folders` on, the whole-folder `folder` archive; else, with `touched_files` on and the root passing every folder refusal above, a `touched` chain; else nothing. The same probe, TTL and kept answer serve both flags.
  ...
  - **Policy off.** With `touched_files` off or absent nothing is written for such a folder: `captureBase` is `not_archivable` after the probe, and no touched path is kept. A touched chain's captures pause (`not_archivable`) while it is off and catch up once it is on (the touched set is kept). The server refuses a `touched` create from a user whose policy is off with `422 archive_marker_not_allowed`: the drain drops the session's chain (its jobs, the session stopped, one `warn` line, no retry) and keeps the policy as off for `touched` until it is asked again (`POLICY_TTL_MS`), so no other chain starts meanwhile. The same holds for `folder`.
- **Gitignored content.** Nothing git ignores is archived: base, delta and final archives, whole-folder and touched-files alike (`ignore.ts`). It can be rebuilt (`node_modules/`, `dist/`, venvs, caches), and it made single projects many gigabytes. Git decides, with all its rules (every `.gitignore`, `.git/info/exclude`, `core.excludesFile`):
  ...
  - **Wire format unchanged.** The manifest's `excluded` object keeps its six counters (section 5.6); the number of ignored entries left out (an ignored directory counts once) is in `ScanResult.ignored` / `TouchedScanResult.ignored` and the `gitignored` field of the "archive queued" log line. The backend's section 5.1 still says gitignored content is included; see the PR for the doc change.
...
- `manifest.test.ts`: UTF-8 byte order; the credential filter; the exclusions (credential, FIFO, app state, `__omnirush__`, unreadable, non-UTF-8) with gitignored `node_modules/`, `.venv/` and `dist/` absent; gitignored content: a repository with a nested `.gitignore`, `.git/info/exclude`, a negation, an ignored file beside a tracked one, a force-added tracked file in an ignored folder (kept) and a nested repository with its own `.gitignore`; an ignored folder of 5,000 files pruned (not one of them `lstat`ed); a folder in no repository with nested `.gitignore` files and a negation; git missing (walkFallback rules); deltas when files become ignored, stop being ignored, or move into an ignored folder; the hash cache; a stopped scan reading nothing; delta for add, modify (content and mode), delete, rename, file to dir, dir to file and symlink retarget; unstable re-send; `.git` changes; manifest JSON, with a delta's `trigger` and a final archive's `reason`; the git block and its `path`; `git status` never rewriting `.git/index`.
- `touched.test.ts`: `scanTouchedFiles` skipping touched files git ignores (root and nested `.gitignore`, an ignored folder, a nested repository) and a delta deleting a file that became ignored; `scanTouchedFiles` with a binary, a PDF, an untouched file, a touched folder (never expanded), a credential file, `__omnirush__`, an app dir, a symlink inside (followed), links out of the root and a folder linked out of it (skipped; every path the scan looked at is inside the root), a missing file and a FIFO; the hash cache used and kept; plain relative paths only; `touchedChange` (added, modified, gone, unreadable kept); `TouchedPathStore` (paths in memory until tracked, the file read back by a new store, a torn line, forget, a session that is something else, clear).
...
- `upload.test.ts` (in-process fake API and S3):
  ...
  - an expired URL (403);
  - 428 and 503;
  - the key route's policy: only `all_folders: true` and `touched_files: true` turn them on, each on its own, and the key works either way;
  - `422 archive_marker_not_allowed` for a `touched` or `folder` create stops the session with that code; any other 422 is `archive_request_invalid`;
...
- `index.test.ts`:
  ...
  - touched files (`touched_files` on): only the touched files, byte for byte (a PDF read, a binary written), untouched files, credentials and a link out of the folder never, the base at the first capture with a touched file, a delta with the changed files and the deleted one, nothing when only an untouched file changed; a git folder archived as today with no touched paths kept; `all_folders` winning over `touched_files`; the policy off, absent or not a boolean: nothing uploaded or kept; the refused folders never archived; finals (the base from an idle final, `app_quit`, `app_start`) and a restart keeping the chain and its touched paths, a session without a base getting it at app start, a deleted session's paths gone; a restart whose key probe fails keeping a chain without a base and its paths, its base catching up with every path once the policy answers; the chain pausing while the policy is off; `422 archive_marker_not_allowed` dropping the chain once, with no retry, and no other touched chain starting until the policy is asked again;
  - final archives: none without a base or when unchanged; the equal-turn wire values (sequence, parent, turn), chained finals, `trigger` and `reason` in the manifest; a server without final archives (`strictTurns`): one refused create, the chain put back, the turn delta queued behind the final captured again, later turns uploaded, one log line; shutdown finals within the budget, a hanging one cut at it and captured at the next start; the app-start candidates (a final due, the most recently active session per folder; not deleted or older than 7 days); the 7-day window counted from the last base, prompt or turn (an unchanged one too), never extended by a final archive (app starts every 6 days with the folder edited: finals stop a week after the last turn), by a refused final's rewind, or in a record from before 1.1.0; a deleted session whose folder is gone.
