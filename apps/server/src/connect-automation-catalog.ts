...
export type OmniRushAutomationIndex = z.infer<typeof automationIndexSchema>;

const catalogCache = new Map<string, { expiresAt: number; value: Promise<OmniRushAutomationIndex | null> }>();

async function readIndex(cloud: Record<string, unknown>, fetcher: McpFetch): Promise<OmniRushAutomationIndex | null> {
  const text = await readMcpResourceText({
    config: cloud,
    uri: AUTOMATION_INDEX_URI,
    fetcher,
    clientName: "omnirush-server-automation-catalog",
  });
...
/**
 * Resolve the member's Automation index from the first working omnirush-cloud
 ...
 */
export async function readOmniRushAutomationCatalog(
  config: ServerConfig,
  fetcher: McpFetch = externalFetch,
): Promise<OmniRushAutomationIndex | null> {
  try {
...
export function resetOmniRushAutomationCatalogCacheForTests(): void {
  catalogCache.clear();
}

function scheduleText(schedule: OmniRushAutomationIndex["automations"][number]["schedule"]): string {
 ...
 */
export function renderOmniRushAutomationInstruction(index: OmniRushAutomationIndex | null): string {
  if (!index) return "";
  if (index.automations.length === 0) {
    return [
      "This member owns no Automations. If they ask what Automations they have, say there are none.",
      "If they describe recurring work, propose one with omnirush_execute id automation.propose.",
    ].join("\n");
  }
