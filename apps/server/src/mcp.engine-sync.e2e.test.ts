...
process.env.OMNIRUSH_MCP_SYNC_RETRY_DELAY_MS = "10";
...
async function createWorkspaceRoot() {
  const root = await mkdtemp(join(tmpdir(), "omnirush-mcp-engine-sync-"));
  roots.push(root);
  return root;
}
...
async function startOmniRushServer(
...
describe("runtime MCP engine sync", () => {
  test("reuses unchanged healthy clients but retries disconnected and changed configurations", async () => {
    ...
    process.env.OMNIRUSH_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    let status = "connected";
    try {
      const mock = startMockOpencode({ liveMcpStatusByName: () => ({ posthog: { status } }) });
      const omnirush = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${mock.server.port}`);
      await writeRuntimeOpencodeConfig(omnirush.config, "ws_1", (current) => ({ ...current, mcp: { posthog: POSTHOG_CONFIG } }));
      const posts = () => mock.requests.filter((entry) => entry.method === "POST" && entry.pathname === "/mcp");
      await syncAllWorkspacesRuntimeMcpToEngine(omnirush.config);
      ...
      await writeRuntimeOpencodeConfig(omnirush.config, "ws_1", (current) => ({ ...current, mcp: { posthog: changedConfig } }));
      ...
      refreshEngineMcpRegistrationFromLiveStatus(omnirush.config, omnirush.config.workspaces[0]!, "posthog", changedConfig, "connected");
      await syncAllWorkspacesRuntimeMcpToEngine(omnirush.config);
  ...
  test("keeps a bootstrapped client alive across repeated deferred syncs until its task finishes", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.OMNIRUSH_RUNTIME_DB;
    const previousDelay = process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS;
    process.env.OMNIRUSH_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS = "10";
    let busy = true;
    try {
      ...
      expect(inspectEngineMcpRegistration(omnirush.config, omnirush.config.workspaces[0]!, "posthog", POSTHOG_CONFIG)).not.toBe("connected");
      busy = false;
      ...
      if (previousDelay === undefined) delete process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS;
      else process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS = previousDelay;
    }
  });
  ...
  test("hot-adds a runtime MCP into the running engine when added", async () => {
    ...
    try {
      ...
      const response = await fetch(`${omnirush.base}/workspace/ws_1/mcp`, {
        method: "POST",
  ...
  test("hot-syncs an external engine without treating its response as trusted registration evidence", async () => {
    ...
    try {
      const mock = startMockOpencode();
      const omnirush = await startOmniRushServer(
        workspaceRoot,
  ...
  test("keeps accepted delivery separate from bounded normalized registration evidence", async () => {
    ...
    try {
      ...
      const inspectRegistration = (name: string, config: Record<string, unknown>) =>
        inspectEngineMcpRegistration(omnirush.config, omnirush.config.workspaces[0]!, name, config);
      ...
      const listResponse = await fetch(`${omnirush.base}/workspace/ws_1/mcp`, {
        headers: auth(omnirush.token),
      });
  ...
  test("records transport-failure provenance and performs one deferred re-sync", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.OMNIRUSH_RUNTIME_DB;
    const previousDeferredDelay = process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS;
    process.env.OMNIRUSH_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS = "50";
    let posthogPosts = 0;
    try {
      const mock = startMockOpencode({
        mcpResponseForName: (name) => {
      ...
      const omnirush = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${mock.server.port}`);
      const workspace = omnirush.config.workspaces[0]!;
      ...
      expect(response.status).toBe(200);
      expect(inspectEngineMcpRegistration(omnirush.config, workspace, "posthog", POSTHOG_CONFIG)).toBe("failed");
      expect(inspectEngineMcpRegistrationDetails(omnirush.config, workspace, "posthog", POSTHOG_CONFIG).source)
        .toBe("transport_failure");

      await waitForRegistration(
        () => inspectEngineMcpRegistration(omnirush.config, workspace, "posthog", POSTHOG_CONFIG),
        "connected",
      );
      ...
      if (previousDeferredDelay === undefined) delete process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS;
      else process.env.OMNIRUSH_MCP_SYNC_DEFERRED_DELAY_MS = previousDeferredDelay;
    }
  });
  ...
  test("serializes workspace MCP sync and coalesces concurrent requests into one latest-state trailing pass", async () => {
    ...
    try {
      ...
      const omnirush = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${mock.server.port}`);
      const writePosthogUrl = (url: string) => writeRuntimeOpencodeConfig(omnirush.config, "ws_1", (current) => ({
        ...current,
        mcp: { posthog: { ...POSTHOG_CONFIG, url } },
      }));

      await writePosthogUrl("https://first.example/mcp");
      const firstSync = syncAllWorkspacesRuntimeMcpToEngine(omnirush.config);
      await firstRegistrationReached;

      await writePosthogUrl("https://intermediate.example/mcp");
      const secondSync = syncAllWorkspacesRuntimeMcpToEngine(omnirush.config);
      await writePosthogUrl("https://latest.example/mcp");
      const thirdSync = syncAllWorkspacesRuntimeMcpToEngine(omnirush.config);
  ...
  test("does not overlap startup registration with explicit cloud reconciliation", async () => {
    ...
    try {
      const mock = startMockOpencode({
        liveMcpStatusByName: () => ({ "omnirush-cloud": { status: "connected" } }),
        mcpResponseForName: (name) => {
      ...
      const startupSync = syncAllWorkspacesRuntimeMcpToEngine(omnirush.config);
      await startupRegistrationReached;
      const explicitReconcile = fetch(`${omnirush.base}/workspace/ws_1/mcp/omnirush-cloud/reconcile`, {
        method: "POST",
        headers: auth(omnirush.token),
        body: JSON.stringify({
          config: {
      ...
      expect(registrationNames.filter((name) => name === "omnirush-cloud").length).toBeGreaterThanOrEqual(1);
      expect(maxRegistrationsInFlight).toBe(1);
    } finally {
      releaseStartupRegistration(Response.json({ posthog: { status: "connected" } }));
      if (previousDb === undefined) delete process.env.OMNIRUSH_RUNTIME_DB;
  ...
  test("scopes registration evidence to the concrete OmniRush.ai server instance", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.OMNIRUSH_RUNTIME_DB;
    process.env.OMNIRUSH_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    try {
      ...
      const serverA = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${engineA.server.port}`);
      const serverB = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${engineB.server.port}`);
  ...
  test("invalidates registration evidence when the engine endpoint changes", async () => {
    ...
    try {
      const engineA = startMockOpencode();
      const engineB = startMockOpencode();
      const omnirush = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${engineA.server.port}`);
      const workspace = omnirush.config.workspaces[0]!;
      ...
      expect(response.status).toBe(200);
      expect(inspectEngineMcpRegistration(omnirush.config, workspace, "posthog", POSTHOG_CONFIG)).toBe("connected");

      workspace.baseUrl = `http://127.0.0.1:${engineB.server.port}`;
      expect(inspectEngineMcpRegistration(omnirush.config, workspace, "posthog", POSTHOG_CONFIG))
        .toBe("not-recorded");
  ...
  test("invalidates registration evidence when a managed engine restarts at the same endpoint", async () => {
    ...
    try {
      ...
      const omnirush = await startOmniRushServer(workspaceRoot, baseUrl, {
  ...
  test("invalidates registration evidence on stop and requires a new server generation", async () => {
    ...
    try {
      const engine = startMockOpencode();
      const omnirush = await startOmniRushServer(workspaceRoot, `http://127.0.0.1:${engine.server.port}`);
      const workspace = omnirush.config.workspaces[0]!;
      ...
      await omnirush.server.stop(true);
      expect(inspectEngineMcpRegistration(omnirush.config, workspace, "posthog", POSTHOG_CONFIG))
        .toBe("not-recorded");

      const restarted = await startServer(omnirush.config) as Served;
  ...
  test("expires registration evidence after the bounded freshness window", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    const previousDb = process.env.OMNIRUSH_RUNTIME_DB;
    const previousMaxAge = process.env.OMNIRUSH_MCP_REGISTRATION_MAX_AGE_MS;
    process.env.OMNIRUSH_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OMNIRUSH_MCP_REGISTRATION_MAX_AGE_MS = "100";
    try {
      const engine = startMockOpencode({
      ...
      await Bun.sleep(150);
      expect(inspectEngineMcpRegistrationDetails(omnirush.config, workspace, "posthog", POSTHOG_CONFIG))
        .toMatchObject({ status: "not-recorded", errorSummary: null });
    } finally {
      if (previousDb === undefined) delete process.env.OMNIRUSH_RUNTIME_DB;
      else process.env.OMNIRUSH_RUNTIME_DB = previousDb;
      if (previousMaxAge === undefined) delete process.env.OMNIRUSH_MCP_REGISTRATION_MAX_AGE_MS;
      else process.env.OMNIRUSH_MCP_REGISTRATION_MAX_AGE_MS = previousMaxAge;
    }
  });
  ...
  test("invalidates registration evidence before an engine reload attempt", async () => {
    ...
    try {
      ...
      rejectDispose = true;
      const reload = await fetch(`${omnirush.base}/workspace/ws_1/engine/reload`, {
        method: "POST",
        headers: auth(omnirush.token),
      });
  ...
  test("cloud plugin install writes a remote MCP and hot-syncs it into the engine", async () => {
    ...
    try {
      ...
      const response = await fetch(`${omnirush.base}/workspace/ws_1/cloud-plugins`, {
        method: "POST",
        headers: auth(omnirush.token),
        body: JSON.stringify({
          marketplaceId: null,
      ...
      expect((await readRuntimeOpencodeConfig(omnirush.config, "ws_1")).mcp?.brief).toMatchObject({
        type: "remote",
  ...
  test("cloud plugin install warns for dropped MCP payloads while still installing skills", async () => {
    ...
    try {
      ...
      expect((await readRuntimeOpencodeConfig(omnirush.config, "ws_1")).mcp?.broken).toBeUndefined();
  ...
  test("re-registers runtime MCPs with the engine after a reload", async () => {
    ...
    try {
      ...
      const addResponse = await fetch(`${omnirush.base}/workspace/ws_1/mcp`, {
        method: "POST",
        headers: auth(omnirush.token),
        body: JSON.stringify({ name: "posthog", config: POSTHOG_CONFIG }),
      });
      ...
      const reloadResponse = await fetch(`${omnirush.base}/workspace/ws_1/engine/reload`, {
        method: "POST",
        headers: auth(omnirush.token),
      });
  ...
  test("pushes toggled enabled state to the engine", async () => {
    ...
    try {
      ...
      const toggleResponse = await fetch(`${omnirush.base}/workspace/ws_1/mcp/posthog/enabled`, {
        method: "POST",
        headers: auth(omnirush.token),
  ...
  test("disconnects a removed MCP from the engine", async () => {
    ...
    try {
      ...
      const removeResponse = await fetch(`${omnirush.base}/workspace/ws_1/mcp/posthog`, {
        method: "DELETE",
        headers: auth(omnirush.token),
      });
  ...
  test("startup sync pushes runtime MCPs for every workspace", async () => {
    ...
    const previousDb = process.env.OMNIRUSH_RUNTIME_DB;
    process.env.OMNIRUSH_RUNTIME_DB = join(rootA, "runtime.sqlite");
    try {
      const mock = startMockOpencode();
      ...
      await writeGlobalRuntimeOpencodeConfig(config, (current) => ({
        ...current,
        mcp: { ...current.mcp, "omnirush-cloud": POSTHOG_CONFIG },
      }));
      ...
      const cloudSyncs = syncs.filter((entry) => (entry.body as { name?: string } | null)?.name === "omnirush-cloud");
  ...
  test("MCP add still succeeds when the engine is unreachable", async () => {
    ...
    process.env.OMNIRUSH_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    try {
      const omnirush = await startOmniRushServer(workspaceRoot, "http://127.0.0.1:9");
  ...
  test("engine reload reports engine-unreachable when the engine is down", async () => {
    ...
    try {
      ...
      const response = await fetch(`${omnirush.base}/workspace/ws_1/engine/reload`, {
        method: "POST",
        headers: auth(omnirush.token),
      });
