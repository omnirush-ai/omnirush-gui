/**
 * Side-effect proof for the omnirush.json -> runtime.sqlite migration.
 *
 ...
 *   - workspace creation writes the omnirush config to the runtime DB, NOT to
 *     `.opencode/omnirush.json`.
 *   - GET /config still returns the same omnirush payload shape (version,
 *     workspace, authorizedRoots).
 *   - a legacy on-disk `.opencode/omnirush.json` (pre-migration install) is
...
async function withSandbox(fn: (input: { root: string; config: ServerConfig }) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "omnirush-config-migration-"));
...
describe("omnirush config DB migration", () => {
  test("creating a workspace seeds the runtime DB and writes no omnirush.json", async () => {
    await withSandbox(async ({ root, config }) => {
      ...
      try {
        ...
        const create = await fetch(`http://127.0.0.1:${server.port}/workspaces/local`, {
          method: "POST",
          headers: { "x-omnirush-host-token": config.hostToken, "content-type": "application/json" },
        ...
        expect(await fileExists(join(workspacePath, ".opencode", "omnirush.json"))).toBe(false);
        ...
        const stored = await readOmniRushWorkspaceConfig(config, workspaceId);
        ...
        const body = (await configRes.json()) as { omnirush: Record<string, unknown> };
        expect(body.omnirush.version).toBe(1);
        expect(body.omnirush.authorizedRoots).toEqual([resolve(workspacePath)]);
      } finally {
  ...
  test("back-compat: a legacy omnirush.json is migrated into the DB on read", async () => {
    await withSandbox(async ({ root, config }) => {
      ...
      await writeFile(
        join(workspacePath, ".opencode", "omnirush.json"),
        JSON.stringify({
          version: 1,
      ...
      try {
        ...
        expect(Object.keys(await readOmniRushWorkspaceConfig(legacyConfig, workspaceId)).length).toBe(0);
        ...
        expect((body.omnirush.workspace as { name?: string } | undefined)?.name).toBe("Legacy");
        expect((body.omnirush.blueprint as { emptyState?: { title?: string } } | undefined)?.emptyState?.title)
          .toBe("Legacy starter");

        // Side effect: the legacy contents now live in the DB.
        const stored = await readOmniRushWorkspaceConfig(legacyConfig, workspaceId);
  ...
  test("existing local workspace without legacy file gets default DB config on read", async () => {
    await withSandbox(async ({ root, config }) => {
      ...
      try {
        expect(await fileExists(join(workspacePath, ".opencode", "omnirush.json"))).toBe(false);
        expect(Object.keys(await readOmniRushWorkspaceConfig(existingConfig, workspaceId)).length).toBe(0);
        ...
        const body = (await configRes.json()) as { omnirush: Record<string, unknown> };
        expect(body.omnirush.version).toBe(1);
        expect(body.omnirush.authorizedRoots).toEqual([workspacePath]);
        expect((body.omnirush.workspace as { preset?: string } | undefined)?.preset).toBe("starter");

        const stored = await readOmniRushWorkspaceConfig(existingConfig, workspaceId);
        expect(stored.authorizedRoots).toEqual([workspacePath]);
      } finally {
        await server.stop(true);
      }
