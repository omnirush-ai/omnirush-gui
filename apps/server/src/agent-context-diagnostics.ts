...
const REQUIRED_CLOUD_AGENT_TOOL_IDS = REQUIRED_CLOUD_TOOL_IDS.map(
  (toolId) => `${OMNIRUSH_CLOUD_MCP_NAME}_${toolId}`,
);
...
function assessEffectiveToolPolicy(
  ...
  const omnirushAgent = snapshot.agents.find((agent) => agent.name === "omnirush");
  if (!omnirushAgent) {
    return {
      status: "unavailable",
      decisions: {},
      deniedToolIds: [],
      unavailableReasons: ["effective_omnirush_agent_missing"],
    };
  }
  if (
    snapshot.defaultAgent !== "omnirush"
    || omnirushAgent.hidden
    || (omnirushAgent.mode !== "primary" && omnirushAgent.mode !== "all")
  ) {
    return {
      status: "unavailable",
      decisions: {},
      deniedToolIds: [],
      unavailableReasons: ["effective_omnirush_agent_unusable_as_default"],
    };
  }
  const decisions: Record<string, EffectiveToolPolicyDecision> = {};
  for (const toolId of REQUIRED_CLOUD_AGENT_TOOL_IDS) {
    decisions[toolId] = effectiveToolDecision(omnirushAgent.permission, toolId);
  }
...
function promptEvidence(configuredAgent: Record<string, unknown> | null) {
  ...
  return {
    ...
    markers: {
      ...
      artifacts: prompt.includes("## OmniRush.ai Artifacts"),
    },
  };
}
...
async function inspectProjectAgent(workspace: WorkspaceInfo, signal?: AbortSignal): Promise<ProjectAgentInspection> {
  ...
  try {
    ...
    return {
      ...
      agentConfigOverride: Object.hasOwn(agents, "omnirush"),
      agentFileOverride: await Promise.all([
        exists(join(workspace.path, ".opencode", "agent", "omnirush.md")),
        exists(join(workspace.path, ".opencode", "agents", "omnirush.md")),
  ...
  try {
    return {
      inventory: await inspectMcpLayersFromRuntimeSnapshot(workspace.path, runtime, {
        ...
        toolPolicy: {
          agentName: "omnirush",
          mcpName: OMNIRUSH_CLOUD_MCP_NAME,
...
export function cloudCatalogCheck(probe: CloudCatalogProbe): AgentContextDiagnosticCheck {
  ...
  if (probe.status === "observed") {
    ...
    return diagnosticCheck({
      ...
      message: transientUnauthorizedRecovered
        ? "The OmniRush.ai runtime saw a transient HTTP 401 during the independent Cloud MCP initialize request and recovered on retry; this points to a proxy or auth-gateway blip, not a revoked OmniRush.ai Cloud credential."
        : exact
        ? "The canonical OmniRush.ai Cloud catalog exposes exactly the two required capability tools."
        : "The OmniRush.ai Cloud catalog does not match the required two-tool contract.",
      owner: transientUnauthorizedRecovered ? "network-admin" : exact ? "omnirush-server" : "omnirush-support",
      action: transientUnauthorizedRecovered
        ...
        : "Review the OmniRush.ai Cloud deployment and restore the canonical capability catalog.",
    });
  }
  ...
  let message = "The selected workspace does not have a usable managed OmniRush.ai Cloud MCP configuration.";
  let owner: AgentContextDiagnosticCheck["owner"] = "omnirush-server";
  let action = "Reconnect OmniRush.ai Cloud from Settings > Connect and rerun diagnostics.";
  ...
  switch (probe.code) {
    ...
    case "remote_workspace_unavailable":
      ...
      action = "Run diagnostics on the OmniRush.ai server that owns the workspace.";
      break;
    case "cloud_mcp_missing":
      owner = "omnirush-client";
      message = "The selected client workspace has no synced OmniRush.ai Cloud MCP entry.";
      break;
    case "cloud_mcp_disabled":
      owner = "omnirush-client";
      message = "The selected workspace OmniRush.ai Cloud MCP entry is disabled.";
      action = "Enable or reconnect OmniRush.ai Cloud from Settings > Connect, then rerun diagnostics.";
      break;
    case "cloud_mcp_not_remote":
      message = "The managed OmniRush.ai Cloud entry is not configured as a remote MCP.";
      action = "Reconnect OmniRush.ai Cloud to restore its managed remote configuration.";
      break;
    case "invalid_endpoint":
      message = "The managed OmniRush.ai Cloud endpoint is not credential-safe or does not end at the required /mcp/agent route.";
      action = "Reconnect OmniRush.ai Cloud to restore its managed endpoint, then rerun diagnostics.";
      break;
    case "untrusted_endpoint":
      status = "warning";
      evidenceKind = "unavailable";
      message = probe.enterpriseActivationPresent
        ? "The runtime endpoint probe was not performed: this installation is enterprise activated, but against a different control-plane origin than the configured OmniRush.ai Cloud MCP. No request was sent, so this is a configuration mismatch, not a network, TLS, or MCP failure."
      ...
      action = probe.enterpriseActivationPresent
        ? "Reconcile the enterprise activation origin with the configured OmniRush.ai Cloud MCP origin, or have an administrator add the exact endpoint origin to OMNIRUSH_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS, then rerun diagnostics."
        : "Activate this installation against your on-prem Den, or have an administrator set OMNIRUSH_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS on the OmniRush.ai desktop/server process to the exact endpoint origin, then rerun diagnostics.";
      break;
    case "credential_missing":
    case "duplicate_authorization":
      owner = "omnirush-client";
      message = "The managed OmniRush.ai Cloud entry does not contain one unambiguous authentication value.";
      action = "Reconnect OmniRush.ai Cloud so the client can replace the managed credential, then rerun diagnostics.";
      break;
    case "timeout":
    case "network_error":
    case "http_error":
      ...
      if (probe.httpStatus === 451) {
        message = "The independent runtime endpoint probe received HTTP 451, which indicates an egress proxy, firewall, or allowlist deny rather than an OmniRush.ai Cloud catalog shape problem.";
        action = "Add the configured OmniRush.ai Cloud MCP host and its documented redirect targets to the corporate allowlist, then rerun diagnostics.";
      } else {
        message = "The independent runtime endpoint probe could not be completed through the configured network path.";
        action = "Verify server egress, DNS, TLS, proxy policy, and the configured OmniRush.ai Cloud service, then rerun diagnostics.";
      }
      break;
    ...
    case "dns_error":
      ...
      action = "Verify DNS resolution from the OmniRush.ai server, then rerun diagnostics.";
      break;
    case "connection_refused":
      owner = "network-admin";
      message = "The configured OmniRush.ai Cloud service refused the independent runtime probe connection.";
    ...
    case "tls_error":
      owner = "network-admin";
      message = "The independent runtime endpoint probe failed TLS certificate validation or negotiation on the OmniRush.ai runtime trust store.";
    ...
    case "unauthorized":
      owner = "omnirush-client";
      message = "OmniRush.ai Cloud rejected the configured credential during the independent runtime probe.";
      action = "Reconnect OmniRush.ai Cloud so the client can replace the managed credential, then rerun diagnostics.";
      break;
    case "forbidden":
      owner = "organization-admin";
      message = "OmniRush.ai Cloud rejected the probe for membership, scope, or policy reasons rather than credential validity.";
    ...
    case "mcp_route_not_found":
      ...
      action = "Verify the OmniRush.ai Cloud or on-prem Den deployment version exposes the /mcp/agent route, then rerun diagnostics.";
      break;
    case "rate_limited":
      status = "warning";
      owner = "omnirush-support";
      message = "OmniRush.ai Cloud rate-limited the independent runtime probe.";
      action = "Wait before rerunning diagnostics; contact OmniRush.ai support if rate limiting persists.";
      break;
    case "gateway_unavailable":
      owner = "network-admin";
      message = "A gateway or upstream in front of the OmniRush.ai Cloud service reported it unavailable during the independent runtime probe.";
      action = "Check service status and gateway health, wait briefly, then rerun diagnostics.";
      break;
    case "probe_busy":
      status = "warning";
      evidenceKind = "unavailable";
      owner = "omnirush-server";
    ...
    case "invalid_catalog":
      owner = "omnirush-support";
      message = "OmniRush.ai Cloud returned a response that does not satisfy the bounded MCP handshake protocol contract.";
      action = "Review the OmniRush.ai Cloud deployment and restore a conformant MCP handshake response.";
      break;
    case "required_tools_missing":
      owner = "omnirush-support";
      message = "The OmniRush.ai Cloud catalog handshake succeeded, but the catalog does not contain both required capability tools.";
      action = "Review the OmniRush.ai Cloud deployment and restore the canonical capability catalog.";
      break;
  }
  return diagnosticCheck({
    ...common,
...
export function cloudDifferentialCheck(probe: CloudCatalogProbe, engineReachableNow: boolean): AgentContextDiagnosticCheck {
  ...
  if (verdict === "runtime_and_engine_connected") {
    return diagnosticCheck({
      ...
      message: "The independent OmniRush.ai runtime probe and the engine registration evidence both report the OmniRush.ai Cloud endpoint as reachable.",
      owner: "omnirush-server",
  ...
  if (verdict === "runtime_connected_engine_failed") {
    return diagnosticCheck({
      ...
      status: "failed",
      evidenceKind: "derived",
      message: "The OmniRush.ai runtime reached the Cloud endpoint directly, but the engine registration evidence reports a failure; the engine-side connection path or registration lifecycle is implicated, not the endpoint.",
      owner: "opencode-engine",
      action: "Reconnect OmniRush.ai Cloud or restart the selected workspace engine, then rerun diagnostics; the endpoint itself is reachable from this machine.",
    });
  }
  if (verdict === "runtime_failed_engine_connected") {
    return diagnosticCheck({
      ...
      status: "warning",
      evidenceKind: "derived",
      message: "The engine registration evidence reports a live connection, but the independent runtime probe failed; the OmniRush.ai runtime network path is implicated rather than the endpoint or the engine.",
      owner: "network-admin",
      action: "Compare proxy, DNS, and trust-store configuration between the OmniRush.ai runtime and the engine process, then rerun diagnostics.",
    });
  }
  ...
  if (verdict === "engine_evidence_stale_or_unavailable") {
    return diagnosticCheck({
      ...
      status: "warning",
      evidenceKind: "derived",
      message: "The engine registration evidence for OmniRush.ai Cloud is stale or was never recorded, so only the independent runtime observation is current.",
      owner: "opencode-engine",
...
export function cloudEndpointTransportCheck(
  ...
  if (notRequired) {
    return diagnosticCheck({
      ...
      message: "The managed OmniRush.ai Cloud MCP registration is connected; the differential check compares runtime and engine reachability, so a separate TLS-layer probe was not needed.",
      owner: "omnirush-server",
      action: "No action is required.",
      details,
    });
  ...
  if (probe.verifiedHandshake === "ok") {
    return diagnosticCheck({
      ...
      message: "The OmniRush.ai Cloud endpoint completed a credential-free TLS handshake; use the differential check for authentication, MCP protocol, and engine-registration attribution.",
      owner: "omnirush-server",
  ...
  if (
    ...
    return diagnosticCheck({
      ...
      owner: "network-admin",
      action: "Fix or bypass the egress device that stalls TLS 1.3 ClientHello traffic for OmniRush.ai Cloud hosts, or temporarily force TLS 1.2 where your policy permits it, then rerun diagnostics.",
      details,
    });
  }
  if (probe.verifiedHandshake === "failed" && probe.verifyErrorCode === "ETIMEDOUT") {
    if (
      ...
      return diagnosticCheck({
        ...
        owner: "network-admin",
        action: "Verify the egress proxy and firewall pass TLS ClientHello traffic to OmniRush.ai Cloud hosts, then compare with a known Node or openssl TLS 1.2-only probe and confirm every OmniRush.ai runtime honors any temporary TLS-version pinning policy.",
        details,
      });
    }
    return diagnosticCheck({
      ...
      owner: "network-admin",
      action: "Verify that the egress proxy and firewall pass TLS 1.3 ClientHello traffic to OmniRush.ai Cloud hosts; compare with a TLS 1.2-only probe or temporarily force TLS 1.2 where policy permits, then rerun diagnostics.",
      details,
    });
  }
  if (isCloudEndpointCertificateVerificationFailure(probe)) {
    ...
    if (/corporate|interception|proxy|mitm/iu.test(servedIssuer)) {
      return diagnosticCheck({
        ...
        message: "The OmniRush.ai Cloud endpoint appears to be TLS-inspected or re-signed by a corporate proxy; the runtime does not trust that inspecting issuer.",
        owner: "network-admin",
        action: "Install the corporate inspection root for the OmniRush.ai runtime with NODE_EXTRA_CA_CERTS, or bypass TLS inspection for OmniRush.ai Cloud hosts, then rerun diagnostics.",
        details,
      });
    }
    if (probe.verifyErrorCode === "UNABLE_TO_VERIFY_LEAF_SIGNATURE" && probe.servedChainLength === 1) {
      return diagnosticCheck({
        ...
        message: "The OmniRush.ai Cloud endpoint served a leaf-only TLS chain and verification failed with UNABLE_TO_VERIFY_LEAF_SIGNATURE; the missing intermediate/fullchain must be repaired before blaming credentials or application logic.",
        owner: "network-admin",
        action: "Serve the complete certificate chain from the endpoint or let the OmniRush.ai runtime add the AIA intermediate to NODE_EXTRA_CA_CERTS, then rerun diagnostics.",
        details,
      });
    }
    return diagnosticCheck({
      ...
      message: `The OmniRush.ai Cloud endpoint TLS handshake failed with ${probe.verifyErrorCode ?? "a certificate verification error"}; the OS or corporate CA chain is not visible to this runtime.`,
      owner: "network-admin",
      action: "Provide the corporate CA chain to OmniRush.ai with NODE_EXTRA_CA_CERTS or fix the server to present its full certificate chain; the served-chain evidence below shows what the endpoint sent.",
      details,
    });
  }
  if (probe.verifiedHandshake === "failed") {
    return diagnosticCheck({
      ...
      message: "The OmniRush.ai Cloud endpoint could not be reached with a credential-free TCP/TLS handshake.",
      owner: "network-admin",
  ...
  return diagnosticCheck({
    ...
    owner: "omnirush-server",
    action: "Review the managed OmniRush.ai Cloud MCP configuration if transport evidence is needed.",
    details,
  });
}
  ...
  if (request.organizationConnectionsProbe.status === "skipped") {
    ...
    return diagnosticCheck({
      ...
      message: remotePrivacy
        ? "Local Den organization topology was intentionally omitted from the remote OmniRush.ai diagnostics request."
        : "Organization connection readiness was not observed for this run.",
      owner: remotePrivacy ? "omnirush-client" : "member",
...
function engineAgentCheck(
  ...
  const agent = snapshot.agents.find((candidate) => candidate.name === "omnirush");
  return diagnosticCheck({
    id: "engine-agent",
    status: agent ? "passed" : "failed",
    evidenceKind: "observed",
    code: agent ? "effective_omnirush_agent_observed" : "effective_omnirush_agent_missing",
    message: agent
      ? "The selected engine resolved the OmniRush.ai agent."
      : "The selected engine did not resolve an OmniRush.ai agent.",
    owner: agent ? "opencode-engine" : "omnirush-server",
    action: agent
      ? "No action is required."
      : "Restore the OmniRush.ai runtime agent injection and restart the selected workspace engine.",
    details: {
      engineApiReadPerformed: true,
      effectiveAgentCount: snapshot.agents.length,
      omnirushAgentPresent: Boolean(agent),
      omnirushAgentHidden: agent?.hidden ?? null,
...
function runtimeHealthCheck(
  ...
  return diagnosticCheck({
    ...
    code,
    message,
    owner: corrupt ? "omnirush-server" : engineConfigured ? "omnirush-server" : "member",
    action: status === "passed"
      ? "No action is required."
      : corrupt
        ? "Repair the OmniRush.ai runtime state before relying on injected configuration."
        : remote
          ? "Run diagnostics on the OmniRush.ai server that owns the workspace."
...
export async function runAgentContextDiagnostics(input: {
  ...
  const expectedRuntimeConfig = buildOmniRushRuntimeConfigObjectFromSnapshot(globalRuntimeInspection.config);
  const expectedAgents = isRecord(expectedRuntimeConfig.agent) ? expectedRuntimeConfig.agent : {};
  const expectedAgent = isRecord(expectedAgents.omnirush) ? expectedAgents.omnirush : null;
  const effectiveOmniRushAgent = effectiveEngine?.agents.find((agent) => agent.name === "omnirush") ?? null;
  const effectiveAgentModeUsable = effectiveOmniRushAgent?.mode === "primary"
    || effectiveOmniRushAgent?.mode === "all";
  const effectiveAgentUsable = Boolean(
    effectiveOmniRushAgent
    && effectiveEngine?.defaultAgent === "omnirush"
    && !effectiveOmniRushAgent.hidden
  ...
  const reportedAgent = effectiveEngine
    ? effectiveOmniRushAgent
      ? { prompt: effectiveOmniRushAgent.prompt }
      : null
    : expectedAgent;
  ...
  const canonicalConnectPluginSpec = expectedPlugins.find(
    (spec) => pluginLabel(spec) === "omnirush-extensions-preview",
  ) ?? null;
  ...
  const selectedCloudMcpPresent = Object.hasOwn(runtimeMcpMap(runtime), OMNIRUSH_CLOUD_MCP_NAME);
  ...
  const engineConfigItems = effectiveEngine?.mcps.map((item) => ({
    ...
    disabledByTools: item.name === OMNIRUSH_CLOUD_MCP_NAME
  ...
  const runtimeCloudItem = combinedInventoryItems.find((item) =>
    item.source === "config.remote" && item.name === OMNIRUSH_CLOUD_MCP_NAME,
  );
  if (
    runtimeCloudItem
  ...
  const runtimeCloudConfig = runtimeMcpMap(runtime)[OMNIRUSH_CLOUD_MCP_NAME] ?? null;
  ...
  const cloudProbe = await probeOmniRushCloudCatalog({
  ...
  const checks: AgentContextDiagnosticCheck[] = [
    ...
    diagnosticCheck({
      ...
      message: !connectSnapshotAvailable
        ...
        : crossWorkspaceSteeringDrift
          ? "Global Connect steering sees OmniRush.ai Cloud, but the selected workspace does not contain that managed MCP."
          : "The expected Connect steering branch is internally consistent for the selected workspace.",
      owner: !connectSnapshotAvailable || crossWorkspaceSteeringDrift ? "omnirush-server" : "omnirush-client",
      action: !connectSnapshotAvailable
        ? "Verify the OmniRush.ai server runtime state and rerun diagnostics."
        : crossWorkspaceSteeringDrift
          ? "Reconnect or sync OmniRush.ai Cloud for the selected workspace."
          : "No action is required.",
      details: {
    ...
    diagnosticCheck({
      ...
      code: effectiveEngine
        ? !effectiveOmniRushAgent
          ? "effective_omnirush_agent_missing"
          : effectiveEngine.defaultAgent !== "omnirush"
            ? "effective_default_agent_mismatch"
            : effectiveOmniRushAgent.hidden
              ? "effective_omnirush_agent_hidden"
              : !effectiveAgentModeUsable
                ? "effective_omnirush_agent_not_primary"
            : "effective_omnirush_agent_selected"
        : projectOverrideDetected
          ? "configured_agent_has_override_layers"
          : "runtime_agent_intent_only",
      message: effectiveEngine
        ? !effectiveOmniRushAgent
          ? "The effective engine configuration does not contain the OmniRush.ai agent."
          : effectiveEngine.defaultAgent !== "omnirush"
            ? "The effective engine default does not select the OmniRush.ai agent."
            : effectiveOmniRushAgent.hidden
              ? "The effective OmniRush.ai agent is hidden and cannot be used as the default agent."
              : !effectiveAgentModeUsable
                ? "The effective OmniRush.ai agent is subagent-only and cannot be used as the default agent."
            : "The effective engine default selects the resolved OmniRush.ai agent."
        : projectOverrideDetected
          ? "The configured OmniRush.ai agent intent has project override layers and could not be confirmed live."
          : "Only the configured OmniRush.ai agent intent was available; effective resolution was not observed.",
      ...
      action: effectiveEngine && effectiveAgentUsable
        ? "No action is required."
        : effectiveEngine
          ? "Restore the OmniRush.ai agent and default-agent injection, then restart the selected workspace engine."
          : "Check the selected workspace engine health and rerun diagnostics.",
      details: {
        configuredAgentPresent: Boolean(expectedAgent),
        effectiveAgentPresent: effectiveEngine ? Boolean(effectiveOmniRushAgent) : null,
        effectiveDefaultAgentIsOmniRush: effectiveEngine ? effectiveEngine.defaultAgent === "omnirush" : null,
        effectiveAgentHidden: effectiveOmniRushAgent?.hidden ?? null,
        effectiveAgentMode: effectiveOmniRushAgent?.mode ?? null,
    ...
    diagnosticCheck({
      ...
      message: promptMatchesCanonicalIntent
        ? effectiveEngine
          ? "The effective OmniRush.ai base prompt exactly matches the canonical configured injection and contains every required marker."
          : "The configured OmniRush.ai base prompt intent matches its canonical generated injection and contains every required marker."
        : !promptMarkersPresent
          ? effectiveEngine
            ? "The effective OmniRush.ai base prompt is missing one or more required markers."
            : "The configured OmniRush.ai base prompt intent is missing one or more required markers."
          : effectiveEngine
            ? "The effective OmniRush.ai base prompt contains the markers but does not match the canonical configured injection."
            : "The configured OmniRush.ai base prompt markers are present, but its digest does not match the canonical generated injection.",
      owner: effectiveEngine ? "opencode-engine" : "omnirush-server",
      action: promptMatchesCanonicalIntent
        ? "No action is required."
        : "Restore the canonical OmniRush.ai runtime agent definition.",
      details: {
        ...prompt.markers,
    ...
    diagnosticCheck({
      ...
      action: cloudToolPolicyStatus === "denied"
        ? "Allow the denied omnirush-cloud capability tool IDs in top-level or OmniRush.ai agent permission policy, then rerun diagnostics."
        : cloudToolPolicyStatus === "unavailable"
          ? "Check the selected workspace engine health and rerun diagnostics."
      ...
      details: {
        searchCapabilities: reportedToolPermission(`${OMNIRUSH_CLOUD_MCP_NAME}_search_capabilities`),
        executeCapability: reportedToolPermission(`${OMNIRUSH_CLOUD_MCP_NAME}_execute_capability`),
    ...
    diagnosticCheck({
      ...
      action: canonicalPluginSpecMatched
        ? "No action is required."
        : "Restore the canonical OmniRush.ai runtime plugin bundle.",
      details: {
    ...
    diagnosticCheck({
      ...
      message: effectiveEngine
        ? inventoryTotal > 200
          ...
          : "The selected engine's merged MCP configuration and OmniRush.ai-managed dynamic injection intent were inventoried as separate evidence sources."
        : layerHealthProblem
    ...
    diagnosticCheck({
      ...
      message: failedRegistrationCount > 0
        ...
        : missingRegistrationCount > 0
          ...
          : remoteMcps.length > 0
            ? "Every enabled OmniRush.ai-managed MCP has a current connected registration result; configured-disabled entries are not treated as injected tools."
            : "No server-managed MCP registration was available to inspect.",
      owner: failedRegistrationCount > 0 || missingRegistrationCount > 0 ? "opencode-engine" : "omnirush-server",
    ...
    diagnosticCheck({
      ...
      owner: "opencode-engine",
      action: "Review the bounded OmniRush.ai Cloud catalog probe and exact managed registration response evidence instead.",
      details: {
  ...
  const report: AgentContextDiagnosticsReport = {
    ...
    agent: {
      ...
      configuredOmniRushAgent: {
        state: effectiveEngine
          ? effectiveOmniRushAgent ? "present" : "missing"
          : expectedAgent?.disable === true
            ? "configured-disabled"
            : expectedAgent
              ? "present"
              : "missing",
        mode: effectiveOmniRushAgent
          ? effectiveOmniRushAgent.mode
