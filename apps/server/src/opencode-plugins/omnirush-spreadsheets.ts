...
import { WorkspaceFileError, omniRushspaceFileForReading, omniRushspaceFileForWriting } from "./workspace-file-identity.js";
import {
...
/**
 * OmniRush.ai Spreadsheets Plugin
 *
...
async function writeWorkbookFile(root: string, file: WorkspaceFile, bytes: Uint8Array, overwrite: boolean): Promise<{ replaced: boolean }> {
  ...
  let opened;
  try {
    opened = await omniRushspaceFileForWriting(realRoot, path, existing, label);
  } catch (error) {
...
export const OmniRushSpreadsheets = async (factoryInput?: unknown) => {
