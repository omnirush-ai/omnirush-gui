...
function uiControlDiscoveryPaths(): string[] {
  const explicit = process.env.OMNIRUSH_UI_CONTROL_DISCOVERY?.trim();
  if (explicit) return [explicit];
  return [
    join(userAppDataDir(), "ai.omnirush.desktop", "omnirush-ui-control.json"),
    join(userAppDataDir(), "ai.omnirush.desktop.dev", "omnirush-ui-control.json"),
...
async function discoverUiBridge(): Promise<UiBridge | null> {
  ...
  const discovery = process.env.OMNIRUSH_UI_CONTROL_DISCOVERY?.trim();
