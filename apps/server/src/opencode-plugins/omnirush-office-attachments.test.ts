...
import { omnirushOfficeAttachmentsPluginPath } from "../omnirush-extensions-plugin-path.js";
import { OmniRushOfficeAttachments } from "./omnirush-office-attachments.js";
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "omnirush-office-plugin-"));
  try {
    await fn(root);
  } finally {
...
async function transform(root: string, messages: unknown[]) {
  const plugin = await OmniRushOfficeAttachments({ directory: root });
...
describe("OmniRushOfficeAttachments", () => {
  ...
  test("rejects file URLs that resolve outside the workspace through a symlink", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "omnirush-office-outside-"));
      try {
        const docx = docxFixture();
  ...
  test("never materializes attachment bytes through a symlinked inbox folder", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "omnirush-office-inbox-outside-"));
      try {
  ...
  test("materializes into the real inbox folder and reuses identical bytes", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(first).toBe(`.opencode/omnirush/inbox/chat-attachments/${sha256(docx).slice(0, 16)}-QuarterlyBrief.docx`);
      await expect(readdir(join(root, ".opencode", "omnirush", "inbox", "chat-attachments"))).resolves.toEqual([`${sha256(docx).slice(0, 16)}-QuarterlyBrief.docx`]);
  ...
  test("uses factory directory and ignores Daytona non-git worktree root", async () => {
    await withWorkspace(async (root) => {
      const docx = docxFixture();
      const plugin = await OmniRushOfficeAttachments({ directory: root, worktree: "/" });
      ...
      expect(materialized).toContain(".opencode/omnirush/inbox/chat-attachments/");
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    ...
    expect(plugin).toContain(omnirushOfficeAttachmentsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("omnirush-office-attachments.ts");
  });

  test("module exposes only the plugin factory", async () => {
    const mod = await import("./omnirush-office-attachments.js");
    expect(Object.keys(mod)).toEqual(["OmniRushOfficeAttachments"]);
  });
});
