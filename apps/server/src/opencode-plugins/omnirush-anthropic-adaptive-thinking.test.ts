...
import { OmniRushAnthropicAdaptiveThinking } from "./omnirush-anthropic-adaptive-thinking.js";

async function runHook(apiId: string, options: Record<string, unknown>) {
  const hooks = await OmniRushAnthropicAdaptiveThinking();
  const output: { options: Record<string, unknown> } = { options };
  await hooks["chat.params"]({ model: { id: apiId, api: { id: apiId } } }, output);
  return output.options;
}

describe("OmniRushAnthropicAdaptiveThinking chat.params", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./omnirush-anthropic-adaptive-thinking.js");
    expect(Object.keys(mod)).toEqual(["OmniRushAnthropicAdaptiveThinking"]);
  });
});
