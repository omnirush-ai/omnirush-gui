...
import { omnirushSpreadsheetsPluginPath } from "../omnirush-extensions-plugin-path.js";
import { buildZip, excelSerialToIso, isDateNumberFormat, listZipEntries, openXlsxWorkbook, readZipEntryData, renderSheetTable, utf8Text, writeXlsxWorkbook } from "@omnirush/workbook";
import { OmniRushSpreadsheets } from "./omnirush-spreadsheets.js";

const PACKAGE_ROOT = join(dirname(fileURLToPath(import.meta.url)), "../..");

type Plugin = Awaited<ReturnType<typeof OmniRushSpreadsheets>>;
...
async function withWorkspace(fn: (root: string, plugin: Plugin) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "omnirush-spreadsheets-"));
  try {
    await fn(root, await OmniRushSpreadsheets({ directory: root }));
  } finally {
...
describe("OmniRushSpreadsheets", () => {
  ...
  test("keeps every path inside the workspace and explains unsupported files", async () => {
    await withWorkspace(async (root, plugin) => {
      const outside = await mkdtemp(join(tmpdir(), "omnirush-spreadsheets-outside-"));
      try {
  ...
  test("appends one spreadsheets instruction section to the existing system entry", async () => {
    const plugin = await OmniRushSpreadsheets({ directory: tmpdir() });
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    ...
    expect(plugin).toContain(omnirushSpreadsheetsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("omnirush-spreadsheets.ts");
  });

  test("module exposes only the plugin factory", async () => {
    const mod = await import("./omnirush-spreadsheets.js");
    expect(Object.keys(mod)).toEqual(["OmniRushSpreadsheets"]);
  });
});
