...
import { omnirushFeatureContributionSchema } from "@omnirush/types/omnirush-provider";

import { buildOmniRushProviderContributions } from "./omnirush-provider-adapters.js";

describe("OmniRush.ai provider adapters", () => {
  test("normalizes sessions and extensions into semantic contributions", () => {
    const contributions = buildOmniRushProviderContributions([]);
    ...
    for (const contribution of contributions) {
      expect(omnirushFeatureContributionSchema.safeParse(contribution).success).toBe(true);
    }
  });
  ...
  test("keeps known Connect skills direct and search available for unknown capabilities", () => {
    ...
    expect(connect?.guidance).toEqual([{
      ...
      title: "Customer briefing",
      description: "Prepare a customer briefing from connected sources.",
      provider: { id: "omnirush-cloud", kind: "connect" },
      loading: "catalog",
    }]);
    expect(connect?.affordances.map((affordance) => ({
    ...
    }))).toEqual([
      {
        ...
        executor: { kind: "tool", tool: "omnirush-cloud_search_capabilities" },
      },
      {
        id: "connect.capability.execute",
        executor: { kind: "tool", tool: "omnirush-cloud_execute_capability" },
      },
    ]);
    expect(
  ...
  test("exposes an Automations proposal affordance that writes nothing", () => {
    const proposal = buildOmniRushProviderContributions([])
