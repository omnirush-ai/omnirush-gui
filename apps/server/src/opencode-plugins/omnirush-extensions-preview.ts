...
import { z } from "zod";
import { visualizationSchema } from "@omnirush/types/visualization";
import type { OmniRushAffordanceEffects } from "@omnirush/types/omnirush-affordance";
import { automationProposalSchema } from "@omnirush/types/automations";
import {
...
import {
  buildOmniRushProviderContributions,
  type ConnectSkillDescriptor,
  type EngineMcpDescriptor,
} from "./omnirush-provider-adapters.js";
...
const omnirushAffordanceRequestSchema = z.object({
  id: z.string().trim().min(1).describe("Semantic affordance id from omnirush_context."),
  args: z.record(z.string(), z.unknown()).optional().describe("JSON arguments for the affordance."),
  expectedRevision: z.number().int().nonnegative().optional().describe("Context revision from omnirush_context. Use for commands to prevent stale writes."),
...
const sessionSearchArgsSchema = z.object({
  query: z.string().trim().min(1).describe("Text to search for across OmniRush.ai session titles and message transcripts."),
  workspaceId: z.string().trim().optional().describe("Optional OmniRush.ai workspace id/name to limit the search."),
...
const sessionCreateArgsSchema = z.object({
  sessions: z.array(z.object({
    title: z.string().trim().min(1).max(120).describe("Short title shown in the OmniRush.ai session list."),
  ...
  workspaceId: z.string().trim().optional().describe("Optional OmniRush.ai workspace id/name. Defaults to the workspace containing the current session."),
});
...
const OMNIRUSH_IDENTITY_INSTRUCTION =
  `## OmniRush.ai identity
You are the OmniRush.ai assistant. In ordinary conversation, identify the app as OmniRush.ai.
If the user asks which model or provider is running, use the exact selection supplied by runtime context. Never rename one vendor's model as another vendor's model, invent an identity, or hide a known selection. If the selection is unavailable, say you cannot see it rather than guessing.`;

const OMNIRUSH_AGENT_SURFACE_INSTRUCTION =
  `## OmniRush.ai app context
For lightweight UI mockups, wireframes, and design iterations, use omnirush_visualization to show a native OmniRush.ai-styled sketch in the conversation. Keep the design id when revising, increment revision, and send the complete updated design. Mock controls are illustrative; use the normal app-building workflow when a working app is requested.
Use omnirush_context when the request depends on the current OmniRush.ai screen, open tabs, split view, focused pane, sidebar, side panel, settings panel, or available app actions.
Each affordance declares its effects and executor. Use omnirush_query only for side-effect-free app affordances. Use omnirush_execute for app commands without activating the desktop window. If executor names another tool, call that exact tool instead.
...
To open settings or navigate the app, use omnirush_execute with ids from omnirush_context such as settings.panel.open — never browser_* tools for the OmniRush.ai app itself.`;

// External-web mechanics only: the app-surface section above owns the rule
// that browser_* tools never drive the OmniRush.ai app itself.
const OMNIRUSH_BROWSER_INSTRUCTION =
...
When browser.release_tab is available, keep the chosen tabId and release it through omnirush_execute only after all running and queued browser calls have finished. This permits the person to suspend the page. Before any later use, call browser.restore_tab through omnirush_execute with that tabId, then observe and rediscover website tools; never reuse old observations, tool references, or targets after release.
...
All methods preserve the same conversation and tab. Observe before each action; references expire after page changes. After navigation, observe and rediscover tools. Never call arbitrary browser_eval or connect directly to CDP to bypass the host. Never control OmniRush.ai's own UI through browser tools.
...
type OmniRushWorkspace = z.infer<typeof workspaceSchema>;
...
type CreatedOmniRushSessionResult = {
  ok: true;
  sessionId: string;
  title: string;
  started: boolean;
...
type FailedOmniRushSessionResult = {
  ok: false;
  title: string;
  error: string;
};
...
function preserveMcpResult(output: unknown): void {
  ...
  output.metadata = {
    ...
    omnirushMcpApp: appResult,
  };
}

const affordanceReadEffects: OmniRushAffordanceEffects = { data: "read", ui: "none", external: false };
const affordanceWriteEffects: OmniRushAffordanceEffects = { data: "write", ui: "none", external: false };
const affordanceExternalWriteEffects: OmniRushAffordanceEffects = { data: "write", ui: "none", external: true };
// A proposal writes nothing anywhere: it is rendered for a person to act on.
const affordanceProposalEffects: OmniRushAffordanceEffects = { data: "none", ui: "none", external: false };

function affordanceResult(
  id: string,
  result: unknown,
  effects: OmniRushAffordanceEffects,
) {
...
type OmniRushEngineMcpStatusFunction = OmniRushEngineMcpStatusClient["mcp"]["status"];

function isOmniRushEngineMcpStatusFunction(value: unknown): value is OmniRushEngineMcpStatusFunction {
  return typeof value === "function";
}

function readEngineMcpStatusClient(value: unknown): OmniRushEngineMcpStatusClient | undefined {
  ...
  if (!isOmniRushEngineMcpStatusFunction(status)) return undefined;
...
async function serverGet(path: string): Promise<unknown> {
  ...
  if (!response.ok) throw new Error(errorMessage(payload, "OmniRush.ai server request failed"));
  return payload;
}
...
async function readEngineMcpDescriptors(
  client: OmniRushEngineMcpStatusClient | undefined,
...
async function readOmniRushAgentContext(
  engineMcpStatusClient: OmniRushEngineMcpStatusClient | undefined,
  ...
  const contributions = buildOmniRushProviderContributions(skills, mcps);
...
async function queryOmniRushAffordance(rawArgs: unknown): Promise<unknown> {
  const request = omnirushAffordanceRequestSchema.parse(rawArgs);
  if (request.id === "session.search") {
    return affordanceResult(
      request.id,
      await searchOmniRushSessions(request.args ?? {}),
  ...
  if (request.id === "session.read") {
    return affordanceResult(
      request.id,
      await readOmniRushSession(request.args ?? {}),
      affordanceReadEffects,
    );
  }
  ...
  if (request.id.startsWith("connect.")) {
    return unavailableAffordance(
      request.id,
      "This affordance declares a dedicated Connect executor. Call the tool named in omnirush_context.",
    );
  }
  const result = await uiControlRequest("query", request);
  return isRecord(result) && typeof result.ok === "boolean"
    ? result
    : unavailableAffordance(request.id, "OmniRush.ai UI query returned an invalid response.");
}
...
async function executeOmniRushAffordance(
  ...
  if (request.id === "session.create") {
    return affordanceResult(
      request.id,
      await createOmniRushSessions(request.args ?? {}, context),
...
function workspaceLabel(workspace: OmniRushWorkspace): string {
...
function titleSearchResult(workspace: OmniRushWorkspace, session: SessionInfo, queryLower: string): SessionSearchResult | null {
...
function messageSearchResult(workspace: OmniRushWorkspace, session: SessionInfo, messages: SessionMessage[], queryLower: string): SessionSearchResult | null {
...
async function listOmniRushWorkspaces(): Promise<OmniRushWorkspace[]> {
  return workspaceListEnvelopeSchema.parse(await serverGet("/workspaces")).items;
}

function filterWorkspaces(workspaces: OmniRushWorkspace[], workspaceId?: string): OmniRushWorkspace[] {
...
async function listWorkspaceSessions(workspace: OmniRushWorkspace, limit: number): Promise<SessionInfo[]> {
...
async function assertSessionInWorkspace(workspace: OmniRushWorkspace, session: SessionInfo): Promise<void> {
...
async function readWorkspaceSession(workspace: OmniRushWorkspace, sessionId: string): Promise<SessionInfo> {
...
async function readSessionMessages(workspace: OmniRushWorkspace, sessionId: string, limit: number): Promise<SessionMessage[]> {
...
async function searchOmniRushSessions(rawArgs: unknown): Promise<object> {
  ...
  const workspaces = filterWorkspaces(await listOmniRushWorkspaces(), args.workspaceId);
  if (!workspaces.length) {
    return { ok: false, error: args.workspaceId ? `No workspace matched ${args.workspaceId}` : "No OmniRush.ai workspaces are available" };
  }

  const sessions: Array<{ workspace: OmniRushWorkspace; session: SessionInfo }> = [];
...
async function readOmniRushSession(rawArgs: unknown): Promise<object> {
  ...
  return { ok: false, error: `Session ${args.sessionId} was not found in matching OmniRush.ai workspaces` };
}
...
async function createOmniRushSessions(rawArgs: unknown, context: OpenCodeContext): Promise<object> {
  ...
  const results = await Promise.all(args.sessions.map(async (session): Promise<CreatedOmniRushSessionResult | FailedOmniRushSessionResult> => {
    try {
  ...
  const created = results.filter((result): result is CreatedOmniRushSessionResult => result.ok);
  const failures = results.filter((result): result is FailedOmniRushSessionResult => !result.ok);
...
async function postJson(path: string, body: ExtensionActionPayload | Record<string, unknown>, signal?: AbortSignal): Promise<unknown> {
  ...
  const payload = await parseResponse(response);
  if (!response.ok) {
    throw new Error(errorMessage(payload, "OmniRush.ai extension call failed"));
  }
  return payload;
}
...
export const OmniRushExtensionsPreview = async (factoryInput?: unknown) => {
  ...
  "chat.headers": async (input: { sessionID: string; model: { providerID: string }; message: { id: string } }, output: { headers: Record<string, string> }) => {
    if (input.model.providerID !== "omnirush") return;
    output.headers["x-omnirush-session-id"] = input.sessionID;
    output.headers["x-omnirush-task-id"] = input.message.id;
  },
  "tool.execute.after": async (_input: unknown, output: unknown) => {
    ...
    // so OmniRush.ai can host the UI without replaying the tool call.
  ...
  "experimental.chat.system.transform": async (input: unknown, output: { system: string[] }) => {
    ...
    const [extensionInstruction, skillInstruction, automationInstruction] = await Promise.all([
      resolveOmniRushExtensionDiscoveryInstruction(mergedInput, fetch, {
        client: engineMcpStatusClient,
        directory: engineMcpStatusDirectory,
      }),
      resolveOmniRushConnectSkillInstruction(mergedInput, fetch),
      resolveOmniRushAutomationInstruction(mergedInput, fetch),
    ]);
    ...
    appendAgentInstructions(
      output.system,
      createInstructionSection("identity", OMNIRUSH_IDENTITY_INSTRUCTION),
      createInstructionSection("agent-surface", OMNIRUSH_AGENT_SURFACE_INSTRUCTION),
      createInstructionSection("browser", OMNIRUSH_BROWSER_INSTRUCTION),
  ...
  tool: {
    ...
    omnirush_context: {
      description: "Read one semantic snapshot of OmniRush.ai: current screen, retained conversation tabs, split view and focused pane, sidebar and side panel state, settings panel, provider contributions, remote skill guidance, and available affordances with explicit effects and executors.",
      args: {},
      async execute() {
        return JSON.stringify(
          await readOmniRushAgentContext(engineMcpStatusClient, engineMcpStatusDirectory),
          null,
          2,
        );
      },
    ...
    omnirush_query: {
      description: "Run a side-effect-free OmniRush.ai affordance whose executor is OmniRush.ai. Use the exact id and arguments from omnirush_context. This reads backend or app state without navigation or window focus.",
      args: omnirushAffordanceRequestSchema.shape,
      async execute(rawArgs: unknown) {
        return JSON.stringify(await queryOmniRushAffordance(rawArgs), null, 2);
      },
    },
    omnirush_execute: {
