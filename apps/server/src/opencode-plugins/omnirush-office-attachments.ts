...
import {
  ...
  type XlsxSheetData,
  type ZipEntry,
} from "@omnirush/workbook";
import { omniRushspaceFileForReading, omniRushspaceFileForWriting } from "./workspace-file-identity.js";
...
const MATERIALIZED_DIR = join(".opencode", "omnirush", "inbox", "chat-attachments");
...
async function readWorkspaceFile(realRoot: string, path: string, label: string): Promise<Buffer> {
  ...
  const { handle, info } = await omniRushspaceFileForReading(realRoot, path, label);
  try {
...
async function writeMaterializedFile(realRoot: string, target: string, bytes: Buffer): Promise<void> {
  const { handle } = await omniRushspaceFileForWriting(realRoot, target, null, "Materialized Office attachment");
  try {
...
function normalizedText(part: OfficeFilePart, materialized: MaterializedAttachment | null, extractedText: string, error?: string): string {
  return [
    "OmniRush.ai normalized an Office attachment before sending this request to the model.",
...
export const OmniRushOfficeAttachments = async (factoryInput?: unknown) => {
