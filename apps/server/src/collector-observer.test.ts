...
function fakeArchive() {
  ...
  return { turns, turnCompleted: (_sessionId: string, messages: unknown) => { turns.push(messages as unknown[] | null); } };
}

function observe(input: { collector: ObservedCollector; archive: ReturnType<typeof fakeArchive>; observers: SessionObservers; target: EngineTarget; timing: Partial<ObserverTiming> }) {
...
describe("collector observer", () => {
  ...
  test("the server stopping mid-turn ends the observation quietly and leaves the checkpoint where it was", async () => {
    ...
    await observe({ collector, archive: fakeArchive(), observers, target: engine.target, timing: clock.timing });
