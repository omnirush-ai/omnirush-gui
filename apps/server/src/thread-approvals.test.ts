...
async function serverConfig(): Promise<ServerConfig> {
  const root = await mkdtemp(join(tmpdir(), "omnirush-thread-approvals-"));
...
describe("thread approval replayer", () => {
  ...
  test("ignores directories OmniRush.ai does not own and reconnects after the stream drops", async () => {
    const config = await serverConfig();
    const engine = fakeEngine();
    cleanups.push(engine.stop);
    const replayer = startThreadApprovalReplayer({
