...
export function resolveModelProviderDisplayName(
  ...
  _reportedModelName?: string | null,
) {
...
export function dedupeModelOptions<T extends Pick<ModelOption, "providerID" | "modelID">>(
  options: readonly T[],
): T[] {
  const result: T[] = [];
  ...
  for (const option of options) {
    const key = `${option.providerID.trim().toLowerCase()}\u0000${option.modelID.trim().toLowerCase()}`;
...
const STARTUP_PREF_KEY = "omnirush.startupPref";
const LEGACY_PREF_KEY = "omnirush.modePref";
const LEGACY_PREF_KEY_ALT = "omnirush_mode_pref";
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.omnirushHostUrl ?? ""}`.toLowerCase();
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(omnirush|opencode)/i, // macOS app bundle identifiers
  /\.omnirush\.dev\//i, // omnirush.ai dev paths
