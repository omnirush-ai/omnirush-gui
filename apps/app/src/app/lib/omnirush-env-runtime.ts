const PENDING_CHANGES_KEY = "omnirush.settings.environment.pendingChanges";
...
export function buildOmniRushEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "omnirush"}::${runtime || "runtime"}`;
}

export function readOmniRushEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeOmniRushEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
