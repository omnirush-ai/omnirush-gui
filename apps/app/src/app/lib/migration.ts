...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^omnirush\.react\.activeWorkspace$/,
  /^omnirush\.react\.sessionByWorkspace$/,
  /^omnirush\.server\.list$/,
  /^omnirush\.server\.active$/,
  /^omnirush\.server\.urlOverride$/,
  /^omnirush\.server\.token$/,
];
...
function electronMigrationBridge(): ElectronMigrationBridge | null {
  ...
  const bridge = (window as unknown as {
    __OMNIRUSH_ELECTRON__?: { migration?: ElectronMigrationBridge };
...
export const MIGRATION_DEFER_KEY = "omnirush.migration.deferredUntil";
