...
`apps/app` is a React 19 + Vite app. It is the UI for every OmniRush.ai
deployment: the Electron desktop shell loads it, plain web serves it, and it
talks to omnirush-server / opencode / Den over HTTP. (The Solid runtime it
...
│   ├── lib/                   Clients + bridges: opencode, omnirush-server, den, desktop (IPC),
...
2. Leaf modules (`runtime-env`, `desktop-types`, `den-types`, `extensions`)
   ...
   (`opencode`, `omnirush-server`, `den`) import leaves — never the `utils/`
...
- The legacy `omnirush.react.activeWorkspace` and
  `omnirush.react.sessionByWorkspace` values are only restore/fallback memory.
...
`shell/omnirush-context-projector.ts` combines route state, the workbench store,
UI chrome state, and panel-tab state into the shared
`OmniRushContextSnapshot` contract. `OmniRushContextPublisher` publishes that
snapshot through `window.__omnirushControl`; the desktop loopback bridge and
`omnirush-ui-mcp` expose the same contract without requiring every element to
