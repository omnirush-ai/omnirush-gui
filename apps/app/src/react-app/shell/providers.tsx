...
import { hydrateOmniRushServerSettingsFromEnv } from "@/app/lib/omnirush-server";
...
function resolveDefaultServerUrl(): string {
  ...
  const omnirushUrl =
    typeof import.meta.env?.VITE_OMNIRUSH_URL === "string"
      ...
      : "";
  if (omnirushUrl) {
    return `${omnirushUrl.replace(/\/+$/, "")}/opencode`;
  }
...
export function AppProviders({ children }: AppProvidersProps) {
  ...
  useEffect(() => {
    // Start the dev observability forwarder. Reads the current omnirush-server
    // URL on every flush so reconnects after port changes still work. In prod
    // builds `startDebugLogger` is a no-op.
    startDebugLogger({
      serverUrl: async () => (await resolveOmniRushConnection()).normalizedBaseUrl,
    });
    return () => {
      stopDebugLogger();
    };
