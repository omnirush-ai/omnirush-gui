...
import type { OmniRushAffordanceRequest } from "@omnirush/types/omnirush-affordance";
import {
  createOmniRushServerClient,
  type OmniRushUiControlRequest,
} from "../../../app/lib/omnirush-server";
import { resolveOmniRushConnection } from "../omnirush-connection";
import type { OmniRushControlAPI } from "./control-provider";

const wait = (ms: number) => new Promise<void>((resolve) => window.setTimeout(resolve, ms));

function hasAffordanceId(input: unknown): input is OmniRushAffordanceRequest {
  return input !== null
    && typeof input === "object"
    && "id" in input
...
export function useUiControlMailbox(apiRef: RefObject<OmniRushControlAPI | null>): void {
  useEffect(() => {
