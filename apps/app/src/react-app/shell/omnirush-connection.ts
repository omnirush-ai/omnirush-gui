...
import { isWebDeployment } from "../../app/lib/omnirush-deployment";
import { omnirushServerInfo, type OmniRushServerInfo } from "../../app/lib/desktop";
import { isDesktopRuntime } from "../../app/utils";

export type OmniRushConnectionSource = "desktop-runtime" | "stored-settings" | "same-origin" | "gateway" | "empty";

export type ResolvedOmniRushConnection = {
  normalizedBaseUrl: string;
  resolvedToken: string;
  resolvedHostToken: string;
  hostInfo: OmniRushServerInfo | null;
  source: OmniRushConnectionSource;
};
...
export function isStaleStoredDesktopConnection(input: {
  ...
  return (
    isLoopbackOmniRushServerUrl(input.storedBaseUrl) ||
 ...
 */
export async function resolveOmniRushConnection(): Promise<ResolvedOmniRushConnection> {
  const gatewayOrigin = getOmniRushGatewayOrigin();
  if (gatewayOrigin) {
    return {
      normalizedBaseUrl: normalizeOmniRushServerUrl(gatewayOrigin) ?? "",
      resolvedToken: readOmniRushGatewayDenToken(),
      resolvedHostToken: "",
      hostInfo: null,
      source: "gateway",
    };
  ...
  if (isDesktopRuntime()) {
    try {
      ...
      const normalizedBaseUrl =
        normalizeOmniRushServerUrl(info.baseUrl ?? info.connectUrl ?? info.lanUrl ?? info.mdnsUrl ?? "") ??
        "";
  ...
  const settings = readOmniRushServerSettings();
  const normalizedBaseUrl = normalizeOmniRushServerUrl(settings.urlOverride ?? "") ?? "";
  const sameOriginBaseUrl =
    !normalizedBaseUrl && !isDesktopRuntime() && isWebDeployment() && typeof window !== "undefined"
      ? normalizeOmniRushServerUrl(window.location.origin) ?? ""
      : "";
  const resolvedToken = settings.token?.trim() ?? "";
  const resolvedHostToken =
    normalizedBaseUrl && isLoopbackOmniRushServerUrl(normalizedBaseUrl)
