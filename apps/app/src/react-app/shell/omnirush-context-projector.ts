...
import type {
  OmniRushConversationLayout,
  OmniRushContextSnapshot,
  OmniRushPanelTab,
  OmniRushResourceDescriptor,
  OmniRushScreen,
} from "@omnirush/types/omnirush-context";
...
type OmniRushContextProjectorInput = {
  route: string;
  revision: number;
  capturedAt: string;
  ...
  availableAffordances: OmniRushAffordanceDescriptor[];
};
...
export function screenFromRoute(route: string): OmniRushScreen {
...
function panelTab(tab: PanelTabStore["sessions"][string]["tabs"][number]): OmniRushPanelTab {
  if (tab.type === "browser") {
    return {
      id: tab.id,
      kind: "browser",
...
export function buildOmniRushContext(
  input: OmniRushContextProjectorInput,
): OmniRushContextSnapshot {
  const primary = input.workbench.primary;
  const secondary = input.workbench.secondary;
  const layout: OmniRushConversationLayout = primary && secondary
    ? {
        kind: "split",
  ...
  const provider: OmniRushProviderRef = { id: "omnirush-ui", kind: "builtin" };
