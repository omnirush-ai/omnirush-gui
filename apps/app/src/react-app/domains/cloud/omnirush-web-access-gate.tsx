...
import { isOmniRushGatewayRuntime } from "@/app/lib/gateway-runtime";
...
import {
  ...
  type OmniRushWebAccessCheck,
  type OmniRushWebAccessGateState,
} from "./omnirush-web-access-state";
...
export function OmniRushWebAccessGateScreen(props: {
  state: Exclude<OmniRushWebAccessGateState, "inactive" | "granted">;
  ...
  return (
    <main
      ...
      data-testid="omnirush-web-access-gate"
...
export function OmniRushWebAccessGate({ children }: { children: ReactNode }) {
  const denAuth = useDenAuth();
  const platform = usePlatform();
  const gatewayMode = isOmniRushGatewayRuntime();
  ...
  const expectedScope =
    ...
    && verifiedOrganizationId === organizationId
      ...
      : null;
  const [check, setCheck] = useState<OmniRushWebAccessCheck | null>(null);
  ...
  useEffect(() => {
    ...
    void client.getOmniRushWebAccess(organizationId).then(
      (access) => {
        if (cancelled) return;
        setCheck({
          scope: expectedScope,
  ...
  const state = resolveOmniRushWebAccessGateState({
    gatewayMode,
