...
export type OmniRushConnectStateStatus = "available" | "missing" | "invalid" | "unreadable";

export type OmniRushConnectStateSummary = {
...
export function resolveOmniRushConnectStateSummary(
  status: OmniRushConnectStateStatus,
  connectEnabled: boolean,
): OmniRushConnectStateSummary {
  if (status === "missing") {
    return {
...
export type OmniRushConnectStatus = {
  state: "checking" | "ready" | "needs_attention";
  label: "Checking" | "Ready" | "Needs attention";
  description: string;
};

export function omniRushConnectAttentionTitle(description: string): string {
  return `One possible issue: ${description}`;
}

export function resolveOmniRushConnectStatus(
  signedIn: boolean,
  maintenance: SessionCloudMcpMaintenanceState | undefined,
): OmniRushConnectStatus | null {
