...
export function ComputerUseControls() {
  ...
  const state = useQuery({ queryKey: ["computer-use", "host"], enabled: Boolean(window.__OMNIRUSH_ELECTRON__?.invokeDesktop),
    queryFn: async () => {
      const invoke = window.__OMNIRUSH_ELECTRON__?.invokeDesktop;
