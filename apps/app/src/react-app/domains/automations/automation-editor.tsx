...
import type { AutomationExecutionTarget, AutomationSchedule, CreateAutomation } from "@omnirush/types/automations"
...
import { AUTOMATION_INTERNAL_MODEL, automationPickerOptions, describeAutomationModel } from "./automation-model-options"
...
function defaultInput(modelOptions: readonly AutomationModelOption[]): CreateAutomation {
  const first = modelOptions[0] ?? AUTOMATION_INTERNAL_MODEL
  return {
    name: "",
    instructions: "",
    ...
    >
      ...
      <div className="rounded-xl border border-border bg-muted/30 p-3 text-sm text-muted-foreground" data-automation-placement={props.placement}>
        {cloud
          ...
          : "Runs on your desktop computer. Keep OmniRush.ai open, signed in, and connected at the scheduled time. If your desktop stays unavailable, the run is marked as missed."}
      </div>
