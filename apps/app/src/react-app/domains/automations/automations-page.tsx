...
import { AUTOMATION_INTERNAL_MODEL, type AutomationModelOption, type AutomationProviderCatalog } from "./automation-model-options"
...
function inputDefaults(models: readonly AutomationModelOption[]): CreateAutomation {
  const first = models[0] ?? AUTOMATION_INTERNAL_MODEL
  return {
    name: "",
    instructions: "",
...
export function AutomationsPage(props: { providerCatalog?: AutomationProviderCatalog; workspaceId?: string | null } = {}) {
  ...
  const internalStarterInRuntime = props.providerCatalog === undefined || Boolean(
    props.providerCatalog[AUTOMATION_INTERNAL_MODEL.providerId]?.[AUTOMATION_INTERNAL_MODEL.modelId],
  )
  ...
  // Desktop can use the internal proxy starter directly; Cloud runs use the
  // organization's managed provider inventory.
  const models = useMemo(
    () => automationModelOptions(providersQuery.data ?? [], {
      includeInternalStarter: placement === "desktop" && internalStarterInRuntime,
    }),
    [internalStarterInRuntime, placement, providersQuery.data],
  )
