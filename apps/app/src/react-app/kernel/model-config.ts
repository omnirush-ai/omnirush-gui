...
export const storedDefaultModelChangedEvent = "omnirush.defaultModelChanged";
...
const parseStoredModel = (value: unknown) => {
  ...
  if (
    ...
    return isSupportedModelProvider(record.providerID)
  ...
  try {
    ...
    for (const [sessionId, value] of Object.entries(parsed)) {
      if (typeof value === "string") {
        ...
        if (model && isSupportedModelProvider(model.providerID)) next[sessionId] = { model };
        continue;
      }
