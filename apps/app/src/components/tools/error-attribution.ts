...
import { connectionActionPayloadSchema, type ConnectionActionPayload } from "@omnirush/types/connection-action-app"
...
const OMNIRUSH_CLOUD_CAPABILITY_TOOLS = new Set([
  "omnirush-cloud_search_capabilities",
  "omnirush-cloud_execute_capability",
  "omnirush-cloud_connection_action",
])
...
export function connectionCardPayloadFromChatToolResult(
  ...
  if (toolName === "omnirush-cloud_search_capabilities" && (!isRecord(input) || input.intent !== "connect")) return null
  ...
  for (const candidate of candidates) {
    const payload = connectionActionPayloadSchema.safeParse(
      isRecord(candidate) && omnirushCloudMcpConnectionActionSchema.safeParse(candidate).success
...
export function reconnectActionFromChatToolResult(
  ...
  if (candidates.length === 0) {
    ...
    if (!payload || payload.actor !== "member" || payload.action?.surface !== "omnirush_your_connections"
  ...
  for (const connectionStatus of candidates) {
    const parsedStatus = omnirushCloudMcpConnectionActionSchema.safeParse(connectionStatus)
    if (!parsedStatus.success) continue
    const status = parsedStatus.data
    if (status.authType !== "oauth" || status.credentialMode !== "per_member" || status.actor !== "member"
      || status.action.surface !== "omnirush_your_connections"
