 ...
 */
export function SubagentRunLine({ part, className, parentActive = true, parentLastProgressAt }: SubagentRunLineProps) {
  ...
  const progressAnchor = child
    ? Math.max(child.runStartedAt || 0, child.lastProgressAt)
    : parentLastProgressAt ?? startedAt ?? 0
  const noNewActivity = hasNoNewActivity({
    active: inFlight, lastProgressAt: progressAnchor, now: Date.now(),
  })
