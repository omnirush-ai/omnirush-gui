...
const CODE_COPY_ICON = `<svg data-omnirush-code-copy-icon="" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-3.5 w-3.5" aria-hidden="true"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>`;
const CODE_COPIED_ICON = `<svg data-omnirush-code-copy-check-icon="" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-3.5 w-3.5" aria-hidden="true" hidden><path d="M20 6 9 17l-5-5"/></svg>`;
...
function codeCopyButton() {
  return `<button type="button" data-omnirush-code-copy="" class="absolute right-2 top-2 z-10 inline-flex h-7 w-7 items-center justify-center rounded-md border border-border/70 bg-background/95 text-muted-foreground shadow-sm transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Copy code block" title="Copy code block">${CODE_COPY_ICON}${CODE_COPIED_ICON}<span data-omnirush-code-copy-label="" class="sr-only" aria-live="polite">Copy code block</span></button>`;
}

function codeWrapButton() {
  return `<button type="button" data-omnirush-code-wrap="" class="absolute right-11 top-2 z-10 inline-flex h-7 w-7 items-center justify-center rounded-md border border-border/70 bg-background/95 text-muted-foreground shadow-sm transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Enable word wrap" aria-pressed="false" title="Enable word wrap">${CODE_WRAP_ICON}</button>`;
}

function chatCodeBlockContainer(html: string, shiki: boolean) {
  const shikiAttribute = shiki ? ` data-omnirush-shiki="true"` : "";

  return `<div data-omnirush-code-block=""${shikiAttribute} class="relative my-4 overflow-hidden rounded-[18px] border border-border/70 bg-gray-2/60 font-mono text-xs leading-6 text-foreground">${codeWrapButton()}${codeCopyButton()}${html}</div>`;
}

function chatCodeBlockHtml(text: string, lang: string | undefined) {
  return chatCodeBlockContainer(
    `<pre data-omnirush-code-scroll="" class="overflow-x-auto px-4 pb-3 pt-11"><code${codeLanguageClass(lang)}>${escapeHtml(text)}</code></pre>`,
    false,
  );
}
...
function mermaidBlockHtml(text: string, presentation: MarkdownPresentation) {
  ...
  return `<div data-omnirush-mermaid="" data-omnirush-mermaid-state="source" aria-busy="false" class="my-4 overflow-hidden rounded-[18px] border ${borderClass} ${backgroundClass}"><div class="flex min-h-10 items-center gap-2 border-b ${borderClass} px-3 py-2"><span class="me-auto text-xs font-medium text-muted-foreground">Mermaid diagram</span><div role="group" aria-label="Diagram view" class="flex items-center gap-1"><button type="button" data-omnirush-mermaid-view="rendered" class="${buttonClass}" aria-pressed="false" disabled>Rendered</button><button type="button" data-omnirush-mermaid-view="source" class="${buttonClass}" aria-pressed="true">Source</button><button type="button" data-omnirush-mermaid-download="" class="${buttonClass}" aria-label="Download diagram as SVG" hidden>Download SVG</button></div></div><div data-omnirush-mermaid-rendered="" class="overflow-auto p-4 [&amp;&gt;svg]:mx-auto [&amp;&gt;svg]:h-auto [&amp;&gt;svg]:max-w-full" hidden></div><pre data-omnirush-mermaid-source="" class="overflow-x-auto p-4 text-xs leading-6 text-foreground"><code class="language-mermaid">${escapeHtml(text)}</code></pre><p data-omnirush-mermaid-status="" class="border-t ${borderClass} px-3 py-2 text-xs text-muted-foreground" aria-live="polite">Diagram source</p></div>`;
}
...
export function syncMarkdownImagePreviews(root: HTMLElement) {
  const previews = root.querySelectorAll("[data-omnirush-image-preview]");
...
export function setCodeCopyButtonState(button: HTMLButtonElement, copied: boolean) {
  const label = button.querySelector("[data-omnirush-code-copy-label]");
  if (label) label.textContent = copied ? "Code block copied" : "Copy code block";

  button.querySelector("[data-omnirush-code-copy-icon]")?.toggleAttribute("hidden", copied);
  button.querySelector("[data-omnirush-code-copy-check-icon]")?.toggleAttribute("hidden", !copied);
...
export function setCodeWrapButtonState(button: HTMLButtonElement, wrapped: boolean) {
  const codeBlock = button.closest("[data-omnirush-code-block]");
  const pre = codeBlock?.querySelector("pre");

  for (const [className, enabled] of Object.entries(codeWrapClassStates(wrapped))) {
    for (const container of codeBlock?.querySelectorAll("[data-omnirush-code-scroll]") ?? []) {
...
function sanitizeMarkdownHtml(value: string) {
  ...
  return DOMPurify.sanitize(value, {
    ...
    ADD_ATTR: [
      ...
      "checked",
      "class",
      "data-omnirush-math-error",
      "data-omnirush-code-block",
      "data-omnirush-code-copy",
      "data-omnirush-code-copy-check-icon",
      "data-omnirush-code-copy-icon",
      "data-omnirush-code-copy-label",
      "data-omnirush-code-scroll",
      "data-omnirush-code-wrap",
      "aria-label",
      "aria-pressed",
      "data-omnirush-image-preview",
      "data-omnirush-inline-code-path",
      "data-omnirush-link-href",
      "data-omnirush-link-chevron",
      "data-omnirush-shiki",
      "decoding",
      "disabled",
      "hidden",
...
function markdownProfileForPresentation(presentation: MarkdownPresentation): MarkdownProfile {
  if (presentation === "surface") {
    return {
      ...
      shikiContainer: `<div data-omnirush-shiki="true" class="my-4 overflow-x-auto rounded-[18px] border border-dls-border/70 bg-gray-1/80 p-4 text-xs leading-6">%s</div>`,
  ...
  return {
    ...
    shikiContainer: chatCodeBlockContainer(`<div data-omnirush-code-scroll="" class="overflow-x-auto px-4 pb-3 pt-11">%s</div>`, true),
...
function renderVideo(href: string, label: string) {
  ...
  const fileLink = remote
    ...
    : `href="#" data-omnirush-inline-code-path="${escapeAttribute(href)}"`;
  return `<span class="my-4 inline-block w-full max-w-lg align-top"><video data-omnirush-video-path="${escapeAttribute(href)}"${source} controls playsinline preload="metadata" aria-label="${escapeAttribute(label)}" class="block max-h-80 w-full rounded-lg border border-border/70 bg-black"></video><span data-omnirush-video-error="" hidden class="text-sm text-muted-foreground">Video preview unavailable. Open the file to play it.</span><a ${fileLink} class="text-sm text-indigo-10">${escapeHtml(label)}</a></span>`;
}
...
function renderLink(profile: MarkdownProfile, href: string, title: string | null | undefined, text: string) {
  ...
  if (profile.linkPresentation === "chat") {
    ...
    if (isFilePath) {
      ...
      return `<span class="inline-flex items-stretch overflow-hidden rounded-md border border-border/60 bg-muted/40 text-xs font-medium text-foreground align-middle"><a href="${safe}" data-omnirush-link-href="${originalHref}"${titleAttr} target="_blank" rel="noreferrer noopener" class="inline-flex items-center gap-1 px-1.5 py-0.5 no-underline transition-colors hover:bg-muted">${fileIcon}${text}</a><button type="button" data-omnirush-link-chevron="${originalHref}" class="inline-flex items-center border-l border-border/60 px-1 transition-colors hover:bg-muted" aria-label="Open with">${chevron}</button></span>`;
    }
    ...
    return `<a href="${safe}" data-omnirush-link-href="${originalHref}"${titleAttr} target="_blank" rel="noreferrer noopener" class="text-indigo-10 no-underline transition-colors hover:text-indigo-8">${faviconHtml}${text}</a>`;
  }
...
function renderImage(profile: MarkdownProfile, href: string, title: string | null | undefined, text: string) {
  ...
  if (profile.imagePresentation === "chat") {
    return `<button type="button" data-omnirush-image-preview="" class="my-4 inline-block max-w-full cursor-zoom-in align-top text-left transition-opacity hover:opacity-90" aria-label="Expand ${escapeAttribute(text)}"><img src="${safe}" alt="${escapeAttribute(text)}"${titleAttr} loading="lazy" decoding="async" class="block h-auto w-auto rounded-lg border border-border/70 object-contain" style="max-height: ${MARKDOWN_IMAGE_PREVIEW_MAX_HEIGHT}px; max-width: ${MARKDOWN_IMAGE_PREVIEW_MAX_WIDTH}px"></button>`;
  }
...
function createMarkedOptions(profile: MarkdownProfile, presentation: MarkdownPresentation, isAsync: boolean) {
  return {
    ...
    renderer: {
      html({ text }) {
        return profile.rawHtmlMode === "shiki-only" && !text.includes('data-omnirush-shiki="true"') ? "" : text;
      },
      paragraph({ tokens }) {
      ...
      codespan({ text }) {
        ...
        const pathAttributes = path
          ? ` data-omnirush-inline-code-path="${escapeAttribute(path)}" role="button" tabindex="0" aria-label="Open ${escapeAttribute(path)}"`
          : "";
