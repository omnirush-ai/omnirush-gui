    ...
    value: {
      ...
      __OMNIRUSH_ELECTRON__: {
        invokeDesktop: async (command: string) => {
          ...
          if (command === "omnirushServerRestart") {
...
function createStore() {
  return createOmniRushServerStore({
    ...
    selectedWorkspaceDisplay: () => ({
      id: "workspace_local",
      name: "Local",
      path: "/tmp/omnirush-restart-guard",
      preset: "starter",
      workspaceType: "local",
    }),
...
describe("local omnirush server recovery restart guard", () => {
  test("a fresh store with a healthy live server never restarts it", async () => {
    ...
    const client = await store.ensureLocalOmniRushServerClient();
