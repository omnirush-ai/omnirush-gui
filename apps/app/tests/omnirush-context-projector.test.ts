...
import {
  buildOmniRushContext,
  screenFromRoute,
} from "../src/react-app/shell/omnirush-context-projector";
import type { WorkbenchSnapshot } from "../src/react-app/domains/session/chat/workbench-store";

type ContextProjectorInput = Parameters<typeof buildOmniRushContext>[0];
...
describe("OmniRush.ai context projector", () => {
  test("projects the focused split session and its panel state", () => {
    const context = buildOmniRushContext(baseInput);
  ...
  test("does not leak artifact tabs into a non-panel surface", () => {
    const context = buildOmniRushContext({
      ...baseInput,
      ui: {
        ...baseInput.ui,
    ...
    panelSessions: {
      "session-a": {
        tabs: [{
          id: "browser-one",
          type: "browser",
          label: "OmniRush.ai docs",
          url: "https://docs.omnirush.so",
          favicon: null,
          status: "ready",
          canGoBack: false,
          canGoForward: false,
