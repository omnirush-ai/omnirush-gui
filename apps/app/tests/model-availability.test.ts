...
describe("computeModelAvailability", () => {
  ...
  test("cloud sync while OmniRush.ai Models is reconciling stays pending", () => {
    const cloudModel = { providerID: "omnirush", modelID: "gpt-5.5" };
    expect(
      computeModelAvailability(
        cloudModel,
        context({ signedIn: true, cloudProviderSyncReady: true, omniRushModelsSyncing: true }),
      ),
    ).toEqual({ status: "pending" });
  });
