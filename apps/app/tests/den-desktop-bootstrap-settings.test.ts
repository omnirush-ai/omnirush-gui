...
describe("desktop Den bootstrap settings", () => {
  ...
  test("reads the desktop base URL from bootstrap instead of stale localStorage", async () => {
    window.localStorage.setItem("omnirush.den.baseUrl", "https://stale.example.com");
    window.localStorage.setItem("omnirush.den.apiBaseUrl", "https://api.example.com");
  ...
  test("saves base URL changes to bootstrap and clears legacy endpoint storage", async () => {
    ...
    expect(window.localStorage.getItem("omnirush.den.baseUrl")).toBeNull();
    expect(window.localStorage.getItem("omnirush.den.apiBaseUrl")).toBeNull();
  ...
  test("session or server changes invalidate configured Cloud MCP token markers", async () => {
    ...
    window.localStorage.setItem(CLOUD_MCP_SYNC_MARKER_STORAGE_KEY, "stale-marker");
    window.localStorage.setItem("omnirush.react.dashboardTileCache.v1.user_alice.org_ops", "private report");
    window.localStorage.setItem("omnirush.react.dashboardTileCache.v1.user_bob.org_finance", "private forecast");
    window.localStorage.setItem("unrelated.preference", "keep me");
    clearDenSession();
    expect(window.localStorage.getItem(CLOUD_MCP_SYNC_MARKER_STORAGE_KEY)).toBeNull();
    expect(window.localStorage.getItem("omnirush.react.dashboardTileCache.v1.user_alice.org_ops")).toBeNull();
    expect(window.localStorage.getItem("omnirush.react.dashboardTileCache.v1.user_bob.org_finance")).toBeNull();
