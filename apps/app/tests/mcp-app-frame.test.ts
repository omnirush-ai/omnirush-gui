...
function fixture(overrides: Partial<OmniRushMcpAppResource> = {}): OmniRushMcpAppResource {
  return {
    serverName: "fixture",
    toolName: "render",
...
describe("MCP App iframe policy", () => {
  test("connection status execution renders the native card even without preserved app metadata", () => {
    const part: DynamicToolUIPart = {
      type: "dynamic-tool", toolName: "omnirush-cloud_execute_capability", toolCallId: "status-probe",
      ...
      output: { schemaVersion: "1", connectionId: "emc_notes", connectionName: "Notes", state: "needs_connection",
        ...
        action: { type: "connect", label: "Connect Notes", surface: "omnirush_your_connections" } },
    }
  ...
  test("an unsupported first-party connection launch cannot fall back to the legacy iframe", () => {
    const part: DynamicToolUIPart = {
      type: "dynamic-tool", toolName: "omnirush-cloud_execute_capability", toolCallId: "old-status-probe",
      state: "output-available", input: {}, output: {},
      callProviderMetadata: { omnirush: { mcpResult: { content: [], _meta: { "omnirush/mcpApp": {
        toolName: "connection_action", resourceUri: "ui://omnirush/connection-action/v1/view.html", arguments: { connectionId: "emc_notes" },
      } } } } },
    }
  ...
  test("accepts a same-server generated App launch without a connection reference", () => {
    expect(gatewayMcpAppLaunch({
      "omnirush/mcpApp": {
        ...
        resourceUri: "ui://omnirush/artifacts/atlas/views/1/index.html",
  ...
  test("uses the opaque message origin for packaged file hosts", () => {
    ...
    const client = createOmniRushServerClient({ baseUrl: "http://localhost:61856" })
  ...
  test("keeps ordinary tools silent while surfacing advertised resource failures", () => {
    expect(isActionableMcpAppResolutionError(new OmniRushServerError(503, "mcp_unreachable", "offline"))).toBe(false)
    expect(isActionableMcpAppResolutionError(new OmniRushServerError(404, "resource_read_failed", "missing"))).toBe(true)
  ...
  test("formats safe, copyable handshake diagnostics", () => {
    const details = formatMcpAppDiagnostic({
      ...
      resourceUri: "ui://omnirush/artifacts/arv_1/views/avr_2/index.html",
    ...
    expect(details).toContain("Resource: ui://omnirush/artifacts/arv_1/views/avr_2/index.html")
...
test("only canonical completed gateway search results render connector setup suggestions", () => {
  ...
  const part = { type: "dynamic-tool", toolName: "omnirush-cloud_search_capabilities", toolCallId: "catalog", state: "output-available", input: { query: "Slack", intent: "connect" }, output: JSON.stringify({ connectorCatalog: catalog }) } satisfies import("ai").DynamicToolUIPart;
