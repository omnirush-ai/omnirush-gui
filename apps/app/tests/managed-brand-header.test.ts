...
describe("managed brand header", () => {
  test("shows a wordmark when supplied and a branded fallback otherwise", () => {
    const source = readFileSync(sidebarPath, "utf8");

    expect(source).toMatch(/\{brandLogoUrl \? \([\s\S]*?data-testid="brand-logo"[\s\S]*?<img/);
    expect(source).toContain("useBrandAppName");
    expect(source).toContain("data-sidebar-brand");
    expect(source).toContain("title={brandAppName}>{brandAppName}</span>");
