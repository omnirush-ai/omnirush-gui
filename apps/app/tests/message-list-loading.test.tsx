...
const task: TaskToolPart = {
  ...
  callProviderMetadata: { omnirush: { childSessionId: "child" } },
};
...
describe("task-linked meaningful progress", () => {
  ...
  test("busy polls, identical snapshots, user follow-ups and unrelated sessions do not reset silence", async () => {
    ...
    try {
      ...
      expect(html).not.toContain('data-subagent-activity="no-new-activity"');
      ...
      expect(container.innerHTML).not.toContain('data-subagent-activity="no-new-activity"');
