...
describe("Automation model options", () => {
  test("always offers the normalized internal starter model", () => {
  ...
  test("can omit the internal starter when only organization models are allowed", () => {
  ...
  test("expands the member's managed omnirush.ai aliases even when Den stores no model rows", () => {
    ...
    expect(options.some((option) => option.providerId === "lpr_member_omnirush")).toBe(false)
  })
  ...
  test("offers the runtime's reasoning levels for the selected model only", () => {
    ...
    const starter = picker.find((option) => option.modelID === "z-ai/glm-5.2")
    ...
    // The internal starter model is absent from the local catalog here, so it
    // still lists — just without reasoning levels.
    expect(starter?.isFree).toBe(false)
    expect(starter?.behaviorOptions).toEqual([])
  })
})
...
describe("Automation proposal model resolution", () => {
  ...
  test("defaults an omitted model to the internal starter model", () => {
    expect(resolveProposalModel(undefined, [])).toEqual({
      model: { providerId: "omnirush", modelId: "z-ai/glm-5.2", variant: null },
      resolution: "default",
    })
  })
