...
export async function onboardingWorld(seed: Seed) {
  const directory =
    process.env.OMNIRUSH_EVAL_FILM_DIR || seed.tmpPath("onboarding-film");
  const world = await signupWorkspace(seed, {
    filmDirectory: directory,
    viewport: { width: 1600, height: 940 },
  });
  world.owner.name = "Alex";
  world.owner.email = "alex@omnirush.test";
  world.owner.password = "OmniRush.ai-demo-9274!";
...
export async function desktopOnboardingWorld(seed: Seed) {
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (message.method === "Page.frameRequestedNavigation" && typeof params.url === "string" && params.url.startsWith("omnirush://")) {
