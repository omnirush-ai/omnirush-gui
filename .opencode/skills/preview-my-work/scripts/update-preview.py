...
receipt_dir = Path(os.environ.get("OMNIRUSH_WORLD_SNAPSHOT_DIR", root / "evals/results/.worlds/scripts"))
...
if web:
    for proc in Path("/proc").iterdir():
        ...
        try:
            ...
            allowed = {"PATH", "HOME", "PNPM_HOME", "DEN_WEB_PORT", "DEN_BASE_URL", "DEN_API_BASE", "DEN_AUTH_ORIGIN", "DEN_AUTH_FALLBACK_BASE", "NEXT_PUBLIC_OMNIRUSH_AUTH_CALLBACK_URL", "DEN_ORG_MODE", "OMNIRUSH_DEV_MODE", "DEN_WEB_ALLOWED_DEV_ORIGINS"}
...
with open("/tmp/preview-update.log", "w") as log:
    ...
    if web:
        subprocess.run(["pnpm", "--filter", "@omnirush-ee/den-web", "build"], cwd=root, env=env, stdout=log, stderr=subprocess.STDOUT, check=True)
if web:
    ...
    with open("/tmp/den-web.log", "a") as log:
        subprocess.Popen(["pnpm", "--filter", "@omnirush-ee/den-web", "exec", "next", "start", "--hostname", "0.0.0.0", "--port", "3005"], cwd=root, env=env, stdout=log, stderr=subprocess.STDOUT, start_new_session=True)
