...
journey. Pass/fail evidence comes from an `@omnirush/testkit` spec and its
...
imports `test` from `@omnirush/testkit`; screenshots and validation claims are
...
```text
omnirush-eval-artifacts:/daytona-artifacts
```
