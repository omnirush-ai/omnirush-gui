...
const paths = {
  ...
  models: path.resolve("ee/apps/inference/src/models/omnirush-models.json"),
  aliases: path.resolve("packages/types/src/den/inference.ts"),
}

function usage() {
  console.log(`Usage:
  omnirush-models.mjs search <query>
  omnirush-models.mjs add <query-or-id>
  omnirush-models.mjs remove <id> [<id2>]
  omnirush-models.mjs discount <usageFactor> <id> [<id2>]
  omnirush-models.mjs sync
  omnirush-models.mjs validate`)
}
...
async function readOmniRushModels() {
...
function resolveExactOmniRushIds(omnirushModels, ids) {
  const resolved = []
  for (const id of ids) {
    if (omnirushModels[id]) {
      resolved.push(id)
      continue
    }

    const matches = findMatches(omnirushModels, id)
    console.error(`OmniRush.ai model "${id}" was not exact. Please clarify with exact id:`)
    displayMatches(matches)
    process.exitCode = 2
    return null
  }
...
async function syncAliases(models, usageOverrides = new Map()) {
  ...
  const entries = Object.entries(models).map(([id, model]) => {
    ...
    const displayName = `OmniRush.ai: ${model.name}`
...
async function validate() {
  const models = await readOmniRushModels()
  ...
  console.log(`validated ${Object.keys(models).length} OmniRush.ai model(s)`)
}
...
async function remove(ids) {
  const models = await readOmniRushModels()
  const resolved = resolveExactOmniRushIds(models, ids)
  if (!resolved) return
...
if (command === "search") {
  ...
  await syncAll(await readOmniRushModels())
