...
OmniRush.ai discovers and authorizes Den-managed external MCP connections without
...
deployment. OmniRush.ai derives both public OAuth URLs exclusively from this
...
OmniRush.ai as a web OAuth client. Each self-hosted Den deployment has its own
callback and metadata URL.

## OAuth runtime and callback compatibility

Den uses `@omnirush/enterprise-mcp-client` for discovery, authorization,
...
In Cloud → Connections, enter the MCP server URL. OmniRush.ai automatically runs
...
When authorization starts, OmniRush.ai uses this registration priority:
...
challenge nor the administrator selects scopes, OmniRush.ai falls back to the
...
- **Additional permission required**: review and approve the newly challenged
  scope before reconnecting; OmniRush.ai does not silently expand access.
