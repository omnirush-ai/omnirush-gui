...
OmniRush.ai supports MCP Apps delivered by standard MCP servers connected through
OmniRush.ai Connect. A server advertises the stable
...
per-member credentials, and tool policy. The OmniRush.ai Cloud control server
publishes a member-scoped resource at:

```text
omnirush://connect/mcp-servers/index.json
```
...
an audience or capability header. Desktop never writes `omnirush-connect-*`
...
central `omnirush-cloud` `search_capabilities` and `execute_capability` tools.
...
OmniRush.ai access grants, disabled-tool policy, and approval rules still apply at
...
metadata (`kind: mcp_app`, `mcpApp.resourceUri`, `omnirush/mcpApp` meta) is an
...
disconnects stale
`omnirush-connect-*` OpenCode entries while preserving user-authored MCPs and
...
- no `ui://omnirush/library-apps/...` resources are registered;
