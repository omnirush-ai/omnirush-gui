...
or host API access. The next engine never inherits the general OmniRush.ai client
credential; the legacy engine retains its existing credential for built-in tools.

The browser restriction applies to OmniRush.ai's built-in browser. Native fetch
...
with `OMNIRUSH_EVAL_ENGINE=v1` and `OMNIRUSH_EVAL_ENGINE=v2`, using the full feature
commit as `OMNIRUSH_EVAL_REF` and `OMNIRUSH_EVAL_DAYTONA_REF` for remote placement.
