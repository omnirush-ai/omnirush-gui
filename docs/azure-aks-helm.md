# Deploy OmniRush.ai EE on Azure with AKS and Helm

Status: self-host operator guide
Related: `packaging/helm/omnirush-ee`, `packaging/helm/omnirush-ee/examples/values.azure-ingress.yaml`

This is the recommended Azure path for a first production-like OmniRush.ai EE
...
add-on's managed NGINX Ingress controller so OmniRush.ai gets stable HTTPS host
...
production workloads through November 2026. The current OmniRush.ai chart emits
...
OmniRush.ai. Azure Load Balancer is a layer 4 load balancer. It is useful for TCP
...
identities, and network security groups. The OmniRush.ai Helm chart owns OmniRush.ai
...
The OmniRush.ai EE release artifact is already a Helm chart, and AKS gives the
...
not a different OmniRush.ai packaging format.
...
export RESOURCE_GROUP=omnirush-ee-rg
export AKS_CLUSTER=omnirush-ee
export VNET_NAME=omnirush-ee-vnet
...
database. This is an Azure subscription/region eligibility issue, not OmniRush.ai
quota usage.
...
subscription cannot use, not that OmniRush.ai has consumed quota. Pick an allowed
...
- Region capacity varies. If AKS Automatic reports SKU/capacity failures, retry
  in a known-good region before debugging the OmniRush.ai chart.
...
export MYSQL_ADMIN_USER=omnirush
...
indicate an Azure MySQL subscription/region eligibility issue before OmniRush.ai or
...
```text
mysql://omnirush:<password>@<server>.mysql.database.azure.com:3306/omnirush_den?sslaccept=accept
```

Use `?sslaccept=accept` for the simple private-MySQL smoke path. This keeps TLS
on without requiring a cloud CA bundle to be mounted into the OmniRush.ai image.
...
```bash
cp packaging/helm/omnirush-ee/examples/values.azure-ingress.yaml values.azure.yaml
```
...
helm template omnirush-ee oci://ghcr.io/omnirush-ai/charts/omnirush-ee \
  ...
  -f values.azure.yaml > /tmp/omnirush-rendered.yaml
...
ingress:
  tls:
    - secretName: omnirush-ee-tls
```

Create that secret from a certificate that covers both OmniRush.ai hosts:

```bash
kubectl create namespace omnirush-ee

kubectl create secret tls omnirush-ee-tls \
  --namespace omnirush-ee \
...
- `omnirush.example.com` -> the application routing public IP.
- `api.omnirush.example.com` -> the same application routing public IP.
...
**Identifier (Entity ID)** is the OmniRush.ai auth origin and the certificate saved
in OmniRush.ai is the active Entra token-signing certificate.
...
| Browser shows certificate warnings | TLS secret is missing, wrong, or does not cover both hosts | Recreate `omnirush-ee-tls` or configure the platform certificate automation |
