...
import type {
  ...
  PutV1AdminOrganizationsByOrganizationIdOmniRushWebAccessResponses,
...
export class DenClient extends HeyApiClient {
  ...
  public putV1AdminOrganizationsByOrganizationIdOmniRushWebAccess<ThrowOnError extends boolean = false>(
    parameters: {
      organizationId: string;
    },
    options?: Options<never, ThrowOnError>,
    ...
    return (options?.client ?? this.client).put<
      ...
      ThrowOnError
    >({
      url: "/v1/admin/organizations/{organizationId}/omnirush-web-access",
      ...options,
      ...params,
    });
  }
   ...
   *
   * Starts the active organization's OmniRush.ai Cloud browser instance when needed and returns its browser URL once ready.
   */
   ...
   *
   * Starts or wakes the caller's own OmniRush.ai Cloud browser instance when needed and returns the collaborator token only to the trusted gateway.
   */
  ...
  /**
   * Send current user the OmniRush.ai desktop download link
   *
   * Emails the authenticated user a link to download the OmniRush.ai desktop app.
   */
   ...
   * List Automations
   *
   * Den schedules Automations and keeps durable run history. Automations created by Desktop run on the owner's connected desktop; Automations created by Web run in OmniRush.ai Cloud. If no desktop runner is connected when a desktop occurrence is due, that occurrence is recorded as missed. Creation makes an Automation active immediately and uses the owner's current OmniRush.ai Connect integrations. Deactivation stops future runs but does not cancel a run already in progress.
   */
  ...
  /**
   * Create an active OmniRush.ai Cloud Automation
   *
   * Den schedules Automations and keeps durable run history. Automations created by Desktop run on the owner's connected desktop; Automations created by Web run in OmniRush.ai Cloud. If no desktop runner is connected when a desktop occurrence is due, that occurrence is recorded as missed. Creation makes an Automation active immediately and uses the owner's current OmniRush.ai Connect integrations. Deactivation stops future runs but does not cancel a run already in progress. This is the Web and Cloud Chat creation surface. Placement is fixed to OmniRush.ai Cloud and the Automation can wake a stopped Cloud container without a desktop. Create only when the person explicitly asks to create or schedule it; there is no draft step.
   */
  ...
  /**
   * Save a successful Code Mode run as a Workflow inside an OmniRush.ai Connect Plugin
   */
   ...
   *
   * Returns OmniRush.ai Models enablement and limit context for the active organization.
   */
   ...
   *
   * Enables or disables OmniRush.ai Models for the active organization.
   */
  ...
  /**
   * Read the OmniRush.ai Models task analytics choice
   */
  ...
  /**
   * Choose whether to collect task analytics included with OmniRush.ai Models
   */
  ...
  /**
   * Read provider-reported consumption for OmniRush.ai Models
   */
   ...
   *
   * Mints a shareable OmniRush.ai desktop install link for a signed-in organization member. Older active links remain valid unless an owner or admin explicitly requests rotation.
   */
  ...
  /**
   * Download current user's managed OmniRush.ai desktop
   *
  ...
  /**
   * Download managed OmniRush.ai desktop
   *
   ...
   *
   * Admin-only. Lets an org bring its own OAuth app (client id + secret) for a native provider such as google-workspace, instead of relying on an OmniRush.ai-owned client.
   */
   ...
   *
   * The provider redirects here with code+state after the member consents. Identity is carried entirely by the signed state token, not a session cookie, since the redirect may arrive in a fresh browser context. Serves a small static HTML page that deep-links back to OmniRush.ai.
   */
   ...
   *
   * Authenticated host transport for omnirush-cloud-uploads. The route immediately forwards the file to Google and does not persist it or expose its bytes to the model.
   */
   ...
   *
   * Authenticated host transport for omnirush-cloud-uploads. The route immediately creates the draft and does not persist attachment bytes or expose them to the model.
   */
   ...
   *
   * Creates a plain-text Gmail draft in the calling member own mailbox. For workspace attachments, use the omnirush-cloud-uploads gmail_create_draft_with_attachments action so file bytes stay outside model context. Set threadId for replies and forwards. Always share the returned draftUrl.
   */
   ...
   * List my library
   *
   * Lists the Workflows, Remote MCP Apps, plugins, and connections the caller can use, with every applicable access edge. Workflows and Remote MCP Apps remain config objects contained by their parent OmniRush.ai Connect Plugin.
   */
   ...
   *
   * Creates OmniRush.ai plugins and connector mappings from selected discovery candidates.
   */
   ...
   *
   * Returns connection tokens and the resolved OmniRush.ai connect URL for an existing worker.
   */
   ...
   *
   * Receives a batch of telemetry events from the OmniRush.ai app or workers. Auth provides org and member identity. Unknown event types and disallowed fields are dropped. Always returns 204.
   */
