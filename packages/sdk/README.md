...
`@omnirush/sdk` is a TypeScript client for the **Den cloud API**. It is a private
workspace package for now; this change does not publish it to npm. It does not
wrap the local OmniRush.ai server or the OpenCode engine API.
...
const den = createDenClient({
  ...
  // Defaults to https://api.omnirushlabs.com. Set this for a self-hosted Den.
...
organization key (`x-api-key`), and `orgId` for `x-omnirush-org-id`. Session-only
