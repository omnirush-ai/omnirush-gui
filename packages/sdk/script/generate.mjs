...
const temporary = await mkdtemp(join(tmpdir(), "omnirush-sdk-"));
...
try {
  ...
  execFileSync("pnpm", ["--filter", "@omnirush/mcp-apps", "build"], { cwd: repoDir, stdio: "inherit" });
  const input = join(temporary, "openapi.json");
  execFileSync("pnpm", ["--filter", "@omnirush-ee/den-api", "exec", "tsx", "--conditions=development",
