export const OmniRushConnectInstaller = () => {
  ...
  const CODEX_RECONNECT_COMMAND = `codex mcp logout omnirush
codex mcp login omnirush`;
  const OPENCODE_AUTH_COMMAND = "opencode mcp auth omnirush";
  const OPENCODE_RECONNECT_COMMAND = `opencode mcp logout omnirush
opencode mcp auth omnirush`;
  const installs = [
    ...
    },
    {
      ...
      helper: "Add OmniRush.ai once, then sign in with Codex's MCP login command.",
      supportStatus: "Setup only",
      supportExplanation: "Setup guide only: add OmniRush.ai, run codex mcp login omnirush, and reconnect with logout then login. Native proof must be rerun on this exact branch.",
      copyText: `codex mcp add omnirush --url ${MCP_SERVER_URL}`,
    ...
    },
    {
      ...
      copyText: `claude mcp add --transport http omnirush ${MCP_SERVER_URL}`,
    },
    {
      id: "opencode",
      label: "OpenCode",
  ...
  "mcp": {
    ...
    },
    {
      ...
      copyText: `code --add-mcp '{"name":"omnirush","type":"http","url":"${MCP_SERVER_URL}"}'`,
    },
    {
      id: "any-client",
      label: "Any client",
  ...
  return (
    <div id="connect-mcp-install" className="not-prose my-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-950">
      ...
      <div className="overflow-x-auto border-b border-gray-100 px-4 pt-4 dark:border-white/10" role="tablist" aria-label="OmniRush.ai MCP client install options">
      ...
      <div className="flex flex-col gap-2 border-t border-gray-100 px-5 py-4 text-xs sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button type="button" aria-label="Copy the OmniRush.ai MCP server URL" onClick={() => copy("server-url", MCP_SERVER_URL)} className="shrink-0 font-medium underline">
