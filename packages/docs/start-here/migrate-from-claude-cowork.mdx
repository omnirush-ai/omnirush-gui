...
description: "Move your files, skills, plugins, MCP servers, and team setup to OmniRush.ai."
---

You do not need to rebuild your setup to move from Claude Cowork to OmniRush.ai. Start with one real task, bring over the capabilities it needs, and keep Cowork available until that task works end to end.

## What carries over

| From Claude Cowork | In OmniRush.ai | What to do |
| --- | --- | --- |
| Local files and folders | Workspace files | Open the same folder or repository as an OmniRush.ai workspace. |
| `SKILL.md` skills | Skills | Copy the skill folder into your workspace or install it as a plugin. |
| Claude-compatible plugins | Plugins | Import the plugin or marketplace manifest; OmniRush.ai normalizes its skills, commands, and MCP dependencies. |
| MCP servers | Connectors and custom apps | Reconnect the server through OmniRush.ai Connect or add its URL as a custom app. |
| Claude models | Anthropic provider | Add an Anthropic API key, or provision Anthropic centrally through OmniRush.ai Cloud. |
...
## 1. Install OmniRush.ai and open the same work

1. [Download OmniRush.ai](https://github.com/omnirush-ai/omnirush-gui/releases/latest) for macOS, Windows, or Linux.
2. Create a workspace from the folder or repository you used with Cowork.
3. Ask OmniRush.ai to summarize the workspace before changing anything. This confirms it can read the files you expect.

Your files stay where they already are. OmniRush.ai does not require you to upload a duplicate copy before you can work with them.

## 2. Choose your models

OmniRush.ai is model-agnostic. You can keep using Anthropic models, add another provider, or let your organization provision models centrally.
...
OmniRush.ai uses the same `SKILL.md` format. Copy a skill directory into the workspace, or import the plugin that contains it.
...
See [Connectors, skills, and plugins](/start-here/do-work-with-it/skills-plugins-and-mcp) for how OmniRush.ai organizes these pieces. For team distribution, see [Publish and copy a skill](/start-here/do-work-with-it/publish-and-copy-a-skill).

## 4. Reconnect apps and MCP servers

Credentials do not move between products. Reconnect each service so OmniRush.ai receives its own authorized access.
...
Do not ask every teammate to repeat your local setup by hand. In OmniRush.ai Cloud, publish approved skills and connections once, assign them to the right teams, and apply desktop policies centrally.
...
- [ ] Cowork remains available until the representative workflow passes in OmniRush.ai.

Once the checklist passes, make OmniRush.ai the default for that workflow and migrate the next one.
