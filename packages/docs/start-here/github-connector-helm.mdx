...
description: "Create a GitHub App and configure the OmniRush.ai EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted OmniRush.ai deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
config:
  public:
    ...
    apiOrigin: "https://api.omnirush.example.com"
```

The GitHub App needs these OmniRush.ai URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://omnirush.example.com` |
...
## What OmniRush.ai does with it

When an admin clicks **Connect GitHub** in OmniRush.ai:
...
```text
https://omnirush.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to OmniRush.ai, show repositories visible to the installation, and allow discovery on a selected repository.
