---
title: "Self-host"
description: "Run OmniRush.ai on your own server or infrastructure."
---

Most users just use the [desktop app](https://github.com/omnirush-ai/omnirush-gui/releases/latest). Self-hosting is for teams that want OmniRush.ai to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

omnirush.ai does not run a hosted Den, so team features such as Collections, shared providers, and hosted workspaces need a self-hosted Den. Rolling this out across a company? See [Enterprise](/cloud/enterprise) or email [info@omnirush.ai](mailto:info@omnirush.ai?subject=Enterprise%20deployment).
...
OmniRush.ai is split into normal services:

- **OmniRush.ai app**: the desktop or web client people use.
...
- **Inference service**: optional OmniRush.ai Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want OmniRush.ai Cloud-style accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.
...
The chart pulls the matching GHCR images for `omnirush-den-api`, `omnirush-den-web`, and optional `omnirush-inference` unless you mirror them into an internal registry.
...
For containerized services, start from the [Docker packaging](https://github.com/omnirush-ai/omnirush-gui/tree/main/packaging/docker) in the repo.
...
- Hosted OmniRush.ai uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Keep the local Docker MySQL URL (`mysql://root:password@127.0.0.1:3306/omnirush_den`) for development only; it is not a production security posture.
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. OmniRush.ai uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
Private deployments should keep public signup disabled and use the one-time [first-administrator setup flow](/self-host/deploy-to-your-cloud/first-administrator). Configuring an owner or platform-admin email does not create an account or password, and OmniRush.ai does not ship a default administrator password.
...
If you do not deploy Den, worker access is token-based with `OMNIRUSH_TOKEN` and `OMNIRUSH_HOST_TOKEN`.

## Minimal working stack

### OmniRush.ai Cloud and the desktop app

OmniRush.ai cloud-style self-hosting is composed of three main pieces:
...
OmniRush.ai does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**OmniRush.ai Cloud**](/cloud/get-started) runs hosted workers for your team.

Cloud also gives you org-wide primitives you'd otherwise have to build: [Collections](/cloud/share-with-your-team/collections), [shared LLM providers](/cloud/share-with-your-team/managed-llm-provider), [RBAC](/cloud/members-and-rbac), and [OmniRush.ai Web](/cloud/run-in-the-cloud/shared-workspace).

## Looking for support for a larger rollout?

If you're rolling OmniRush.ai out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
...
You can check the [**Enterprise**](/cloud/enterprise) page or email [info@omnirush.ai](mailto:info@omnirush.ai?subject=Enterprise%20deployment).
