...
description: "Sign in to OmniRush.ai and connect the accounts your agent can use"
---

<Info>
**OmniRush.ai Cloud organization setup.** This path requires an OmniRush.ai Cloud sign-in and an organization that has made services available to you. To stay local, keep using OmniRush.ai for free and [add a custom MCP server](/start-here/connect-your-stack/add-an-mcp-server) to your workspace. Admins can manage team connections with [Connectors](/cloud/share-with-your-team/shared-mcp-connections).
</Info>

Use `Settings` > `OmniRush.ai Connect` for services your organization makes available,
...
Connect requires signing in to an OmniRush.ai account and joining an organization
that has Connect enabled. This is separate from signing in to an LLM provider
such as ChatGPT.

1. Open the OmniRush.ai desktop app.
2. Click `Sign in`, or open `Settings` > `OmniRush.ai Connect` and choose `Sign in`.
3. Complete sign-in in your browser and return to OmniRush.ai.
4. Choose the OmniRush.ai organization you want to use if prompted.
...
1. Open `Settings` > `OmniRush.ai Connect`.
2. Find the service under `Needs your sign-in`.
3. Click `Connect` and approve access on the provider's sign-in page.
4. Return to OmniRush.ai. The service moves to `Ready to use` automatically.

Some connections are managed by your organization. If a service is not
listed, ask an organization admin to publish it from the OmniRush.ai Cloud
...
send you back to `Settings` > `OmniRush.ai Connect` with the action to take instead of
...
- A **connector** is an MCP service your organization adds on its own, without
  ...
  OmniRush.ai Cloud `Connectors` page and grant member or team access.
...
- The **MCP Gateway** is the hosted endpoint that lets external MCP clients
  use capabilities from an OmniRush.ai organization. See
  [OmniRush.ai MCP Gateway](/cloud/run-in-the-cloud/cloud-mcp).
