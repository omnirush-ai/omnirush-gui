...
description: "Diagnose DNS, TLS, proxy, catalog, and Den outbound issues in locked-down OmniRush.ai deployments."
---

Use this page when OmniRush.ai reaches the wrong server, cannot complete TLS, is blocked by a proxy, or needs proof that Den can reach an approved diagnostic origin. For non-network runtime, MCP, and stale-tool problems, use [Diagnostic prompts](/start-here/troubleshooting/diagnostic-prompts).
...
If raw download is blocked, copy `scripts/support/omnirush-doctor.ps1` to the machine through your approved offline channel and run:
...
| --- | --- | --- |
| `LIKELY TLS INTERCEPTION` | The leaf issuer is an internal or proxy CA instead of the expected public issuer. | Allowlist the OmniRush.ai hosts through the proxy, or install the enterprise root in every OmniRush.ai trust surface that needs it. |
...
Settings → Debug includes a cloud catalog diagnostic that probes the managed `omnirush-cloud` MCP entry and verifies that Cloud exposes exactly `search_capabilities` and `execute_capability`.

For self-hosted Den origins, set `OMNIRUSH_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS` before OmniRush.ai launches:

```bash
OMNIRUSH_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS=https://omnirush.example.com
```
...
- Den's built-in default diagnostics origin belongs to the retired hosted service and no longer resolves, so set `DEN_DIAGNOSTICS_ORIGIN` to a diagnostics app you deploy (see below).
