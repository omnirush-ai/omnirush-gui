...
description: "How OmniRush.ai can answer questions about past sessions."
---

OmniRush.ai's cross-chat memory currently comes from saved session history. It is not a separate long-term memory database, and it does not automatically inject every old chat into every new prompt.

When an agent is running inside the OmniRush.ai app, it can use OmniRush.ai's semantic UI actions to look up past chats:
...
Because this uses the OmniRush.ai UI, the agent may briefly navigate away from your current session to inspect another one. You can ask it to return to the current session afterward.

## Current limits

- Session lookup is based on the sessions OmniRush.ai has loaded for the current app and workspace context.
...
- The agent can only read sessions available to the current OmniRush.ai app and user.

## How to ask

Give the agent whatever clue you have: a session ID, title, person name, project name, workspace, or rough topic. For example: `Use the OmniRush.ai app UI to find the customer migration chat and tell me what I said about customer references.`
