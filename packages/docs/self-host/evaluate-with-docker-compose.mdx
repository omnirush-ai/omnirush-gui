...
description: "Run the OmniRush.ai control plane from published images without cloning or building the repository."
---

Use the pull-only Docker Compose stack to evaluate Den web, Den API, and organization management on one machine. The stack downloads published OmniRush.ai images and MySQL, runs database migrations, and starts the browser experience at `http://localhost:3005`.
...
| --- | --- | --- |
| `OMNIRUSH_WEB_PORT` | `3005` | Host port for Den web. |
| `OMNIRUSH_API_PORT` | `8788` | Host port for Den API. |
| `OMNIRUSH_AUTH_SECRET` | Required | Better Auth signing secret of at least 32 random characters. |
| `OMNIRUSH_DB_ENCRYPTION_KEY` | Required | Key of at least 32 random characters used to encrypt selected database values. |
| `OMNIRUSH_ORG_NAME` | `OmniRush.ai Evaluation` | Name of the single organization. |
| `OMNIRUSH_ALLOW_SIGNUP` | `true` | Whether anyone who can reach the site can create an account. |
