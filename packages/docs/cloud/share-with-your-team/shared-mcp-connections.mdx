...
omnirush.ai does not run a hosted Den, so there is no shared hosted redirect
URL. For a self-hosted instance whose public Den API is
`https://api.omnirush.example.com`, register this exact redirect URL with the
OAuth provider:
