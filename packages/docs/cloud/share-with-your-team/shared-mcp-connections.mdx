...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in OmniRush.ai Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in OmniRush.ai Cloud

1. Open `Connectors` in the OmniRush.ai Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in OmniRush.ai Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted OmniRush.ai Cloud instance, the dashboard is
`https://app.omnirushlabs.com` and its public Den API origin is
`https://api.omnirushlabs.com`. Register this exact redirect URL with the OAuth
provider:

```text
https://api.omnirushlabs.com/v1/mcp-connections/oauth/callback
```

For a self-hosted instance whose public Den API is
`https://api.omnirush.example.com`, register:

```text
https://api.omnirush.example.com/v1/mcp-connections/oauth/callback
```
...
Slack's MCP server does not support automatic OAuth client registration, so a Slack admin creates a Slack app once and OmniRush.ai Cloud uses its credentials for every member.
...
6. In OmniRush.ai Cloud, add the Slack connection and paste the app's `Client ID` and `Client Secret` from `Basic Information` > `App Credentials`.
...
Granted members find the connection in `Settings` > `OmniRush.ai Connect`, grouped by what
they need to do:
...
1. Sign in to OmniRush.ai, then open `Settings` > `OmniRush.ai Connect`.
...
4. The browser shows `Connected` — return to OmniRush.ai.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in OmniRush.ai Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through OmniRush.ai Connect,
...
action and direct you to `Settings` > `OmniRush.ai Connect`.

## Expose a connection directly as an MCP server

By default the agent reaches every connection through OmniRush.ai Connect's two
...
that reads the OmniRush.ai Connect server index.

What stays the same:

- Sign-in still happens in OmniRush.ai Cloud. The desktop app never receives the
  provider's URL or token; it talks to Den's per-connection endpoint with the
  member's own OmniRush.ai credential.
...
Directly exposed connections appear in OpenCode as `omnirush-direct-<name>-…`.
...
services managed through OmniRush.ai Cloud.
