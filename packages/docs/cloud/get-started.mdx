---
title: "Get started with OmniRush.ai Cloud"
...
<Note>
OmniRush.ai Cloud is optional. The local desktop app stays free, requires no Cloud sign-in, and supports your own providers, skills, plugins, and custom MCP servers. Use Cloud when an organization needs to assign and manage those resources for a team.
</Note>

OmniRush.ai Cloud runs on a [self-hosted Den](/start-here/self-host); omnirush.ai does not run a hosted Den. It is for teams that want shared identity, Collections, managed connections and models, desktop policies, or hosted workspaces.

First, create or join an organization and connect the desktop app to it.

<Frame>
  ![OmniRush.ai Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open your Den web origin (for example `https://omnirush.example.com`) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if OmniRush.ai Cloud asks for billing.
...
1. Open `/dashboard/org-settings` on your Den web origin.
...
4. If the browser does not return to OmniRush.ai automatically, click `Paste sign-in code`, paste the `omnirush://den-auth?...` link or one-time code from OmniRush.ai Cloud, then click `Finish sign-in`.

## Cloud URL used by the desktop app

The desktop app uses a single Cloud URL: the Den web `baseUrl`. There is no
hosted default, because omnirush.ai does not run a hosted Den.
...
- OmniRush.ai Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.

The desktop app does not store or read a separate API URL. On startup it reads
the Cloud URL from `desktop-bootstrap.json`; if that file is missing and no
Cloud URL was set through a connect link or `Settings > Advanced`, no Den is
used and Cloud features stay off. When you change the Cloud URL in
`Settings > Account`, OmniRush.ai writes the new `baseUrl` back to the bootstrap
...
- Use [OmniRush.ai Web](/cloud/run-in-the-cloud/shared-workspace) to give members a hosted OmniRush.ai workspace in the browser, with nothing to install.
