...
description: "Current Google Workspace SCIM support for OmniRush.ai"
---

Use [Google Workspace SAML SSO](/cloud/sso-google-workspace) when members
should authenticate to OmniRush.ai with Google. Google Workspace custom SAML apps do
not currently expose a generic outbound SCIM connector that can provision users
and groups into OmniRush.ai.

## Current support

OmniRush.ai supports SCIM provisioning through identity providers that can send
SCIM requests to the OmniRush.ai SCIM base URL with a bearer token. Google
...
For a custom SAML app created for OmniRush.ai, Google Admin provides SAML settings,
optional SAML attribute mappings, and service access controls, but does not show
the SCIM fields OmniRush.ai needs:
...
Until Google adds OmniRush.ai as a supported automated-provisioning app or exposes
generic outbound SCIM settings for custom apps, Google Workspace can be used for
OmniRush.ai SSO but not for OmniRush.ai SCIM lifecycle provisioning.

## What still works with Google Workspace

Google SAML SSO can authenticate OmniRush.ai members and create first-login members
through OmniRush.ai's just-in-time SSO provisioning. JIT provisioning is not the
same as SCIM:

- It happens when a user signs in successfully.
- It can create the OmniRush.ai organization membership with the default `Member`
...
- It does not deactivate OmniRush.ai access when a Google user is suspended or
  deleted.
- It does not create SCIM-managed OmniRush.ai teams from Google groups.

Use OmniRush.ai member management, invitations, or another supported SCIM identity
provider for lifecycle automation.

## How to confirm in Google Admin

After setting up the OmniRush.ai custom SAML app, open the Google Admin console and
check:

1. **Apps → Web and mobile apps → OmniRush.ai**.
...
If those sections are not present, the app cannot consume the OmniRush.ai SCIM base
URL or bearer token.

Google's general automated user provisioning page points administrators to
app-specific provisioning guides for supported apps. If OmniRush.ai appears in that
supported app list in the future, follow the Google app-specific provisioning
guide and enter the OmniRush.ai SCIM values from **Settings → SCIM**.

## Do not expose the SCIM token unnecessarily

Only create or rotate an OmniRush.ai SCIM connector when you have an identity
...
that supports OmniRush.ai's SCIM connector:
...
SSO and SCIM lifecycle tests pass and at least one OmniRush.ai owner has a working
recovery path.
