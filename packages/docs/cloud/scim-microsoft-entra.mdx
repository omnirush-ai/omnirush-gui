...
description: "Provision OmniRush.ai members and teams from Microsoft Entra ID"
---

Use Microsoft Entra ID provisioning to create, update, and deactivate OmniRush.ai
members automatically. Entra security groups can also become OmniRush.ai teams
...
- An OmniRush.ai organization with Enterprise SSO and SCIM.
- An OmniRush.ai owner or super-admin.
- A working, domain-verified, tested, and enabled OmniRush.ai SAML connection.
  Complete and test
...
SCIM. Make sure at least one OmniRush.ai owner can sign in through SAML and that
the current SSO configuration has passed OmniRush.ai's **Test SSO before enabling
it** flow before relying on Entra as the identity source.

## 1. Get the OmniRush.ai connection values

In OmniRush.ai, select the organization you want to connect, then open
**Settings → SCIM**.
...
3. Copy the **SCIM bearer token** immediately. OmniRush.ai shows the full token
   only after creating or rotating it.
4. To synchronize Entra groups as OmniRush.ai teams, select **Enable team sync**
...
| --- | --- |
| Tenant URL | OmniRush.ai **Settings → SCIM → SCIM base URL**. It normally ends in `/api/auth/scim/v2`. |
...
2. Select the same enterprise application used for OmniRush.ai SAML SSO.
3. Open **Provisioning**.
...
6. Paste the OmniRush.ai **SCIM base URL** into **Tenant URL**.
7. Paste the OmniRush.ai **SCIM bearer token** into **Secret Token**.
...
principal names use an `onmicrosoft.com` domain but users sign in to OmniRush.ai
...
create a second OmniRush.ai member for the same person.
...
not enough; assign it to the OmniRush.ai enterprise application as well.
...
3. Confirm that import, scope evaluation, matching, and the final action all
   succeed.
4. In OmniRush.ai, open **Members** and confirm that the member appears with the
   expected email.
...
6. In OmniRush.ai, open **Members → Teams** and confirm that the group appears as a
...
sends a group-create request to OmniRush.ai. Add members and select them explicitly
...
5. Refresh OmniRush.ai **Members** and **Teams** to verify the final state.
...
1. Remove a test user from the Entra group and confirm that OmniRush.ai removes the
   user from the SCIM-managed team.
2. Disable or unassign a test user and confirm the expected OmniRush.ai
   deprovisioning behavior.
3. Add another assigned group and confirm that OmniRush.ai creates a second
   SCIM-managed team.

Change SCIM-managed teams in Entra, not in OmniRush.ai. OmniRush.ai keeps manually
created teams separate from identity-provider-managed teams.

### Optional: grant organization Admin through an Entra group

After verifying the synchronized membership, an OmniRush.ai owner or super-admin
...
| --- | --- | --- |
| **Test Connection** fails | Incorrect Tenant URL, expired or rotated token, or copied whitespace | Copy the base URL and a current token again from OmniRush.ai, then retest |
...
| On-demand group provisioning reports an internal server error without an OmniRush.ai request | The group is empty or no members were selected for the on-demand run | Add members, select up to five under **Selected users**, and retry |
| A group provisions but no OmniRush.ai team appears | OmniRush.ai team synchronization is disabled | Open **Settings → SCIM** and enable **Create teams from SCIM groups** |
| Entra creates a duplicate OmniRush.ai member | SAML and SCIM use different email identifiers | Align the SAML NameID and the SCIM `userName` mapping |
| Entra reports success but OmniRush.ai still shows the old state | The UI or incremental cycle has not refreshed | Refresh OmniRush.ai, check Entra **Provisioning logs**, and review the SCIM health status in OmniRush.ai |
