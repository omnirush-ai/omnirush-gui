...
import { createAutomationSchema, createCloudAutomationSchema, updateAutomationSchema } from "@omnirush/types/automations"
...
describe("saved Script Automations", () => {
  ...
  test("keeps legacy agent creation compatible and versions exact script identities", () => {
    expect(createAutomationSchema.safeParse({
      ...
      schedule,
      model: { providerId: "omnirush", modelId: "model" },
    }).success).toBe(true)
