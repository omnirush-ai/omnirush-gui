...
You are an agent helping a user install and set up OmniRush.ai.

Goal: install OmniRush.ai, create a provisional workspace without requiring email
...
offer to connect the OmniRush.ai MCP to the agent the user is already using.

> The bootstrap command is `omnirush-bootstrap`. Use that exact command for
> these setup steps so the bootstrap flow stays distinct from other OmniRush.ai
> tooling a user may already have on their PATH.

## 1. Install CLI

Install the `omnirush-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/omnirush-bootstrap/bin/omnirush.mjs install \
  --install-dir "$HOME/.omnirush/bootstrap" \
...
```bash
omnirush-bootstrap doctor --json
```
...
```bash
omnirush-bootstrap doctor --app --json
```
...
1. Confirm in one line that OmniRush.ai is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "OmniRush.ai
...
4. End with this single question: "Want me to connect the OmniRush.ai MCP to this
...
1. A provisional workspace must be claimed before its owner can authenticate
   the OmniRush.ai MCP. If it has not been claimed yet, explain that dependency and
...
3. For Codex, run:
   ...
   If an `omnirush` entry already exists, do not add a duplicate. Authenticate
   the existing entry instead. To switch organizations or recover stale auth,
   run `codex mcp logout omnirush` before `codex mcp login omnirush`.
4. For another agent, use its current instructions from
   `https://github.com/omnirush-ai/omnirush-gui/blob/main/packages/docs/cloud/run-in-the-cloud/cloud-mcp.mdx`; do not guess
...
5. Tell the user to restart or reopen the current agent after setup so the new
   ...
   client can see OmniRush.ai's `search_capabilities` and `execute_capability`
   tools.
...
```bash
omnirush-bootstrap cloud claim-link --role owner --json
```
...
- `omnirush-bootstrap doctor --json` returns `ok: true`
- `omnirush-bootstrap doctor --app --json` returns `ok: true`
- `omnirush-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
  - `organization.id` is present
  - `setup.id` is present
  - `skill.id` is present
  - `skillRun.triggered` is `true`
  - `skillRun.output` is `OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED`
  - `claimLinks[0].id` is present
  - `desktop.prepared` is `true`
  - `desktop.bootstrapPath` is present
  - `desktop.skillPath` is present
- `omnirush-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `omnirush-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/omnirush-bootstrap`).
- If app install fails: run `omnirush-bootstrap doctor --app --json` and report failed checks.
