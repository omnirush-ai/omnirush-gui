// Live local E2E test evidence for `omnirush install app` using a real macOS DMG.
//
// The flow creates a tiny OmniRush.ai.app fixture, packages it as a .dmg, writes an
// install manifest with a SHA-256 digest, installs the bootstrap CLI into a temp
// bin dir, then runs the installed `omnirush install app` command against the
// manifest and verifies it with `omnirush doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("omnirush install app DMG test evidence requires macOS")
}

const here = dirname(fileURLToPath(import.meta.url))
const packageRoot = resolve(here, "..")
const repoRoot = resolve(packageRoot, "..", "..")
const cli = join(packageRoot, "bin", "omnirush.mjs")
const outDir = join(repoRoot, "evals", "results", "omnirush-app-install-dmg")
const temp = join(tmpdir(), `omnirush-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
const sourceDir = join(temp, "source")
const appFixture = join(sourceDir, "OmniRush.ai.app")
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const appDir = join(temp, "Applications")
const dmgPath = join(temp, "OmniRush.ai.dmg")
const manifestPath = join(temp, "omnirush-install-manifest.json")
const installedOmniRush = join(binDir, "omnirush-bootstrap")
...
<key>CFBundleExecutable</key><string>OmniRush.ai</string>
<key>CFBundleIdentifier</key><string>com.omnirush.fixture</string>
<key>CFBundleName</key><string>OmniRush.ai</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "OmniRush.ai"), "#!/usr/bin/env sh\necho OmniRush.ai fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "OmniRush.ai"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "OmniRush.ai", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  const digest = sha256(dmgPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-fixture",
    appName: "OmniRush.ai.app",
    artifacts: {
      darwin: {
        [process.arch]: {
          type: "dmg",
  ...
  prove("A real macOS DMG install manifest is available", {
    action: "Create OmniRush.ai.app fixture, package it with hdiutil, and write manifest JSON",
  ...
  prove("The bootstrap CLI can be installed from a script", {
    action: "node bin/omnirush.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and an omnirush executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installedOmniRush))

  const installApp = run(installedOmniRush, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  const appPath = join(appDir, "OmniRush.ai.app")
  prove("The installed CLI can download, verify, mount, and install OmniRush.ai.app from a DMG", {
    action: "omnirush install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
    assert: "exit 0, checksum recorded, and OmniRush.ai.app copied into app dir",
  ...
  const doctor = run(installedOmniRush, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "omnirush doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with omnirushApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "omnirushApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
...
<title>OmniRush.ai App DMG Install — test evidence</title>
...
</head><body><main><h1>OmniRush.ai App DMG Install — test evidence</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Screenshots: ${frames.length}</div>
