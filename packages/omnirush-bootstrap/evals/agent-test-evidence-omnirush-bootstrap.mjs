// Live E2E test evidence for the script-installable OmniRush.ai bootstrap CLI.
//
...
//   DEN_API_E2E_BASE_URL=... pnpm exec node evals/agent-test-evidence-omnirush-bootstrap.mjs
...
const temp = mkdtempSync(join(tmpdir(), "omnirush-bootstrap-e2e-"))
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const installedOmniRush = join(binDir, process.platform === "win32" ? "omnirush-bootstrap.cmd" : "omnirush-bootstrap")
const outDir = join(repoRoot, "evals", "results", "omnirush-bootstrap-cli")
...
try {
  ...
  prove("A bootstrap script can install the omnirush CLI", {
  ...
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installedOmniRush)

  const doctor = run(installedOmniRush, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "omnirush doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installedOmniRush, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
    ...
    skillName,
    "--json",
  ], { env: { ...process.env, OMNIRUSH_OWNER_PASSWORD: randomPassword() }, timeout: 30_000 })
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "omnirush cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
  ...
  }, onboard.status === 0 && onboard.json?.ok === true && onboard.json?.organization?.id && onboard.json?.invitation?.invitationId && onboard.json?.skill?.id && onboard.json?.skill?.title === skillName && onboard.json?.skillRun?.triggered === true && onboard.json?.skillRun?.output === "OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED")
} finally {
...
<title>OmniRush.ai Bootstrap CLI — live E2E test evidence</title>
...
<h1>OmniRush.ai Bootstrap CLI — live E2E test evidence</h1>
