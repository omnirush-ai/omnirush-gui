...
// bootstrap actions from other OmniRush.ai commands a user may already have.
const COMMAND_NAME = "omnirush-bootstrap"
const DEFAULT_OMNIRUSH_MARKETPLACE_NAME = "OmniRush.ai Marketplace"
...
function printHelp() {
  console.log([
    ...
    "",
    "Usage:",
    "  omnirush-bootstrap install [--bin-dir <path>] [--install-dir <path>] [--source <path>] [--json]",
    "  omnirush-bootstrap install app --manifest <url-or-file> [--app-dir <path>] [--json]",
    "  omnirush-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  OMNIRUSH_OWNER_PASSWORD=<password> omnirush-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  omnirush-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  omnirush-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
    "  install          Install the omnirush-bootstrap CLI into a user bin dir",
...
function defaultInstallDir() {
  return process.env.OMNIRUSH_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".omnirush", "bootstrap")
}

function defaultBinDir() {
  return process.env.OMNIRUSH_BIN_DIR || join(process.env.HOME || process.cwd(), ".local", "bin")
}

function defaultAppDir() {
  return process.env.OMNIRUSH_APP_DIR || (process.platform === "darwin"
    ? join(process.env.HOME || process.cwd(), "Applications")
    : process.platform === "win32"
      ? join(process.env.LOCALAPPDATA || join(process.env.HOME || process.cwd(), "AppData", "Local"), "OmniRush.ai")
      : join(process.env.HOME || process.cwd(), ".local", "share", "omnirush"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.OMNIRUSH_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "omnirush", "desktop-bootstrap.json")
}

function defaultSkillsDir() {
  return process.env.OMNIRUSH_SKILLS_DIR || join(configHomeDir(), "opencode", "skills")
}

function defaultDeviceKeyPath() {
  return process.env.OMNIRUSH_DEVICE_KEY_PATH || join(configHomeDir(), "omnirush", "bootstrap-device-key.json")
}
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "omnirush-bootstrap-skill"
}
...
function runInstall(args) {
  ...
  const installedCli = join(installDir, "omnirush.mjs")
  ...
  jsonOut({ ok: true, message: `OmniRush.ai CLI installed at ${executable}`, install: manifest }, json)
}
...
async function runInstallApp(args) {
  const json = hasFlag(args.flags, "json")
  const manifestLocation = getFlag(args.flags, "manifest") || process.env.OMNIRUSH_INSTALL_MANIFEST
  ...
  const workDir = join(tmpdir(), `omnirush-app-install-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  try {
    ...
    const artifactPath = join(workDir, artifact.fileName || "OmniRush.ai.dmg")
    ...
    writeFileSync(join(appDir, "omnirush-app-install.json"), JSON.stringify(install, null, 2))
    jsonOut({ ok: true, message: `OmniRush.ai app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  checks.push({ name: "omnirushExecutable", ok: executableOk, value: executable })
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    ...
    checks.push({ name: "omnirushApp", ok: existsSync(appPath), value: appPath })
  ...
  jsonOut({ ok, message: ok ? "OmniRush.ai doctor: ok" : "OmniRush.ai doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by omnirush bootstrap.\nomnirushBootstrapTrigger: bootstrap.verify\nomnirushBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm OmniRush.ai cloud onboarding can create and trigger a deterministic skill.`
}
...
async function createCloudSkillPlugin(baseUrl, auth, input) {
  ...
  const marketplace = marketplaces.body.items.find((item) => item?.name === DEFAULT_OMNIRUSH_MARKETPLACE_NAME) || marketplaces.body.items[0]
  if (!marketplace?.id) {
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "omnirushBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "omnirushBootstrapOutput")
...
async function createDesktopHandoff(baseUrl, auth) {
  const handoff = await request(baseUrl, "/v1/auth/desktop-handoff", {
    method: "POST",
    headers: auth,
    body: JSON.stringify({ desktopScheme: "omnirush" }),
  })
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "omnirush-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function resolveOwnerPassword(flags) {
  ...
  const envName = getFlag(flags, "owner-password-env", "OMNIRUSH_OWNER_PASSWORD")
...
async function runCloudOnboard(args) {
  ...
  const skillOutput = getFlag(args.flags, "skill-output", "OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED")
  ...
  jsonOut({
    ok: true,
    message: "OmniRush.ai cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  ...
  const skill = {
    ...response.body.skill,
    skillText: skillText(response.body.skill.title, response.body.skill.output || "OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED"),
  }

  const skillRun = runBootstrapSkill(skill, { trigger: "bootstrap.verify" })
  if (!skillRun.triggered || skillRun.output !== "OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED") {
    throw new Error(`skill_trigger_failed: ${JSON.stringify(skillRun)}`)
  }
  ...
  jsonOut({
    ok: true,
    message: "OmniRush.ai workspace bootstrap complete",
    organization: response.body.organization,
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "omnirush-bootstrap cloud claim-link --role ${link.role}" to view`
