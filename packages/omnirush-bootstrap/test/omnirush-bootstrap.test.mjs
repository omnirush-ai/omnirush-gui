...
const cli = join(root, "bin", "omnirush.mjs")
const temp = mkdtempSync(join(tmpdir(), "omnirush-bootstrap-test-"))
...
try {
  ...
  const executableName = process.platform === "win32" ? "omnirush-bootstrap.cmd" : "omnirush-bootstrap"
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "cob_test", title: "First OmniRush.ai Skill", output: "OMNIRUSH_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  await withStubDenApi(
    ...
    async (baseUrl) => {
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ brandAppName: "OmniRush.ai Demo A" })}\n`,
        "utf8",
      )
      ...
      assert.equal(writtenExplicit.brandAppName, "OmniRush.ai Demo A", "prepared desktop setup must preserve its profile identity")
    },
  )
