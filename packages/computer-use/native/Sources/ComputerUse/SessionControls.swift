...
final class ControlLease {
    ...
    init() throws {
        let directory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("OmniRush.ai/ComputerUse", isDirectory: true)
...
final class SessionControls: NSObject {
    ...
    private func publish(_ values: [String: Any]) {
        ...
        hostState["id"] = hostID
        guard let data = try? JSONSerialization.data(withJSONObject: ["jsonrpc": "2.0", "method": "omnirush/ui", "params": hostState]) else { return }
    ...
    func chooseWindow(app: AppIdentity, mode: AccessMode, windows: [WindowTarget], purpose: String) async throws -> WindowTarget {
        ...
        alert.messageText = "Allow OmniRush.ai to use \(app.name)?"
    ...
    func show(app: AppIdentity, target: WindowTarget, mode: AccessMode, purpose: String) {
        ...
        panel.title = "OmniRush.ai Computer Use"
        ...
        let status = NSTextField(wrappingLabelWithString: "OmniRush.ai is working. You can take over at any time.")
        ...
        stopObserver = DistributedNotificationCenter.default().addObserver(forName: Notification.Name("ai.omnirush.desktop.computer-use.stop"), object: nil, queue: .main) { [weak self] _ in
...
final class PermissionSetup: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let title = NSTextField(labelWithString: "Choose what OmniRush.ai can use")
        ...
        let footer = NSTextField(wrappingLabelWithString: "After allowing access, return to Library → Computer Use in OmniRush.ai to finish setup. If macOS asks you to restart, quit and reopen OmniRush.ai. Windows and Linux desktop control are not available in this version.")
    ...
    @objc private func stopAll() {
        DistributedNotificationCenter.default().postNotificationName(Notification.Name("ai.omnirush.desktop.computer-use.stop"), object: nil, userInfo: nil, deliverImmediately: true)
    }
}
