...
#
# Usage (from _repos/omnirush repo root):
#   packaging/docker/den-dev-up.sh
#
# Outputs:
# - OmniRush.ai web app URL
...
PROJECT="omnirush-den-dev-$DEV_ID"
...
DEN_SINGLE_ORG_NAME="${DEN_SINGLE_ORG_NAME:-OmniRush.ai}"
...
DEN_MYSQL_URL=mysql://root:password@127.0.0.1:$DEN_MYSQL_PORT/omnirush_den
...
echo "" >&2
echo "OmniRush.ai web UI:        http://localhost:$DEN_WEB_PORT" >&2
echo "OmniRush.ai web UI (LAN/public):      http://$PUBLIC_HOST:$DEN_WEB_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "OmniRush.ai web UI (LAN IP):          http://$LAN_IPV4:$DEN_WEB_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "OmniRush.ai web UI (Tailscale):       http://$TAILSCALE_DNS_NAME:$DEN_WEB_PORT" >&2
fi
...
echo "MySQL:                 mysql://root:password@127.0.0.1:$DEN_MYSQL_PORT/omnirush_den" >&2
