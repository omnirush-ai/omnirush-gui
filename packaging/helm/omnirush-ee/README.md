# OmniRush.ai EE Helm Chart

Initial Helm chart for the OmniRush.ai EE Den stack:
...
```bash
helm upgrade --install omnirush-ee oci://ghcr.io/omnirush-ai/charts/omnirush-ee \
  --version REPLACE_OMNIRUSH_VERSION \
  -f values.prod.yaml
```
...
```yaml
image:
  tag: "REPLACE_OMNIRUSH_VERSION"

config:
  tenancy:
    # Default chart behavior is single-org for private/self-hosted installs.
    # Hosted OmniRush.ai Cloud should set this to "multi_org" explicitly.
    mode: "single_org"
  ...
  public:
    ...
    webOrigin: "https://omnirush.example.com"
    ...
    apiOrigin: ""
    mcpResourceUrl: ""
    mcpClaimNamespace: "https://omnirush.example.com"
    desktopDenBaseUrl: ""
    corsOrigins: ""
    ...
    authCallbackUrl: "https://omnirush.example.com"
  githubConnector:
    appId: ""
    clientId: ""
...
secret:
  values:
    ...
    databaseUrl: "mysql://omnirush:REPLACE_ME@mysql.example.internal:3306/omnirush_den?sslaccept=accept"
    betterAuthSecret: "REPLACE_WITH_AT_LEAST_32_CHARACTERS"
    denDbEncryptionKey: "REPLACE_WITH_AT_LEAST_32_CHARACTERS"
    emailFrom: "OmniRush.ai <no-reply@example.com>"
    smtpHost: "smtp.example.com"
    smtpPort: "587"
    smtpUser: "omnirush@example.com"
    smtpPass: "REPLACE_ME"
    smtpSecure: "false"
...
ingress:
  ...
  className: nginx
  web:
    host: omnirush.example.com
  api:
    host: api.omnirush.example.com
```
...
```bash
helm template omnirush-ee ./packaging/helm/omnirush-ee -f values.prod.yaml
helm upgrade --install omnirush-ee ./packaging/helm/omnirush-ee -f values.prod.yaml
```
...
unambiguous. Hosted OmniRush.ai Cloud explicitly enables availability.
...
### OmniRush.ai Web rollout

OmniRush.ai Web is unavailable by default for self-hosted and customer-managed
deployments. OmniRush.ai Cloud enables it in its deployment values with:

```yaml
config:
  public:
    omnirushWebEnabled: "true"
```

The chart renders this value as `DEN_OMNIRUSH_WEB_ENABLED`. A raw environment
...
configure `STRIPE_SECRET_KEY` and `STRIPE_OMNIRUSH_WEB_PRICE_ID` before the
...
secret:
  create: false
  existingSecret: omnirush-ee-secrets
```
...
Set optional `DATABASE_REDIS_URL` to enable Den API Redis-backed session and query caching. Set `DAYTONA_API_KEY` when `config.provisioner.mode` is `daytona`. Set `POLAR_ACCESS_TOKEN` when Polar feature gating is enabled. Set `OPENROUTER_MANAGEMENT_API_KEY` when enabling OmniRush.ai Models management.
...
secret:
  values:
    databaseRedisUrl: "rediss://redis-master.omnirush.svc.cluster.local:6379"
```
...
Use `customCa` when OmniRush.ai must trust a private certificate authority for
...
```bash
kubectl create secret generic omnirush-custom-ca \
  --namespace omnirush \
...
customCa:
  enabled: true
  existingSecret: omnirush-custom-ca
  existingConfigMap: ""
  key: ca.crt
```
...
```bash
kubectl create configmap omnirush-custom-ca \
  --namespace omnirush \
  --from-file=ca.crt=./corp-root-ca.pem
```
...
customCa:
  enabled: true
  existingSecret: ""
  existingConfigMap: omnirush-custom-ca
  key: ca.crt
```
...
`/etc/omnirush/custom-ca/ca-bundle.pem` and sets `NODE_EXTRA_CA_CERTS` to that
...
secret:
  values:
    databaseUrl: "mysql://omnirush:REPLACE_DB_PASSWORD@mysql.example.internal:3306/omnirush_den?sslmode=verify-full"
```
...
roots your OmniRush.ai deployment should trust. On CA rotation, update the existing
Secret or ConfigMap and restart the running workloads so Node reloads the CA
file, for example:

```bash
kubectl rollout restart deployment/omnirush-ee-den-api --namespace omnirush
kubectl rollout restart deployment/omnirush-ee-den-web --namespace omnirush
kubectl rollout restart deployment/omnirush-ee-inference --namespace omnirush
```
...
The chart configures telemetry export from OmniRush.ai; it does not install an
...
`localhost`, because that would refer to the OmniRush.ai container itself.

First create the namespace used by this example:

```bash
kubectl create namespace omnirush
```
...
namespace as OmniRush.ai**:

```bash
kubectl create secret generic omnirush-otel-headers \
  --namespace omnirush \
...
observability:
  backend: otel
  serviceNames:
    denApi: omnirush-den-api
    denWeb: omnirush-den-web
  otel:
    ...
    tracesSamplerArg: ""
    headers:
      existingSecret: omnirush-otel-headers
...
Install or upgrade OmniRush.ai with the values file:

```bash
helm upgrade --install omnirush-ee ./packaging/helm/omnirush-ee \
  --namespace omnirush \
  --create-namespace \
...
The commands below assume the Helm release is named `omnirush-ee`. If you use a
different release name, run `kubectl get deployments,services --namespace
omnirush` to find the generated resource names.

Confirm that the workloads are ready:

```bash
kubectl get pods --namespace omnirush
kubectl rollout status deployment/omnirush-ee-den-api --namespace omnirush
kubectl rollout status deployment/omnirush-ee-den-web --namespace omnirush
```
...
```bash
kubectl describe deployment/omnirush-ee-den-api --namespace omnirush
kubectl describe deployment/omnirush-ee-den-web --namespace omnirush
```
...
`omnirush-otel-headers`.

Generate a request that crosses both services. Keep this port-forward running:

```bash
kubectl port-forward service/omnirush-ee-den-web 3005:3005 --namespace omnirush
```
...
Your observability backend should show `omnirush-den-web` and
`omnirush-den-api`, with one connected trace for the request. Logs from both
...
- `observability.otel.endpoint` is a base endpoint. OmniRush.ai appends
...
- The Secret must be in the OmniRush.ai release namespace, and its key must match
...
observability:
  ...
  sentry:
    dsnSecret:
      existingSecret: omnirush-sentry-runtime
      key: SENTRY_DSN
...
- Setup URL: `https://omnirush.example.com/dashboard/integrations/github`
- Webhook URL: `https://api.omnirush.example.com/v1/webhooks/connectors/github`
...
- `DEN_API_BASE=http://<release>-omnirush-ee-den-api:8788`
- `DEN_AUTH_FALLBACK_BASE=http://<release>-omnirush-ee-den-api:8788`
- `INFERENCE_PROXY_BASE_URL=http://<release>-omnirush-ee-inference:8791` when `inference.enabled=true`
...
config:
  public:
    ...
    installerReleaseRepo: "omnirush-ai/omnirush-gui"

installerArtifacts:
  enabled: true
  existingClaim: omnirush-desktop-artifacts
  mountPath: /var/lib/omnirush/installer-artifacts
```
...
The organization download page hands the normal OmniRush.ai app its Den
...
setup stays in the **Open OmniRush.ai** deep-link step after installation.
...
- `omnirush-mac-arm64-<version>.dmg`
- `omnirush-mac-x64-<version>.dmg`
- `omnirush-win-x64-<version>.exe`
- `omnirush-linux-x86_64-<version>.AppImage`
- `omnirush-linux-arm64-<version>.AppImage`
