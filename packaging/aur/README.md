...
Use this directory to build and install the OmniRush.ai package locally on Arch Linux without Docker.
...
- Installs the Electron bundle under `/opt/omnirush`.
- Adds `/usr/bin/omnirush`, a desktop entry, and the OmniRush.ai icon.
...
2. build an Arch package such as `omnirush-<version>-1-x86_64.pkg.tar.zst`
3. install it locally with `pacman`

After install, `omnirush` is available as the desktop launcher. The bundled sidecars remain inside `/opt/omnirush`; the package does not claim the standalone `opencode` command.
...
```bash
sudo pacman -U omnirush-<version>-1-x86_64.pkg.tar.zst
```
